...
async fn err_of(setup: &[&str], sql: &str) -> String {
    ...
    c.execute(sql)
        .await
        .expect_err("bad ON CONFLICT target must be rejected")
        .to_string()
}
...
async fn ok(setup: &[&str], sql: &str) {
    ...
    c.execute(sql)
        .await
        .expect("valid ON CONFLICT target must succeed");
}
    ...
    asupersync::test_utils::run_test(|| async {
        ...
        assert_eq!(
            err_of(
                ...
                "INSERT INTO t VALUES(1,2) ON CONFLICT(nope) DO NOTHING"
            )
            .await,
            "no such column: nope",
        );
        assert_eq!(
            err_of(
                &["CREATE TABLE t(a PRIMARY KEY, b)"],
                "INSERT INTO t VALUES(1,2) ON CONFLICT(nope) DO UPDATE SET b=2"
            )
            .await,
            "no such column: nope",
        );
        ...
        ok(
            ...
            "INSERT INTO t VALUES(1,2) ON CONFLICT(b) DO NOTHING",
        )
        .await;
        ok(
            &[
