...
fn two_writers_append_once_and_the_loser_rereads_a_fresh_snapshot() {
    asupersync::test_utils::run_test(|| async {
        ...
        winner.execute("CREATE TABLE counter(id INTEGER PRIMARY KEY, value INTEGER)").await.unwrap();
        winner.execute("INSERT INTO counter VALUES (1, 0)").await.unwrap();
        winner.execute("CREATE TABLE events(id INTEGER PRIMARY KEY, label TEXT)").await.unwrap();
        ...
        let committed_value = loser.transaction_with_retry(RetryPolicy::default(), async |tx| {
            let value: i64 = tx.query_row("SELECT value FROM counter WHERE id = 1")
                .await?.get_typed(0)?;
            ...
            if observations.len() == 1 {
                ...
                first.execute("UPDATE counter SET value = value + 1 WHERE id = 1").await?;
                first.execute("INSERT INTO events VALUES (1, 'winner')").await?;
                first.commit().await?;
            }
            tx.execute_with_params("UPDATE counter SET value = ?1 WHERE id = 1",
                &[SqliteValue::Integer(value + 1)]).await?;
            tx.execute("INSERT INTO events VALUES (2, 'loser')").await?;
            Ok(value + 1)
        }).await.unwrap();

        assert_eq!(observations, [0, 1], "the loser must rerun its reads, not just COMMIT");
        assert_eq!(committed_value, 2);
        assert!(!winner.in_transaction());
        assert!(!loser.in_transaction());
        let rows = loser.query("SELECT label FROM events ORDER BY id").await.unwrap();
        assert_eq!(rows.len(), 2);
        assert_eq!(rows[0].get_typed::<String>(0).unwrap(), "winner");
        assert_eq!(rows[1].get_typed::<String>(0).unwrap(), "loser");
        assert_eq!(loser.query_row("SELECT value FROM counter WHERE id = 1")
            .await.unwrap().get_typed::<i64>(0).unwrap(), 2);
        assert_eq!(loser.query_row("PRAGMA integrity_check").await.unwrap()
            .get_typed::<String>(0).unwrap(), "ok");
        ...
        let count: i64 = oracle.query_row("SELECT count(*) FROM events", [], |row| row.get(0)).unwrap();
        let counter: i64 = oracle.query_row("SELECT value FROM counter WHERE id = 1", [], |row| row.get(0)).unwrap();
...
fn corruption_is_not_retried_and_partial_writes_are_rolled_back() {
    asupersync::test_utils::run_test(|| async {
        ...
        let failure = conn.transaction_with_retry(RetryPolicy::default(), async |tx| {
            calls.set(calls.get() + 1);
            tx.execute("INSERT INTO t VALUES (1)").await?;
            Err::<(), _>(FrankenError::DatabaseCorrupt { detail: "injected nontransient error".into() })
        }).await.unwrap_err();
        assert_eq!(failure.reason, RetryStopReason::NonTransient);
        assert_eq!(failure.attempts, 1);
        assert_eq!(calls.get(), 1);
        assert!(matches!(failure.last_error.as_deref(), Some(FrankenError::DatabaseCorrupt { .. })));
...
fn invalid_limits_and_an_expired_budget_do_not_begin_or_call_user_code() {
    asupersync::test_utils::run_test(|| async {
        ...
        let cases = [
            (RetryPolicy { max_attempts: 0, ..normal }, RetryStopReason::InvalidPolicy),
            (RetryPolicy { max_rollback_attempts: 0, ..normal }, RetryStopReason::InvalidPolicy),
            (RetryPolicy { max_backoff: Duration::ZERO, ..normal }, RetryStopReason::InvalidPolicy),
            (RetryPolicy { timeout: Duration::ZERO, ..normal }, RetryStopReason::DeadlineExceeded),
        ];
        for (policy, expected) in cases {
            let calls = Cell::new(0);
            let failure = conn.transaction_with_retry(policy, async |_| {
                calls.set(calls.get() + 1);
                Ok(())
            }).await.unwrap_err();
...
fn rejecting_a_nested_call_does_not_rollback_the_callers_transaction() {
    asupersync::test_utils::run_test(|| async {
        ...
        let failure = conn.transaction_with_retry(RetryPolicy::default(), async |_| Ok(()))
            .await.unwrap_err();
...
fn dropping_a_pending_body_rolls_back_before_the_next_sql_statement() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let mut operation = Box::pin(conn.transaction_with_retry(RetryPolicy::default(), async |tx| {
                tx.execute("INSERT INTO t VALUES (1)").await?;
                written.set(true);
            ...
            poll_fn(|cx| {
                ...
                if written.get() { Poll::Ready(()) } else { Poll::Pending }
            }).await;
            assert!(conn.in_transaction());
...
fn dropping_an_unpolled_helper_does_not_start_a_transaction() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert!(!called.get());
        assert!(!conn.in_transaction());
        conn.execute("CREATE TABLE still_usable(id INTEGER)").await.unwrap();
    });
}
...
fn a_body_finishing_after_the_deadline_is_rolled_back_not_committed() {
    asupersync::test_utils::run_test(|| async {
        let conn = Connection::open(":memory:").await.unwrap();
        conn.execute("CREATE TABLE t(value INTEGER)").await.unwrap();
        let policy = RetryPolicy { timeout: Duration::from_millis(100), ..RetryPolicy::default() };
        let failure = conn.transaction_with_retry(policy, async |tx| {
            tx.execute("INSERT INTO t VALUES (1)").await?;
...
fn the_final_failure_does_not_wait_for_another_backoff() {
    asupersync::test_utils::run_test(|| async {
        ...
        let failure = conn.transaction_with_retry(policy, async |_| Err::<(), _>(FrankenError::Busy))
            .await.unwrap_err();
