...
fn run_shape(n: i64, fk_on: bool, composite_unique: bool) -> f64 {
    ...
    conn.execute(PARENT_DDL).unwrap();
    let item_ddl = if composite_unique {
        ITEM_DDL_WITH_UNIQUE
    } else {
        ITEM_DDL_NO_UNIQUE
    };
    conn.execute(item_ddl).unwrap();
    ...
    let rows = conn.query("SELECT COUNT(*) FROM item").unwrap();
...
fn issue_116_quadratic_scaling_repro() {
    ...
    for &n in &sizes {
        fk_unique.push((n, run_shape(n, true, true)));
    }

    println!("\n--- 2x2 isolation at N=2000 ---");
    let iso_n = 2000_i64;
    let t_fk_uniq = run_shape(iso_n, true, true);
    let t_fk_nouniq = run_shape(iso_n, true, false);
    let t_nofk_uniq = run_shape(iso_n, false, true);
    let t_nofk_nouniq = run_shape(iso_n, false, false);
