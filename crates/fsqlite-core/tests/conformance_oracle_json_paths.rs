...
async fn assert_query_matches(fconn: &Connection, rconn: &rusqlite::Connection, sql: &str) {
    let franken = franken_rows(fconn, sql).await;
...
INSERT INTO docs VALUES (1, '{"name":"Ada","age":37}');"#;
        ...
        for sql in cases {
            assert_query_matches(&fconn, &rconn, sql).await;
        }
    });
}
