...
class Target {
  constructor(path = ":memory:", initialize = true) {
    ...
    this.db.exec("PRAGMA recursive_triggers=ON;PRAGMA foreign_keys=ON;PRAGMA busy_timeout=0;");
  ...
  rows() {
    return this.db
      .prepare("SELECT * FROM t ORDER BY id")
      .all()
      .map((row) => ({ ...row }));
  }
  close() {
...
const count = (target) =>
  target.db.prepare(`SELECT count(*) AS n FROM ${CHANGESET_OUTBOX_TABLE}`).get().n;
...
test("source rows and real SQLite binary changeset are atomically recorded", async () => {
  ...
  try {
    const recorded = await outbox.record(async (tx) => {
      await add(tx);
      return 42;
    }, options());
    assert.equal(recorded.replayed, false);
    assert.equal(recorded.value, 42);
    assert.equal(recorded.delivery.sequence, 1n);
    const page = await outbox.pending();
    assert.deepEqual(page, [recorded.delivery]);
    const read = await outbox.read("source:1");
    assert.ok(read.changeset instanceof Uint8Array);
    assert.equal(read.changeset.byteLength, read.delivery.byteLength);
    assert.equal(read.delivery.changes, 1);
    assert.equal(receiver.db.applyChangeset(read.changeset), true);
    ...
    assert.equal((await outbox.read("source:1")).changeset, null);
...
test("redelivery does not execute work or invent the original callback value", async () => {
  const target = new Target(),
    outbox = new ChangesetOutbox(target);
  let called = 0;
  try {
    const work = async (tx) => {
      called++;
      await add(tx);
      return { anything: "not persisted" };
    };
    const first = await outbox.record(work, options()),
      second = await outbox.record(work, options());
    assert.equal(called, 1);
    assert.equal(second.replayed, true);
    assert.equal("value" in second, false);
    assert.deepEqual(second.delivery, first.delivery);
...
test("acknowledgement reclaims payload bytes but retains operation deduplication", async () => {
  ...
  try {
    ...
    const retry = await outbox.record(() => {
      throw new Error("must not replay");
    }, options());
    assert.equal(retry.replayed, true);
    assert.equal(retry.delivery.acknowledged, true);
    assert.equal(
      target.db.prepare(`SELECT length(payload) AS n FROM ${CHANGESET_OUTBOX_TABLE}`).get().n,
      0,
    );
  } finally {
    target.close();
...
test("wrong and unknown acknowledgements do not remove pending payloads", async () => {
  ...
  try {
    const first = await outbox.record(add, options());
    await assert.rejects(outbox.acknowledge("source:1", "0".repeat(64)), {
      code: "ERR_FSQLITE_OUTBOX_ACK",
    });
    await assert.rejects(outbox.acknowledge("missing", first.delivery.sha256), {
      code: "ERR_FSQLITE_OUTBOX_ACK",
    });
...
test("only an exact acknowledged identity can be explicitly forgotten", async () => {
  ...
  try {
    const first = await outbox.record(add, options());
    await assert.rejects(outbox.forgetAcknowledged("source:1", first.delivery.sha256), {
      code: "ERR_FSQLITE_OUTBOX_STATE",
    });
    await outbox.acknowledge("source:1", first.delivery.sha256);
    await assert.rejects(outbox.forgetAcknowledged("source:1", "0".repeat(64)), {
      code: "ERR_FSQLITE_OUTBOX_STATE",
    });
    ...
    assert.equal(await outbox.read("source:1"), null);
    assert.equal(target.rows().length, 1);
    const next = await outbox.record(
      (tx) => tx.execute("INSERT INTO t VALUES(2,'second')"),
      options("source:2"),
    );
...
test("payload pages use monotonic sequences, skip acknowledged entries and never load blobs", async () => {
  ...
  try {
    ...
    for (let i = 0; i < 5; i++)
      items.push(
        await outbox.record(
          (tx) => tx.execute("INSERT INTO t VALUES(?,?)", [BigInt(i), BigInt(i)]),
          options(`source:${i}`),
        ),
      );
    await outbox.acknowledge("source:1", items[1].delivery.sha256);
    target.sql = [];
    const a = await outbox.pending({ limit: 2 }),
      b = await outbox.pending({ limit: 2, after: a.at(-1).sequence });
    assert.deepEqual(
      [...a, ...b].map((x) => x.deliveryId),
      ["source:0", "source:2", "source:3", "source:4"],
    );
    assert.equal(
      target.sql.some((sql) => sql.startsWith("SELECT payload FROM")),
      false,
    );
...
test("payload is owned and verified on every read", async () => {
  ...
  try {
    ...
    const a = await outbox.read("source:1"),
      original = a.changeset.slice();
    a.changeset.fill(0);
...
test("callback failure rolls back application data and creation of a new outbox", async () => {
  const target = new Target(),
    outbox = new ChangesetOutbox(target),
    failure = new Error("work");
  try {
    await assert.rejects(
      outbox.record(async (tx) => {
        await add(tx);
        throw failure;
      }, options()),
      (e) => e === failure,
...
test("failed payload retention rolls back source DML and frees capture TEMP state", async () => {
  const target = new Target(),
    outbox = new ChangesetOutbox(target, { maxPayloadBytes: 1 });
  try {
...
test("retained-entry capacity rejects before callback but permits existing-ID recovery", async () => {
  const target = new Target(),
    outbox = new ChangesetOutbox(target, { maxEntries: 1 });
  let called = false;
  try {
    const first = await outbox.record(add, options());
    await assert.rejects(
    ...
    await outbox.acknowledge("source:1", first.delivery.sha256);
    await assert.rejects(
      outbox.record(() => {}, options("source:2")),
      { code: "ERR_FSQLITE_OUTBOX_FULL" },
    );
...
test("acknowledging payloads restores byte capacity without forgetting IDs", async () => {
  const target = new Target(),
    outbox = new ChangesetOutbox(target, { maxPayloadBytes: 32 });
  try {
    const a = await outbox.record(add, options());
    await assert.rejects(
      outbox.record((tx) => tx.execute("INSERT INTO t VALUES(2,'second')"), options("source:2")),
      { code: "ERR_FSQLITE_OUTBOX_FULL" },
    );
    assert.equal(target.rows().length, 1);
    await outbox.acknowledge("source:1", a.delivery.sha256);
    await outbox.record(
      (tx) => tx.execute("INSERT INTO t VALUES(2,'second')"),
      options("source:2"),
    );
    assert.equal(target.rows().length, 2);
    assert.equal(count(target), 2);
  } finally {
    target.close();
  }
});
test("deferred commit errors roll back both source rows and retained payload", async () => {
  const target = new Target();
  target.db.exec(
    "CREATE TABLE p(id PRIMARY KEY);CREATE TABLE child(id PRIMARY KEY,pid REFERENCES p DEFERRABLE INITIALLY DEFERRED);",
  );
  const outbox = new ChangesetOutbox(target);
  try {
    await assert.rejects(
      outbox.record((tx) => tx.execute("INSERT INTO child VALUES(1,9)"), {
        deliveryId: "source:bad",
        tables: ["child"],
      }),
      /FOREIGN KEY/,
    );
    assert.equal(target.db.prepare("SELECT count(*) AS n FROM child").get().n, 0);
    assert.deepEqual(await outbox.pending(), []);
  } finally {
    target.close();
  }
  ...
  try {
    await assert.rejects(
      target.transaction(async () => {
        const result = await outbox.record(add, options());
        await outbox.acknowledge("source:1", result.delivery.sha256);
...
test("scope identity is case-insensitive and order-independent, but cannot be changed on retry", async () => {
  const target = new Target();
  target.db.exec("CREATE TABLE u(id PRIMARY KEY)");
  const outbox = new ChangesetOutbox(target);
  try {
    ...
    assert.equal(
      (
        await outbox.record(
          ...
          { deliveryId: "source:1", tables: ["t", "U"] },
        )
      ).replayed,
      true,
    );
    ...
    await assert.rejects(
      outbox.record(() => {}, { deliveryId: "source:1", tables: ["t", "u"], indirect: true }),
      { code: "ERR_FSQLITE_OUTBOX_REUSE" },
    );
  } finally {
    target.close();
...
test("lost source commit acknowledgement recovers the retained delivery without repeating SQL", async () => {
  ...
  try {
    ...
    const recovered = await outbox.record(work, options());
...
test("lost local ACK acknowledgement is safe to repeat and retains source deduplication", async () => {
  const target = new Target(),
    normal = new ChangesetOutbox(target);
  try {
    const first = await normal.record(add, options());
    let lose = true;
    const uncertain = new ChangesetOutbox({
      transaction: async (work) => {
        const result = await target.transaction(work);
        if (lose) {
          lose = false;
...
test("file-backed outbox survives a child-process reopen and is independently deliverable", async () => {
  const directory = mkdtempSync(join(tmpdir(), "fsqlite-outbox-")),
  ...
  const child = spawnSync(
    process.execPath,
    [...process.execArgv, "--input-type=module", "-e", program],
    { encoding: "utf8", timeout: 20000 },
  );
  assert.equal(child.status, 0, child.stderr);
  assert.match(child.stdout, /reopened-and-delivered/);
  const reopened = new Target(path, false);
  try {
    const recovered = await new ChangesetOutbox(reopened).read("source:1");
    assert.equal(recovered.delivery.acknowledged, true);
...
test("two file connections overlap at one absent ID; no duplicate source transaction commits", async () => {
  const directory = mkdtempSync(join(tmpdir(), "fsqlite-outbox-race-")),
    path = join(directory, "source.db");
  const a = new Target(path),
    oa = new ChangesetOutbox(a);
  a.db.exec("PRAGMA journal_mode=WAL");
  await oa.record(() => {}, options("setup"));
  const b = new Target(path, false),
    ob = new ChangesetOutbox(b);
  let release, entered;
  ...
  try {
    const pending = oa.record(async (tx) => {
      entered();
      await held;
      await add(tx);
    }, options());
    await started;
    const other = await ob.record(
      (tx) => tx.execute("INSERT INTO t VALUES(2,'winner')"),
      options(),
    );
    assert.equal(other.replayed, false);
    release();
    await assert.rejects(pending, /locked|busy/i);
  ...
  test(`corrupt ${label} rejects without replaying source work`, async () => {
    ...
    try {
      ...
      await assert.rejects(
        ...
        (e) => /^ERR_FSQLITE_OUTBOX_/.test(e.code),
      );
      assert.equal(called, false);
    } finally {
      target.close();
...
test("existing incompatible outbox schema is rejected, not overwritten", async () => {
  const target = new Target();
  target.db.exec(
    `CREATE TABLE ${CHANGESET_OUTBOX_TABLE}(precious);INSERT INTO ${CHANGESET_OUTBOX_TABLE} VALUES(42)`,
  );
  try {
    await assert.rejects(new ChangesetOutbox(target).record(add, options()), {
      code: "ERR_FSQLITE_OUTBOX_SCHEMA",
    });
    assert.equal(
      target.db.prepare(`SELECT precious FROM ${CHANGESET_OUTBOX_TABLE}`).get().precious,
      42,
    );
    assert.deepEqual(target.rows(), []);
  } finally {
...
test("application triggers or new indexes on the outbox are rejected", async () => {
  for (const ddl of [
    `CREATE TRIGGER evil AFTER UPDATE ON ${CHANGESET_OUTBOX_TABLE} BEGIN DELETE FROM t;END`,
    `CREATE INDEX extra ON ${CHANGESET_OUTBOX_TABLE}(sha256)`,
  ]) {
    const target = new Target(),
      outbox = new ChangesetOutbox(target);
    try {
...
test("read-only discovery does not create an outbox", async () => {
  ...
  try {
    assert.deepEqual(await outbox.pending(), []);
    assert.equal(await outbox.read("missing"), null);
    assert.equal(await outbox.forgetAcknowledged("missing", "0".repeat(64)), false);
    assert.equal(
      target.db
        .prepare("SELECT count(*) AS n FROM sqlite_schema WHERE name=?")
        .get(CHANGESET_OUTBOX_TABLE).n,
...
test("empty changesets retain a stable operation receipt and valid empty payload", async () => {
  const target = new Target(),
    outbox = new ChangesetOutbox(target);
  try {
    const first = await outbox.record(() => 42, options());
    assert.equal(first.delivery.changes, 0);
    assert.equal(first.delivery.byteLength, 0);
    assert.deepEqual(decodeChangeset((await outbox.read("source:1")).changeset), []);
    assert.equal(
      (
        await outbox.record(() => {
          throw new Error("replay");
...
test("input failures do not enter a transaction", async () => {
  const outbox = new ChangesetOutbox({
    transaction: () => {
      throw new Error("entered");
    },
  });
  for (const deliveryId of ["", "x".repeat(513), "nul\0", "\ud800"])
    await assert.rejects(
      outbox.record(() => {}, options(deliveryId)),
      { code: "ERR_FSQLITE_OUTBOX_INPUT" },
    );
  for (const page of [{ limit: 0 }, { limit: 257 }, { after: -1n }, { after: 1 }])
    await assert.rejects(outbox.pending(page), { code: "ERR_FSQLITE_OUTBOX_INPUT" });
});

test("outbox insertion failure after captured DML rolls back the source operation", async () => {
  const target = new Target(),
    failure = new Error("injected storage failure");
  const execute = target.execute.bind(target);
  target.execute = async (sql, params) => {
    if (sql.startsWith('INSERT OR ABORT INTO main."__fsqlite_changeset_outbox"')) throw failure;
    return execute(sql, params);
  };
  const outbox = new ChangesetOutbox(target);
  try {
    await assert.rejects(outbox.record(add, options()), (e) => e === failure);
    assert.deepEqual(target.rows(), []);
    assert.deepEqual(await outbox.pending(), []);
    assert.deepEqual(
      target.db
...
test("monotonic outbox sequences preserve signed-int64 precision above 2^53", async () => {
  ...
  try {
    ...
    target.db
      ...
      .run(9007199254740993n, CHANGESET_OUTBOX_TABLE);
    const result = await outbox.record(add, options());
    assert.equal(result.delivery.sequence, 9007199254740994n);
    assert.deepEqual(
      (await outbox.pending({ after: 9007199254740993n })).map((x) => x.sequence),
      [9007199254740994n],
    );
  } finally {
    target.close();
  ...
  try {
    ...
    target.query = async (sql, params) => {
      ...
      if (sql.startsWith("SELECT type, sql FROM main.sqlite_schema") && result.rowArrays.length) {
        result.rowArrays[0][1] = result.rowArrays[0][1]
          .replace(/CREATE TABLE/, "create   table")
          .replace(/AUTOINCREMENT/, "autoincrement");
      }
      return result;
    };
    assert.equal((await outbox.pending()).length, 1);
    target.query = async (sql, params) => {
      const result = await query(sql, params);
      if (sql.startsWith("SELECT type, sql FROM main.sqlite_schema") && result.rowArrays.length) {
        result.rowArrays[0][1] = result.rowArrays[0][1].replace(
          /AUTOINCREMENT/,
          "/* AUTOINCREMENT */",
        );
      }
...
for (const cut of ["before", "after"])
  test(`SIGKILL ${cut} COMMIT preserves the source/outbox atomic decision`, async () => {
    const directory = mkdtempSync(join(tmpdir(), `fsqlite-outbox-${cut}-`)),
      path = join(directory, "source.db");
    const seed = new Target(path);
    seed.close();
    const moduleUrl = new URL("../src/changeset-outbox.ts", import.meta.url).href;
    ...
    assert.match(child.stdout, new RegExp(`reached-${cut}-commit`));
    const target = new Target(path, false),
      outbox = new ChangesetOutbox(target);
    try {
      if (cut === "before") {
        assert.deepEqual(target.rows(), []);
