...
fn values_as_row_source() {
    asupersync::test_utils::run_test(|| async {
        ...
        agree(
            &[],
            "SELECT column1, column2 FROM (VALUES (1,'x'),(2,'y')) ORDER BY column1",
            "VALUES in a derived table",
        )
        .await;
        agree(
            &[],
            "SELECT 1 AS n UNION ALL VALUES (2),(3)",
            "VALUES in a compound",
        )
        .await;
        agree(
            &[],
            "WITH v(a,b) AS (VALUES (1,10),(2,20)) SELECT a, b FROM v ORDER BY a",
            "VALUES in a CTE",
        )
        .await;
        agree(
...
fn jsonb_roundtrip() {
    asupersync::test_utils::run_test(|| async {
        agree(
            &[],
            "SELECT json(jsonb('{\"a\":1,\"b\":[2,3]}'))",
            "jsonb round-trip via json()",
        )
        .await;
        agree(
            &[],
            "SELECT json_extract(jsonb('{\"a\":{\"b\":5}}'), '$.a.b')",
            "json_extract over jsonb",
        )
        .await;
    });
}
...
fn conditional_scalars() {
    asupersync::test_utils::run_test(|| async {
        agree(
            &[],
            "SELECT coalesce(NULL, NULL, 3, 4), coalesce(NULL, NULL)",
            "coalesce",
        )
        .await;
        agree(
            &[],
            "SELECT ifnull(NULL, 'x'), ifnull(5, 'x'), nullif(1,1), nullif(1,2)",
            "ifnull/nullif",
        )
        .await;
        agree(
            &[],
            "SELECT iif(1>0, 'yes', 'no'), iif(0, 'yes', 'no'), iif(NULL, 'y', 'n')",
            "iif",
        )
        .await;
...
fn between_in_typeof_minmax() {
    asupersync::test_utils::run_test(|| async {
        agree(
            ...
            "SELECT x FROM t WHERE x BETWEEN 5 AND 10 ORDER BY x",
            "BETWEEN",
        )
        .await;
        agree(
            &[],
            "SELECT 1 IN (1,2,3), 'a' IN ('x','a'), 5 IN (SELECT 5), NULL IN (1,2)",
            "IN mixed/subquery/NULL",
        )
        .await;
        agree(
            &[],
            "SELECT typeof(1), typeof(1.5), typeof('s'), typeof(NULL), typeof(X'00')",
            "typeof across storage classes",
        )
        .await;
        agree(
            &[],
            "SELECT min(3,1,2), max(3,1,2), min('b','a','c')",
            "scalar (multi-arg) min/max",
        )
        .await;
    });
}
