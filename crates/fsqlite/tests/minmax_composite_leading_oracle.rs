...
fn minmax_composite_leading_matches_sqlite() {
    asupersync::test_utils::run_test(|| async {
        ...
        cmp(&f, &r, "SELECT MAX(a) FROM n").await;
        cmp(&f, &r, "SELECT MIN(a) FROM n").await;
        ...
        cmp(&f, &r, "SELECT MAX(a) FROM e").await;
        cmp(&f, &r, "SELECT MIN(a) FROM e").await;

        // Opcode gate: MAX(a) seeks the composite index end (Last); the NOCASE leading term does not.
        assert!(
            has_op(&f, "SELECT MAX(a) FROM t", "Last").await,
            "MAX(a) (composite leading term) must seek the index end (Last)"
        );
        assert!(
            !has_op(&f, "SELECT MAX(nc) FROM t", "Last").await,
