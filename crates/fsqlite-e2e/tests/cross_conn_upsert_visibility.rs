    ...
    asupersync::test_utils::run_test(|| async {
...

        {
            ...
            c.execute("CREATE TABLE kv (k TEXT PRIMARY KEY, v INTEGER)")
                .await
                .unwrap();
            c.execute("INSERT INTO kv VALUES ('key1', 100)")
                .await
                .unwrap();
        }

        {
            ...
            let rows = c
                .query("SELECT COUNT(*) FROM kv WHERE k='key1'")
                .await
                .unwrap();
            assert_eq!(col(&rows, 0, 0), "1", "DO NOTHING: should still have 1 row");

            let rows = c.query("SELECT v FROM kv WHERE k='key1'").await.unwrap();
            assert_eq!(
                col(&rows, 0, 0),
                "100",
...
fn cross_conn_three_connections_same_key() {
    asupersync::test_utils::run_test(|| async {
        ...
        for i in 1..=3 {
            ...
            let rows = c
                .query("SELECT COUNT(*) FROM t WHERE slug='x'")
                .await
                .unwrap();
            assert_eq!(
                col(&rows, 0, 0),
            ...
            let rows = c.query("SELECT val FROM t WHERE slug='x'").await.unwrap();
            assert_eq!(
                col(&rows, 0, 0),
                format!("{i}"),
