...
fn fresh() -> (Connection, rusqlite::Connection) {
    ...
    for i in 1..=300_i64 {
        let cc = if i % 13 == 0 { "NULL".to_owned() } else { format!("{}", i % 12) }; // NULLs sprinkled in the indexed col
...
fn check(dml: &str, no_rewind: bool) {
    let (f, r) = fresh();
    if no_rewind {
        assert!(!has_op(&f, dml, "Rewind"), "index-eq DML must not full-scan (Rewind): `{dml}`");
    } else {
