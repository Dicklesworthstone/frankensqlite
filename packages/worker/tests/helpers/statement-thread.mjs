...
const fixture = sqliteSnapshotWorker({
  statementFree(sql) {
    if (workerData?.freeFailure && sql.includes('fail_free')) throw new Error('reference finalization failure');
  },
});
parentPort.on('message', request => {
  if (request.testControl === 'stats') {
    parentPort.postMessage({ testControl: 'stats', id: request.id,
      prepared: fixture.host.preparedStatements, requests: fixture.host.requestQueue });
    return;
  }
  void fixture.host.handle(request).then(response => {
    if (response.kind === 'ready') {
      if (workerData?.omitPolicy) delete response.data.preparedStatementLimits;
      if (workerData?.responsePolicy !== undefined) response.data.preparedStatementLimits = workerData.responsePolicy;
    }
