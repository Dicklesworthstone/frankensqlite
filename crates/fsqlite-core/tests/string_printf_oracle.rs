...
fn substr_indexing() {
    asupersync::test_utils::run_test(|| async {
        agree("SELECT substr('hello', 2, 3), substr('hello', 2), substr('hello', 2, 99)", "substr basic + open-ended + past-end length").await;
        agree("SELECT substr('hello', 0, 3)", "substr with start 0").await;
        agree("SELECT substr('hello', -3, 2)", "substr with negative start (from end)").await;
        agree("SELECT substr('hello', 4, -2)", "substr with negative length").await;
        agree("SELECT substr('hi', 5, 3)", "substr starting past the end is empty").await;
    });
}
...
fn instr_replace_trim() {
    asupersync::test_utils::run_test(|| async {
        agree("SELECT instr('hello world', 'o'), instr('hello', 'z'), instr('abcabc', 'bc')", "instr").await;
        agree("SELECT replace('aXbXc', 'X', '-'), replace('aaa', 'a', 'bb'), replace('x', 'y', 'z')", "replace").await;
        agree("SELECT '['||trim('  hi  ')||']', '['||ltrim('  hi')||']', '['||rtrim('hi  ')||']'", "trim/ltrim/rtrim default").await;
        agree("SELECT trim('xxhixx', 'x'), ltrim('...hi', '.'), rtrim('hi!!!', '!')", "trim/ltrim/rtrim with charset").await;
    });
}
...
fn case_length_hex_quote_char() {
    asupersync::test_utils::run_test(|| async {
        agree("SELECT upper('Hello123'), lower('Hello123')", "upper/lower ASCII").await;
        agree("SELECT length('hello'), length(''), length('cafe')", "length in characters").await;
        agree("SELECT hex('AB'), hex(255)", "hex of text and integer").await;
        agree("SELECT quote('a''b'), quote(42), quote(NULL), quote(3.5)", "quote of text/int/null/real").await;
        agree("SELECT char(72, 105), unicode('A'), unicode('z')", "char/unicode").await;
    });
}
...
fn printf_specifiers() {
    asupersync::test_utils::run_test(|| async {
        agree("SELECT printf('%d|%5d|%-5d|%05d', 42, 42, 42, 42)", "printf integer width/left/zero padding").await;
        agree("SELECT printf('%x %X %o', 255, 255, 8)", "printf hex/octal").await;
        agree("SELECT printf('[%s][%10s][%-10s]', 'hi', 'hi', 'hi')", "printf string width").await;
        agree("SELECT printf('%.2f %.0f %e', 3.14159, 2.5, 12345.0)", "printf explicit-precision float").await;
...
fn concat_and_typeof() {
    asupersync::test_utils::run_test(|| async {
        agree("SELECT 'a' || NULL, 'a' || 'b' || 'c'", "|| NULL propagation").await;
        agree("SELECT typeof(substr('x',1)), typeof(length('x')), typeof(printf('%d',1))", "result affinities").await;
    });
}
