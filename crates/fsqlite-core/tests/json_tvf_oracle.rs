...
fn json_each_standalone() {
    asupersync::test_utils::run_test(|| async {
        agree(
            ...
            "json_each over array",
        )
        .await;
        agree(
            &[],
            "SELECT key, value FROM json_each('{\"a\":1,\"b\":2,\"c\":3}') ORDER BY key",
            "json_each over object",
        )
        .await;
        agree(
            &[],
            "SELECT value FROM json_each('{\"arr\":[7,8,9]}', '$.arr') ORDER BY value",
            "json_each with path",
        )
        .await;
        agree(
            &[],
            "SELECT value, type FROM json_each('[1,\"s\",null,true,2.5]') ORDER BY id",
            "json_each value types",
        )
        .await;
    });
}
...
fn deep_paths_and_types() {
    asupersync::test_utils::run_test(|| async {
        agree(
            &[],
            "SELECT json_extract('{\"x\":1,\"y\":2,\"z\":3}', '$.x', '$.z')",
            "json_extract multi-path -> array",
        )
        .await;
        agree(&[], "SELECT json_type('{\"a\":[1,2]}','$.a'), json_type('{\"a\":[1,2]}','$.a[0]'), json_type('{\"o\":{}}','$.o')", "json_type at paths").await;
        agree(
            &[],
            "SELECT json_array_length('{\"a\":{\"b\":[1,2,3,4]}}', '$.a.b')",
            "json_array_length at path",
        )
        .await;
        agree(
            &[],
            "SELECT json_extract('{\"a\":{\"b\":{\"c\":[0,{\"d\":42}]}}}', '$.a.b.c[1].d')",
            "deep json_extract path",
        )
        .await;
        agree(
            &[],
            "SELECT '{\"a\":{\"b\":[10,20]}}' -> '$.a' ->> '$.b[1]'",
            "chained ->/->>",
        )
        .await;
        agree(
            &[],
            "SELECT json_group_array(value) FROM json_each('[3,1,2]')",
            "json_group_array over json_each (integrates bd-json-group-aggregates-pdvx5)",
        )
        .await;
    });
}
    ...
    asupersync::test_utils::run_test(|| async {
        agree(
            &[
                ...
                "INSERT INTO t VALUES (1,'[\"x\",\"y\"]'),(2,'[\"y\",\"z\"]')",
            ],
    ...
    asupersync::test_utils::run_test(|| async {
        agree(
            &[
                ...
                "INSERT INTO t VALUES (1,'[1,2,3]'),(2,'[9]')",
            ],
