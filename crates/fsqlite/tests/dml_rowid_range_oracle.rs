...
fn check(dml: &str, no_rewind: bool) {
    let (f, r) = fresh();
    if no_rewind {
        assert!(!has_op(&f, dml, "Rewind"), "rowid-range DML must not full-scan (Rewind): `{dml}`");
    } else {
        assert!(has_op(&f, dml, "Rewind"), "control DML should full-scan: `{dml}`");
    }
    f.execute(dml).unwrap();
    r.execute_batch(dml).unwrap();
    assert_eq!(frank_state(&f), sqlite_state(&r), "state diverged after `{dml}`");
}
#[test]
fn dml_rowid_range_matches_sqlite() {
    ...
    check("UPDATE t SET id = id + 10000 WHERE id BETWEEN 60 AND 62", true); // rewrites the ROWID
    ...
    check("UPDATE t SET x = 'c' WHERE id BETWEEN 50 AND 100 AND c = 5", false);
}
