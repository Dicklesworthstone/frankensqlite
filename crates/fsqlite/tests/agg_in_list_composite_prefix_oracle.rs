...
fn agg_in_list_composite_prefix_matches_sqlite() {
    asupersync::test_utils::run_test(|| async {
        ...
        cmp(&f, &r, "SELECT COUNT(*) FROM t WHERE a IN (3)", Some(true)).await;
        cmp(&f, &r, "SELECT SUM(v) FROM t WHERE a IN (3)", Some(true)).await;
        ...
        .await;
        cmp(&f, &r, "SELECT SUM(v) FROM t WHERE a IN (999)", Some(true)).await;
        // Single value with many rows.
        cmp(&f, &r, "SELECT COUNT(*) FROM t WHERE a IN (7)", Some(true)).await;
        // MIN/MAX/AVG over the composite prefix seek.
        cmp(&f, &r, "SELECT MIN(v) FROM t WHERE a IN (3, 8)", Some(true)).await;
        cmp(&f, &r, "SELECT MAX(v) FROM t WHERE a IN (3, 8)", Some(true)).await;
        cmp(&f, &r, "SELECT AVG(v) FROM t WHERE a IN (7, 9)", Some(true)).await;

        // Control: single-column index still seeks with the exact 2-field probe (byte-identical path).
        let f2 = Connection::open(":memory:").await.unwrap();
        ...
        cmp(&f, &r, "SELECT COUNT(*) FROM t WHERE v = 100", Some(false)).await;
    });
}
