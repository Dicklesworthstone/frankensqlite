...
async fn named_series() -> Connection {
    ...
    connection.execute("CREATE VIRTUAL TABLE seq USING generate_series").await.unwrap();
    connection
}
    ...
    asupersync::test_utils::run_test(|| async {
        ...
        let rows = integer_rows(&connection,
            "SELECT value, start, stop, step FROM seq \
             WHERE stop=11 AND value>=4 AND step=2 AND start=1 AND value<=8 ORDER BY value", 4).await;
        assert_eq!(rows, vec![vec![5,1,11,2], vec![7,1,11,2]]);
        let rows = integer_rows(&connection,
            "SELECT value FROM seq WHERE start=2 AND step=0 AND stop=4 ORDER BY value", 1).await;
...
fn duplicate_hidden_and_unconsumed_value_constraints_remain_effective() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert!(connection.query(
            "SELECT value FROM seq WHERE start=1 AND start=3 AND stop=7").await.unwrap().is_empty());
        let rows = integer_rows(&connection,
            "SELECT value FROM seq WHERE start=1 AND stop=9 AND step=2 \
             AND value>=2 AND value>=5 AND value<>7 ORDER BY value", 1).await;
    ...
    asupersync::test_utils::run_test(|| async {
        ...
        let rows = integer_rows(&connection,
            "SELECT value, start, stop, step FROM seq WHERE start=10 AND stop=-10 AND step=-3 \
             AND value>-4.5 AND value<=6.25 ORDER BY value", 4).await;
        assert_eq!(rows, vec![vec![-2,10,-10,-3],vec![1,10,-10,-3],vec![4,10,-10,-3]]);
        let rows = integer_rows(&connection,
            "SELECT value FROM seq WHERE start=10 AND stop=-10 AND step=-3 \
             AND rowid>=1 AND rowid<=7 ORDER BY value DESC", 1).await;
...
fn real_predicates_preserve_fractional_and_large_integer_boundaries() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert!(connection.query(
            "SELECT value FROM seq WHERE start=0 AND stop=5 AND value=2.5").await.unwrap().is_empty());
        let rows = integer_rows(&connection,
            "SELECT value FROM seq WHERE start=9007199254740991 AND stop=9007199254740995 \
             AND value>9007199254740992.0 ORDER BY value", 1).await;
        assert_eq!(rows, vec![vec![9_007_199_254_740_993],vec![9_007_199_254_740_994],vec![9_007_199_254_740_995]]);
    });
}
...
fn empty_and_failed_scans_do_not_contaminate_later_scans() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert!(connection.query("SELECT value FROM seq WHERE stop=5").await.is_err());
        assert_eq!(integer_rows(&connection,
            "SELECT value FROM seq WHERE start=2 AND stop=3 ORDER BY value", 1).await,
            vec![vec![2],vec![3]]);
        assert_eq!(integer_rows(&connection,
            "SELECT value FROM generate_series(2,4) ORDER BY value", 1).await,
...
fn constrained_series_aggregates_and_limits_use_only_matching_rows() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert_eq!(integer_rows(&connection,
            "SELECT value FROM generate_series(9,1,-2) ORDER BY value", 1).await,
