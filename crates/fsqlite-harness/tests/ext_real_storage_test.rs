...
async fn query_col_text(conn: &Connection, sql: &str) -> String {
    let rows = conn.query(sql).await.expect("query should succeed");
...
async fn query_col_blob(conn: &Connection, sql: &str) -> Vec<u8> {
    let rows = conn.query(sql).await.expect("query should succeed");
    assert!(!rows.is_empty(), "expected at least one row from: {sql}");
    match rows[0].get(0).expect("column 0") {
...
async fn row_count(conn: &Connection, sql: &str) -> usize {
    conn.query(sql).await.expect("query should succeed").len()
}
...
fn json_text_storage_round_trip() {
    asupersync::test_utils::run_test(|| async {
...

        {
            ...
            let stored = query_col_text(&conn, "SELECT data FROM docs WHERE id = 1").await;
    ...
    asupersync::test_utils::run_test(|| async {
...

        {
            ...
            conn.execute("CREATE TABLE cfg (key TEXT PRIMARY KEY, val TEXT)")
                ...
                .expect("create");
            conn.execute(
                r#"INSERT INTO cfg VALUES ('settings', '{"theme":"dark","font_size":14}')"#,
            )
            .await
            .expect("insert");
        }

        {
            let conn = Connection::open(&path).await.expect("reopen");
            let original =
                query_col_text(&conn, "SELECT val FROM cfg WHERE key = 'settings'").await;

            let updated =
                json_ext::json_set(&original, &[("$.font_size", SqliteValue::Integer(16))])
                    .expect("json_set");
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen2");
            let stored = query_col_text(&conn, "SELECT val FROM cfg WHERE key = 'settings'").await;
...
fn json_array_storage_and_inspection() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen");
            let stored = query_col_text(&conn, "SELECT items FROM lists WHERE id = 1").await;
...
fn json_object_construction_round_trip() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen");
            let stored = query_col_text(&conn, "SELECT data FROM objects WHERE id = 1").await;
...
fn json_remove_and_patch_persistence() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen");
            let original = query_col_text(&conn, "SELECT data FROM patches WHERE id = 1").await;
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen2");
            let stored = query_col_text(&conn, "SELECT data FROM patches WHERE id = 1").await;
...
fn session_changeset_encode_store_decode() {
    asupersync::test_utils::run_test(|| async {
...

        {
            let conn = Connection::open(&path).await.expect("reopen");
            let stored_blob =
                query_col_blob(&conn, "SELECT data FROM changesets WHERE id = 1").await;
...
fn session_changeset_invert_round_trip() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen");
            let blob = query_col_blob(&conn, "SELECT data FROM inv WHERE id = 1").await;
...
fn session_patchset_round_trip() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen");
            let stored = query_col_blob(&conn, "SELECT data FROM patches WHERE id = 1").await;
...
fn misc_decimal_precision_round_trip() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen");
            let stored = query_col_text(&conn, "SELECT val FROM decimals WHERE id = 1").await;
...
fn misc_uuid_round_trip() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen");
            let stored = query_col_text(&conn, "SELECT val FROM uuids WHERE id = 1").await;
...
fn misc_uuid_blob_text_conversion_round_trip() {
    asupersync::test_utils::run_test(|| async {
...

        {
            let conn = Connection::open(&path).await.expect("reopen");
            let stored_blob =
                query_col_blob(&conn, "SELECT data FROM uuid_blobs WHERE id = 1").await;
...
fn fts3_query_parsing_on_stored_content() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen");
            let _stored = query_col_text(&conn, "SELECT content FROM notes WHERE id = 1").await;
...
fn multi_extension_data_on_single_db() {
    asupersync::test_utils::run_test(|| async {
...

        {
            ...
            conn.execute(
                r#"INSERT INTO json_store VALUES (1, '{"type":"point","x":5.0,"y":10.0}')"#,
            )
            .await
            .expect("insert json");
...

        {
            ...
            assert_eq!(row_count(&conn, "SELECT * FROM json_store").await, 1);
            assert_eq!(row_count(&conn, "SELECT * FROM spatial_store").await, 1);
            assert_eq!(row_count(&conn, "SELECT * FROM change_log").await, 1);
            assert_eq!(row_count(&conn, "SELECT * FROM docs").await, 1);

            // JSON: extract point
            let json_data = query_col_text(&conn, "SELECT data FROM json_store WHERE id = 1").await;
            ...
            let cs_blob = query_col_blob(&conn, "SELECT cs FROM change_log WHERE id = 1").await;
            ...
            let body = query_col_text(&conn, "SELECT body FROM docs WHERE id = 1").await;
            let tokenizer = Unicode61Tokenizer::new();
...
fn multiple_reopen_cycles_preserve_extension_data() {
    asupersync::test_utils::run_test(|| async {
        ...
        // Cycle 2: mutate
        {
            let conn = Connection::open(&path).await.expect("reopen1");
            let j = query_col_text(&conn, "SELECT j FROM jdata WHERE id = 1").await;
            let modified =
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen3");
            let j = query_col_text(&conn, "SELECT j FROM jdata WHERE id = 1").await;
            let val = json_ext::json_extract(&j, &["$.v"]).expect("extract v");
...
fn large_json_document_storage_integrity() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen");
            let stored = query_col_text(&conn, "SELECT data FROM large WHERE id = 1").await;
            let len = json_ext::json_array_length(&stored, None).expect("array_length");
...
fn jsonb_blob_storage_round_trip() {
    asupersync::test_utils::run_test(|| async {
...

        {
            let conn = Connection::open(&path).await.expect("reopen");
            let stored_blob =
                query_col_blob(&conn, "SELECT data FROM jsonb_store WHERE id = 1").await;
            assert_eq!(
...
fn session_concat_multi_table_round_trip() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&path).await.expect("reopen");
            let blob = query_col_blob(&conn, "SELECT data FROM multi_cs WHERE id = 1").await;
