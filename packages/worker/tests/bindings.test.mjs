...
import { MAX_BIND_PARAMETERS, parameterLayout, resolveBindings } from "../src/bindings.ts";
...
async function fixture(t) {
  ...
  const ok = async (message) => {
    ...
    if (response.kind === "error") assert.fail(JSON.stringify(response.error));
    return response;
  };
...
test("layout matches independent SQLite parameter metadata including holes and aliases", async (t) => {
  ...
  for (const sql of [
    ...
    "SELECT ?,?1",
    "SELECT ?01,?1,?",
    "SELECT $::name(suffix), :a::b, @a(z), :a$b",
    ...
    const oracle = await f.rpc({ op: "prepare", sql }),
      layout = parameterLayout(sql);
    ...
    assert.ok(Object.isFrozen(layout.names));
  }
});
...
test("complete binding supports exact and unambiguous bare names without rewriting slots", () => {
  ...
  assert.deepEqual(
    resolveBindings(p, { "?2": 2, ":x": 3, "?1": 1, "@x": 4, tail: 5 }),
    [1, 2, 3, 4, 5],
  );
  assert.deepEqual(resolveBindings(parameterLayout("SELECT ?5,:v"), { "?5": 5, v: 6 }), [
    null,
    null,
    null,
    null,
...
test("ambiguous, missing, duplicate and unknown names fail explicitly", () => {
  for (const [sql, params, code] of [
    ["SELECT :x,@x", { x: 1 }, "NAME"],
    ["SELECT :x", { x: 1, ":x": 2 }, "NAME"],
    ["SELECT ?01,?1", { "?01": 1, "?1": 1 }, "NAME"],
    ["SELECT :x", { y: 1 }, "NAME"],
    ["SELECT :x", {}, "ARITY"],
    ["SELECT ?", {}, "ARITY"],
  ])
    assert.throws(() => resolveBindings(parameterLayout(sql), params), {
      code: `ERR_FSQLITE_BINDING_${code}`,
    });
});
...
test("strict positional resolver checks actual slot count and scalar types", () => {
  ...
  for (const values of [[], [3], [1, 2, 3, 4]])
    assert.throws(() => resolveBindings(p, values), { code: "ERR_FSQLITE_BINDING_ARITY" });
  for (const value of [undefined, {}, Symbol(), 1n << 63n, -(1n << 63n) - 1n]) {
    assert.throws(() => resolveBindings(parameterLayout("SELECT ?"), [value]), {
      code: "ERR_FSQLITE_BINDING_INPUT",
    });
  }
  assert.throws(() => resolveBindings(parameterLayout("SELECT ?"), new Array(1)), {
    code: "ERR_FSQLITE_BINDING_INPUT",
  });
});
...
test("parameter bombs and malformed tokens are bounded before core work", () => {
  for (const sql of [
    "SELECT ?0",
    `SELECT ?${MAX_BIND_PARAMETERS + 1}`,
    `SELECT ?${"9".repeat(1000)}`,
    "SELECT 'unfinished",
    "SELECT $a(unclosed",
    "SELECT $a(with space)",
    "SELECT :",
...
test("prepared bindings reuse a real native statement and reset values between calls", async (t) => {
  ...
  assert.equal(p.data.parameterCount, 1);
  assert.deepEqual(p.data.parameterNames, [":v"]);
  ...
  assert.equal(
    (await f.send({ kind: "statement-query", statementId: p.data.statementId, params: { v: 2 } }))
      .kind,
    "error",
  );
});
...
test("missing or extra named values and script tails never execute a partial write", async (t) => {
  ...
  for (const params of [{ id: 1 }, { id: 1, v: "x", oops: 0 }]) {
    ...
    assert.equal(
      (await f.send({ kind: "query", sql: "INSERT INTO t VALUES(:id,:v) RETURNING id", params }))
        .kind,
      "error",
    );
  ...
  assert.equal(
    (
      await f.send({
        kind: "execute",
        sql: "INSERT INTO t VALUES(:id,:v); COMMIT",
        params: { id: 1, v: "x" },
      })
    ).kind,
    "error",
...
test("named transport captures scalars, charges full backing buffers and excludes inherited keys", () => {
  ...
  params.__proto__ = "safe";
  params.v = 9;
  const admission = budget.admit({
    kind: "query",
    requestId: 1,
    sql: "SELECT :__proto__,:v",
    params,
  });
  params.v = 10;
  assert.equal(admission.request.params.v, 9);
  assert.equal(admission.request.params.__proto__, "safe");
  admission.release();
  ...
  assert.throws(
    () =>
      budget.admit({
        ...
        requestId: 2,
        sql: "SELECT :v",
        params: { v: new Uint8Array(1024).subarray(0, 1) },
      }),
...
test("quoted identifiers, comments and Tcl suffixes do not cause false bindings", async (t) => {
  ...
  const data = (
    await f.ok({ kind: "query", sql, params: { x: "hi", "$::a(b-c)": 2, ":a::b": 3, "@a(z)": 4 } })
  ).data;
