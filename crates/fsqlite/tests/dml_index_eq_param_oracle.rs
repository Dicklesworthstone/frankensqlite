...
async fn check_param(dml: &str, params: &[SqliteValue], no_rewind: bool) {
    let (f, r) = fresh().await;
    if no_rewind {
        assert!(
...
fn dml_index_eq_param_matches_sqlite() {
    asupersync::test_utils::run_test(|| async {
        ...
        .await; // text 'abc' -> no match
        check_param("DELETE FROM t WHERE c = ?1", &[sv(SqliteValue::Null)], true).await; // NULL -> matches nothing
...
async fn fresh_text(collation: &str) -> (Connection, rusqlite::Connection) {
    let f = Connection::open(":memory:").await.unwrap();
    let r = rusqlite::Connection::open_in_memory().unwrap();
    let ddl = [
...
async fn ut_state_f(c: &Connection) -> Vec<Vec<String>> {
    let mut r: Vec<Vec<String>> = c
...
async fn check_text(collation: &str, dml: &str, params: &[SqliteValue], no_rewind: bool) {
    let (f, r) = fresh_text(collation).await;
    if no_rewind {
        assert!(
            !has_op(&f, dml, "Rewind").await,
...
fn dml_index_eq_param_text_matches_sqlite() {
    asupersync::test_utils::run_test(|| async {
        ...
        .await; // + residual
        check_text("", "DELETE FROM ut WHERE 'n7' = name", &[], true).await; // literal value on the LHS, no params
...
async fn fresh_num() -> (Connection, rusqlite::Connection) {
    let f = Connection::open(":memory:").await.unwrap();
    let r = rusqlite::Connection::open_in_memory().unwrap();
    for s in [
...
async fn nt_state_f(c: &Connection) -> Vec<Vec<String>> {
    let mut r: Vec<Vec<String>> = c
...
async fn check_num(dml: &str, params: &[SqliteValue], no_rewind: bool) {
    let (f, r) = fresh_num().await;
    if no_rewind {
        assert!(
            !has_op(&f, dml, "Rewind").await,
...
fn dml_index_eq_param_numeric_matches_sqlite() {
    asupersync::test_utils::run_test(|| async {
        ...
        .await; // real 3.0 (whole)
        check_num(
            "DELETE FROM nt WHERE amt = ?1",
            &[SqliteValue::Integer(3)],
            true,
        )
        .await; // int 3 -> REAL 3.0
        check_num(
            "DELETE FROM nt WHERE amt = ?1",
        ...
        .await; // no match
        check_num("DELETE FROM nt WHERE amt = ?1", &[SqliteValue::Null], true).await; // NULL -> none
        check_num(
