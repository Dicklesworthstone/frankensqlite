...
async fn integrity_ok(c: &Connection, label: &str) {
    let rows = c.query("PRAGMA integrity_check").await.unwrap();
...
fn update_rewrite_keeps_every_row_rowid_seek_reachable() {
    asupersync::test_utils::run_test(|| async {
        ...
        for i in 1..=ROWS {
            let probe =
                format!("SELECT count(*) FROM t WHERE id = {i} AND length(v) = {NEW_BYTES}");
            let f = count_f(&frank, &probe).await;
        ...
        assert_eq!(count_f(&frank, "SELECT count(*) FROM t").await, ROWS);
        assert_eq!(count_r(&oracle, "SELECT count(*) FROM t"), ROWS);
        integrity_ok(&frank, "update rewrites").await;
    });
}
...
fn update_rewrite_overflow_rows_stay_seek_reachable() {
    asupersync::test_utils::run_test(|| async {
        ...
        integrity_ok(&frank, "overflow load").await;
        ...
        for i in 1..=OVERFLOW_ROWS {
            let probe =
                format!("SELECT count(*) FROM t WHERE id = {i} AND length(v) = {OVERFLOW_NEW}");
            let f = count_f(&frank, &probe).await;
            let r = count_r(&oracle, &probe);
            assert_eq!(
                f, r,
        ...
        assert_eq!(
            count_f(&frank, "SELECT count(*) FROM t").await,
            OVERFLOW_ROWS
        );
        integrity_ok(&frank, "overflow update rewrites").await;
    });
}
