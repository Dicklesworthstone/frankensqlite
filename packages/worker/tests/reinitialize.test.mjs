...
async function fixture(t, hooks = {}, config = {}) {
  ...
  let created = 0, loaded = 0, exported = 0, requestId = 0;
  const open = async bytes => {
    ...
    try { raw.prepare('SELECT name FROM sqlite_schema').all(); }
    catch (cause) { raw.close(); throw cause; }
    const record = { generation, raw, closed: false, closes: 0, frees: 0, statementFrees: 0 };
    const query = (sql, params = []) => {
      const statement = raw.prepare(sql), columns = statement.columns().map(c => c.name);
      const rows = statement.all(...params);
      return { columns, columnCount: columns.length, columnTypes: [], rows,
        rowArrays: rows.map(row => columns.map(c => row[c])), changes: 0 };
    };
    const handle = {
      get path() { return hooks.path ? hooks.path(generation, path) : path; },
      close() {
        record.closes++; events.push(`close:${generation}`); hooks.close?.(generation);
        if (!record.closed) { raw.close(); record.closed = true; }
      },
      free() {
        record.frees++; events.push(`free:${generation}`); hooks.free?.(generation);
        if (!record.closed) { raw.close(); record.closed = true; }
      },
      ...
      async prepare(sql) {
        const statement = raw.prepare(sql), columns = statement.columns().map(c => c.name);
        let freed = false;
        const check = () => { if (freed) throw new Error('statement freed'); };
        return { sql, columnCount: columns.length, columnNames: () => columns,
          free() { assert.equal(freed, false); freed = true; record.statementFrees++; hooks.statementFree?.(generation); },
          async execute() { check(); return Number(statement.run().changes); },
          async executeWithParams(params) { check(); return Number(statement.run(...params).changes); },
          async query() { check(); return query(sql); }, async queryWithParams(params) { check(); return query(sql, params); },
        };
      },
      async export() {
  ...
  const host = new WorkerConnectionHost({ async load() {
    loaded++; await hooks.load?.(loaded);
  ...
  t.after(async () => {
    ...
    for (const record of records) if (!record.closed) { record.raw.close(); record.closed = true; }
  });
  const initialize = next => send({ kind: 'init', config: next });
  ok(await initialize(config));
  ok(await send({ kind: 'execute-batch', sql: 'CREATE TABLE items(v); INSERT INTO items VALUES(7);' }));
  const statementId = ok(await send({ kind: 'prepare', sql: 'SELECT v FROM items' })).data.statementId;
  const query = async () => ok(await send({ kind: 'query', sql: 'SELECT v FROM items' })).data.rowArrays;
  return { host, send, initialize, query, records, events, storage, statementId, hooks, directory };
}
function malformedImage() {
  const bytes = new Uint8Array(512);
  bytes.set(new TextEncoder().encode('SQLite format 3\0')); bytes[16] = 2;
...
for (const mode of ['import', 'create', 'loader']) {
  test(`failed ${mode} retains unsaved data and the existing prepared handle`, async t => {
    ...
    if (mode === 'create') f.hooks.beforeOpen = () => { throw new Error('create failed'); };
    if (mode === 'loader') f.hooks.load = () => { throw new Error('loader failed'); };
    const result = await f.initialize(mode === 'import' ? { snapshot: malformedImage() } : {});
    error(result);
    assert.deepEqual(await f.query(), [[9]]);
    assert.deepEqual(ok(await f.send({ kind: 'statement-query', statementId: f.statementId })).data.rowArrays, [[9]]);
    assert.equal(f.records[0].closes, 0); assert.equal(f.records[0].statementFrees, 0);
    const bytes = ok(await f.send({ kind: 'export' })).data;
    const path = join(f.directory, 'retained.sqlite'); await writeFile(path, bytes);
...
test('queued requests wait for staged failure, then continue against the retained database', async t => {
  ...
  const f = await fixture(t);
  f.hooks.beforeOpen = async () => { entered.resolve(); await release.promise; throw new Error('late import failure'); };
  const init = f.initialize({ snapshot: malformedImage() });
  await entered.promise;
  const query = f.query(); let settled = false; void query.then(() => { settled = true; });
  await turn(); assert.equal(settled, false); assert.equal(f.host.requestQueue.pendingRequests, 2);
  assert.equal(f.records[0].closes, 0); release.resolve();
  assert.match(error(await init).message, /late import failure/);
  assert.deepEqual(await query, [[7]]); assert.equal(f.host.requestQueue.pendingRequests, 0);
});
...
test('successful replacement publishes once and invalidates old prepared handles without id reuse', async t => {
  ...
  assert.deepEqual(await f.query(), [[7]]);
  assert.deepEqual([f.records[0].closes, f.records[0].frees, f.records[0].statementFrees], [1,1,1]);
  ...
  assert.deepEqual(ok(await f.send({ kind: 'statement-query', statementId: newId })).data.rowArrays, [[7]]);
});
...
test('a loader cannot return the live handle as a candidate and cause self-destruction', async t => {
  ...
  assert.deepEqual(await f.query(), [[7]]);
  assert.deepEqual([f.records[0].closes, f.records[0].frees, f.records[0].statementFrees], [0,0,0]);
});
...
test('candidate destructor failures retain every cause without closing the healthy previous session', async t => {
  const f = await fixture(t);
  f.hooks.path = generation => { if (generation === 2) throw new Error('candidate path'); return 'old'; };
  f.hooks.close = generation => { if (generation === 2) throw new Error('candidate close'); };
  f.hooks.free = generation => { if (generation === 2) throw new Error('candidate free'); };
  ...
  assert.deepEqual(failure.cleanupErrors.map(e => e.message), ['candidate close', 'candidate free']);
  assert.deepEqual(await f.query(), [[7]]); assert.equal(f.records[0].closes, 0);
  assert.deepEqual([f.records[1].closes, f.records[1].frees], [1,1]);
});

test('old-session teardown failure never publishes the candidate and fences queued SQL', async t => {
  const f = await fixture(t);
  f.hooks.statementFree = generation => { if (generation === 1) throw new Error('old statement'); };
  f.hooks.close = generation => { if (generation === 1) throw new Error('old close'); };
  f.hooks.free = generation => { if (generation === 1) throw new Error('old free'); };
  ...
  assert.deepEqual(failure.cause.cleanupErrors.map(e => e.message), ['old close', 'old free']);
  assert.equal(failure.userRecoverable, false); error(await tail, fatal);
  assert.deepEqual([f.records[0].closes, f.records[0].frees, f.records[0].statementFrees], [1,1,1]);
  assert.deepEqual([f.records[1].closes, f.records[1].frees], [1,1]);
});
...
for (const value of [undefined, null, false, 0, '']) {
  test(`a falsy thrown destructor value (${String(value)}) cannot become successful initialization`, async t => {
    const f = await fixture(t);
    f.hooks.statementFree = generation => { if (generation === 1) throw value; };
...
test('transport failure during staging joins candidate creation and closes both generations', async t => {
  ...
  f.hooks.beforeOpen = async () => { entered.resolve(); await release.promise; };
  const init = f.initialize({}); await entered.promise;
  const close = f.host.failTransport(new Error('transport disappeared'));
  await turn(); assert.equal(f.records[0].closes, 0);
  release.resolve();
  assert.match(error(await init).message, /transport disappeared/); ok(await close);
  assert.deepEqual(f.records.map(r => [r.closes,r.frees]), [[1,1],[1,1]]);
});
...
test('ordinary close queued after initialization drains the successful replacement in FIFO order', async t => {
  ...
  await turn(); assert.equal(f.records[0].closes, 0); release.resolve();
  ok(await init, 'ready'); ok(await close, 'close-result');
  assert.deepEqual(f.records.map(r => [r.closes,r.frees]), [[1,1],[1,1]]);
});
...
test('failed snapshot replacement preserves the original store and checkpoint parent revision', async t => {
  ...
  error(await f.initialize({ dbName: 'replacement', persistence: 'indexeddb-snapshot', snapshot: malformedImage() }));
...
test('managed transaction ownership rejects replacement before touching either generation', async t => {
  ...
  assert.deepEqual(await f.query(), [[12]]); assert.equal(f.records.length, 1);
});
