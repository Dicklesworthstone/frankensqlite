...
fn fresh() -> (Connection, rusqlite::Connection) {
    ...
    for i in 1..=300_i64 {
        let cc = if i % 13 == 0 { "NULL".to_owned() } else { format!("{}", i % 12) };
        let s = format!("INSERT INTO t VALUES ({i}, {}, {}, 'v{}');", i % 20, cc, i % 7);
        f.execute(&s).unwrap(); r.execute_batch(&s).unwrap();
    }
    (f, r)
}
...
fn check_param(dml: &str, params: &[SqliteValue], no_rewind: bool) {
    let (f, r) = fresh();
    if no_rewind {
        assert!(!has_op(&f, dml, "Rewind"), "parameterized index-eq must not full-scan (Rewind): `{dml}`");
    }
    f.execute_with_params(dml, params).unwrap_or_else(|e| panic!("frank `{dml}` params {params:?}: {e}"));
    ...
    assert_eq!(frank_state(&f), sqlite_state(&r), "state diverged after `{dml}` params {params:?}");
}
#[test]
fn dml_index_eq_param_matches_sqlite() {
    ...
    check_param("DELETE FROM t WHERE c = ?1", &[sv(SqliteValue::Integer(5))], true);   // integer -> c=5
    check_param("DELETE FROM t WHERE c = ?1", &[sv(SqliteValue::Integer(0))], true);   // many matches
    check_param("DELETE FROM t WHERE c = ?1", &[sv(SqliteValue::Integer(999))], true); // no match
    check_param("DELETE FROM t WHERE c = ?1", &[sv(SqliteValue::Float(5.0))], true);   // real 5.0 -> 5
    check_param("DELETE FROM t WHERE c = ?1", &[sv(SqliteValue::Float(5.5))], true);   // real 5.5 -> no match
    check_param("DELETE FROM t WHERE c = ?1", &[sv(SqliteValue::Text("5".into()))], true);   // text '5' -> 5
    check_param("DELETE FROM t WHERE c = ?1", &[sv(SqliteValue::Text("abc".into()))], true); // text 'abc' -> no match
    ...
    check_param("UPDATE t SET x = ?2 WHERE c = ?1", &[sv(SqliteValue::Integer(7)), sv(SqliteValue::Text("z".into()))], true);
    check_param("UPDATE t SET c = ?2 WHERE c = ?1", &[sv(SqliteValue::Integer(3)), sv(SqliteValue::Integer(103))], true);
