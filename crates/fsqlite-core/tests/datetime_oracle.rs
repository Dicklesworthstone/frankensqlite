...
fn day_arithmetic() {
    asupersync::test_utils::run_test(|| async {
        agree(
            "SELECT date('2024-02-27', '+3 days')",
            "+3 days across month boundary",
        )
        .await;
        agree(
            "SELECT date('2024-03-01', '-1 day')",
            "-1 day back across month boundary",
        )
        .await;
    });
}
...
fn start_of_modifiers() {
    asupersync::test_utils::run_test(|| async {
        agree(
            "SELECT date('2024-06-17', 'start of month')",
            "start of month",
        )
        .await;
        agree(
            "SELECT date('2024-06-17', 'start of year')",
            "start of year",
        )
        .await;
        agree(
            "SELECT datetime('2024-06-17 09:30:00', 'start of day')",
            "start of day",
        )
        .await;
    });
...
fn weekday_modifier() {
    asupersync::test_utils::run_test(|| async {
        ...
        agree(
            "SELECT date('2024-06-17', 'weekday 0')",
            "weekday 0 advances to next Sunday",
        )
        .await;
        agree(
            "SELECT date('2024-06-17', 'weekday 1')",
            "weekday 1 on Monday stays same day",
        )
        .await;
    });
}
...
fn strftime_specifiers() {
    asupersync::test_utils::run_test(|| async {
        agree(
            "SELECT strftime('%Y-%m-%d %H:%M:%S', '2024-01-15 12:34:56')",
            "strftime full",
        )
        .await;
        agree(
...
fn time_arithmetic_and_julianday() {
    asupersync::test_utils::run_test(|| async {
        ...
        agree(
            "SELECT datetime('2024-01-15 23:30:00', '+90 minutes')",
            "+90 minutes across midnight",
        )
        .await;
        agree(
            "SELECT CAST(julianday('2024-03-01') - julianday('2024-02-01') AS INTEGER)",
            "julianday difference in days",
        )
        .await;
        agree(
            "SELECT date('2024-01-15', 'start of month', '+1 month', '-1 day')",
            "chained modifiers = last day of month",
        )
        .await;
    });
}
