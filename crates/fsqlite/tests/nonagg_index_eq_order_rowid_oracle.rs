...
async fn frank_ord(c: &Connection, sql: &str) -> Vec<Vec<String>> {
    c.query(sql)
        .await
...
async fn opcode_present(c: &Connection, sql: &str, prefix: &str) -> bool {
...
async fn opcode_count(c: &Connection, sql: &str, opcode: &str) -> usize {
...
async fn has_seek(c: &Connection, sql: &str) -> bool {
    opcode_present(c, sql, "Seek").await
}
async fn has_sorter(c: &Connection, sql: &str) -> bool {
    opcode_present(c, sql, "Sorter").await
}
...
async fn cmp_ord(f: &Connection, r: &rusqlite::Connection, sql: &str, l: &str) {
    assert_eq!(
        frank_ord(f, sql).await,
        sqlite_ord(r, sql),
...
fn nonagg_index_eq_order_rowid_matches_sqlite() {
    asupersync::test_utils::run_test(|| async {
        ...
        for sql in [
            ...
            cmp_ord(&f, &r, sql, "idx-eq-declines").await;
        }
        // Bare eq (no ORDER BY) still seeks (unchanged).
        assert!(
            has_seek(&f, "SELECT * FROM t WHERE c = 5").await,
        ...
        for sql in [
            ...
            cmp_ord(&f2, &r2, sql, "composite-declines").await;
        }
    });
}
...
fn index_eq_rowid_order_respects_hints_and_collations() {
    asupersync::test_utils::run_test(|| async {
        ...
        for sql in [
            ...
            cmp_ord(&h, &hr, sql, "hint-isolation").await;
            assert_eq!(
                opcode_count(&h, sql, "SeekGE").await,
                0,
                "hinted path must not take idx_h_c: `{sql}`"
            );
            assert_eq!(
                opcode_count(&h, sql, "SeekLE").await,
                0,
                "hinted path must not take idx_h_c: `{sql}`"
            );
        }
        ...
        for sql in [
            ...
            ins(&binary, &binary_r, sql).await;
        }
        let binary_sql = "SELECT id, c FROM b WHERE c = 'a' ORDER BY id DESC";
        cmp_ord(&binary, &binary_r, binary_sql, "binary-column-nocase-index").await;
        assert_eq!(
            opcode_count(&binary, binary_sql, "SeekLE").await,
            0,
            "mismatched NOCASE index must decline"
        );
        assert_eq!(
            opcode_count(&binary, binary_sql, "SeekGE").await,
            0,
            "mismatched NOCASE index must decline"
        );
        ...
        for sql in [
            ...
            ins(&nocase, &nocase_r, sql).await;
        }
        let nocase_sql = "SELECT id, c FROM n WHERE c = 'a' ORDER BY id DESC";
        cmp_ord(&nocase, &nocase_r, nocase_sql, "nocase-column-binary-index").await;
        assert_eq!(
            opcode_count(&nocase, nocase_sql, "SeekLE").await,
            0,
            "mismatched BINARY index must decline"
        );
        assert_eq!(
            opcode_count(&nocase, nocase_sql, "SeekGE").await,
            0,
            "mismatched BINARY index must decline"
        );
        ...
        for sql in [
            ...
            ins(&matched, &matched_r, sql).await;
        }
        ...
        .await;
        assert!(
            opcode_count(&matched, matched_sql, "SeekLE").await > 0,
            "matching NOCASE index must SeekLE"
        );
        assert!(
            opcode_count(&matched, matched_sql, "Prev").await > 0,
            "matching NOCASE index must walk backward"
        );
        assert!(
            !has_sorter(&matched, matched_sql).await,
