...
export class FrankenWorkerClient {
  ...
  readonly #onMessage = (event: WorkerMessageEvent): void => {
    ...
    try {
      ...
      if (source.kind === "worker-fatal") {
        ...
        this.#failTransport(
          transportFailure("Worker could not receive or deliver an operation message", cause),
          true,
        );
        return;
      }
      ...
      const response =
        error === null ? captureResponse(source, kind, id, pending.requestKind) : null;
      ...
      if (
        error.code === "ERR_FSQLITE_SNAPSHOT_CONNECTION_UNUSABLE" ||
  ...
  readonly #onMessageError = (): void => {
    if (!this.#disposed)
      this.#failTransport(transportFailure("Worker response could not be deserialized"));
  };
  ...
  constructor(worker: WorkerLike, limits: Partial<RequestLimits> = {}) {
    ...
    try {
      ...
      try {
        ...
        throw new AggregateError([cause, cleanup], "Worker setup and cleanup failed", { cause });
      }
      throw cause;
    }
  }
  ...
  get snapshotOwnership(): SnapshotOwnership | null {
    return this.#snapshotOwnership;
  }
  ...
  #notifyFailure(error: Error): void {
    ...
    for (const listener of listeners) {
      try {
        ...
        /* Internal owner already has the error. */
      }
    }
  }
    ...
    try {
      if (values === null || values === undefined) {
        throw new BindingError(
          ...
          "Bindings must be an array or a named object",
        );
      }
      ...
      try {
        ...
        const output = new RequestBudget({
          maxPendingBytes: this.#budget.stats.maxPendingBytes,
      ...
      if (error instanceof BindingError || error instanceof RequestAdmissionError) {
        throw new FrankenSQLiteError({
          ...
          userRecoverable: error.userRecoverable,
        });
      }
      throw error;
    }
  ...
  async init(config: InitConfig) {
    ...
    if (
      ready.persistence !== persistence ||
      (isSnapshotPersistenceMode(persistence) && ready.path !== path)
    ) {
      const error = new FrankenSQLiteError({
        ...
        suggestion:
          "Use a matching SDK and worker. No database handle was opened; do not fall back to memory or another snapshot backend.",
      });
    ...
    if (
      (held !== undefined &&
        ((held !== "shared" && held !== "exclusive") || !isSnapshotPersistenceMode(persistence))) ||
      (ownership !== undefined && held !== ownership) ||
      (ownership === undefined && held === "exclusive")
    ) {
      const error = new FrankenSQLiteError({
  ...
  #queryResult<Row extends Record<string, unknown>>(response: WorkerResponse): QueryResult<Row> {
    ...
    try {
      ...
      throw new FrankenSQLiteError({
        ...
        suggestion:
          "The SQL may have executed. Do not automatically retry writes; inspect database state.",
      });
    }
  }
        ...
        (error: unknown) => {
          try {
            ...
            throw new AggregateError(
              [error, cleanupError],
              "FrankenSQLite close and worker cleanup both failed",
              { cause: error },
            );
          }
          throw error;
  ...
  async #sendBulk(
    ...
    const checkAborted = (): void => {
      if (signal.aborted) {
        throw new FrankenSQLiteError({
          ...
          message: "FrankenSQLite bulk execution was cancelled before admission",
          transient: false,
        });
      }
    };
    ...
    try {
      checkAborted();
      const response = this.#send(request, false, () => {
        posted = true;
      });
  ...
  #send(
    ...
    const canCloseHost = (): boolean =>
      allowClosing && request.kind === "close" && this.#hostTerminal && !this.#disposed;
    ...
    if ((this.#terminalError !== null && !canCloseHost()) || (this.#closing && !allowClosing)) {
      release();
      return Promise.reject(
        this.#terminalError ?? new Error("FrankenSQLite worker client is closing"),
      );
    }
      ...
      if (this.#remoteTransportFailure && request.kind !== "close") {
        try {
          ...
          if (cleanup !== error)
            throw new AggregateError(
              [error, cleanup],
              "Worker operation response and transport cleanup both failed",
              { cause: error },
            );
        }
      }
...
function ensureKind<K extends WorkerResponse["kind"]>(
  ...
  if (response.kind !== kind) {
    throw new Error(`Expected worker response kind \`${kind}\`, got \`${response.kind}\``);
  }
...
function responseObject(value: unknown): Record<string, unknown> {
  if (typeof value !== "object" || value === null || Array.isArray(value))
    throw new TypeError("Invalid worker response object");
...
function responseFailure(cause: unknown): FrankenSQLiteError {
  const error = new FrankenSQLiteError({
    ...
    suggestion:
      "SQL or snapshot publication may have executed. Inspect authoritative state; do not blindly retry the operation.",
  });
  error.cause = cause;
  return error;
}
function transportFailure(message: string, cause?: unknown): FrankenSQLiteError {
  const error = new FrankenSQLiteError({
    ...
    message,
    suggestion:
      "Connection outcomes are unknown: SQL or snapshot publication may have executed. Reopen and reconcile authoritative data; do not blindly retry writes.",
  });
...
function captureResponse(
  ...
  const expected: Record<WorkerRequest["kind"], string> = {
    ...
    "execute-batch": "execute-batch-result",
    "execute-many": "execute-many-result",
    query: "query-result",
    prepare: "prepare-result",
    "statement-execute": "execute-result",
    "statement-execute-many": "execute-many-result",
    "statement-query": "query-result",
    "statement-finalize": "statement-finalize-result",
    transaction: "transaction-result",
    "cancel-transaction": "cancel-transaction-result",
    "cancel-bulk": "cancel-bulk-result",
    export: "export-result",
    checkpoint: "checkpoint-result",
    "checkpoint-recover": "checkpoint-result",
    close: "close-result",
  };
  if (
    kind !== expected[request] &&
    !(kind === "query-binary-result" && expected[request] === "query-result")
  ) {
  ...
  switch (kind) {
    case "ready": {
      ...
      const path = field("path", true),
        persistence = field("persistence", true);
      if (
        typeof path !== "string" ||
        !["memory", "opfs", "indexeddb", "indexeddb-snapshot", "opfs-snapshot"].includes(
      ...
      return {
        kind,
        requestId,
        data: Object.freeze(ready) as unknown as import("@frankensqlite/worker").InitResult,
      };
    }
    case "execute-result": {
    ...
    case "execute-many-result": {
      ...
      if (
        !safeCount(executions) ||
        executions > 10000 ||
        !safeCount(changes) ||
        !Array.isArray(counts) ||
        counts.length !== executions
      )
        throw new TypeError("Invalid bulk result");
      let total = 0;
      for (let i = 0; i < counts.length; i++) {
        if (!safeCount(counts[i])) throw new TypeError("Invalid bulk affected-row count");
        total += counts[i];
      }
      if (!Number.isSafeInteger(total) || total !== changes)
        throw new TypeError("Inconsistent bulk counts");
      return {
        kind,
        requestId,
        data: { executions, changes, changesPerExecution: counts as number[] },
      };
    }
    case "query-result": {
      const data = responseObject(source.data);
      if (
        !safeCount(data.columnCount) ||
        !Array.isArray(data.columns) ||
        data.columns.length !== data.columnCount ||
        !Array.isArray(data.columnTypes) ||
        !Array.isArray(data.rows) ||
        !Array.isArray(data.rowArrays) ||
        data.rows.length !== data.rowArrays.length
      )
        throw new TypeError("Invalid query result envelope");
    ...
    case "query-binary-result":
      ...
      return {
        kind,
        requestId,
        encoding: source.encoding as "fqr1",
    ...
    case "prepare-result": {
      ...
      if (
        ...
        data.statementId.length === 0 ||
        typeof data.sql !== "string" ||
        !safeCount(data.columnCount) ||
        !Array.isArray(data.columnNames) ||
        data.columnNames.length !== data.columnCount
      ) {
    ...
    case "checkpoint-result": {
      ...
      if (
        ...
        !safeCount(data.byteLength) ||
        (data.parentRevision !== null && typeof data.parentRevision !== "string")
      )
        throw new TypeError("Invalid checkpoint metadata");
      return { kind, requestId, data: data as unknown as SnapshotMetadata };
    }
    case "cancel-bulk-result":
    case "cancel-transaction-result": {
      if (typeof source.accepted !== "boolean")
        throw new TypeError("Invalid cancellation acknowledgement");
      return { kind, requestId, accepted: source.accepted };
    }
    case "execute-batch-result":
    case "transaction-result":
    case "statement-finalize-result":
    case "close-result":
