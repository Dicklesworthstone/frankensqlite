...
fn bulk_insert_scaling_profile() {
    asupersync::test_utils::run_test(|| async {
        ...
        let conn2 = Connection::open(":memory:").await.expect("open2");
        conn2
        ...
        let conn3 = Connection::open(":memory:").await.expect("open3");
        conn3
            .execute("CREATE TABLE tu (id INTEGER PRIMARY KEY, a INTEGER);")
            .await
            .unwrap();
        conn3.execute("BEGIN").await.unwrap();
        let ins = conn3
            .prepare("INSERT INTO tu VALUES (?1, 0)")
            .await
            .unwrap();
        for i in 1..=n {
            conn3
        ...
        drop(ins);
        let upd = conn3
            .prepare("UPDATE tu SET a = ?1 WHERE id = ?2")
            .await
            .unwrap();
        ...
        drop(upd);
        conn3.execute("COMMIT").await.unwrap();

        // --- PREPARED DELETE by PK: scaling ---
        let conn4 = Connection::open(":memory:").await.expect("open4");
        conn4
            .execute("CREATE TABLE td (id INTEGER PRIMARY KEY, a INTEGER);")
            .await
            .unwrap();
        conn4.execute("BEGIN").await.unwrap();
        let ins2 = conn4
            .prepare("INSERT INTO td VALUES (?1, ?2)")
            .await
            .unwrap();
        for i in 1..=n {
            conn4
        ...
        drop(del);
        conn4.execute("COMMIT").await.unwrap();
        ...
        let conn5 = Connection::open(":memory:").await.expect("open5");
        conn5
            .execute("CREATE TABLE tw (id INTEGER PRIMARY KEY, a INTEGER);")
            .await
            .unwrap();
        conn5.execute("BEGIN").await.unwrap();
        let ins3 = conn5
            .prepare("INSERT INTO tw VALUES (?1, ?2)")
            .await
            .unwrap();
        for i in 1..=n {
            conn5
        ...
        drop(ins3);
        let upd2 = conn5
            .prepare("UPDATE tw SET a = ?1 WHERE id = ?2")
            .await
            .unwrap();
        ...
        drop(upd2);
        conn5.execute("COMMIT").await.unwrap();
