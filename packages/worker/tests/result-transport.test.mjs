...
import { decodeQueryResult, tryEncodeQueryResult, responseTransferList,
  BINARY_RESULT_THRESHOLD } from '../src/result-codec.ts';
import { sqliteSnapshotWorker } from './helpers/snapshot-sqlite-core.mjs';

function result(values = [[1, 'one']], columns = ['id', 'name']) {
  return { columns, columnCount: columns.length, columnTypes: [], changes: 0,
    rowArrays: values, rows: values.map(row => Object.fromEntries(columns.map((key,i) => [key,row[i]]))) };
}
function fixture(data = result()) {
  let queries = 0, loads = 0, creates = 0;
  const core = {
    path: ':memory:', free() {}, close() {}, async executeBatch() {},
    async query() { queries++; return data; },
    async queryWithParams() { queries++; return data; },
    async prepare(sql) { return { sql, columnCount: data.columnCount, columnNames: () => data.columns,
      free() {}, query: () => core.query(), queryWithParams: () => core.query() }; },
  };
  const host = new WorkerConnectionHost({ async load() { loads++; return { FrankenDB: {
    async create() { creates++; return core; }, async import() { creates++; return core; },
  } }; } });
  let id = 1;
  const send = input => host.handle({requestId: id++, ...input});
  const init = resultEncoding => send({kind:'init', config: resultEncoding === undefined ? {} : {resultEncoding}});
...
test('default encoding leaves structured results untouched and never walks result rows', async () => {
  const data = result();
  let visits = 0;
  Object.defineProperty(data, 'rows', { enumerable: true, get() { visits++; return [{id:1,name:'one'}]; } });
...
test('binary negotiation produces one fresh transferable buffer for direct and prepared reads', async () => {
  ...
  for (const request of [
    {kind:'query',sql:'SELECT 1'}, {kind:'query',sql:'SELECT ?',params:[1]},
...
test('auto threshold is measured on the encoded image, including the exact boundary', async () => {
  ...
  for (const delta of [-1,0,1]) {
    const f = fixture(result([[new Uint8Array(BINARY_RESULT_THRESHOLD - overhead + delta)]], ['b']));
...
test('binary mode preserves scalar types, aliases, UTF-16 and empty result metadata', async () => {
  const values=[null,false,true,-0,NaN,Infinity,-Infinity,1.25,-(1n<<63n),(1n<<63n)-1n,'a\0\ud800\udfff',new Uint8Array([0,255]),11,22];
  const names=values.map((_,i)=>'c'+i);
  names[11]='__proto__'; names[12]='duplicate'; names[13]='duplicate';
  for (const data of [result([values],names), result([],['empty']), result([[]],[])]) {
    const f=fixture(data); await f.init('binary');
    const reply=await f.send({kind:'query',sql:'fixture'});
    assert.equal(reply.kind,'query-binary-result');
    const actual=received(reply);
    assert.deepEqual(actual, data);
    if (actual.rows[0]?.__proto__ instanceof Uint8Array) assert.equal(Object.getPrototypeOf(actual.rows[0]),Object.prototype);
  }
});

test('unsupported and extended results fall back without rerunning successful SQL', async () => {
  const data=[result([[1n<<80n]],['x']), {...result(),extra:{keep:true}},
    {...result(),rows:[{id:1,name:'different'}]}, result([[undefined]],['x'])];
  for (const value of data) {
    const f=fixture(value); await f.init('binary');
    const reply=await f.send({kind:'query',sql:'INSERT ... RETURNING'});
    assert.equal(reply.kind,'query-result'); assert.equal(reply.data,value);
...
test('result mode is captured on admission, while independent hosts remain independent', async () => {
  ...
  const begin=a.send({kind:'init',config}); config.resultEncoding='structured-clone';
  await begin; await b.init();
...
test('real SQL results transfer inside a managed scope without changing its rollback', async () => {
  const f=sqliteSnapshotWorker(); let id=1;
  const send=async request=>{
    const reply=await f.host.handle({requestId:id++,...request});
    assert.notEqual(reply.kind,'error',JSON.stringify(reply)); return reply;
  };
  ...
  const write=await send({kind:'query',transactionId:'1',sql:"INSERT INTO t VALUES(1,x'010203') RETURNING id,b"});
...
test('codec refusal after real INSERT RETURNING is a successful structured reply, not a retry', async () => {
  ...
  await f.host.handle({kind:'execute-batch',requestId:2,sql:'CREATE TABLE t(id INTEGER PRIMARY KEY)'});
  const core=f.handles[0], query=core.query.bind(core);
  core.query=async sql=>({...await query(sql), extension:'retained'});
  const reply=await f.host.handle({kind:'query',requestId:3,sql:'INSERT INTO t DEFAULT VALUES RETURNING id'});
  assert.equal(reply.kind,'query-result'); assert.equal(reply.data.extension,'retained');
