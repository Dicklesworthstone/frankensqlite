...
function transportError(code: string, message: string): SerializedFrankenError {
  return {
    ...
    suggestion:
      "SQL or snapshot publication may have executed. Inspect authoritative state; do not blindly retry writes.",
  };
}
...
function reportTransportError(): void {
  ...
  reported = true;
  setTimeout(() => {
    throw new Error("FrankenSQLite worker transport failed; operation outcomes are unknown");
  }, 0);
}

function failTransport(message: string, deliveryFailed = false): void {
  if (terminal) {
    if (deliveryFailed) reportTransportError();
    return;
  }
  terminal = true;
  ...
  const fatal: WorkerFatalMessage = {
    kind: "worker-fatal",
    error: transportError("ERR_FSQLITE_WORKER_TRANSPORT", message),
  };
  try {
    workerScope.postMessage(fatal);
  } catch {
...
async function dispatch(input: unknown): Promise<void> {
  ...
  try {
    ...
    try {
      error = serializeFrankenError(cause);
    } catch {
      error = transportError(
        "ERR_FSQLITE_WORKER_DISPATCH",
        "Worker could not serialize an operation failure",
      );
    }
  ...
  try {
    ...
    const fallback: ErrorResponse = {
      ...
      requestId,
      error: transportError(
        "ERR_FSQLITE_RESPONSE_TRANSFER",
        "Worker could not clone or transfer the operation response",
      ),
    };
    try {
      workerScope.postMessage(fallback);
    } catch {
      failTransport("Worker could not deliver the response or its failure acknowledgement", true);
    }
  }
}
