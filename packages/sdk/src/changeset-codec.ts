...
export type ChangesetChange =
  | { readonly operation: "insert"; readonly indirect: boolean; readonly new: readonly ChangesetValue[] }
  | { readonly operation: "delete"; readonly indirect: boolean; readonly old: readonly ChangesetValue[] }
  | { readonly operation: "update"; readonly indirect: boolean; readonly old: readonly ChangesetField[]; readonly new: readonly ChangesetField[] };
...
export class ChangesetError extends Error {
  constructor(readonly code: "ERR_FSQLITE_CHANGESET_FORMAT" | "ERR_FSQLITE_CHANGESET_LIMIT" |
    "ERR_FSQLITE_CHANGESET_INPUT", message: string, readonly offset?: number) {
    super(message); this.name = "ChangesetError";
  }
}
const DEFAULTS = { maxBytes: 64 * 1024 * 1024, maxTables: 256, maxColumns: 2000,
  maxChanges: 100_000, maxCells: 1_000_000 } as const;
...
function limits(input: ChangesetLimits = {}): Limits {
  ...
  for (const key of Object.keys(DEFAULTS) as (keyof Limits)[]) {
    ...
    if (!Number.isSafeInteger(value) || value < 1 || value > ceiling) {
      throw new ChangesetError("ERR_FSQLITE_CHANGESET_INPUT", `${key} must be an integer in 1..${ceiling}`);
    }
    result[key] = value;
  }
  return result;
...
function limit(message: string): never { throw new ChangesetError("ERR_FSQLITE_CHANGESET_LIMIT", message); }
function input(message: string): never { throw new ChangesetError("ERR_FSQLITE_CHANGESET_INPUT", message); }
function nameBytes(name: string): Uint8Array {
  if (typeof name !== "string" || name.length === 0 || name.length > 1024 || name.includes("\0")) input("Invalid changeset table name");
...
function textBytes(text: string, maximum: number): Uint8Array {
  ...
  if (utf8.decode(bytes) !== text) input("Changeset text must not contain unpaired UTF-16 surrogates");
  return bytes;
}
function validateChange(pk: readonly number[], change: ChangesetChange): void {
  ...
  if (typeof change.indirect !== "boolean" || (before !== undefined && before.length !== pk.length) ||
      (after !== undefined && after.length !== pk.length)) format("Invalid changeset row shape");
  let modified = false;
  for (let i = 0; i < pk.length; i++) {
    const old = before?.[i], next = after?.[i];
    if (change.operation !== "update") {
      const value = change.operation === "insert" ? next : old;
      if (value === undefined || (pk[i] !== 0 && value === null)) format("Incomplete row or NULL primary key");
    } else if (pk[i] !== 0) {
      if (old === undefined || old === null || next !== undefined) format("UPDATE must retain its original primary key");
    } else {
      if ((old === undefined) !== (next === undefined)) format("UPDATE requires paired old/new values");
...
export function decodeChangeset(bytes: Uint8Array, options?: ChangesetLimits): readonly ChangesetTable[] {
  ...
  if (!(bytes.buffer instanceof ArrayBuffer) || (bytes.buffer as ArrayBuffer & { resizable?: boolean }).resizable) {
  ...
  let pos = 0, rows = 0, cells = 0;
  const need = (n: number): void => { if (n > bytes.length - pos) format("Truncated changeset", pos); };
  const byte = (): number => { need(1); return bytes[pos++]!; };
  ...
  const field = (): ChangesetField => {
    ...
    const size = length(policy.maxBytes); need(size);
    const data = bytes.subarray(pos, pos + size); pos += size;
    if (tag === 4) return new Uint8Array(data);
    try { return utf8.decode(data); } catch { return format("Invalid changeset UTF-8", pos - size); }
  };
  ...
  while (pos < bytes.length) {
    ...
    need(count);
    const pk = Array.from(bytes.subarray(pos, pos + count)); pos += count;
    if (!pk.some(value => value !== 0)) format("Changeset table has no primary key", pos);
    const start = pos;
    while (byte() !== 0) { if (pos - start > 1024) limit("Changeset table name exceeds 1024 bytes"); }
    let name: string;
    try { name = utf8.decode(bytes.subarray(start, pos - 1)); } catch { return format("Invalid table name UTF-8", start); }
    ...
    while (pos < bytes.length && bytes[pos] !== 84 && bytes[pos] !== 80) {
      const op = byte(), flag = byte();
      if (![18, 9, 23].includes(op) || flag > 1) format("Invalid change operation or indirect flag", pos - 2);
      ...
      const record = (): readonly ChangesetField[] => Object.freeze(Array.from({ length: count }, field));
      let change: ChangesetChange;
      if (op === 18) change = { operation: "insert", indirect: flag === 1, new: record() as readonly ChangesetValue[] };
      else if (op === 9) change = { operation: "delete", indirect: flag === 1, old: record() as readonly ChangesetValue[] };
      else change = { operation: "update", indirect: flag === 1, old: record(), new: record() };
      validateChange(pk, change); changes.push(Object.freeze(change));
    }
    if (changes.length === 0) format("Table header has no changes", pos);
    result.push(Object.freeze({ name, primaryKey: Object.freeze(pk), changes: Object.freeze(changes) }));
  }
  return Object.freeze(result);
}

/** Encode explicit changes, without silently coercing real values to integers. */
export function encodeChangeset(tables: readonly ChangesetTable[], options?: ChangesetLimits): Uint8Array {
  const policy = limits(options);
  ...
  let buffer = new Uint8Array(Math.min(1024, policy.maxBytes)), pos = 0, rows = 0, cells = 0;
  ...
  const put = (b: number): void => { room(1); buffer[pos++] = b; };
  const data = (b: Uint8Array): void => { room(b.length); buffer.set(b, pos); pos += b.length; };
  ...
  const field = (value: ChangesetField): void => {
    if (value === undefined) { put(0); return; }
    if (value === null) { put(5); return; }
    if (typeof value === "bigint" || typeof value === "number") {
      if (typeof value === "bigint" && (value < -(1n << 63n) || value >= (1n << 63n))) input("Integer must fit signed 64-bit");
      if (typeof value === "number" && Number.isNaN(value)) input("NaN is not a SQLite stored real");
      put(typeof value === "bigint" ? 1 : 2); room(8);
      const view = new DataView(buffer.buffer);
      if (typeof value === "bigint") view.setBigInt64(pos, value); else view.setFloat64(pos, value);
      pos += 8; return;
    }
    if (typeof value === "string") { const b = textBytes(value, policy.maxBytes - pos); put(3); length(b.length); data(b); return; }
    if (value instanceof Uint8Array) { put(4); length(value.length); data(value); return; }
    input("Unsupported changeset value");
  };
  for (let t = 0, n = tables.length; t < n; t++) {
    const table = tables[t]!;
    const name = nameBytes(table.name), sourceKey = table.primaryKey, changes = table.changes;
    ...
    if (!Array.isArray(pk) || pk.length === 0 || pk.length > policy.maxColumns) input("Invalid changeset column count");
    if (!Array.isArray(changes) || changes.length === 0) input("Each table needs changes");
    put(84); length(pk.length);
    for (let i = 0; i < pk.length; i++) {
      const key = pk[i]!;
    ...
    for (let i = 0, total = changes.length; i < total; i++) {
      ...
      if (op !== "insert" && op !== "delete" && op !== "update") input("Invalid changeset operation");
      const records = op === "insert" ? [change.new] : op === "delete" ? [change.old] : [change.old, change.new];
      cells += pk.length * records.length;
      if (cells > policy.maxCells) limit("Changeset exceeds maxCells");
      const indirect = change.indirect;
      if (typeof indirect !== "boolean") input("indirect must be a boolean");
      put(op === "insert" ? 18 : op === "delete" ? 9 : 23); put(Number(indirect));
      for (const record of records) {
        if (!Array.isArray(record) || record.length !== pk.length) input("Invalid changeset row shape");
...
export function invertChangeset(bytes: Uint8Array, options?: ChangesetLimits): Uint8Array {
  const policy = limits(options);
  const tables = decodeChangeset(bytes, policy).map(table => ({ ...table,
    changes: table.changes.map((change): ChangesetChange => {
      if (change.operation === "insert") return { operation: "delete", indirect: change.indirect, old: change.new };
      if (change.operation === "delete") return { operation: "insert", indirect: change.indirect, new: change.old };
      return { operation: "update", indirect: change.indirect,
