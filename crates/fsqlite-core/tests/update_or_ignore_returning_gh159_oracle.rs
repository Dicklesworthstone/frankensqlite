...
fn update_or_ignore_returning_unique_conflict_emits_nothing_gh159() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert_agree(&f, &r, "UPDATE OR IGNORE t SET email='a@x.com' WHERE id=2 RETURNING id,email,v").await;
...
fn update_or_ignore_returning_pk_conflict_emits_nothing_gh159() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert_agree(&f, &r, "UPDATE OR IGNORE t SET id=1 WHERE id=2 RETURNING id,email,v").await;
        assert_agree(&f, &r, "SELECT id,email,v FROM t ORDER BY id").await;
    });
}
...
fn update_or_ignore_returning_no_conflict_still_emits_gh159() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert_agree(&f, &r, "UPDATE OR IGNORE t SET email='c@x.com', v=99 WHERE id=2 RETURNING id,email,v").await;
        assert_agree(&f, &r, "SELECT id,email,v FROM t ORDER BY id").await;
        // Setting a column to its OWN current value is NOT a conflict — must emit.
        assert_agree(&f, &r, "UPDATE OR IGNORE t SET email='a@x.com' WHERE id=1 RETURNING id,email,v").await;
    });
}
...
fn update_or_ignore_returning_without_rowid_gh159() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert_agree(&f, &r, "UPDATE OR IGNORE w SET e='a' WHERE k='q' RETURNING k,e,n").await;
