...
fn cmp(f: &Connection, r: &rusqlite::Connection, sql: &str, no_rewind: Option<bool>) {
    match no_rewind {
        Some(true) => assert!(!has_op(f, sql, "Rewind"), "agg-rowid-IN must not full-scan (Rewind): `{sql}`"),
...
fn agg_rowid_in_matches_sqlite() {
    ...
    for s in ["CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER, w REAL, x INTEGER);"] {
        f.execute(s).unwrap();
    ...
    cmp(&f, &r, "SELECT SUM(v) FROM t WHERE id IN (5, 25, 45)", Some(true));
    cmp(&f, &r, "SELECT AVG(v) FROM t WHERE id IN (5, 25, 45)", Some(true));
    cmp(&f, &r, "SELECT MIN(v) FROM t WHERE id IN (5, 25, 45)", Some(true));
    cmp(&f, &r, "SELECT MAX(v) FROM t WHERE id IN (5, 25, 45)", Some(true));
    cmp(&f, &r, "SELECT COUNT(v) FROM t WHERE id IN (5, 25, 45)", Some(true));
    cmp(&f, &r, "SELECT SUM(w) FROM t WHERE id IN (5, 25, 45)", Some(true)); // real sum
    cmp(&f, &r, "SELECT SUM(v) FROM t WHERE id IN (5, 5, 25)", Some(true)); // dedup: 5 counted once
    cmp(&f, &r, "SELECT SUM(v) FROM t WHERE id IN (5, 99999)", Some(true)); // one absent
    cmp(&f, &r, "SELECT SUM(v) FROM t WHERE id IN (99999, 88888)", Some(true)); // all absent -> NULL
    cmp(&f, &r, "SELECT COUNT(v) FROM t WHERE id IN (99999, 88888)", Some(true)); // all absent -> 0
    cmp(&f, &r, "SELECT SUM(v) FROM t WHERE id IN (41, 82)", Some(true)); // v is NULL at those rows -> NULL
