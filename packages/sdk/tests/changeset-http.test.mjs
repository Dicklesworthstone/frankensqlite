...
import { ChangesetDeliveryPump, ChangesetReceiver } from "../src/changeset-delivery.ts";
import {
  ChangesetHttpError,
...
function ack(meta, extra = {}) {
  return {
    ...
    sha256: meta.sha256,
    byteLength: meta.byteLength ?? meta.changeset.length,
    applied: 0,
    omitted: 0,
    replayed: false,
    confirmed: true,
...
async function server(t, handler) {
  ...
  const http = createServer((q, r) => {
    Promise.resolve(handler(q, r)).catch((e) => {
      errors.push(e);
      r.destroy();
    });
  });
...
const read = async (q) => {
  const parts = [];
  for await (const part of q) parts.push(part);
  return Buffer.concat(parts);
};
const jsonReply = (r, receipt, status = 200) => {
  r.writeHead(status, { "content-type": RECEIPT_TYPE });
  r.end(JSON.stringify(receipt));
};
function client(url, options = {}) {
  return createChangesetHttpTransport(url, { allowInsecureLoopback: true, ...options });
}
function isError(code, outcome = "unknown") {
  return (e) =>
    e instanceof ChangesetHttpError &&
    e.code === `ERR_FSQLITE_HTTP_${code}` &&
    e.outcome === outcome;
}
...
test("native SQLite session bytes cross real HTTP unchanged and replay safely", async (t) => {
  ...
  source
    ...
    .run(9223372036854775807n, "a\0🌍", new Uint8Array([0, 255, 1]));
  ...
  const url = await server(t, async (q, r) => {
    ...
    assert.equal(q.headers["content-type"], REQUEST_TYPE);
    assert.equal(q.headers.authorization, "Bearer test-only");
    ...
    const prior = destination
      .prepare("SELECT digest FROM inbox WHERE id=?")
      .get(received.meta.deliveryId);
    if (!prior) {
      destination.exec("BEGIN");
      assert.equal(destination.applyChangeset(received.bytes), true);
      destination
        .prepare("INSERT INTO inbox VALUES (?,?)")
        .run(received.meta.deliveryId, received.meta.sha256);
  ...
  const row = destination
    .prepare("SELECT CAST(id AS TEXT) AS id, hex(body) AS body, hex(data) AS data FROM notes")
    .get();
...
test("payload and routing fields are captured before asynchronous credentials", async (t) => {
  ...
  const url = await server(t, async (q, r) => {
    const got = frame(await read(q));
    assert.deepEqual(got.bytes, original);
    assert.equal(got.meta.deliveryId, "source:1");
    jsonReply(r, ack(got.meta));
  });
  await client(url, {
    headers: async (meta) => {
      assert.ok(Object.isFrozen(meta));
      bytes.fill(9);
      envelope.deliveryId = "other";
...
test("intrinsic buffer capture ignores subclass getters and iterators", async (t) => {
  ...
  const bytes = new Bytes([1, 2]);
  const envelope = { ...message(new Uint8Array([1, 2])), changeset: bytes };
  const url = await server(t, async (q, r) => {
    const data = frame(await read(q));
    assert.deepEqual([...data.bytes], [1, 2]);
    jsonReply(r, ack(data.meta));
  });
...
for (const url of [
  ...
  test(`reject unsafe endpoint ${url} without making a request`, () => {
    assert.throws(
      () => createChangesetHttpTransport(url, { allowInsecureLoopback: true }),
      isError("INPUT", "not-sent"),
    );
  });
}
test("loopback HTTP requires explicit opt-in", () => {
  assert.throws(
    () => createChangesetHttpTransport("http://127.0.0.1/x"),
    isError("INPUT", "not-sent"),
  );
  assert.throws(
    () =>
      createChangesetHttpTransport("http://127.0.0.1.evil.test/x", { allowInsecureLoopback: true }),
    isError("INPUT", "not-sent"),
  );
});
...
for (const name of [
  ...
  test(`credentials cannot override ${name}`, async () => {
    let calls = 0;
    const deliver = createChangesetHttpTransport("https://example.test/x", {
      headers: () => ({ [name]: "x" }),
      fetch: async () => {
        calls++;
    ...
    await assert.rejects(deliver(message(), {}), isError("INPUT", "not-sent"));
    assert.equal(calls, 0);
  });
}

test("fetch receives explicit no-cookie/no-cache/no-redirect policies", async () => {
  await createChangesetHttpTransport("https://example.test/x", {
    fetch: async (request) => {
...
test("real redirect never forwards a payload or credentials to another endpoint", async (t) => {
  let leaked = 0;
  const other = await server(t, (q, r) => {
    leaked++;
    r.end();
  });
  const first = await server(t, async (q, r) => {
    await read(q);
    r.writeHead(307, { location: other });
    r.end();
  });
  await assert.rejects(
    client(first, { headers: () => ({ authorization: "secret" }) })(message(), {}),
    isError("NETWORK"),
  );
...
for (const status of [202, 204, 400, 401, 409, 429, 500, 503]) {
  test(`HTTP ${status} cannot become a success or an automatic retry`, async (t) => {
    ...
    const url = await server(t, async (q, r) => {
      ...
      await read(q);
      r.writeHead(status);
      r.end("private SQL detail");
    });
    await assert.rejects(
      client(url)(message(), {}),
      (e) => isError("HTTP")(e) && e.status === status && !e.message.includes("private"),
    );
...
for (const [key, value] of Object.entries({
  ...
  test(`reject mismatched receipt ${key}`, async (t) => {
    const url = await server(t, async (q, r) =>
      jsonReply(r, ack(frame(await read(q)).meta, { [key]: value })),
    );
...
test("chunked receipt is bounded by actual bytes without Content-Length", async (t) => {
  const url = await server(t, async (q, r) => {
    await read(q);
    r.writeHead(200, { "content-type": RECEIPT_TYPE });
    r.write(" ".repeat(8192));
    r.end("x");
  });
...
test("oversized declared receipt is cancelled before being read", async () => {
  ...
  await assert.rejects(deliver(message(), {}), isError("LIMIT"));
...
test("a lying or truncated Content-Length never yields a receipt", async () => {
  for (const length of ["1", "4096", "-1", "2, 2", "00"]) {
    const deliver = createChangesetHttpTransport("https://example.test/x", {
      fetch: async () =>
        new Response(JSON.stringify(ack(message())), {
          headers: { "content-type": RECEIPT_TYPE, "content-length": length },
        }),
    });
    await assert.rejects(
      deliver(message(), {}),
      (e) => e instanceof ChangesetHttpError && e.outcome === "unknown",
    );
  }
});
for (const body of ["{", "null", "[]", '{"confirmed":true}', new Uint8Array([0xff])]) {
  test(`malformed receipt ${String(body).slice(0, 24)} rejects`, async () => {
    const deliver = createChangesetHttpTransport("https://example.test/x", {
      fetch: async () => new Response(body, { headers: { "content-type": RECEIPT_TYPE } }),
    });
...
test("wrong receipt media type and compressed responses reject", async () => {
  for (const headers of [
    ...
    { "content-type": RECEIPT_TYPE, "content-encoding": "gzip" },
  ]) {
    const deliver = createChangesetHttpTransport("https://example.test/x", {
...
test("pre-aborted calls and invalid/oversized payloads never dispatch", async () => {
  ...
  await assert.rejects(
    deliver(message(), { signal: AbortSignal.abort("stop") }),
    isError("CANCELLED", "not-sent"),
  );
  ...
  await assert.rejects(
    deliver({ ...message(), changeset: detached }, {}),
    isError("INPUT", "not-sent"),
  );
  assert.equal(calls, 0);
});
test("deadline covers slow headers and signals real fetch cancellation", async (t) => {
  ...
  const url = await server(t, async (q, r) => {
    await read(q);
    observed(); /* deliberately withhold response */
  });
...
test("external cancellation after dispatch remains an unknown outcome", async (t) => {
  ...
  const url = await server(t, async (q, r) => {
    await read(q);
    controller.abort("lost interest");
  });
...
test("credential delay drains before rejecting; no delayed request escapes cancellation", async () => {
  ...
  await assert.rejects(
    deliver(message(), { signal: controller.signal }),
    isError("CANCELLED", "not-sent"),
  );
  assert.equal(calls, 0);
});
test("a lost response never triggers implicit retry and explicit retry preserves identity", async (t) => {
  ...
  const url = await server(t, async (q, r) => {
    ...
    if (attempts === 1) r.destroy();
    else jsonReply(r, ack(meta, { replayed }));
  });
  const deliver = client(url),
    envelope = message();
...
function sqlTarget(db) {
  return {
    async transaction(work, options = {}) {
      const deadline =
        options.timeoutMs === undefined ? Infinity : performance.now() + options.timeoutMs;
      function check() {
        if (options.signal?.aborted || performance.now() >= deadline)
          throw Error("SQL scope cancelled");
      }
      check();
      db.exec("BEGIN");
      try {
...
function httpBridge(getHandler, afterResponse) {
  return async (q, r) => {
    const controller = new AbortController();
    const abort = () => {
      if (!r.writableFinished) controller.abort(new Error("HTTP peer disconnected"));
    };
    q.once("aborted", abort);
    r.once("close", abort);
    try {
      const init = { method: q.method, headers: q.headers, signal: controller.signal };
      if (q.method !== "GET" && q.method !== "HEAD") {
        init.body = Readable.toWeb(q);
        init.duplex = "half";
      }
...
function wire(envelope = message(), edits = {}) {
  ...
  delete metadata.applied;
  delete metadata.omitted;
  delete metadata.confirmed;
  delete metadata.replayed;
  const json = Buffer.from(JSON.stringify(metadata));
  const bytes = Buffer.alloc(8 + json.length + envelope.changeset.length);
  bytes.set([70, 67, 68, 49]);
  bytes.writeUInt32BE(json.length, 4);
  bytes.set(json, 8);
  bytes.set(envelope.changeset, 8 + json.length);
  return bytes;
}
function request(body = wire(), options = {}) {
  return new Request("https://receiver.test/changes", {
    method: "POST",
    body,
    duplex: "half",
    ...options,
    headers: { "content-type": REQUEST_TYPE, ...options.headers },
  });
}
const verifiedDouble = {
  receiverId: "receiver",
...
test("endpoint construction requires explicit authorization and bounded policy", () => {
  for (const options of [
    {},
    { authorize: true },
    { authorize: () => true, maxInFlight: 0 },
    { authorize: () => true, maxMessageBytes: 64 * 1024 * 1024 + 1 },
    { authorize: () => true, timeoutMs: NaN },
  ]) {
    assert.throws(
      () => createChangesetHttpHandler(verifiedDouble, options),
      isError("INPUT", "not-sent"),
    );
  }
});
test("authorization runs before any body read and denial cancels the unread stream", async () => {
  ...
  const handle = createChangesetHttpHandler(
    {
      receiverId: "receiver",
      async receive() {
        receives++;
    ...
    },
    {
      authorize: async (context) => {
        ...
        assert.equal(context.receiverId, "receiver");
  ...
  assert.deepEqual(await response.json(), {
    error: "ERR_FSQLITE_HTTP_REJECTED",
    outcome: "unknown",
  });
});
for (const decision of [
  ...
  {},
  () => {
    throw Error("private authentication secret");
  },
]) {
...
test("delivery authorization can bind authenticated callers to a source namespace", async () => {
  ...
  const handle = createChangesetHttpHandler(
    ...
    },
    {
      ...
      authorizeDelivery: (ctx, meta) => {
        assert.equal(meta.changeset, undefined);
        assert.ok(Object.isFrozen(meta));
        return (
          ctx.headers.get("authorization") === "Bearer sourceA" &&
          meta.deliveryId.startsWith("sourceA:")
        );
      },
    },
  );
  assert.equal(
    (
      await handle(
        request(wire(message(undefined, "sourceB:1")), {
          headers: { authorization: "Bearer sourceA" },
        }),
      )
    ).status,
    403,
  ...
  assert.equal(
    (
      await handle(
        request(wire(message(undefined, "sourceA:1")), {
          headers: { authorization: "Bearer sourceA" },
        }),
      )
    ).status,
...
test("delivery authorization throw/false cannot leak errors or invoke receiver", async () => {
  for (const authorizeDelivery of [
    () => false,
    () => {
      throw Error("private policy");
    },
  ]) {
    let calls = 0;
    const handle = createChangesetHttpHandler(
      ...
      { authorize: () => true, authorizeDelivery },
    );
    const response = await handle(request());
    assert.equal(response.status, 403);
    assert.equal(calls, 0);
...
test("chunked uploads obey actual-byte bounds and release the admission slot", async () => {
  ...
  const handle = createChangesetHttpHandler(
    ...
    { authorize: () => true, maxMessageBytes: 1 },
  );
  assert.equal((await handle(request(body))).status, 413);
  assert.equal(receives, 0);
  assert.equal(cancelled, 1);
...
for (const headers of [
  ...
  test(`endpoint rejects invalid wire headers ${JSON.stringify(headers)}`, async () => {
    ...
    assert.ok([400, 413].includes((await handle(request(wire(), { headers }))).status));
    assert.equal(calls, 0);
  });
}
test("method/media gates run without entering authorization or SQL", async () => {
  let calls = 0;
  const handle = createChangesetHttpHandler(verifiedDouble, {
    authorize: () => {
      calls++;
      return true;
    },
  ...
  assert.equal(
    (await handle(request(wire(), { headers: { "content-type": "application/json" } }))).status,
    415,
  );
  assert.equal(calls, 0);
});
...
test("CORS preflight uses explicit origins/headers and never grants SQL authority", async () => {
  ...
  const response = await handle(
    new Request("https://receiver.test/changes", { method: "OPTIONS", headers }),
  );
...
for (const origin of [
  ...
  "https://evil.test",
  "https://app.test/",
  "https://app.test, https://evil.test",
  "https://user@app.test",
]) {
...
for (const options of [
  ...
  { allowedOrigins: ["https://user@app.test"] },
  { allowedHeaders: ["*"] },
  ...
  test(`invalid CORS configuration ${JSON.stringify(options)} fails at construction`, () => {
    assert.throws(
      () => createChangesetHttpHandler(verifiedDouble, { authorize: () => true, ...options }),
      isError("INPUT", "not-sent"),
    );
  });
}
test("unallowed preflight method/header and missing origin reject", async () => {
  const handle = createChangesetHttpHandler(verifiedDouble, {
    authorize: () => {
      throw Error("preflight must not authorize");
    },
    allowedOrigins: ["https://app.test"],
  });
  for (const headers of [
    {},
    { origin: "https://app.test", "access-control-request-method": "DELETE" },
    {
      origin: "https://app.test",
      "access-control-request-method": "POST",
    ...
    assert.equal(
      (await handle(new Request("https://receiver.test/changes", { method: "OPTIONS", headers })))
        .status,
      403,
    );
  }
...
test("cancellation retains the admission slot until receiver work actually drains", async () => {
  ...
  const first = handle(request(wire(), { signal: controller.signal })).then((result) => {
    finished = true;
    return result;
  });
  await entered.promise;
...
test("stalled uploads time out, cancel the reader and release capacity", async () => {
  ...
  const handle = createChangesetHttpHandler(
    ...
    { authorize: () => true, timeoutMs: 30 },
  );
  assert.equal((await handle(request(stream))).status, 408);
  assert.equal(cancelled, 1);
  assert.equal(calls, 0);
  assert.equal((await handle(request())).status, 200);
});
  ...
  const handle = createChangesetHttpHandler(
    {
      receiverId: "receiver",
      async receive() {
        throw Error("SQL password secret\n stack trace");
      },
    },
    { authorize: () => true },
  );
...
test("actual SDK receiver and inbox apply native session bytes over streaming HTTP", async (t) => {
  ...
  let confirmed = 0;
  const receiver = new ChangesetReceiver(sqlTarget(destination), {
    receiverId: "receiver",
    tables: ["notes"],
    confirmCommit: async () => {
      confirmed++;
  ...
  const handle = createChangesetHttpHandler(receiver, {
    authorize: (ctx) => ctx.headers.get("authorization") === "Bearer test",
  });
  ...
  assert.equal(destination.prepare("SELECT count(*) AS n FROM notes").get().n, 1);
  assert.equal(
    destination.prepare("SELECT count(*) AS n FROM __fsqlite_changeset_receipts").get().n,
    1,
  );
});
test("lost HTTP acknowledgement after commit recovers from a reopened receiver file", async (t) => {
  ...
  const makeHandler = () =>
    createChangesetHttpHandler(
      new ChangesetReceiver(sqlTarget(db), {
        receiverId: "receiver",
        tables: ["notes"],
        confirmCommit: async () => {},
      }),
  ...
  const deliver = client(url),
    envelope = message(nativeInsert(t));
  await assert.rejects(deliver(envelope, {}), isError("NETWORK"));
  db.close();
...
test("failed receiver confirmation returns no ACK and a retry reconfirms its retained decision", async (t) => {
  ...
  db.exec("CREATE TABLE notes(id INTEGER PRIMARY KEY, body TEXT)");
  let confirmations = 0;
  const receiver = new ChangesetReceiver(sqlTarget(db), {
    receiverId: "receiver",
    tables: ["notes"],
    confirmCommit: async () => {
...
test("SQL constraint failure over HTTP rolls back earlier rows and the inbox", async (t) => {
  ...
  await assert.rejects(
    client(url)(message(nativeInsert(t, "INSERT INTO notes VALUES(1,'a'),(2,'b')")), {}),
    isError("HTTP"),
  );
  ...
  assert.equal(
    db
      .prepare("SELECT count(*) AS n FROM sqlite_schema WHERE name='__fsqlite_changeset_receipts'")
      .get().n,
    0,
  );
});
test("digest tampering and unauthorized tables never reach receiver SQL", async (t) => {
  ...
  const handle = createChangesetHttpHandler(
    new ChangesetReceiver(observed, {
      receiverId: "receiver",
      tables: [],
      confirmCommit: async () => {},
    }),
...
test("complete capture/outbox/pump/HTTP/receiver path preserves ordered work after a lost ACK", async (t) => {
  ...
  for (const db of [source, destination])
    db.exec("CREATE TABLE notes(id INTEGER PRIMARY KEY, body TEXT); PRAGMA recursive_triggers=ON");
  const outbox = new ChangesetOutbox(sqlTarget(source));
  let callbacks = 0;
  await outbox.record(
    async (tx) => {
      callbacks++;
      await tx.execute("INSERT INTO notes VALUES(1,'before')");
    },
    { tables: ["notes"], deliveryId: "source:1" },
  );
  await outbox.record(
    async (tx) => {
      callbacks++;
      await tx.execute("UPDATE notes SET body='after' WHERE id=1");
    },
    { tables: ["notes"], deliveryId: "source:2" },
  );
  const receiver = new ChangesetReceiver(sqlTarget(destination), {
    receiverId: "receiver",
    tables: ["notes"],
  ...
  const url = await server(
    ...
    httpBridge(
      () => handle,
      (response, _q, r) => {
        if (response.status === 200 && !dropped) {
          dropped = true;
          r.destroy();
          return true;
        }
  ...
  await assert.rejects(
    ...
    (e) =>
      e.phase === "transport" &&
      e.cause instanceof ChangesetHttpError &&
      e.cause.outcome === "unknown",
  );
  ...
  assert.equal(recovered.deliveries, 2);
  assert.equal(recovered.replays, 1);
  assert.equal(recovered.stopped, "empty");
  ...
  assert.equal(
    source
      .prepare(
        "SELECT count(*) AS n FROM __fsqlite_changeset_outbox WHERE acknowledged=1 AND length(payload)=0",
      )
      .get().n,
    2,
  );
...
test("a separate HTTP receiver process killed after commit recovers its exact retained receipt", {
  timeout: 10_000,
}, async (t) => {
  const path = join(mkdtempSync(join(tmpdir(), "fsqlite-http-kill-")), "receiver.db");
  const init = new DatabaseSync(path);
  init.exec("CREATE TABLE notes(id INTEGER PRIMARY KEY, body TEXT)");
  init.close();
  ...
  `;
  const loaderFlags = process.execArgv.filter(
    (a) => a.startsWith("--experimental-loader=") || a.startsWith("--loader="),
  );
  assert.ok(loaderFlags.length, "run with the documented TypeScript source loader");
  const child = spawn(process.execPath, [...loaderFlags, "--input-type=module", "-e", script], {
  ...
  const startup = await Promise.race([
    ...
    exited.then(() => {
      throw Error(`Receiver exited before startup: ${diagnostics}`);
    }),
  ]);
  const envelope = message(nativeInsert(t));
  ...
  const handler = createChangesetHttpHandler(
    new ChangesetReceiver(sqlTarget(reopened), {
      receiverId: "receiver",
      tables: ["notes"],
      confirmCommit: async () => {},
    }),
  ...
  assert.equal(reopened.prepare("SELECT count(*) AS n FROM notes").get().n, 1);
  assert.equal(
    reopened.prepare("SELECT count(*) AS n FROM __fsqlite_changeset_receipts").get().n,
    1,
  );
});
