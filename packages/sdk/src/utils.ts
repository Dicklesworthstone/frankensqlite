...
export function resolveWorker(worker: FrankenDbOpenOptions["worker"]): WorkerLike {
...
export function captureRequiredCheckpoint(
  options: FrankenDbOpenOptions,
): Readonly<CheckpointRecoveryIdentity> | null {
  ...
  const invalid = (): never => {
    throw new FrankenSQLiteError({
      ...
      transient: false,
      message:
        "requireCheckpoint must identify a publication in the requested snapshot database, without an initialization image",
    });
  };
  ...
  if (
    ...
    path.includes("\0") ||
    path === ":memory:" ||
    !isSnapshotPersistenceMode(persistence) ||
    persistence !== options.persistence ||
    path !== options.dbName ||
    !revision(publicationId) ||
    (parentRevision !== null && !revision(parentRevision)) ||
    publicationId === parentRevision ||
