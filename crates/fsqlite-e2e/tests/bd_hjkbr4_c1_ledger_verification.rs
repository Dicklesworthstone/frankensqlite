...
fn l2_commit_profile_reset() {
    ...
    asupersync::test_utils::run_test(|| async {
        ...
        let conn = fsqlite::Connection::open(":memory:").await.unwrap();
        conn.execute("CREATE TABLE l2 (x INTEGER)").await.unwrap();
        conn.execute("INSERT INTO l2 VALUES (1)").await.unwrap();
...
fn l3_commit_profile_enable_disable() {
    ...
    asupersync::test_utils::run_test(|| async {
        ...
        let conn = fsqlite::Connection::open(":memory:").await.unwrap();
        conn.execute("CREATE TABLE l3 (x INTEGER)").await.unwrap();
        conn.execute("BEGIN").await.unwrap();
        for i in 0..50 {
...
fn l5_prepared_hit_rate_proof() {
    ...
    asupersync::test_utils::run_test(|| async {
        ...
        let _ = conn.query(query).await.unwrap();
...
fn l6_publication_snapshot_advances() {
    ...
    asupersync::test_utils::run_test(|| async {
        ...
        conn.execute("CREATE TABLE l6 (x INTEGER)").await.unwrap();
        ...
        for i in 0..5 {
            ...
            let rows = conn.query("SELECT COUNT(*) FROM l6").await.unwrap();
...
fn l8_file_backed_commit_profile() {
    ...
    asupersync::test_utils::run_test(|| async {
        ...
        let rows = conn.query("SELECT COUNT(*) FROM l8").await.unwrap();
        let count = match &rows[0].values()[0] {
            fsqlite_types::value::SqliteValue::Integer(n) => *n,
            other => panic!("unexpected: {other:?}"),
        };
...
fn l9_commit_profile_evidence_pack() {
    ...
    asupersync::test_utils::run_test(|| async {
        ...
        for q in &queries {
            ...
            for _ in 0..20 {
                let _ = conn.query(q).await.unwrap();
            }
        ...
        // Final verification
        let final_count = conn.query("SELECT COUNT(*) FROM evidence").await.unwrap();
