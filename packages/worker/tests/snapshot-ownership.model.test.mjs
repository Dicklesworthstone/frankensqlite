...
import { SnapshotSessionLease as Lease, resolveSnapshotOwnership } from '../src/snapshot-ownership.ts';

function install(t, manager) {
  const original = Object.getOwnPropertyDescriptor(globalThis, 'navigator');
  Object.defineProperty(globalThis, 'navigator', {configurable:true, value:manager === undefined ? {} : {locks:manager}});
  t.after(() => {
...
function model(t) {
  ...
  const manager = {async request(name, options, callback) {
    ...
    const blocked = [...held].some(lock => lock.name === name &&
      (options.mode === 'exclusive' || lock.mode === 'exclusive'));
    if (blocked) return callback(null);
    const lock = {name, mode:options.mode}; held.add(lock);
    try { return await callback(lock); }
    finally { held.delete(lock); }
  }};
  install(t, manager);
  return {held, calls};
}
...
test('default sessions share ownership and exclude exclusive peers', async t => {
  ...
  assert.equal(a.mode,'shared'); assert.equal(held.size,2);
  await assert.rejects(Lease.acquire('opfs-snapshot','db','exclusive'), denied);
  await a.close();
  await assert.rejects(Lease.acquire('opfs-snapshot','db','exclusive'), denied);
  await b.close(); assert.equal(held.size,0);
  const exclusive = await Lease.acquire('opfs-snapshot','db','exclusive'); await exclusive.close();
});
...
test('an exclusive owner excludes default/shared/exclusive sessions', async t => {
  ...
  await a.close();
  const b = await Lease.acquire('indexeddb-snapshot','db'); await b.close();
});
...
test('unrelated names and storage backends have separate authority', async t => {
  ...
  assert.equal(held.size,12); assert.equal(new Set(calls.map(call => call.name)).size,12);
  await Promise.all(leases.map(lease=>lease.close())); assert.equal(held.size,0);
});
...
test('close returns one joined promise and retires transfer eligibility immediately', async t => {
  ...
  const first = lease.close(); assert.equal(first,lease.close());
  assert.equal(lease.matches('opfs-snapshot','db','exclusive'),false);
  await first; assert.equal(held.size,0); assert.equal(Object.isFrozen(lease),true);
});
...
test('only an unspecified default may operate without Web Locks', async t => {
  ...
  for (const mode of ['shared','exclusive']) {
    await assert.rejects(Lease.acquire('indexeddb-snapshot','db',mode), {code:'ERR_FSQLITE_SNAPSHOT_OWNERSHIP_UNAVAILABLE'});
  }
});

test('invalid identities or explicit policies fail before contacting the manager', async t => {
  const {calls} = model(t);
  for (const args of [['memory','db','shared'],['opfs-snapshot','',undefined],['opfs-snapshot',':memory:',undefined],
    ['opfs-snapshot','a\0b','exclusive'],['opfs-snapshot','x'.repeat(257),'exclusive'],
    ['opfs-snapshot','db',null],['opfs-snapshot','db',false],['opfs-snapshot','db','typo']]) {
...
test('concurrent exclusive attempts grant exactly one lease without waiters', async t => {
  const {held} = model(t);
  const attempts = await Promise.allSettled(Array.from({length:100},()=>Lease.acquire('opfs-snapshot','db','exclusive')));
  assert.equal(attempts.filter(v=>v.status==='fulfilled').length,1);
  assert.equal(attempts.filter(v=>v.status==='rejected' && v.reason.code===denied.code).length,99);
  assert.equal(held.size,1);
  await attempts.find(v=>v.status==='fulfilled').value.close(); assert.equal(held.size,0);
});
...
test('lock-manager refusal and thrown errors never become a granted lease', async t => {
  ...
  install(t,{request(){throw failure;}});
  ...
  await assert.rejects(Lease.acquire('opfs-snapshot','db','exclusive'),{code:'ERR_FSQLITE_SNAPSHOT_OWNERSHIP_UNAVAILABLE'});
});
