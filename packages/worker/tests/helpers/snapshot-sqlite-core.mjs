...
export function sqliteSnapshotWorker(hooks = {}) {
  ...
  async function open(bytes) {
    ...
    let closed = false;
    let exports = 0;
    const parameters = params => params.map(value => typeof value === "boolean" ? Number(value) : value);
    ...
    const core = {
      ...
      async export() {
        events.push("export"); await hooks.beforeExport?.();
  ...
  const worker = {
    requests: [],
    terminateCount: 0,
    addEventListener(type, listener) { (type === "message" ? messages : errors).add(listener); },
    removeEventListener(type, listener) { (type === "message" ? messages : errors).delete(listener); },
    postMessage(request, transfer) {
      ...
      void host.handle(copy).then(response => {
        if (!terminated) for (const listener of messages) listener({ data: structuredClone(response) });
      }, error => { for (const listener of errors) listener({ message: String(error) }); });
    },
    terminate() { this.terminateCount++; terminated = true; },
  };
