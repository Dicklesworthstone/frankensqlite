...
export type ChangesetDeliveryPhase =
  | "admission"
  | "validate"
  | "receiver-apply"
  | "receiver-confirm"
...
export class ChangesetDeliveryError extends Error {
  constructor(
    readonly code:
      | "ERR_FSQLITE_DELIVERY_INPUT"
      | "ERR_FSQLITE_DELIVERY_BUSY"
      | "ERR_FSQLITE_DELIVERY_FAILED"
      | "ERR_FSQLITE_DELIVERY_RECEIPT"
      | "ERR_FSQLITE_DELIVERY_CANCELLED"
      | "ERR_FSQLITE_DELIVERY_TIMEOUT"
      | "ERR_FSQLITE_DELIVERY_LIMIT",
    readonly phase: ChangesetDeliveryPhase,
    readonly deliveryId: string | null,
    cause: unknown,
  ) {
    super(
      `Changeset delivery stopped during ${phase}; retain the same delivery identity and reconcile commit/confirmation state before retrying`,
      { cause },
    );
...
function input(message: string): never {
  return reject("ERR_FSQLITE_DELIVERY_INPUT", message);
}
function failure(
  cause: unknown,
  phase: ChangesetDeliveryPhase,
  id: string | null,
): ChangesetDeliveryError {
  return new ChangesetDeliveryError(
    cause instanceof ChangesetDeliveryError ? cause.code : "ERR_FSQLITE_DELIVERY_FAILED",
    phase,
    id,
    cause,
  );
...
function identity(value: unknown, maximum: number): string {
  if (typeof value !== "string" || !value.length || value.length > maximum || value.includes("\0"))
    input("Invalid delivery/receiver identity");
  const bytes = new TextEncoder().encode(value);
  if (
    bytes.length > maximum ||
    new TextDecoder("utf-8", { ignoreBOM: true }).decode(bytes) !== value
  )
    input("Identity must be bounded valid UTF-8");
  return value;
}
function digest(value: unknown): string {
  if (typeof value !== "string" || !/^[0-9a-f]{64}$/.test(value))
    input("Expected lowercase SHA-256");
  return value;
}
function bound(value: unknown, fallback: number, maximum: number): number {
  ...
  if (typeof n !== "number" || !Number.isSafeInteger(n) || n < 1 || n > maximum)
    input(`Expected an integer in 1..${maximum}`);
  return n;
}
...
async function hash(bytes: Uint8Array): Promise<string> {
  if (globalThis.crypto?.subtle === undefined)
    input("Changeset delivery requires Web Crypto SHA-256");
...
function resultCounts(value: unknown, changes: number): ApplyChangesetResult {
  const applied = field(value, "applied"),
    omitted = field(value, "omitted"),
...
class DeliveryBudget {
  ...
  constructor(options: ChangesetDeliveryOptions = {}) {
    ...
    if (external !== undefined) {
      try {
        ...
        input("signal must be an AbortSignal");
      }
    }
  ...
  checkpoint(): void {
    ...
    if (this.signal.aborted)
      throw new ChangesetDeliveryError(
        this.signal.reason === this.#reason
          ? "ERR_FSQLITE_DELIVERY_TIMEOUT"
          : "ERR_FSQLITE_DELIVERY_CANCELLED",
        "admission",
        null,
...
export class ChangesetReceiver {
  ...
  constructor(target: ChangesetTarget, options: ChangesetReceiverOptions) {
    ...
    for (let i = 0, n = source.length; i < n; i++) {
      const name = identity(source[i], 1024),
        key = fold(name);
      if (seen.has(key) || key.startsWith("sqlite_") || key.startsWith("__fsqlite_"))
        input("Use distinct application tables, not SDK/system tables");
      seen.add(key);
      tables.push(name);
    }
    if (
      typeof confirm !== "function" ||
      (onConflict !== undefined && typeof onConflict !== "function")
    )
      input("A commit confirmation function and valid conflict policy are required");
  ...
  ): Promise<ChangesetDeliveryReceipt> {
    if (this.#active)
      throw new ChangesetDeliveryError(
        "ERR_FSQLITE_DELIVERY_BUSY",
        "admission",
        null,
        new Error("This receiver is active; no additional request was queued"),
      );
    this.#active = true;
    let budget: DeliveryBudget | undefined,
      phase: ChangesetDeliveryPhase = "validate",
      id: string | null = null;
    try {
      budget = new DeliveryBudget(options);
      budget.checkpoint();
      if (
        field(message, "protocol") !== CHANGESET_DELIVERY_PROTOCOL ||
        field(message, "receiverId") !== this.#id
      )
        input("Wrong delivery protocol or receiver");
      id = identity(field(message, "deliveryId"), 512);
      const sha256 = digest(field(message, "sha256")),
        bytes = ownBytes(field(message, "changeset"), this.#maxBytes);
...
export type ChangesetTransport = (
...
function deliveryMetadata(value: unknown): OutboxDelivery {
  const sequence = field(value, "sequence"),
    ...
    changes = field(value, "changes");
  const deliveryId = identity(field(value, "deliveryId"), 512),
  ...
  if (
    ...
    input("Invalid outbox delivery metadata");
...
function verifyReceipt(
  ...
  if (
    field(value, "protocol") !== CHANGESET_DELIVERY_PROTOCOL ||
    field(value, "receiverId") !== receiverId ||
    field(value, "deliveryId") !== delivery.deliveryId ||
    field(value, "sha256") !== delivery.sha256 ||
    field(value, "byteLength") !== delivery.byteLength ||
...
export class ChangesetDeliveryPump {
  ...
  #active = false;
  constructor(
    outbox: Pick<ChangesetOutbox, "pending" | "read" | "acknowledge">,
    options: ChangesetPumpOptions,
  ) {
    this.#outbox = outbox;
    this.#id = identity(options?.receiverId, 256);
    const deliver = options?.deliver,
      confirm = options?.confirmSource;
    const allowOmissions = options?.allowOmissions ?? false;
    if (
      typeof deliver !== "function" ||
      typeof confirm !== "function" ||
      typeof allowOmissions !== "boolean"
    )
      input("Delivery requires a transport, source confirmation and explicit omission policy");
    this.#deliver = deliver;
    this.#confirm = confirm;
    this.#allowOmissions = allowOmissions;
    this.#maxBytes = bound(options?.maxMessageBytes, 8 * 1024 * 1024, HARD_BYTES);
  }
  ...
  async run(options: ChangesetPumpRunOptions = {}): Promise<ChangesetPumpResult> {
    if (this.#active)
      throw new ChangesetDeliveryError(
        ...
        "admission",
        null,
        new Error("This pump is active; no run was queued"),
      );
    this.#active = true;
    let budget: DeliveryBudget | undefined,
      phase: ChangesetDeliveryPhase = "admission",
      id: string | null = null;
    const counts = {
      deliveries: 0,
      bytes: 0,
    ...
    const result = (stopped: "empty" | "limit"): ChangesetPumpResult =>
      Object.freeze({ ...counts, stopped });
    try {
      const maxDeliveries = bound(options.maxDeliveries, 100, 10_000),
        maxBytes = bound(options.maxBytes, HARD_BYTES, 1024 * 1024 * 1024);
      budget = new DeliveryBudget(options);
      budget.checkpoint();
      ...
      for (let selected = 0; selected < maxDeliveries; selected++) {
        ...
        phase = "source-read";
        budget.checkpoint();
        const pending = await this.#outbox.pending({ limit: 1 });
        budget.checkpoint();
        if (!Array.isArray(pending) || pending.length > 1)
          input("Outbox exceeded its requested page bound");
        if (pending.length === 0) return result("empty");
        const next = deliveryMetadata(pending[0]);
        id = next.deliveryId;
        if (next.acknowledged || next.sequence <= lastSequence)
          input("Outbox did not advance past confirmed acknowledgements");
        if (next.byteLength > this.#maxBytes)
          reject(
            "ERR_FSQLITE_DELIVERY_LIMIT",
            "Pending message exceeds maxMessageBytes; it was not skipped",
          );
        if (next.byteLength > maxBytes - counts.bytes) {
          if (counts.deliveries === 0)
            reject(
              "ERR_FSQLITE_DELIVERY_LIMIT",
              "The oldest message cannot fit this run's byte budget",
            );
          return result("limit");
        }
        const loaded = await this.#outbox.read(id);
        budget.checkpoint();
        if (loaded === null)
          input("Selected outbox delivery disappeared; reconcile its retention state");
        const current = deliveryMetadata(field(loaded, "delivery")),
          data = field(loaded, "changeset");
        if (!sameDelivery(next, current))
          input("Outbox delivery changed between selection and read");
        if (current.acknowledged) {
          if (data !== null)
            input("An acknowledged outbox entry retained unexpected payload bytes");
          phase = "source-confirm";
          await this.#confirm();
          budget.checkpoint();
          counts.alreadyAcknowledged++;
        ...
        if (
          ...
          (await hash(bytes)) !== current.sha256 ||
        ...
        const remaining = budget.remainingMs();
        if (remaining !== undefined) transportOptions.timeoutMs = remaining;
        ...
        if (receipt.omitted !== 0 && !this.#allowOmissions)
          reject(
            "ERR_FSQLITE_DELIVERY_RECEIPT",
            "Receiver omissions require explicit sender acceptance",
          );
        budget.checkpoint();
        phase = "source-ack";
        ...
        budget.checkpoint();
        counts.deliveries++;
        counts.bytes += current.byteLength;
        counts.applied += receipt.applied;
        counts.omitted += receipt.omitted;
