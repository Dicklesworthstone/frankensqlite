...
import { applyChangeset, ChangesetApplyError, CHANGESET_RECEIPTS_TABLE } from '../src/changeset-apply.ts';
...
class SqlTarget {
  ...
  afterExecute;
  afterQuery;
  constructor(sql = '', path = ':memory:') { this.db = new DatabaseSync(path); this.db.exec(sql); }
...
test('a retained delivery returns its original result without replaying SQL or callbacks', async t => {
  const actual = target(t, "CREATE TABLE t(id PRIMARY KEY,v); INSERT INTO t VALUES(2,'local'); CREATE TABLE audit(x); CREATE TRIGGER record_insert AFTER INSERT ON t BEGIN INSERT INTO audit VALUES(NEW.id); END;");
  const bytes = wire([insert(1n, 'one'), insert(2n, 'two')]);
  const first = await applyChangeset(actual, bytes, { ...deliveryOptions, onConflict: () => 'omit' });
  assert.deepEqual(first, { applied: 1, omitted: 1, replayed: false });
  const offset = actual.log.length;
  const second = await applyChangeset(actual, bytes, { ...deliveryOptions, onConflict() { assert.fail('duplicate must not re-resolve a conflict'); } });
  assert.deepEqual(second, { applied: 1, omitted: 1, replayed: true });
  assert.ok(Object.isFrozen(second));
  assert.equal(actual.log.slice(offset).some(sql => /^(INSERT|UPDATE|DELETE|CREATE)/.test(sql)), false);
...
test('delivery identity cannot be reused for different bytes, even equal-length payloads', async t => {
  const actual = target(t, 'CREATE TABLE t(id PRIMARY KEY,v);');
  const first = wire([insert(1n, 'one')]), changed = wire([insert(2n, 'two')]);
  assert.equal(first.length, changed.length);
  await applyChangeset(actual, first, deliveryOptions);
  await assert.rejects(applyChangeset(actual, changed, deliveryOptions), code('ERR_FSQLITE_CHANGESET_DELIVERY_REUSE'));
...
test('receipt failures and ordinary constraint failures do not burn delivery identities', async t => {
  const actual = target(t, "CREATE TABLE t(id PRIMARY KEY,v UNIQUE); INSERT INTO t VALUES(8,'occupied');");
  ...
  actual.afterExecute = sql => { if (sql.startsWith(`INSERT OR ABORT INTO ${inbox}`)) throw new Error('receipt write failed'); };
...
test('cancelled applications roll back both row changes and inbox creation', async t => {
  const actual = target(t, 'CREATE TABLE t(id PRIMARY KEY,v);'), cancellation = new AbortController();
  actual.afterExecute = sql => { if (sql.startsWith('INSERT OR ABORT INTO main."t"')) cancellation.abort('cancel'); };
  await assert.rejects(applyChangeset(actual, wire([insert(1n, 'one')]), {
    ...deliveryOptions, signal: cancellation.signal,
  }), code('ERR_FSQLITE_CHANGESET_CANCELLED'));
  assert.deepEqual(actual.rows(), []);
  assert.equal(actual.db.prepare('SELECT count(*) AS n FROM main.sqlite_schema WHERE name=?').get(CHANGESET_RECEIPTS_TABLE).n, 0);
});

test('deferred commit failure cannot persist a successful delivery receipt', async t => {
  const actual = target(t, 'PRAGMA foreign_keys=ON; CREATE TABLE parent(id PRIMARY KEY); CREATE TABLE t(id PRIMARY KEY,v REFERENCES parent(id) DEFERRABLE INITIALLY DEFERRED);');
  const bytes = wire([insert(1n, 8n)]);
  await assert.rejects(applyChangeset(actual, bytes, deliveryOptions), /FOREIGN KEY constraint failed/);
  assert.deepEqual(actual.rows(), []);
...
test('lost transaction acknowledgement is reconciled from the committed inbox', async t => {
  const actual = target(t, 'CREATE TABLE t(id PRIMARY KEY,v);');
  const bytes = wire([insert(1n, 'one')]);
  const lostAck = { async transaction(work, options) {
    await actual.transaction(work, options);
    throw new Error('connection lost after commit');
  } };
  await assert.rejects(applyChangeset(lostAck, bytes, deliveryOptions), /connection lost after commit/);
  assert.deepEqual(await applyChangeset(actual, bytes, deliveryOptions), { applied: 1, omitted: 0, replayed: true });
  assert.deepEqual(actual.rows(), [{ id: 1n, v: 'one' }]);
});

test('rolling back an outer transaction also rolls back the nested delivery receipt', async t => {
  const actual = target(t, 'CREATE TABLE t(id PRIMARY KEY,v);'), bytes = wire([insert(1n, 'one')]);
  await assert.rejects(actual.transaction(async tx => {
...
test('hashing and decoded work capture the same bytes before yielding', async t => {
  const actual = target(t, 'CREATE TABLE t(id PRIMARY KEY,v);');
  const bytes = wire([insert(1n, new Uint8Array([0, 255]))]), original = new Uint8Array(bytes);
  const policy = { ...deliveryOptions };
  const pending = applyChangeset(actual, bytes, policy);
  bytes.fill(0); policy.deliveryId = 'mutated';
...
test('invalid delivery IDs and reserved inbox targets are rejected before SQL', async t => {
  ...
  for (const deliveryId of ['', 'a\0b', '\ud800', 'a'.repeat(513), '界'.repeat(171), 4, null]) {
    await assert.rejects(applyChangeset(actual, bytes, { ...options, deliveryId }), code('ERR_FSQLITE_CHANGESET_INPUT'));
  }
  await assert.rejects(applyChangeset(actual, wire([insert(1n, 'one')], CHANGESET_RECEIPTS_TABLE), {
...
test('empty changesets can be acknowledged without creating the inbox by default', async t => {
  ...
  assert.deepEqual(await applyChangeset(actual, new Uint8Array(), { tables: [] }), { applied: 0, omitted: 0, replayed: false });
  assert.equal(actual.db.prepare('SELECT count(*) AS n FROM main.sqlite_schema').get().n, 0);
  assert.equal((await applyChangeset(actual, new Uint8Array(), { tables: [], deliveryId: 'empty' })).replayed, false);
  assert.equal((await applyChangeset(actual, new Uint8Array(), { tables: [], deliveryId: 'empty' })).replayed, true);
});
...
test('replay is a historical receipt, not a claim that later application data is unchanged', async t => {
  ...
  await assert.rejects(applyChangeset(actual, bytes, { tables: [], deliveryId: deliveryOptions.deliveryId }), code('ERR_FSQLITE_CHANGESET_INPUT'));
});
...
for (const [label, sql] of [
  ...
  ['wrong collation', `CREATE TABLE ${inbox}(delivery_id TEXT NOT NULL PRIMARY KEY COLLATE NOCASE,sha256 TEXT NOT NULL,byte_length INTEGER NOT NULL,applied INTEGER NOT NULL,omitted INTEGER NOT NULL)`],
]) {
  test(`refuse a pre-existing inbox with ${label}`, async t => {
    const actual = target(t, `CREATE TABLE t(id PRIMARY KEY,v); ${sql}`);
    await assert.rejects(applyChangeset(actual, wire([insert(1n, 'one')]), deliveryOptions), code('ERR_FSQLITE_CHANGESET_RECEIPT'));
    assert.deepEqual(actual.rows(), []);
  });
}
...
for (const namespace of ['main', 'temp']) {
  test(`refuse ${namespace} triggers that could alter an inbox decision`, async t => {
    const actual = target(t, 'CREATE TABLE t(id PRIMARY KEY,v);');
    await applyChangeset(actual, new Uint8Array(), { ...options, deliveryId: 'bootstrap' });
    actual.db.exec(`CREATE ${namespace === 'temp' ? 'TEMP ' : ''}TRIGGER inbox_ignore BEFORE INSERT ON ${inbox} BEGIN SELECT RAISE(IGNORE); END;`);
    await assert.rejects(applyChangeset(actual, wire([insert(1n, 'one')]), deliveryOptions), code('ERR_FSQLITE_CHANGESET_RECEIPT'));
    assert.deepEqual(actual.rows(), []);
    assert.equal(inboxCount(actual), 1);
  });
...
test('receipt corruption cannot be accepted as a successful replay', async t => {
  ...
  await assert.rejects(applyChangeset(actual, bytes, deliveryOptions), code('ERR_FSQLITE_CHANGESET_RECEIPT'));
...
test('file-backed delivery receipt survives another process reopening the database', async t => {
  ...
  const actual = new SqlTarget('CREATE TABLE t(id PRIMARY KEY,v); CREATE TABLE audit(x); CREATE TRIGGER seen AFTER INSERT ON t BEGIN INSERT INTO audit VALUES(NEW.id); END;', path);
  const bytes = wire([insert(1n, 'durable')]);
  try { await applyChangeset(actual, bytes, deliveryOptions); }
  ...
  const child = spawnSync(process.execPath, ['--experimental-loader', loader, '--input-type=module', '-e', program], {
...
test('cancellation during hashing prevents transaction admission', async t => {
  const actual = target(t, 'CREATE TABLE t(id PRIMARY KEY,v);'), abort = new AbortController();
  const pending = applyChangeset(actual, wire([insert(1n, 'one')]), { ...deliveryOptions, signal: abort.signal });
...
test('missing Web Crypto rejects receipt mode before SQL, not the ordinary apply path', async t => {
  const actual = target(t, 'CREATE TABLE t(id PRIMARY KEY,v);'), original = Object.getOwnPropertyDescriptor(globalThis, 'crypto');
  Object.defineProperty(globalThis, 'crypto', { configurable: true, value: undefined });
  try {
    await assert.rejects(applyChangeset(actual, wire([insert(1n, 'one')]), deliveryOptions), code('ERR_FSQLITE_CHANGESET_INPUT'));
    assert.equal(actual.scopes, 0);
...
test('two overlapping file connections do not apply a delivery twice', async t => {
  ...
  const left = new SqlTarget('PRAGMA journal_mode=WAL; CREATE TABLE t(id PRIMARY KEY,v); CREATE TABLE audit(x); CREATE TRIGGER seen AFTER INSERT ON t BEGIN INSERT INTO audit VALUES(NEW.id); END;', path);
  const right = new SqlTarget('', path);
  t.after(() => left.db.close()); t.after(() => right.db.close());
  ...
  for (const client of [left, right]) {
    ...
    client.query = async (sql, params) => {
      ...
      if (sql.startsWith('SELECT sha256,') && params[0] === deliveryOptions.deliveryId && result.rowArrays.length === 0) {
        if (++ready === 2) release();
        await barrier;
      }
      return result;
  ...
  const bytes = wire([insert(1n, 'one')]);
  const outcomes = await Promise.allSettled([applyChangeset(left, bytes, deliveryOptions), applyChangeset(right, bytes, deliveryOptions)]);
