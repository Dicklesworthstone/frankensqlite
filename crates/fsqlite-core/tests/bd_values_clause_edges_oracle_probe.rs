...
fn values_clause_edges_match_rusqlite_oracle() {
    asupersync::test_utils::run_test(|| async {
        ...
        check(
            "bare values",
            fq(&f, "VALUES (1,'a'),(2,'b'),(3,'c')").await,
            rq(&r, "VALUES (1,'a'),(2,'b'),(3,'c')"),
            &mut diffs,
        );
        // VALUES in FROM subquery -> default names column1/column2
        check(
            "values in from default names",
            fq(
                &f,
                "SELECT column1, column2 FROM (VALUES (10,'x'),(20,'y')) ORDER BY column1",
            )
            .await,
            rq(
                &r,
        ...
        check(
            "values order limit",
            fq(&f, "VALUES (3),(1),(4),(1),(5),(9) ORDER BY 1 LIMIT 3").await,
            rq(&r, "VALUES (3),(1),(4),(1),(5),(9) ORDER BY 1 LIMIT 3"),
            &mut diffs,
        );
        ...
        check(
            "union of values",
            fq(&f, "VALUES (1),(2) UNION VALUES (2),(3) ORDER BY 1").await,
            rq(&r, "VALUES (1),(2) UNION VALUES (2),(3) ORDER BY 1"),
            &mut diffs,
        );
        ...
        check(
            ...
            fq(
                &f,
                "SELECT column1 FROM (VALUES (1),('two'),(3.5),(NULL)) ORDER BY column1",
            )
            .await,
            rq(
                &r,
        ...
        check(
            ...
            fq(
                &f,
                "SELECT (2,'b') = (SELECT column1,column2 FROM (VALUES (2,'b')))",
            )
            .await,
            rq(
                &r,
        ...
        check(
            ...
            fq(
                &f,
                "SELECT (SELECT max(column1) FROM (VALUES (7),(2),(9),(4)))",
            )
            .await,
            rq(
                &r,
        ...
        ex(
            &f,
            &r,
            "CREATE TABLE dst(id INTEGER, label TEXT, amount REAL)",
        )
        .await;
        ex(
            &f,
            &r,
            "INSERT INTO dst VALUES (1,'one','1'),(2,42,'2.5'),(3,'three',3)",
        )
        .await;
        check(
            "after values insert affinity",
            fq(&f, "SELECT id,label,amount FROM dst ORDER BY id").await,
            rq(&r, "SELECT id,label,amount FROM dst ORDER BY id"),
            &mut diffs,
        );
        ...
        assert!(
            diffs.is_empty(),
            "{} VALUES-clause divergence(s) vs rusqlite:\n{}",
            diffs.len(),
            diffs.join("\n")
        );
    });
