...
fn inner_joins() {
    asupersync::test_utils::run_test(|| async {
        agree(D, "SELECT a.name, b.val FROM a JOIN b ON a.id = b.aid ORDER BY a.name, b.val", "explicit INNER JOIN").await;
        agree(D, "SELECT a.name, b.val FROM a, b WHERE a.id = b.aid ORDER BY a.name, b.val", "comma INNER JOIN").await;
        agree(
            &["CREATE TABLE emp(id INT, mgr INT, name TEXT)", "INSERT INTO emp VALUES (1,NULL,'boss'),(2,1,'a'),(3,1,'b')"],
...
fn left_joins() {
    asupersync::test_utils::run_test(|| async {
        agree(D, "SELECT a.name, b.val FROM a LEFT JOIN b ON a.id = b.aid ORDER BY a.name, b.val", "LEFT JOIN NULL padding").await;
        agree(D, "SELECT a.name FROM a LEFT JOIN b ON a.id = b.aid WHERE b.id IS NULL ORDER BY a.name", "anti-join").await;
...
fn cross_using_natural_derived() {
    asupersync::test_utils::run_test(|| async {
        agree(
            &["CREATE TABLE x(n INT)","CREATE TABLE y(c TEXT)","INSERT INTO x VALUES (1),(2)","INSERT INTO y VALUES ('a'),('b')"],
            "SELECT n, c FROM x CROSS JOIN y ORDER BY n, c",
            "CROSS JOIN cartesian product",
        ).await;
        agree(
            &["CREATE TABLE p(id INT, v INT)","CREATE TABLE q(id INT, w INT)","INSERT INTO p VALUES (1,10),(2,20)","INSERT INTO q VALUES (1,100),(3,300)"],
            "SELECT id, v, w FROM p JOIN q USING (id) ORDER BY id",
            "USING coalesces the join column",
        ).await;
        agree(
            &["CREATE TABLE p(id INT, v INT)","CREATE TABLE q(id INT, w INT)","INSERT INTO p VALUES (1,10),(2,20)","INSERT INTO q VALUES (1,100),(2,200)"],
            "SELECT id, v, w FROM p NATURAL JOIN q ORDER BY id",
            "NATURAL JOIN on all common columns",
        ).await;
        agree(
            &["CREATE TABLE p(id INT, v INT)","CREATE TABLE q(id INT, w INT)","INSERT INTO p VALUES (1,10),(2,20),(3,30)","INSERT INTO q VALUES (1,100),(2,200)"],
