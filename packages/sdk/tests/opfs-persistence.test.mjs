...
function fixture(t) {
  ...
  t.after(async () => {
    ...
    model.restore();
    for (const [key, descriptor] of [['indexedDB', previousIdb], ['IDBObjectStore', previousStore]]) {
...
async function lostPublication(t, persistence, queue = false, publish = true) {
  ...
  const db = queue ? await FrankenDBQueue.open(options, { checkpointOnCommit: true }) : await FrankenDB.open(options);
  if (queue) await db.transaction(tx => tx.execute(schema));
  else { await db.execute(schema); await db.checkpoint(); }
  ...
  value.host.handle = async request => {
    ...
    return { kind: 'worker-fatal', error: { code: 'ERR_FSQLITE_WORKER_TRANSPORT', message: 'Publication delivery lost' } };
  };
  if (queue) {
    await assert.rejects(db.transaction(tx => tx.execute('INSERT INTO documents VALUES (1, ?, NULL)', ['restored'])),
      FrankenCheckpointCommitError);
  } else {
  ...
  assert.deepEqual({ path: ticket.path, persistence: ticket.persistence, parent: ticket.parentRevision },
...
for (const persistence of ['opfs-snapshot', 'indexeddb-snapshot']) {
  test(`${persistence} restores an exact failed publication after retiring the original worker`, async t => {
    ...
    assert.equal(opened.worker.requests.some(request => request.kind === 'checkpoint'), false);
    ...
    assert.deepEqual((await rows(reader.db)).rowArrays, [[1, 'restored', null], [2, 'next', null]]);
  });
  ...
  test(`${persistence} queued recovery ticket remains available after failed close`, async t => {
    ...
    const value = setup.connection();
    const opened = await FrankenDBQueue.open({ dbName: 'restart', persistence, requireCheckpoint: setup.ticket, worker: value.worker },
    ...
    await opened.transaction(tx => tx.execute('INSERT INTO documents VALUES (2, ?, NULL)', ['next']));
    await opened.close();
    const reader = await setup.open('restart', { persistence });
    assert.deepEqual((await rows(reader.db)).rowArrays, [[1, 'restored', null], [2, 'next', null]]);
  });
  ...
  test(`${persistence} requireCheckpoint refuses a superseded publication without claiming it never committed`, async t => {
    ...
    await peer.db.close();
    await assert.rejects(setup.open('restart', { persistence, requireCheckpoint: setup.ticket }),
      error => error.code === 'ERR_FSQLITE_SNAPSHOT_NOT_CONFIRMED' && error.suggestion.includes('does not prove'));
    const reader = await setup.open('restart', { persistence });
    assert.equal(reader.db.snapshotRevision, latest.revision);
    assert.deepEqual((await rows(reader.db)).rowArrays, [[1, 'restored', null], [2, 'later', null]]);
  });

  test(`${persistence} requireCheckpoint refuses absent storage and incorrect parent lineage`, async t => {
    const setup = await lostPublication(t, persistence);
    await assert.rejects(setup.open('restart', { persistence,
      requireCheckpoint: { ...setup.ticket, parentRevision: crypto.randomUUID() } }),
    { code: 'ERR_FSQLITE_SNAPSHOT_NOT_CONFIRMED' });
    await assert.rejects(setup.open('missing', { persistence,
      requireCheckpoint: { ...setup.ticket, path: 'missing' } }), { code: 'ERR_FSQLITE_SNAPSHOT_NOT_CONFIRMED' });
  });
  ...
  test(`${persistence} requireCheckpoint verifies stored bytes rather than trusting a revision alone`, async t => {
    ...
    if (persistence === 'opfs-snapshot') {
      ...
      const record = setup.indexeddb.databases.get('frankensqlite:snapshot:v1:restart').values.get('head');
...
test('checkpoint recovery identities validate before worker allocation or input transfer', async () => {
  const valid = { path: 'restart', persistence: 'opfs-snapshot', publicationId: crypto.randomUUID(), parentRevision: null };
  let creates = 0;
  const worker = () => { creates++; throw new Error('must not allocate'); };
  for (const requireCheckpoint of [null, [], {}, { ...valid, path: 'elsewhere' }, { ...valid, persistence: 'indexeddb-snapshot' },
    { ...valid, publicationId: 'bad' }, { ...valid, parentRevision: valid.publicationId },
    { ...valid, parentRevision: undefined }, { ...valid, parentRevision: 'bad' }]) {
    await assert.rejects(FrankenDB.open({ dbName: 'restart', persistence: 'opfs-snapshot', requireCheckpoint, worker }),
      { code: 'ERR_FSQLITE_SNAPSHOT_INPUT' });
  }
  const image = new Uint8Array(512);
  await assert.rejects(FrankenDB.open({ dbName: 'restart', persistence: 'opfs-snapshot',
    requireCheckpoint: valid, snapshot: image, worker }), { code: 'ERR_FSQLITE_SNAPSHOT_INPUT' });
  await assert.rejects(FrankenDB.open({ dbName: 'restart', persistence: 'memory', requireCheckpoint: valid, worker }),
    { code: 'ERR_FSQLITE_SNAPSHOT_INPUT' });
...
test('required checkpoint input is captured before an asynchronous open can mutate it', async t => {
  ...
  const entered = deferred(), release = deferred();
  const value = setup.connection({ beforeImport: async () => { entered.resolve(); await release.promise; } });
  const opening = FrankenDB.open({ dbName: 'restart', persistence: 'opfs-snapshot', requireCheckpoint: ticket, worker: value.worker });
  await entered.promise;
  ticket.publicationId = crypto.randomUUID(); ticket.parentRevision = null;
  release.resolve();
...
test('recovery identity is absent during pending publication and clears after same-session confirmation', async t => {
  ...
  await db.execute(schema);
  const entered = deferred(), release = deferred();
  model.hooks.afterClose = async () => { entered.resolve(); await release.promise; throw new Error('lost'); };
...
for (const persistence of ['opfs-snapshot', 'indexeddb-snapshot']) {
  test(`${persistence} required initial checkpoint cannot be silently ignored`, async t => {
    const setup = fixture(t);
    const value = setup.connection();
    const requireCheckpoint = { path: 'never-published', persistence, publicationId: crypto.randomUUID(), parentRevision: null };
    await assert.rejects(FrankenDB.open({ dbName: 'never-published', persistence, requireCheckpoint, worker: value.worker }),
      { code: 'ERR_FSQLITE_SNAPSHOT_NOT_CONFIRMED' });
    assert.equal(value.worker.terminateCount, 1);
