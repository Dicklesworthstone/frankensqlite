...
const code = (name) => (error) =>
  error?.code === name ||
  (error?.cause && code(name)(error.cause)) ||
  error?.errors?.some(code(name));
...
function disk(path, expected) {
  ...
  try {
    assert.deepEqual(
      db.prepare("SELECT id FROM items ORDER BY id").all().map(Object.values),
      expected.map((id) => [id]),
    );
...
test(
  "worker transport: actual clone failure after autocommit RETURNING reports uncertainty without rerunning SQL",
  limits,
  async (t) => {
    const f = await thread(t),
      db = await FrankenDB.open({ worker: f.worker });
    await db.execute("CREATE TABLE items(id INTEGER PRIMARY KEY)");
    const sql = "INSERT INTO items VALUES(1) RETURNING id";
    await f.configure({ uncloneable: sql });
    await assert.rejects(
      db.query(sql),
      (error) => code("ERR_FSQLITE_RESPONSE_TRANSFER")(error) && error.transient === false,
    );
...
test(
  "worker transport: actual clone failure inside a managed transaction rolls back and releases the connection",
  limits,
  async (t) => {
    const f = await thread(t),
      db = await FrankenDB.open({ worker: f.worker });
    ...
    await assert.rejects(
      ...
      code("ERR_FSQLITE_RESPONSE_TRANSFER"),
    );
    assert.equal(f.messages.filter((m) => m.fixture === "sql" && m.sql === sql).length, 1);
...
test(
  "worker transport: even an error serializer that repeatedly throws gets a correlated primitive failure",
  limits,
  async (t) => {
    const f = await thread(t),
      db = await FrankenDB.open({ worker: f.worker });
    await db.execute("CREATE TABLE items(id INTEGER PRIMARY KEY)");
    await f.configure({ badError: "SELECT 1" });
    await assert.rejects(db.query("SELECT 1"), code("ERR_FSQLITE_WORKER_DISPATCH"));
    assert.deepEqual((await db.query("SELECT 2 AS n")).rowArrays, [[2n]]);
    await db.close();
    disk(db.path, []);
  },
);
...
test(
  "worker transport: response and fallback failure produce one global fatal notice, then real acknowledged close",
  limits,
  async (t) => {
    const f = await thread(t),
      db = await FrankenDB.open({ worker: f.worker });
    await db.execute("CREATE TABLE items(id INTEGER PRIMARY KEY)");
    await f.configure({ sendFailure: "responses" });
    await assert.rejects(
    ...
    assert.equal(
      f.messages.filter((m) => m.fixture === "sql" && m.sql === "INSERT INTO items VALUES(2)")
        .length,
      0,
    );
    assert.ok(f.messages.some((m) => m.fixture === "closed"));
    disk(db.path, [1]);
  },
);

test(
  "worker transport: when even fatal delivery fails a real worker error event settles the original request",
  limits,
  async (t) => {
    const f = await thread(t),
      db = await FrankenDB.open({ worker: f.worker });
    await db.execute("CREATE TABLE items(id INTEGER PRIMARY KEY)");
    await f.configure({ sendFailure: "all" });
    const write = observe(db.execute("INSERT INTO items VALUES(1)"));
    const fault = await f.until((m) => m.fixture === "error");
    await write.settled;
    assert.match(fault.message, /outcomes are unknown/);
    assert.equal(write.outcome.status, "rejected");
    assert.equal(
      f.messages.filter((m) => m.fixture === "sql" && m.sql === "INSERT INTO items VALUES(1)")
        .length,
      1,
    );
...
test(
  "worker transport: receiver decode fault fences queued writes and close waits for active SQL",
  limits,
  async (t) => {
    const f = await thread(t),
      db = await FrankenDB.open({ worker: f.worker });
    ...
    await f.configure({ pause: sql });
    const active = observe(db.execute(sql));
    await f.paused();
    const queued = observe(db.execute("INSERT INTO items VALUES(2)"));
    f.decode();
    ...
    assert.equal(
      f.messages.some((m) => m.fixture === "closed"),
      false,
    );
    assert.equal(active.outcome.status, "pending");
    assert.equal(queued.outcome.status, "pending");
    f.resume();
    await Promise.all([close.settled, active.settled, queued.settled]);
    ...
    assert.equal(
      f.messages.some((m) => m.fixture === "sql" && m.sql === "INSERT INTO items VALUES(2)"),
      false,
    );
...
test(
  "worker transport: malformed request without an id stops acceptance but preserves later close acknowledgement",
  limits,
  async (t) => {
    const f = await thread(t),
      db = await FrankenDB.open({ worker: f.worker });
    await db.execute("CREATE TABLE items(id INTEGER PRIMARY KEY)");
    f.native.postMessage(null);
    await f.until((m) => m.kind === "worker-fatal" && !m.audit);
...
test(
  "worker transport: lost close reply after a fatal notice surfaces an error instead of waiting forever",
  limits,
  async (t) => {
    const f = await thread(t),
      db = await FrankenDB.open({ worker: f.worker });
    await db.execute("CREATE TABLE items(id INTEGER PRIMARY KEY)");
    await f.configure({ pause: "SELECT 1" });
    const call = observe(db.query("SELECT 1"));
    await f.paused();
    f.decode();
    await f.until((m) => m.kind === "worker-fatal" && !m.audit);
    ...
    const fault = await f.until((m) => m.fixture === "error");
    await Promise.all([closed.settled, call.settled]);
    assert.match(fault.message, /outcomes are unknown/);
    assert.equal(closed.outcome.status, "rejected");
    assert.equal(f.terminated, 1);
    disk(db.path, []);
  },
);
...
test(
  "worker transport: idle subscriptions and live query demands fail on owner messageerror without another SQL call",
  limits,
  async (t) => {
    const f = await thread(t),
      q = await FrankenDBQueue.open({ worker: f.worker });
    await q.transaction((tx) => tx.execute("CREATE TABLE items(id INTEGER PRIMARY KEY)"));
    const changes = await q.changes(["items"]),
      live = await watchQuery(q, "SELECT * FROM items", { tables: ["items"] });
    await live.next();
    const event = observe(changes.next()),
      read = observe(live.next()),
      done = observe(live.done);
    f.messageError();
    await Promise.all([event.settled, read.settled, done.settled]);
    assert.equal(event.outcome.status, "rejected");
    assert.equal(read.outcome.status, "rejected");
    assert.equal(done.outcome.status, "rejected");
    assert.ok(code("ERR_FSQLITE_WORKER_TRANSPORT")(done.outcome.reason));
    await q.close().catch(() => {});
...
test(
  "worker transport: idle table scan releases snapshot and rejects prefix completion on a receive failure",
  limits,
  async (t) => {
    const f = await thread(t),
      q = await FrankenDBQueue.open({ worker: f.worker });
    ...
    f.messageError();
    await assert.rejects(scan.done, code("ERR_FSQLITE_WORKER_TRANSPORT"));
...
test(
  "worker transport: snapshot pool observes idle client failure, closes all replicas, and never serves a replacement",
  limits,
  async (t) => {
    const writer = await thread(t),
      db = await FrankenDB.open({ worker: writer.worker });
    await db.executeBatch(
      "CREATE TABLE items(id INTEGER PRIMARY KEY);INSERT INTO items VALUES(1);",
    );
    const image = await db.export();
    await db.close();
    const replicas = [await thread(t), await thread(t)];
    let creations = 0;
    const pool = await FrankenSnapshotPool.open(image, {
      workers: 2,
      worker: () => replicas[creations++].worker,
    });
    replicas[0].messageError();
    await assert.rejects(pool.query("SELECT * FROM items"), code("ERR_FSQLITE_POOL_UNUSABLE"));
    await pool.close().catch(() => {});
    assert.equal(creations, 2);
    assert.equal(pool.stats.state, "closed");
    assert.equal(replicas[0].wrappers.size, 0);
    assert.equal(replicas[1].wrappers.size, 0);
    assert.equal(replicas[1].terminated, 1);
  },
);

test(
  "worker transport: fatal during managed SQL drains the host before transaction failure can dispose it",
  limits,
  async (t) => {
    const f = await thread(t),
      q = await FrankenDBQueue.open({ worker: f.worker });
    ...
    const sql = "INSERT INTO items VALUES(1)";
    await f.configure({ pause: sql });
    const job = observe(q.transaction((tx) => tx.execute(sql)));
    await f.paused();
    ...
    f.resume();
    await Promise.all([job.settled, successor.settled]);
    await q.close().catch(() => {});
    assert.ok(code("ERR_FSQLITE_WORKER_TRANSPORT")(job.outcome.reason));
    assert.equal(successorRan, false);
    assert.equal(successor.outcome.status, "rejected");
    assert.ok(f.messages.some((m) => m.fixture === "closed"));
    disk(q.path, []);
  },
);
...
test(
  "worker transport: fatal during COMMIT returns uncertainty after drain, while the real committed write survives",
  limits,
  async (t) => {
    const f = await thread(t),
      db = await FrankenDB.open({ worker: f.worker });
    await db.execute("CREATE TABLE items(id INTEGER PRIMARY KEY)");
    await f.configure({ pause: "COMMIT" });
    const job = observe(db.transaction((tx) => tx.execute("INSERT INTO items VALUES(1)")));
    await f.paused();
    f.decode();
    await f.until((m) => m.kind === "worker-fatal" && !m.audit);
    await drain();
...
test(
  "worker transport: failed initialization joins an outstanding import/create before destroying its wrapper",
  limits,
  async (t) => {
    const f = await thread(t);
    await f.configure({ pauseInit: true });
    const opening = observe(FrankenDB.open({ worker: f.worker }));
    await f.paused();
    f.decode();
    await f.until((m) => m.kind === "worker-fatal" && !m.audit);
    await drain();
    assert.equal(opening.outcome.status, "pending");
    assert.equal(f.terminated, 0);
    f.resume();
    await opening.settled;
    assert.ok(code("ERR_FSQLITE_WORKER_TRANSPORT")(opening.outcome.reason));
    assert.ok(f.messages.some((m) => m.fixture === "closed"));
    assert.equal(f.terminated, 1);
    assert.equal(f.wrappers.size, 0);
  },
