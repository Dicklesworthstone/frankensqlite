...
function changed(): FrankenSQLiteError {
  return new FrankenSQLiteError({
    ...
    message:
      "A watched table or its notification triggers changed; unsubscribe before changing that schema",
    transient: false,
  });
}
...
export function captureTables(input: readonly string[]): readonly string[] {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (
      ...
      name.length > 1024 ||
      name.includes("\0") ||
      fold(name).startsWith("sqlite_") ||
      fold(name).startsWith("__fsqlite_watch_")
    ) {
      throw invalid(
        "Use nonempty, non-reserved table names of at most 1024 characters, without NUL",
      );
    }
...
export class TableChangeJournal {
  ...
  get tables(): readonly string[] {
    return Object.freeze([...this.#watches.keys()]);
  }
  ...
  async configure(
    ...
    await db.transaction(async (tx) => {
      ...
      if (!this.#initialized) {
        await tx.execute(
          `CREATE TEMP TABLE ${quote(this.#name)} (id INTEGER PRIMARY KEY, dirty INTEGER NOT NULL) WITHOUT ROWID`,
        );
      }
      for (const watch of this.#watches.values()) {
        if (definitions.has(watch.name)) continue;
        for (const trigger of watch.triggers.keys())
          await tx.execute(`DROP TRIGGER IF EXISTS temp.${quote(trigger)}`);
      ...
      for (const definition of definitions.values()) {
        ...
        for (const event of EVENTS) {
          ...
          await tx.execute(
            `CREATE TEMP TRIGGER ${quote(trigger)} AFTER ${event} ON main.${quote(definition.name)} ` +
              `BEGIN UPDATE ${quote(this.#name)} SET dirty = 1 WHERE id = ${id}; END`,
          );
          const metadata = await tx.query(
            "SELECT sql FROM temp.sqlite_master WHERE type = 'trigger' AND name = ?",
            [trigger],
          );
    ...
    return Object.freeze(
      names.map((name) => [...next.keys()].find((key) => fold(key) === fold(name))!),
    );
  }
  ...
  runWithRetry<T>(
    ...
    return db.transactionWithRetry(
      (outer, attempt) => this.#collect(outer, (tx) => work(tx, attempt)),
      options,
    );
  }

  async #collect<T>(
    outer: FrankenTransaction,
    work: (tx: FrankenTransaction) => T | Promise<T>,
  ): Promise<{ value: T; tables: readonly string[] }> {
    ...
    for (const row of result.rowArrays) {
      ...
      const name =
        typeof id === "number"
          ? byId.get(id)
          : typeof id === "bigint" && id <= BigInt(Number.MAX_SAFE_INTEGER)
            ? byId.get(Number(id))
            : undefined;
    ...
    if (tables.length !== 0)
      await outer.execute(`UPDATE temp.${quote(this.#name)} SET dirty = 0 WHERE dirty = 1`);
  ...
  async #verify(tx: FrankenTransaction, watches: ReadonlyMap<string, Watch>): Promise<void> {
    ...
    const triggers = await tx.query(
      `SELECT name, sql FROM temp.sqlite_master WHERE type = 'trigger' AND tbl_name IN (${names.map(() => "?").join(",")})`,
      names,
    );
