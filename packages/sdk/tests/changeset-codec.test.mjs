...
import { decodeChangeset, encodeChangeset, invertChangeset, ChangesetError } from '../src/changeset-codec.ts';
...
function db(t, schema = 'CREATE TABLE t(id INTEGER PRIMARY KEY, v)') {
  const d = new DatabaseSync(':memory:'); d.exec(schema); t.after(() => d.close()); return d;
}
function rows(d, name = 't') {
  const s = d.prepare(`SELECT * FROM "${name.replaceAll('"', '""')}" ORDER BY 1`);
  s.setReadBigInts(true);
  return s.all().map(row => Object.values(row).map(value => value instanceof Uint8Array ? [...value] : value));
}
function roundtrip(bytes) { assert.deepEqual(encodeChangeset(decodeChangeset(bytes)), bytes); }
...
test('empty SQLite session is an empty valid changeset', t => {
  const d = db(t), s = d.createSession();
  ...
  assert.deepEqual(invertChangeset(s.changeset()), new Uint8Array()); s.close();
});
test('native inserts round-trip exactly across every SQLite storage class', t => {
  const d = db(t), s = d.createSession();
  const values = [null, '', 'hello\0world', '\uFEFFUnicode λ🚀', Buffer.from([0, 255]),
    -(1n << 63n), (1n << 63n) - 1n, 1.5, Infinity, -Infinity];
  values.forEach((v, i) => d.prepare('INSERT INTO t VALUES (?,?)').run(i + 1, v));
  const bytes = s.changeset(); s.close();
  roundtrip(bytes);
  const restored = db(t);
...
test('generated UPDATE distinguishes untouched fields, SQL NULL and empty blobs', t => {
  ...
  const s = d.createSession(); d.exec("UPDATE t SET a=NULL,c=X''");
  const bytes = s.changeset(); s.close(); roundtrip(bytes);
...
test('native delete inversion restores all fields', t => {
  const d = db(t); d.exec("INSERT INTO t VALUES (1,'gone')");
  const s = d.createSession(); d.exec('DELETE FROM t'); const bytes = s.changeset(); s.close();
  roundtrip(bytes); assert.equal(decodeChangeset(bytes)[0].changes[0].operation, 'delete');
  assert.equal(d.applyChangeset(invertChangeset(bytes)), true);
...
test('composite WITHOUT ROWID key order bytes survive decoding and inversion', t => {
  ...
  d.exec("INSERT INTO t VALUES('a',1,'old')"); const s = d.createSession();
  d.exec("UPDATE t SET c='new'"); const bytes = s.changeset(); s.close(); roundtrip(bytes);
...
test('multiple tables and mixed operations round-trip and undo a native session', t => {
  ...
  const d = db(t, schema); d.exec("INSERT INTO t VALUES(1,'update'),(2,'delete'); INSERT INTO u VALUES('x',7)");
  const initial = [rows(d), rows(d, 'u')], s = d.createSession();
  d.exec("UPDATE t SET v='updated' WHERE id=1; DELETE FROM t WHERE id=2; INSERT INTO t VALUES(3,'new'); UPDATE u SET b=8");
  const bytes = s.changeset(); s.close(); roundtrip(bytes);
...
test('primary-key UPDATE generated as DELETE/INSERT remains interoperable', t => {
  const d = db(t); d.exec("INSERT INTO t VALUES(1,'v')"); const s = d.createSession();
  d.exec('UPDATE t SET id=9'); const bytes = s.changeset(); s.close(); roundtrip(bytes);
...
test('JavaScript-authored typed records apply using SQLite native applyChangeset', t => {
  ...
  const bytes = encodeChangeset([table([insert([7n, 42n, 42, 'data', new Uint8Array([8])])], [1,0,0,0,0])]);
...
test('UTF-8 table names, quotes and BOM text survive exact serialization', t => {
  ...
  roundtrip(bytes); assert.equal(d.applyChangeset(bytes), true);
...
test('blob decode owns a tight copy and sliced input uses correct offsets', () => {
  ...
  const backing = new Uint8Array(bytes.length + 16); backing.set(bytes, 7);
  const decoded = decodeChangeset(backing.subarray(7, 7 + bytes.length));
  backing.fill(0);
  const blob = decoded[0].changes[0].new[1];
  assert.deepEqual(blob, new Uint8Array([1,2,3])); assert.equal(blob.buffer.byteLength, 3);
  assert.equal(Object.isFrozen(decoded), true); assert.equal(Object.isFrozen(decoded[0].changes[0].new), true);
});
test('native patchsets are rejected rather than misread as complete before images', t => {
  const d = db(t), s = d.createSession(); d.exec('INSERT INTO t VALUES(1,2)');
  assert.throws(() => decodeChangeset(s.patchset()), formatError);
  assert.throws(() => invertChangeset(s.patchset()), formatError); s.close();
});
test('all truncated prefixes of a one-change record reject (except empty stream)', () => {
  ...
  for (let n = 1; n < bytes.length; n++) assert.throws(() => decodeChangeset(bytes.slice(0, n)), ChangesetError);
});
test('malformed tags, headers, UTF-8, varints and missing primary keys fail closed', () => {
  for (const bytes of [
    [83], [84,0], [84,2,0,0,116,0], [84,2,1,0,255,0], [84,128,2],
    [84,2,1,0,116,0,255,0], [84,2,1,0,116,0,18,2],
    [84,2,1,0,116,0,18,0,6,0], [84,2,1,0,116,0,18,0,5,5],
...
test('invalid UPDATE/INSERT record semantics cannot be encoded', () => {
  for (const change of [insert([1n, undefined]), insert([null, 'bad']),
...
test('unsupported numbers, ill-formed strings and conflicting table headers reject', () => {
  ...
  assert.throws(() => encodeChangeset([table([insert([1n, 'x'])]), table([insert([2n, 'y'])], [1,0], 'T')]), formatError);
  assert.throws(() => encodeChangeset([table([insert([1n, 'x'])], [1,0], 'bad\0name')]), ChangesetError);
});
test('byte, table, column, change and field-slot bounds apply to both directions', () => {
  const source = [table([insert([1n, 'a']), insert([2n,'b'])]), table([insert([3n,'c'])],[1,0],'u')];
  const bytes = encodeChangeset(source);
  for (const policy of [{maxBytes:8},{maxTables:1},{maxColumns:1},{maxChanges:2},{maxCells:5}]) {
    assert.throws(() => decodeChangeset(bytes, policy), limitError);
    assert.throws(() => encodeChangeset(source, policy), ChangesetError);
  }
  for (const policy of [{maxBytes:0},{maxTables:Infinity},{maxCells:1.5},{maxChanges:100001}]) {
...
test('length varint boundaries 127/128/16383/16384 and large blobs match native', t => {
  const d = db(t), s = d.createSession();
  for (const n of [0, 127, 128, 16383, 16384]) d.prepare('INSERT INTO t VALUES(?,?)').run(n, Buffer.alloc(n, 13));
  const bytes = s.changeset(); s.close(); roundtrip(bytes);
  assert.equal(d.applyChangeset(invertChangeset(bytes)), true); assert.deepEqual(rows(d), []);
});
test('shared, detached and resizable buffers cannot race decoding', () => {
  ...
  const view = new Uint8Array(1); structuredClone(view.buffer, { transfer: [view.buffer] });
  assert.throws(() => decodeChangeset(view));
  assert.throws(() => decodeChangeset(new Uint8Array(new ArrayBuffer(1,{maxByteLength:4}))), ChangesetError);
});
test('deterministic native session workloads preserve bytes and round-trip database state', t => {
  for (let seed = 1; seed <= 30; seed++) {
    ...
    try {
      ...
      const initial = rows(d), s = d.createSession();
      let state = seed;
      for (let i=0;i<80;i++) {
        state = (Math.imul(state,1664525)+1013904223) >>> 0; const id = state % 35 + 1;
