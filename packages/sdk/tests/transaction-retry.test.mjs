...
const {
  ...
  runTransactionRetry,
  FrankenTransactionRetryError,
} = production("transaction-retry");
const names = {
  5: "SQLITE_BUSY",
...
const busy = (extendedCode = 517, extra = {}) =>
  new FrankenSQLiteError({
    code: names[extendedCode],
    sqliteCode: 5,
    extendedCode,
    transient: true,
    message: "conflict",
...
class SqliteTransport {
  constructor(state) {
    this.state = state;
    this.resultEncoding = "objects";
  }
  assertOpen() {
    if (this.state.closed) throw this.state.closed;
  }
  async init() {
    return { path: this.state.path, persistence: "memory" };
  }
  observeFailure() {
    return () => {};
  }
  async transaction(action, id, parentId) {
    ...
    try {
      ...
      if (action === "begin") {
        ...
        if (action === "commit")
          s.raw.exec(frame.parentId === undefined ? "COMMIT" : `RELEASE child_${id}`);
        else
          s.raw.exec(
  ...
  cancelTransaction(id) {
    this.state.events.push({ action: "cancel", id });
  }
  async execute(sql, params = []) {
    this.assertOpen();
    await this.state.beforeExecute?.(sql);
    try {
      return Number(this.state.raw.prepare(sql).run(...params).changes);
    } catch (error) {
      throw sqliteError(error);
    }
  }
  ...
  async executePrepared(id, params = []) {
    this.assertOpen();
    try {
      return Number(this.state.statements.get(id).run(...params).changes);
    } catch (error) {
      throw sqliteError(error);
    }
  }
...
replacements.set("./utils", {
  normalizeOpenOptions: (options) => options ?? {},
  resolveWorker: (worker) => worker,
});
replacements.set("./stream", {
  checkStreamCancellation() {
    throw new Error("stream is outside this test");
  },
  executeRowStream() {
    throw new Error("stream is outside this test");
  },
...
replacements.set("@frankensqlite/worker", {
  ...
  parameterLayout(sql) {
    assert.ok(
      !/[?:@$]/.test(sql),
      "this transport fixture only describes zero-parameter prepared SQL",
    );
...
async function fixture(t, file = false) {
  ...
  t.after(() => {
    if (!state.closed) raw.close();
  });
  const db = await FrankenDB.open({ worker: state });
  raw.exec(
    "CREATE TABLE counts(value INTEGER); INSERT INTO counts VALUES(0); CREATE TABLE audit(v); CREATE TRIGGER log_update AFTER UPDATE ON counts BEGIN INSERT INTO audit VALUES(new.value); END",
  );
  return { db, state, raw };
}
...
for (const code of [5, 261, 517, 773]) {
  test(`classifier accepts confirmed BUSY code ${code}`, () =>
    assert.equal(isTransactionConflict(busy(code)), true));
}
for (const [name, error] of [
  ...
  [
    "storage publication CAS conflict",
    new FrankenSQLiteError({ code: "ERR_FSQLITE_SNAPSHOT_CONFLICT", message: "stale" }),
  ],
  ...
  [
    "fatal wrapper with busy cause",
    new FrankenSQLiteError({
      code: "ERR_FSQLITE_BULK_CONNECTION_UNUSABLE",
      message: "fatal",
      cause: { code: "SQLITE_BUSY", message: "busy" },
    }),
  ],
]) {
...
test("classifier validates every aggregate member without custom iterators/every or sparse holes", () => {
  ...
  const wrapped = new FrankenSQLiteError({
    code: "ERR_FSQLITE_TRANSACTION_ABORTED",
...
test("successful callback runs once, returns its value, and releases ownership", async (t) => {
  ...
  const value = await db.transactionWithRetry(async (tx, info) => {
    assert.deepEqual(info, { attempt: 1, maxAttempts: 4 });
    assert.ok(Object.isFrozen(info));
    await tx.execute("UPDATE counts SET value=value+1");
    return { committed: true };
  }, fast);
  assert.deepEqual(value, { committed: true });
  ...
  const result = await db.transactionWithRetry(async (tx, info) => {
    if (info.attempt === 2)
      await assert.rejects(
        escaped.query("SELECT value FROM counts"),
        (e) => e.code === "ERR_FSQLITE_TRANSACTION_CLOSED",
      );
    escaped = tx;
...
test("REAL COMMIT BUSY rolls back earlier writes and trigger effects before replay", async (t) => {
  ...
  reader.exec("BEGIN");
  reader.prepare("SELECT * FROM counts").all();
...
test("replay waits for rollback acknowledgement, including prepared cleanup", async (t) => {
  ...
  assert.equal(state.attempts, 1);
  assert.equal(state.statements.size, 0);
...
test("failed BEGIN can retry, but never rolls back an existing manual transaction", async (t) => {
  const { db, state, raw } = await fixture(t);
  state.beforeBoundary = (action) => {
    if (action === "begin" && state.attempts < 3) throw busy(5);
  };
  ...
  await assert.rejects(
    ...
    (e) => e.code === "SQLITE_ERROR",
  );
  assert.equal(callbacks, 0);
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 7);
  raw.exec("ROLLBACK");
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 1);
});
...
test("ordinary transaction remains single-attempt (retry is explicitly opt-in)", async (t) => {
  ...
  await assert.rejects(
    db.transaction((tx) => tx.execute("UPDATE counts SET value=1")),
    (e) => e === conflict,
  );
  assert.equal(state.attempts, 1);
});
...
test("attempt exhaustion preserves the LAST conflict object and no partial writes", async (t) => {
  ...
  await assert.rejects(
    db.transactionWithRetry((tx) => tx.execute("UPDATE counts SET value=value+1"), {
      ...fast,
      maxAttempts: 3,
    }),
    (e) => e === last,
...
for (const explicit of [false, true]) {
  test(`${explicit ? "manual" : "automatic"} prepared cleanup failure is never replayed, even when BUSY-shaped`, async (t) => {
    ...
    await assert.rejects(
      db.transactionWithRetry(async (tx) => {
        const prepared = await tx.prepare("UPDATE counts SET value=value+1");
        await prepared.execute();
...
test("rollback failure and worker cleanup failure are preserved, never retried or masked by abort", async (t) => {
  ...
  const original = busy(),
    rollback = busy(5),
    disposal = new Error("transport disposal");
  state.disposeError = disposal;
  state.beforeBoundary = (action) => {
    if (action === "commit") throw original;
    if (action === "rollback") {
      control.abort("cancel racing cleanup");
      throw rollback;
    }
  };
  await assert.rejects(
    db.transactionWithRetry((tx) => tx.execute("UPDATE counts SET value=1"), {
      ...fast,
      signal: control.signal,
    }),
    (error) => {
      assert.ok(error instanceof AggregateError);
      assert.deepEqual(error.errors, [original, rollback, disposal]);
      return true;
    },
  );
  assert.equal(state.attempts, 1);
  assert.equal(actions(state).filter((x) => x === "dispose").length, 1);
...
test("lost COMMIT acknowledgement cannot duplicate an already committed write", async (t) => {
  ...
  await assert.rejects(
    db.transactionWithRetry((tx) => tx.execute("UPDATE counts SET value=value+1"), fast),
    AggregateError,
  );
  assert.equal(state.attempts, 1);
  assert.equal(peer.prepare("SELECT value FROM counts").get().value, 1);
});

test("mixed application/SQL errors and constraint failures do not replay", async (t) => {
  const { db, state, raw } = await fixture(t);
  const application = new Error("do not repeat side effect");
  state.beforeExecute = (sql) => {
    if (sql === "UPDATE counts SET value=2") throw busy();
  };
  await assert.rejects(
    db.transactionWithRetry(async (tx) => {
    ...
    (error) => error instanceof AggregateError && error.errors.includes(application),
  );
  assert.equal(state.attempts, 1);
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 0);
});
...
test("connection lease survives backoff and rejects foreign SQL, close and competing retries", async (t) => {
  ...
  const { db, state } = await fixture(t);
  const rolled = deferred();
  state.beforeBoundary = (action) => {
    if (action === "commit" && state.attempts === 1) throw busy();
  };
  state.afterBoundary = (action) => {
    if (action === "rollback") rolled.resolve();
  };
...
test("pre-abort preserves the caller reason and starts no transaction", async (t) => {
  ...
  const reason = () => "not cloneable";
  controller.abort(reason);
  await assert.rejects(
    db.transactionWithRetry(() => {}, { ...fast, signal: controller.signal }),
    (e) => {
      assert.ok(e instanceof FrankenTransactionRetryError);
      assert.equal(e.code, "ERR_FSQLITE_TRANSACTION_RETRY_CANCELLED");
...
test("abort during callback waits for callback drain and successful rollback", async (t) => {
  ...
  const controller = new AbortController();
  let settled = false;
  const pending = db.transactionWithRetry(
    async (tx) => {
      await tx.execute("UPDATE counts SET value=1");
      started.resolve();
      await release.promise;
  ...
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 1);
  release.resolve();
  await assert.rejects(pending, (e) => e.code === "ERR_FSQLITE_TRANSACTION_RETRY_CANCELLED");
  assert.equal(state.attempts, 1);
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 0);
});
...
test("deadline covers slow BEGIN, joins it and rolls back without ever calling the callback", async (t) => {
  ...
  await assert.rejects(
    pending,
    (e) => e.code === "ERR_FSQLITE_TRANSACTION_RETRY_TIMEOUT" && e.attempts === 1,
  );
  assert.equal(callbacks, 0);
  assert.deepEqual(actions(state), ["begin", "rollback"]);
});
...
test("deadline is not reset between attempts and task yields admit cancellation", async (t) => {
  ...
  await assert.rejects(
    ...
    (e) => e.code === "ERR_FSQLITE_TRANSACTION_RETRY_TIMEOUT",
  );
  assert.ok(state.attempts < 100);
  assert.ok(state.attempts > 0);
  const controller = new AbortController();
  setTimeout(() => controller.abort("yield reached"), 0);
  await assert.rejects(
    db.transactionWithRetry((tx) => tx.execute("UPDATE counts SET value=1"), {
      ...fast,
      maxAttempts: 100,
    ...
    (e) => e.code === "ERR_FSQLITE_TRANSACTION_RETRY_CANCELLED",
  );
});
...
for (const invalid of [
  ...
  test(`invalid retry policy ${JSON.stringify(invalid)} runs no SQL`, async (t) => {
    const { db, state } = await fixture(t);
    await assert.rejects(db.transactionWithRetry(() => {}, invalid));
    assert.equal(state.attempts, 0);
    await db.execute("UPDATE counts SET value=1");
  });
}
...
test("scheduler requires explicit recovery evidence; a BUSY-shaped rejection alone is insufficient", async () => {
  ...
  const error = busy();
  await assert.rejects(
    runTransactionRetry(async () => {
      calls++;
      throw error;
    }, resolveTransactionRetryOptions(fast)),
    (e) => e === error,
  );
...
async function queueFixture(t, options = {}, file = false) {
  const path = file
    ? join(mkdtempSync(join(tmpdir(), "fsqlite-queue-retry-")), "db.sqlite")
    : ":memory:";
  const raw = new DatabaseSync(path);
  const state = {
    raw,
  ...
  raw.exec(
    "CREATE TABLE counts(value INTEGER); INSERT INTO counts VALUES(0); CREATE TABLE other(value INTEGER); INSERT INTO other VALUES(0); CREATE TABLE audit(v); CREATE TRIGGER log_update AFTER UPDATE ON counts BEGIN INSERT INTO audit VALUES(new.value); END",
  );
  return { queue, state, raw };
}

test("queued retry publishes only the final attempt's tables, not rolled-back dirty bits", async (t) => {
  const { queue, state, raw } = await queueFixture(t);
  const events = [],
    delivered = deferred();
  await queue.subscribe(["counts", "other"], (change) => {
    events.push(change);
    delivered.resolve();
  });
  ...
  state.beforeBoundary = (action) => {
    if (
      action === "commit" &&
      state.stack.at(-1)?.parentId === undefined &&
      state.attempts === base + 1
    )
      throw busy();
  };
  const attempts = [];
  ...
  assert.deepEqual(events[0].tables, ["other"]);
  assert.equal(events[0].commits, 1n);
  assert.equal(queue.changeSequence, 1n);
  assert.equal(queue.stats.failedJobs, 0);
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 0);
...
test("queued journal postlude conflict replays the callback and postlude in a new outer transaction", async (t) => {
  ...
  state.beforeQuery = (sql) => {
    if (failRead && sql.startsWith("SELECT id, dirty")) {
      failRead = false;
      throw busy();
    }
  };
  await queue.transactionWithRetry(async (tx) => {
    callbacks++;
    await tx.execute("UPDATE counts SET value=value+1");
  }, fast);
  const change = await delivered.promise;
  assert.equal(callbacks, 2);
  assert.equal(change.commits, 1n);
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 1);
  assert.equal(raw.prepare("SELECT count(*) n FROM audit").get().n, 1);
});
...
test("queued watched transaction recovers a REAL SQLite WAL snapshot conflict", async (t) => {
  ...
  assert.deepEqual(reads, [0, 10]);
  assert.equal(peer.prepare("SELECT value FROM counts").get().value, 11);
  assert.equal((await delivered.promise).commits, 1n);
  assert.equal(queue.changeSequence, 1n);
});
...
test("queued retry retains FIFO capacity through rollback/backoff; close drains all accepted work", async (t) => {
  ...
  const first = queue.transactionWithRetry(
    (tx) => tx.execute("UPDATE counts SET value=value+1"),
    fast,
  );
  await rolling.promise;
  const second = queue.transaction(async (tx) => {
    secondStarted = true;
    return (await tx.query("SELECT value FROM counts")).rows[0].value;
  });
  await assert.rejects(
    queue.transactionWithRetry(() => {}, fast),
    (e) => e.code === "ERR_FSQLITE_JOB_QUEUE_FULL",
  );
  ...
  assert.equal(queue.stats.activeJobs, 1);
  assert.equal(queue.stats.waitingJobs, 1);
  release.resolve();
  await first;
  assert.equal(await second, 1);
  await closing;
  assert.equal(queue.stats.completedJobs, 2);
  assert.equal(queue.stats.acceptedJobs, 2);
  assert.equal(queue.stats.failedJobs, 0);
  assert.equal(queue.stats.state, "closed");
});
...
for (const mode of ["abort", "timeout"]) {
  test(`waiting retry job ${mode} starts no transaction or callback`, async (t) => {
    ...
    const entered = deferred(),
      release = deferred();
    const blocker = queue.transaction(async () => {
      entered.resolve();
      await release.promise;
    });
    ...
    let callbacks = 0;
    const controller = new AbortController();
    const waiting = queue.transactionWithRetry(
      () => {
        callbacks++;
      },
    ...
    const rejection = assert.rejects(
      ...
      (e) =>
        e.code ===
        (mode === "timeout" ? "ERR_FSQLITE_JOB_WAIT_TIMEOUT" : "ERR_FSQLITE_JOB_CANCELLED"),
    );
...
test("active queued cancellation publishes nothing and later jobs still run", async (t) => {
  ...
  await queue.subscribe(["counts"], (change) => {
    events.push(change);
  });
  state.beforeBoundary = (action) => {
    if (action === "commit" && state.stack.at(-1)?.parentId === undefined) throw busy();
  };
  state.afterBoundary = (action) => {
    if (action === "rollback") controller.abort("stop retries");
  };
  await assert.rejects(
    queue.transactionWithRetry((tx) => tx.execute("UPDATE counts SET value=5"), {
      ...fast,
      signal: controller.signal,
    }),
    (e) => e.code === "ERR_FSQLITE_TRANSACTION_RETRY_CANCELLED",
  ...
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 0);
  assert.equal(queue.changeSequence, 0n);
  await queue.transaction((tx) => tx.query("SELECT value FROM counts"));
  await sleep(5);
  assert.deepEqual(events, []);
  assert.equal(queue.stats.failedJobs, 1);
});
...
test("queued exhaustion counts one failed job and emits no phantom later notification", async (t) => {
  ...
  state.beforeBoundary = (action) => {
    if (action === "commit" && state.stack.at(-1)?.parentId === undefined && failures-- > 0)
      throw busy();
  };
  await assert.rejects(
    queue.transactionWithRetry((tx) => tx.execute("UPDATE counts SET value=5"), {
    ...
    (e) => e.code === "SQLITE_BUSY_SNAPSHOT",
  );
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 0);
  await queue.transactionWithRetry((tx) => tx.query("SELECT value FROM counts"), fast);
  await sleep(5);
  assert.deepEqual(events, []);
  assert.equal(queue.changeSequence, 0n);
  assert.equal(queue.stats.failedJobs, 1);
...
test("queued retry policy is captured once before admission and cannot change while waiting", async (t) => {
  ...
  const pending = queue.transactionWithRetry(
    (tx) => tx.execute("UPDATE counts SET value=1"),
    policy,
  );
...
test("queued invalid or reentrant policy rejects before reserving a job", async (t) => {
  ...
  await assert.rejects(
    queue.transactionWithRetry(() => {}, { maxAttempts: 101 }),
    (e) => e.code === "ERR_FSQLITE_TRANSACTION_RETRY_INPUT",
  );
  assert.equal(queue.stats.acceptedJobs, 0);
  assert.equal(queue.stats.rejectedJobs, 1);
  let closing;
  await assert.rejects(
    queue.transactionWithRetry(() => {}, {
      get maxAttempts() {
        closing = queue.close();
        return 4;
      },
    }),
    (e) => e.code === "ERR_FSQLITE_JOB_QUEUE_CLOSED",
  );
  await closing;
  assert.equal(queue.stats.acceptedJobs, 0);
  assert.equal(queue.stats.rejectedJobs, 2);
});
...
test("listener failure after queued commit never causes transaction replay", async (t) => {
  ...
  let callbacks = 0;
  const subscription = await queue.subscribe(["counts"], () => {
    throw failure;
  });
...
test("ordinary watched transactions retain child rollback and one-commit notifications", async (t) => {
  ...
  await queue.transaction(async (tx) => {
    try {
      await tx.transaction(async (child) => {
        await child.execute("UPDATE other SET value=7");
        throw new Error("undo child");
      });
    } catch {}
    await tx.execute("UPDATE counts SET value=1");
  });
...
test("watched schema mutation refuses replay and restores the table", async (t) => {
  ...
  await assert.rejects(
    queue.transactionWithRetry(async (tx) => {
      callbacks++;
      await tx.execute("DROP TABLE counts");
    }, fast),
    (e) => e.code === "ERR_FSQLITE_SUBSCRIPTION_SCHEMA",
  );
  assert.equal(callbacks, 1);
  assert.equal(queue.changeSequence, 0n);
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 0);
...
test("queued retries remain usable after the last subscription is removed", async (t) => {
  ...
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 1);
  assert.equal(
    raw.prepare("SELECT count(*) n FROM temp.sqlite_master WHERE type='trigger'").get().n,
    0,
  );
});
...
for (const timeoutMs of [0, -1, 1.5, NaN, Infinity, 2 ** 31, "10", null]) {
  test(`deadline contract rejects invalid timeout ${String(timeoutMs)} before BEGIN`, async (t) => {
    const { db, state } = await fixture(t);
    await assert.rejects(
      db.transaction(() => {}, { timeoutMs }),
      (e) => e.code === "ERR_FSQLITE_TRANSACTION_INPUT",
    );
    assert.equal(state.attempts, 0);
  });
}
...
test("deadline contract checks delayed BEGIN before entering application callback", async (t) => {
  ...
  assert.equal(callbacks, 0);
  assert.deepEqual(actions(state), ["begin", "cancel", "rollback"]);
});
  ...
  await db.transaction(
    async (tx) => {
      await assert.rejects(
        tx.transaction(
          async (child) => {
            await child.execute("UPDATE counts SET value=9");
            now = 120;
          },
          { timeoutMs: 10 },
        ),
  ...
  await assert.rejects(
    db.transaction(
      async (tx) => {
        ...
        now = 120;
        await assert.rejects(
          tx.execute("UPDATE counts SET value=9"),
          (e) => e.code === "ERR_FSQLITE_TRANSACTION_TIMEOUT",
        );
        await assert.rejects(
          statement.execute(),
  ...
  await assert.rejects(
    db.transaction(
      async (tx) => {
        ...
        await assert.rejects(
          tx.execute("UPDATE counts SET value=2"),
          (e) => e.code === "ERR_FSQLITE_TRANSACTION_TIMEOUT",
        );
      },
      { timeoutMs: 10 },
  ...
  await assert.rejects(
    db.transactionWithRetry(
      (tx) =>
        tx.transaction(
          async (child) => {
            await child.execute("UPDATE counts SET value=1");
            now = 120;
          },
          { timeoutMs: 1000 },
        ),
...
test("deadline contract drains unawaited in-flight SQL before rollback and rejection", async (t) => {
  ...
  const pending = db.transaction(
    (tx) => {
      void tx.execute("UPDATE counts SET value=1");
    },
    { timeoutMs: 10 },
  );
  pending.then(
...
test("deadline contract preserves application failures instead of replacing them with a timeout", async (t) => {
  ...
  await assert.rejects(
    ...
    (error) => {
      assert.ok(error instanceof AggregateError);
      assert.ok(error.errors.includes(application));
      assert.ok(error.errors.some((e) => e.code === "ERR_FSQLITE_TRANSACTION_TIMEOUT"));
      return true;
    },
  );
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 0);
...
test("deadline contract preserves nested caller cancellation reasons after both rollbacks", async (t) => {
  ...
  await assert.rejects(
    db.transactionWithRetry(
      ...
      { ...fast, signal: control.signal },
    ),
    (error) => {
      assert.equal(error.code, "ERR_FSQLITE_TRANSACTION_RETRY_CANCELLED");
...
test("deadline contract never hides a mixed application failure under nested retry cancellation", async (t) => {
  const { db, raw } = await fixture(t);
  const control = new AbortController();
  const application = new Error("retain me");
  await assert.rejects(
    db.transactionWithRetry(
      (tx) =>
        tx.transaction(async (child) => {
    ...
    (error) => {
      ...
      assert.ok(leaves(error).includes(application));
      return true;
    },
  );
  assert.equal(raw.prepare("SELECT value FROM counts").get().value, 0);
...
test("deadline contract captures queued timeouts at submission, not after waiting", async (t) => {
  ...
  let reads = 0,
    limit = 10;
  const pending = queue.transaction(
    async (tx) => {
      await tx.execute("UPDATE counts SET value=1");
      now += 20;
    },
...
test("deadline contract starts the queued active budget after waiting, not at submission", async (t) => {
  ...
  await entered.promise;
  const pending = queue.transaction((tx) => tx.execute("UPDATE counts SET value=1"), {
    timeoutMs: 10,
  });
  now = 1000;
  release.resolve();
...
test("deadline contract rolls back watched writes when journal postlude exceeds the budget", async (t) => {
  ...
  state.beforeQuery = (sql) => {
    if (sql.startsWith("SELECT id, dirty")) now = 120;
  };
  await assert.rejects(
    queue.transaction((tx) => tx.execute("UPDATE counts SET value=1"), { timeoutMs: 10 }),
    (e) => e.code === "ERR_FSQLITE_TRANSACTION_TIMEOUT",
  );
