...
async function fixture(t, hooks = {}) {
  ...
  const state = async () => {
    const data = success(await send({ kind: "query", sql: "SELECT n FROM data ORDER BY id" })).data
      .rowArrays;
    const audit = success(await send({ kind: "query", sql: "SELECT n FROM audit ORDER BY rowid" }))
      .data.rowArrays;
    return { data, audit };
  };
...
const shapes = [
  ...
  [
    "trigger CASE and embedded terminators",
    `CREATE TABLE q(a); CREATE TRIGGER "tr;END" AFTER INSERT ON q BEGIN UPDATE q SET a=CASE WHEN new.a=1 THEN 2 ELSE 3 END; SELECT '; END; COMMIT'; END; INSERT INTO q VALUES(1);`,
  ],
  [
    "TEMP trigger",
    `CREATE TABLE q(a); CREATE /*x*/ TEMPORARY TRIGGER tr AFTER INSERT ON q BEGIN UPDATE q SET a=a+1; UPDATE q SET a=a+1; END; INSERT INTO q VALUES(1);`,
  ],
  [
    "Tcl parameter suffix",
    `CREATE TABLE q(a); INSERT INTO q VALUES($a::b(foo;COMMIT;'bar)); INSERT INTO q VALUES(5);`,
  ],
  ...
  [
    ...
    `CREATE TABLE q(a); INSERT INTO q VALUES(9); /* unfinished`,
  ],
  ["last statement without semicolon", `CREATE TABLE q(a); INSERT INTO q VALUES(42)`],
  [
    "table rebuild",
    `CREATE TABLE old(a); INSERT INTO old VALUES(1),(2); CREATE TABLE q(a); INSERT INTO q SELECT a+1 FROM old; DROP TABLE old; CREATE INDEX qi ON q(a);`,
  ],
];
for (const [name, sql] of shapes) {
  test(`statement boundaries preserve SQLite behavior: ${name}`, async () => {
    const whole = new DatabaseSync(":memory:"),
      sliced = new DatabaseSync(":memory:");
    try {
      whole.exec(sql);
      const calls = [];
      await executeManagedBatch(
        {
          async executeBatch(part) {
            calls.push(part);
            sliced.exec(part);
          },
        },
      ...
      assert.deepEqual(
        sliced.prepare("SELECT * FROM q ORDER BY rowid").all(),
        whole.prepare("SELECT * FROM q ORDER BY rowid").all(),
      );
      assert.equal(sliced.prepare("PRAGMA integrity_check").get().integrity_check, "ok");
      assert.ok(calls.length >= 2);
      if (name.includes("trigger"))
        assert.equal(calls.filter((s) => /CREATE.*TRIGGER/is.test(s)).length, 1);
    } finally {
      whole.close();
      sliced.close();
    }
...
test("single statements retain the no-task-yield path and comments/empty input contract", async (t) => {
  ...
  for (const sql of ["", "; ;", "--comment", "/*comment*/"])
    assert.throws(() => validateManagedSql(sql, true));
...
test("production host runs a managed schema/data script and commits its trigger effects", async (t) => {
  ...
  success(
    await f.script(
      "CREATE TABLE extra(n); INSERT INTO extra VALUES(10); UPDATE data SET n=1; UPDATE data SET n=2;",
    ),
  );
  success(await f.boundary("commit"));
  assert.deepEqual(await f.state(), { data: [[2]], audit: [[1], [2]] });
  assert.equal(
    success(await f.send({ kind: "query", sql: "PRAGMA integrity_check" })).data.rowArrays[0][0],
    "ok",
  );
});
...
test("a genuine SQLite constraint failure is retained and rolls back the entire script", async (t) => {
  ...
  success(await f.begin());
  const error = failure(
    await f.script("UPDATE data SET n=5; INSERT INTO data VALUES(2,5); UPDATE data SET n=6;"),
    "ERR_SQLITE_ERROR",
  );
...
test("rollback failure closes the worker instead of releasing partial script work", async (t) => {
  ...
  const f = await fixture(t, {
    beforeBatch(sql) {
      if (failRollback && sql === "ROLLBACK") throw new Error("rollback I/O fault");
    },
  });
  success(await f.begin());
  success(await f.script("UPDATE data SET n=5;"));
...
test("manual scripts retain core-managed boundaries and partial-failure behavior", async (t) => {
  ...
  failure(
    await f.send({ kind: "execute-batch", sql: "UPDATE data SET n=8; INVALID SQL;" }),
    "ERR_SQLITE_ERROR",
  );
