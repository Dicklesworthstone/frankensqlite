...
fn explicit_transaction_commit_persists() {
    asupersync::test_utils::run_test(|| async {
...

        {
            ...
            conn.execute("INSERT INTO t1 VALUES (10)")
                .await
                .expect("ins1");
            conn.execute("INSERT INTO t1 VALUES (20)")
                .await
                .expect("ins2");
            conn.execute("INSERT INTO t1 VALUES (30)")
                .await
                .expect("ins3");
...
fn create_drop_table_persists_schema_changes() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&db_str).await.expect("open");
            conn.execute("CREATE TABLE keep (a INTEGER)")
                .await
                .expect("t1");
            conn.execute("CREATE TABLE temp_table (b TEXT)")
                .await
                .expect("t2");
            conn.execute("INSERT INTO keep VALUES (42)")
                .await
                .expect("ins");
...

        {
            ...
            let rows = conn.query("SELECT a FROM keep").await.expect("query keep");
            assert_eq!(rows.len(), 1, "bead_id={BEAD_ID} case=ddl_keep_exists");

            // Drop one table
            conn.execute("DROP TABLE temp_table").await.expect("drop");
        }

        {
            let conn = Connection::open(&db_str).await.expect("reopen2");
            // keep should still exist
            let rows = conn.query("SELECT a FROM keep").await.expect("query");
            ...
            let result = conn.query("SELECT b FROM temp_table").await;
            assert!(
                result.is_err(),
...
fn large_text_payload_round_trip() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&db_str).await.expect("reopen");
            let rows = conn.query("SELECT txt FROM t1").await.expect("query");
            assert_eq!(
                rows[0].get(0).unwrap(),
...
fn all_sqlite_types_round_trip_on_file_backend() {
    asupersync::test_utils::run_test(|| async {
...

        {
            let conn = Connection::open(&db_str).await.expect("reopen");
            let rows = conn
                .query("SELECT a, b, c, d, e FROM t1")
                .await
                .expect("query");
...
fn concurrent_transaction_rollback_does_not_persist() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&db_str).await.expect("reopen");
            conn.execute("BEGIN CONCURRENT").await.expect("begin");
...
fn database_file_grows_with_data() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = Connection::open(&db_str).await.expect("open");
            conn.execute("CREATE TABLE t1 (data TEXT)")
                .await
                .expect("ddl");
        }
...
fn deterministic_seed_workload_produces_consistent_results() {
    asupersync::test_utils::run_test(|| async {
        ...
        let result1 = run_seeded_workload(seed).await;
        let result2 = run_seeded_workload(seed).await;
...
async fn run_seeded_workload(seed: u64) -> (usize, i64) {
    let (_dir, db_str) = temp_db();
    let conn = Connection::open(&db_str).await.expect("open");
    ...
    // Reopen and measure
    let conn = Connection::open(&db_str).await.expect("reopen");
    let count_rows = conn.query("SELECT count(*) FROM t1").await.expect("count");
