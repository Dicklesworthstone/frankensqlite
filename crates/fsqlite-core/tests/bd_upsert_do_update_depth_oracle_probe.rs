...
fn upsert_do_update_depth_match_rusqlite_oracle() {
    asupersync::test_utils::run_test(|| async {
        ...
        check(
            "excluded accumulate",
            fq(&f, "SELECT id,qty,hits FROM inv ORDER BY id").await,
            rq(&r, "SELECT id,qty,hits FROM inv ORDER BY id"),
            &mut diffs,
        );
        ...
        check(
            ...
            fq(&f, "SELECT id,qty FROM inv ORDER BY id").await,
            rq(&r, "SELECT id,qty FROM inv ORDER BY id"),
            &mut diffs,
        );
        ...
        ex(
            &f,
            &r,
            "INSERT INTO inv(id,qty) VALUES (1,999) ON CONFLICT(id) DO NOTHING",
        )
        .await;
        check(
            "do nothing keeps old",
            fq(&f, "SELECT id,qty FROM inv WHERE id=1").await,
            rq(&r, "SELECT id,qty FROM inv WHERE id=1"),
            &mut diffs,
        );
        ...
        check(
            "conflict on email target",
            fq(&f, "SELECT id,email,handle,cnt FROM u ORDER BY id").await,
            rq(&r, "SELECT id,email,handle,cnt FROM u ORDER BY id"),
            &mut diffs,
        );
        ...
        check(
            ...
            fq(&f, "SELECT a,b,note FROM k ORDER BY a,b").await,
            rq(&r, "SELECT a,b,note FROM k ORDER BY a,b"),
            &mut diffs,
        );
        ...
        ex(
            &f,
            &r,
            "INSERT INTO m(id,v) VALUES (1,100),(2,200) ON CONFLICT(id) DO UPDATE SET v=excluded.v",
        )
        .await;
        check(
            "multi-row mixed upsert",
            fq(&f, "SELECT id,v FROM m ORDER BY id").await,
            rq(&r, "SELECT id,v FROM m ORDER BY id"),
            &mut diffs,
        );

        assert!(
            diffs.is_empty(),
            "{} UPSERT DO-UPDATE divergence(s) vs rusqlite:\n{}",
            diffs.len(),
            diffs.join("\n")
        );
    });
