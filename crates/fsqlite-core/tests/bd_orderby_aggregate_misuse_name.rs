...
async fn err_of(sql: &str) -> String {
    ...
    c.execute(sql).await.expect_err("aggregate in ORDER BY is a misuse").to_string()
}

#[test]
fn orderby_aggregate_misuse_names_aggregate() {
    asupersync::test_utils::run_test(|| async {
        assert_eq!(err_of("SELECT a FROM t ORDER BY sum(a)").await, "misuse of aggregate: sum()");
        assert_eq!(err_of("SELECT a FROM t ORDER BY max(a)").await, "misuse of aggregate: max()");
        assert_eq!(err_of("SELECT a FROM t ORDER BY count(*)").await, "misuse of aggregate: count()");
        assert_eq!(err_of("SELECT a FROM t ORDER BY total(b)").await, "misuse of aggregate: total()");
