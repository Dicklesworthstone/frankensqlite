...
fn cmp(f: &Connection, r: &rusqlite::Connection, sql: &str, no_rewind: Option<bool>) {
    match no_rewind {
        Some(true) => assert!(!has_op(f, sql, "Rewind"), "rowid-IN COUNT must not full-scan (Rewind): `{sql}`"),
        Some(false) => assert!(has_op(f, sql, "Rewind"), "control COUNT should full-scan: `{sql}`"),
...
fn count_rowid_in_matches_sqlite() {
    ...
    cmp(&f, &r, "SELECT COUNT(*) FROM t WHERE id IN (5, 25, 45)", Some(true));            // all present -> 3
    cmp(&f, &r, "SELECT COUNT(*) FROM t WHERE id IN (99999, 5, 250)", Some(true));        // one absent -> 2
    cmp(&f, &r, "SELECT COUNT(*) FROM t WHERE id IN (5, 5, 25, 25, 45)", Some(true));     // duplicates -> 3
    cmp(&f, &r, "SELECT COUNT(*) FROM t WHERE id IN (99999, 88888)", Some(true));         // all absent -> 0
    cmp(&f, &r, "SELECT COUNT(*) FROM t WHERE id IN (1)", Some(true));                    // single -> 1
    cmp(&f, &r, "SELECT COUNT(*) FROM t WHERE id IN (1, 2, 3, 298, 299, 300)", Some(true)); // boundary -> 6
    cmp(&f, &r, "SELECT COUNT(*) FROM t WHERE 7 = id OR id = 8", Some(true));             // OR-of-rowid-eq (an IN)
    // Control: a non-rowid IN still full-scans (Rewind), and stays correct.
    cmp(&f, &r, "SELECT COUNT(*) FROM t WHERE a IN (3, 5)", Some(false));                 // a is not the rowid
    // Regression: rowid IN + residual is served by an existing indexed path (not mine); verify the count.
    cmp(&f, &r, "SELECT COUNT(*) FROM t WHERE id IN (5, 25, 45) AND c = 1", None);
}
