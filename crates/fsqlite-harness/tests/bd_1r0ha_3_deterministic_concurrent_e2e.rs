...
fn test_e2e_10_writers_disjoint_no_conflicts() {
    asupersync::test_utils::run_test(|| async {
        ...
        let setup_conn = open_conn(&path_str).await;
        for w in 0..10 {
            setup_conn
                .execute(&format!(
        ...
        let verify_conn = open_conn(&path_str).await;
        for w in 0..10 {
                            ...
                            match conn.query("SELECT val FROM counter WHERE id = 1;").await {
                                Ok(rows) => {
                                    ...
                                    let val = match rows[0].values()[0] {
                                        fsqlite_types::value::SqliteValue::Integer(v) => v,
                                        ref other => {
                                            panic!("reader {r}: expected Integer, got {other:?}")
                                        }
                                    };
    ...
    asupersync::test_utils::run_test(|| async {
        ...
        let handles: Vec<_> = (0..num_writers)
            .map(|w| {
                ...
                thread::spawn(move || {
                    let mut latencies_us: Vec<u64> = Vec::with_capacity(ops_per_writer);
                    asupersync::test_utils::run_test(|| async {
                        let conn = open_conn(&p).await;
                        b.wait();
