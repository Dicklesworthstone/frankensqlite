...
fn cmp(f: &Connection, r: &rusqlite::Connection, sql: &str, l: &str) { assert_eq!(frank(f, sql), sqlite(r, sql), "[{l}] diverged: `{sql}`"); }
fn check(label: &str, ddl: &[&str]) {
    ...
    for sql in seeks { cmp(&f, &r, sql, label); assert!(has_seek(&f, sql), "[{label}] IN+residual must seek: `{sql}`"); }
    // Exact IN (no residual) must STILL seek (byte-identical to before this change).
    for sql in ["SELECT id FROM t WHERE a IN (1,2,3)", "SELECT x FROM t WHERE a IN (5,6,7)"] {
        cmp(&f, &r, sql, label); assert!(has_seek(&f, sql), "[{label}] exact IN must still seek: `{sql}`");
    }
}
#[test]
fn nonagg_in_residual_matches_sqlite() {
    check("single", &["CREATE TABLE t (id INTEGER PRIMARY KEY, a INTEGER, c INTEGER, x TEXT, s TEXT);", "CREATE INDEX idx_a ON t(a);", "CREATE INDEX idx_c ON t(c);"]);
    check("shadowed", &["CREATE TABLE t (id INTEGER PRIMARY KEY, a INTEGER, c INTEGER, x TEXT, s TEXT);", "CREATE INDEX idx_ax ON t(a, x);", "CREATE INDEX idx_a ON t(a);"]);
}
