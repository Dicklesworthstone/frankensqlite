...
function ok(response, kind) {
  assert.equal(response.kind, kind, JSON.stringify(response));
...
async function fixture(t, hooks = {}, name = token()) {
  const model = installIndexedDbModel(),
    f = sqliteSnapshotWorker(hooks);
  let id = 0;
  const send = (request) => f.host.handle({ requestId: ++id, ...request });
  const ready = ok(
    await send({ kind: "init", config: { persistence: "indexeddb-snapshot", dbName: name } }),
    "ready",
  );
...
const confirm = (send, publicationId, parentRevision) =>
  send({ kind: "checkpoint-recover", publicationId, parentRevision });

test("storage confirms an exact publication only after verifying stored bytes; no write or export", async (t) => {
  const f = await fixture(t);
  assert.equal(f.ready.checkpointRecovery, 1);
  await f.send({ kind: "execute", sql: "INSERT INTO items VALUES(7)" });
  const id = token(),
    saved = ok(await publish(f.send, id), "checkpoint-result");
  assert.equal(saved.revision, id);
  const head = structuredClone(f.state.values.get("head")),
    before = [...f.events];
  t.mock.method(ModelObjectStore.prototype, "put", () => {
    throw new Error("recovery may not write");
  });
...
test("lost storage acknowledgement recovers the parent-held host and permits the next CAS", async (t) => {
  const f = await fixture(t),
    original = IndexedDbSnapshotStore.prototype.save;
  let once = true;
  ...
  const id = token();
  error(await publish(f.send, id), "ERR_FSQLITE_WORKER");
...
for (const mutation of [
  ...
  test(`recovery refuses ${mutation} without changing storage or host revision`, async (t) => {
    ...
    const before = structuredClone(f.state.values),
      events = [...f.events];
    error(
      await confirm(f.send, id, null),
      ["bad-hash", "bad-bytes", "wrong-name"].includes(mutation)
...
test("a newer local publication cannot be regressed by an old recovery request", async (t) => {
  const f = await fixture(t),
    first = token(),
    second = token();
  await publish(f.send, first);
  await publish(f.send, second);
  error(await confirm(f.send, first, null), "ERR_FSQLITE_SNAPSHOT_NOT_CONFIRMED");
  const next = ok(await publish(f.send, token()), "checkpoint-result");
  assert.equal(next.parentRevision, second);
});
...
test("a different writer winning CAS is never adopted as this worker publication", async (t) => {
  ...
  const peer = await IndexedDbSnapshotStore.open(f.name);
  t.after(() => peer.close());
...
test("recover holds the FIFO through asynchronous hashing and close", async (t) => {
  ...
  const recovering = confirm(f.send, id, null);
  await reached.promise;
  const closed = f.send({ kind: "close" });
  let settled = false;
  closed.then(() => {
    settled = true;
  });
  ...
  release.resolve();
  ok(await recovering, "checkpoint-result");
  ok(await closed, "close-result");
});
...
test("transport failure while hashing cannot resurrect a checkpoint revision", async (t) => {
  ...
  release.resolve();
  error(await recovering, "ERR_FSQLITE_WORKER");
  await closing;
  assert.equal(f.state.values.get("head").revision, id);
});
...
test("managed ownership refuses recovery, including recovery using its own scope id", async (t) => {
  ...
  await publish(f.send, id);
  ok(
    await f.send({ kind: "transaction", action: "begin", transactionId: "1" }),
    "transaction-result",
  );
  ...
  ok(
    await f.send({ kind: "transaction", action: "rollback", transactionId: "1" }),
    "transaction-result",
  );
});
...
for (const input of [undefined, "", "not-uuid", 7, null, "a".repeat(1000)]) {
  test(`admission refuses malformed recovery identity ${JSON.stringify(input)}`, () => {
    ...
    assert.equal(budget.stats.pendingRequests, 0);
...
test("admission captures identity getters once and accounts only known fields", () => {
  ...
  assert.ok(budget.stats.pendingBytes > 128);
  lease.release();
  assert.equal(budget.stats.pendingBytes, 0);
});
...
test("ordinary legacy save still generates its own identity", async (t) => {
  installIndexedDbModel();
  const store = await IndexedDbSnapshotStore.open(token());
  t.after(() => store.close());
  ...
  await assert.rejects(
    store.save(snapshotImage(), saved.revision, saved.revision),
    (e) => e.code === "ERR_FSQLITE_SNAPSHOT_INPUT",
  );
});
