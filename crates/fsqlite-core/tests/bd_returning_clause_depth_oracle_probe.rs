...
async fn fq_sorted(conn: &Connection, sql: &str) -> Vec<Vec<String>> {
    let mut rows = match conn.query(sql).await {
        Ok(rows) => rows
            .iter()
            .map(|r| r.values().iter().map(tag_f).collect::<Vec<_>>())
            .collect::<Vec<_>>(),
...
async fn exf(conn: &Connection, sql: &str) {
    let _ = conn.execute(sql).await;
}
fn exr(conn: &rusqlite::Connection, sql: &str) {
...
fn returning_clause_depth_match_rusqlite_oracle() {
    asupersync::test_utils::run_test(|| async {
        ...
        for s in ["CREATE TABLE t(id INTEGER PRIMARY KEY, a INTEGER, b TEXT)"] {
            exf(&f, s).await;
            exr(&r, s);
        }
        ...
        check(
            ...
            fq_sorted(
                &f,
                "INSERT INTO t(id,a,b) VALUES (1,10,'x') RETURNING id, a, b",
            )
            .await,
            rq_sorted(
                &r,
        ...
        check(
            "insert returning star+expr",
            fq_sorted(
                &f,
                "INSERT INTO t(id,a,b) VALUES (2,20,'y') RETURNING *, a*2 AS dbl",
            )
            .await,
            rq_sorted(
                &r,
        ...
        check(
            ...
            fq_sorted(
                &f,
                "INSERT INTO t(id,a,b) VALUES (3,30,'p'),(4,40,'q'),(5,50,'r') RETURNING id, a",
            )
            .await,
            rq_sorted(
                &r,
        ...
        check(
            "update returning post-image",
            fq_sorted(
                &f,
                "UPDATE t SET a=a+100 WHERE id<=2 RETURNING id, a, a-100 AS was",
            )
            .await,
            rq_sorted(
                &r,
        ...
        check(
            ...
            rq_sorted(&r, "UPDATE t SET b=b||'!' RETURNING *"),
            &mut diffs,
        );
        ...
        check(
            ...
            rq_sorted(&r, "DELETE FROM t WHERE id IN (4,5) RETURNING id, b"),
            &mut diffs,
        );
        ...
        check(
            ...
            fq_sorted(
                &f,
                "UPDATE t SET a=a WHERE id=3 RETURNING id, abs(a), typeof(b), length(b)",
            )
            .await,
            rq_sorted(
                &r,
        ...
        check(
            ...
            rq_sorted(&r, "SELECT id,a,b FROM t"),
            &mut diffs,
        );

        assert!(
            diffs.is_empty(),
            "{} RETURNING-clause divergence(s) vs rusqlite:\n{}",
            diffs.len(),
            diffs.join("\n")
        );
    });
