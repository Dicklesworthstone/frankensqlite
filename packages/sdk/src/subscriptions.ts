...
export function mergeChanges(
  previous: CommittedTableChange | null, next: CommittedTableChange,
...
export class ChangeObserver {
  ...
  constructor(tables: readonly string[], listener: TableChangeListener,
    detach: () => void, release: () => void) {
    ...
    void done.catch(() => {});
    const observer = this;
    this.handle = Object.freeze({ tables: names, done,
      get state() { return observer.#state; },
  ...
  publish(tables: readonly string[], sequence: bigint): void {
    ...
    const change = Object.freeze({ tables: Object.freeze(matching.sort()),
      firstSequence: sequence, lastSequence: sequence, commits: 1n });
  ...
  fail(cause: unknown): void {
    ...
    if (this.#state === "failed") {
      if (cause !== this.#failure) this.#failure = new AggregateError([this.#failure, cause],
        "Subscription connection and listener both failed", { cause: this.#failure });
      return;
    }
    this.#failure = cause;
    this.#state = "failed";
  ...
  #schedule(): void {
    if (this.#state !== "active" || this.#running || this.#timer !== undefined || this.#pending === null) return;
  ...
  async #deliver(): Promise<void> {
    ...
    this.#pending = null;
    this.#running = true;
    try { await this.#listener!(change); }
  ...
  const iterator: TableChangeStream = Object.freeze({
    ...
    next(): Promise<IteratorResult<CommittedTableChange>> {
      ...
      if (waiting !== null) return Promise.reject(new FrankenSQLiteError({
        code: "ERR_FSQLITE_SUBSCRIPTION_NEXT_PENDING", transient: false,
