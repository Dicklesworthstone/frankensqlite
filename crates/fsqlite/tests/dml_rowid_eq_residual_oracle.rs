...
fn check(dml: &str, no_rewind: bool) {
    let (f, r) = fresh();
    if no_rewind {
        assert!(!has_op(&f, dml, "Rewind"), "rowid-eq-residual DML must not full-scan (Rewind): `{dml}`");
    } else {
...
fn dml_rowid_eq_residual_matches_sqlite() {
    ...
    check("UPDATE t SET c = c + 1 WHERE id = 50 AND c BETWEEN 2 AND 8", true); // range residual
