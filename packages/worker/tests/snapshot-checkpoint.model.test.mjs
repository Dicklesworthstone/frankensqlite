...
test("checkpoint: real SQLite data, blobs, indexes and schema survive reopen", async () => {
  const a = await open();
  assert.equal(a.db.path, a.name);
  assert.equal(a.db.persistence, "indexeddb-snapshot");
  assert.equal(a.db.snapshotRevision, null);
  await a.db.executeBatch(
    schema + "; CREATE INDEX by_name ON items(name); CREATE TABLE payloads(b BLOB)",
  );
  await a.db.executeMany("INSERT INTO items VALUES (?, ?)", [
    [1, "Ada"],
    [2, "Grace"],
  ]);
  ...
  assert.deepEqual(await rows(b.db), [
    [1, "Ada"],
    [2, "Grace"],
  ]);
  ...
  assert.equal(b.db.snapshotRevision, saved.revision);
...
test("checkpoint: close does not silently publish uncheckpointed memory writes", async () => {
  const a = await open();
  await a.db.executeBatch(schema);
  await a.db.execute("INSERT INTO items VALUES (1,'saved')");
  await a.db.checkpoint();
  await a.db.execute("INSERT INTO items VALUES (2,'not saved')");
  await a.db.close();
  const b = await open(a.name);
  assert.deepEqual(await rows(b.db), [[1, "saved"]]);
  await b.db.close();
});
...
test("checkpoint: a new unsaved database stays absent", async () => {
  ...
  assert.deepEqual(
    (await b.db.query("SELECT name FROM sqlite_master WHERE type='table'")).rowArrays,
    [],
  );
  await b.db.close();
});
...
test("checkpoint: stale sessions cannot overwrite another session and keep their local work", async () => {
  const a = await open();
  await a.db.executeBatch(schema);
  const first = await a.db.checkpoint();
  const b = await open(a.name);
  assert.equal(b.db.snapshotRevision, first.revision);
  await a.db.execute("INSERT INTO items VALUES (1,'winner')");
  const winner = await a.db.checkpoint();
  ...
  assert.equal(c.db.snapshotRevision, winner.revision);
...
test("checkpoint: managed transactions reject foreign checkpoint admission", async () => {
  ...
  await a.db.close();
  const b = await open(a.name);
  assert.deepEqual(await rows(b.db), [[1, "inside"]]);
  await b.db.close();
});
...
test("checkpoint: manual BEGIN is not rolled back or persisted by a failed probe", async () => {
  ...
  await assert.rejects(
    a.db.checkpoint(),
    (error) => error.code === "ERR_FSQLITE_SNAPSHOT_TRANSACTION" && !!error.cause,
  );
  assert.deepEqual(a.events.slice(start), ["BEGIN"]);
  assert.equal(a.db.snapshotRevision, saved.revision);
  assert.deepEqual(await rows(a.db), [[1, "tentative"]]);
  await a.db.executeBatch("ROLLBACK");
  assert.deepEqual(await rows(a.db), []);
  await a.db.checkpoint();
  await a.db.close();
});
...
test("checkpoint: a SAVEPOINT opened via a prepared statement is detected without SQL guessing", async () => {
  const a = await open();
  await a.db.executeBatch(schema);
  const statement = await a.db.prepare("SAVEPOINT manual");
  ...
  await a.db.close();
  const b = await open(a.name);
  assert.deepEqual(await rows(b.db), [[1, "pending"]]);
  await b.db.close();
});
...
test("checkpoint: quota failure keeps the old revision and all in-memory work for retry", async () => {
  const a = await open();
  await a.db.executeBatch(schema);
  const before = await a.db.checkpoint();
  ...
  try {
    await assert.rejects(a.db.checkpoint(), /quota exhausted/);
  } finally {
    ModelObjectStore.prototype.put = original;
  }
  assert.equal(a.db.snapshotRevision, before.revision);
  assert.deepEqual(await rows(a.db), [[1, "retry"]]);
  const old = await open(a.name);
  assert.deepEqual(await rows(old.db), []);
  await old.db.close();
  const saved = await a.db.checkpoint();
  assert.equal(saved.parentRevision, before.revision);
  await a.db.close();
  const b = await open(a.name);
  assert.deepEqual(await rows(b.db), [[1, "retry"]]);
  await b.db.close();
});
...
test("checkpoint: corrupt saved bytes reject initialization and never create an empty core", async () => {
  ...
  await a.db.checkpoint();
  await a.db.close();
  const head = state(a.name).values.get("head");
  new Uint8Array(head.bytes)[200] ^= 1;
  const f = sqliteSnapshotWorker();
  await assert.rejects(
    FrankenDB.open({ dbName: a.name, persistence: "indexeddb-snapshot", worker: f.worker }),
    code("ERR_FSQLITE_SNAPSHOT_CORRUPT"),
  );
  assert.deepEqual(f.counts(), { creates: 0, imports: 0 });
  assert.equal(f.worker.terminateCount, 1);
  assert.equal(state(a.name).connections.size, 0);
  assert.equal(state(a.name).values.get("head").revision, head.revision);
});
...
test("checkpoint: failed core import releases storage without a create fallback", async () => {
  ...
  const f = sqliteSnapshotWorker({
    beforeImport() {
      throw new Error("core import refused");
    },
  });
  await assert.rejects(
    FrankenDB.open({ dbName: a.name, persistence: "indexeddb-snapshot", worker: f.worker }),
...
test("checkpoint: an initialization image seeds only an absent snapshot name", async () => {
  const a = await open();
  await a.db.executeBatch(schema);
  await a.db.execute("INSERT INTO items VALUES (1,'imported')");
  const bytes = await a.db.export();
  await a.db.close();
  const f = sqliteSnapshotWorker();
  const name = crypto.randomUUID();
  const db = await FrankenDB.import(new Uint8Array(bytes), {
    worker: f.worker,
    persistence: "indexeddb-snapshot",
    dbName: name,
  });
  assert.deepEqual(await rows(db), [[1, "imported"]]);
  await db.checkpoint();
  await db.close();
  const g = sqliteSnapshotWorker();
  await assert.rejects(
    FrankenDB.import(new Uint8Array(bytes), {
      worker: g.worker,
      persistence: "indexeddb-snapshot",
      dbName: name,
    }),
  ...
  assert.deepEqual(await rows(b.db), [[1, "imported"]]);
  await b.db.close();
});
...
test("checkpoint: failed probe rollback makes the host terminal and releases all handles", async () => {
  ...
  const a = await open(undefined, {
    beforeBatch(sql) {
      if (fail && sql === "ROLLBACK") throw new Error("rollback failed");
    },
  });
  await a.db.executeBatch(schema);
  const saved = await a.db.checkpoint();
  ...
  await assert.rejects(
    a.db.checkpoint(),
    (error) => error.code === "ERR_FSQLITE_SNAPSHOT_CONNECTION_UNUSABLE" && !!error.cause,
  );
...
test("checkpoint: 10000-row import and failed bulk trigger effects round-trip correctly", async () => {
  ...
  await a.db.executeBatch(
    schema +
      "; CREATE TABLE audit(id INTEGER); CREATE TRIGGER log_insert AFTER INSERT ON items BEGIN INSERT INTO audit VALUES(new.id); END;",
  );
  ...
  await a.db.checkpoint();
  await assert.rejects(
    a.db.executeMany("INSERT INTO items VALUES (?, ?)", [
      [10001, "valid"],
      [0, "conflict"],
    ]),
  );
  ...
  await a.db.close();
  const b = await open(a.name);
  assert.deepEqual((await b.db.query("SELECT COUNT(*), SUM(id) FROM items")).rowArrays, [
    [10000, 49995000],
  ]);
...
test("checkpoint: export failure cannot advance storage or poison an otherwise usable connection", async () => {
  ...
  await a.db.executeBatch(schema);
  const baseline = await a.db.checkpoint();
  await a.db.execute("INSERT INTO items VALUES (1,'memory')");
  fail = true;
  await assert.rejects(a.db.checkpoint(), /export failed/);
  assert.equal(a.db.snapshotRevision, baseline.revision);
  assert.deepEqual(await rows(a.db), [[1, "memory"]]);
...
test("checkpoint: queued SQL cannot modify the exported image before publication", async () => {
  ...
  await a.db.executeBatch(schema);
  await a.db.execute("INSERT INTO items VALUES (1,'first')");
  const pending = a.db.checkpoint();
  await started;
  ...
  const b = await open(a.name);
  assert.deepEqual(await rows(b.db), [[1, "first"]]);
...
test("checkpoint: eviction/recreation cannot turn an old revision into a matching token", async () => {
  ...
  assert.equal(a.db.snapshotRevision, baseline.revision);
  const b = await open(a.name);
  await b.db.executeBatch(schema);
  const fresh = await b.db.checkpoint();
  assert.notEqual(fresh.revision, baseline.revision);
  await assert.rejects(a.db.checkpoint(), code("ERR_FSQLITE_SNAPSHOT_CONFLICT"));
  await Promise.all([a.db.close(), b.db.close()]);
});
