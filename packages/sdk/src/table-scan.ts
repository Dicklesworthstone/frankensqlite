...
export async function createTablePageReader(tx: FrankenTransaction, settings: Settings) {
  ...
  const matches = listed.rowArrays.filter(
    (row) =>
      row[0] === "main" && typeof row[1] === "string" && fold(row[1]) === fold(settings.name),
  );
  if (matches.length !== 1 || matches[0]![2] !== "table") {
    throw unsupported(
      "Table scans require an ordinary main table, not a view, virtual or shadow table",
    );
  }
  const metadata = matches[0]!,
    table = identifier(metadata[1]);
  const withoutRowid = integer(metadata[4]);
  if (withoutRowid > 1 || integer(metadata[3]) > MAX_COLUMNS)
    throw unsupported("Unsupported table layout");
  const info = await tx.query(`PRAGMA main.table_xinfo(${literal(table)})`);
  if (info.rowArrays.length === 0 || info.rowArrays.length !== integer(metadata[3]))
    throw badResult();
  const all = new Map<string, string>();
  for (const row of info.rowArrays) {
    const name = identifier(row[1]),
      hidden = integer(row[6]);
    if (hidden !== 0 && hidden !== 2 && hidden !== 3)
      throw unsupported("Hidden virtual columns are not supported");
  ...
  if (withoutRowid === 0) {
    ...
    if (name === undefined)
      throw unsupported("All hidden rowid aliases are shadowed by declared columns");
    ...
    for (const row of index.rowArrays) {
      ...
      const name = identifier(row[2]),
        descending = integer(row[3]);
      if (
        integer(row[0]) !== keys.length ||
        !all.has(fold(name)) ||
        descending > 1 ||
        keys.some((key) => fold(key.name) === fold(name))
      )
        throw badResult();
      keys.push({
        name,
        descending: (descending === 1) !== settings.reverse,
        collation: identifier(row[4]),
        alias: "",
      });
    }
    if (keys.length === 0 || keys.length > MAX_KEYS)
      throw unsupported(`Use a primary key with 1..${MAX_KEYS} columns`);
  }
  ...
  keys.forEach((key, i) => {
    ...
    key.alias = alias;
    labels.add(fold(alias));
  });
  const column = (name: string): string => `s.${quote(name)}`;
  const expression = (key: Key): string =>
    column(key.name) + (key.collation === null ? "" : ` COLLATE ${quote(key.collation)}`);
  const order = keys
    .map((key) => `${expression(key)} ${key.descending ? "DESC" : "ASC"}`)
    .join(", ");
  const select =
    `SELECT ${keys
      .map((key) => `${column(key.name)} AS ${quote(key.alias)}`)
      .concat(columns.map(column))
      .join(", ")} ` + `FROM main.${quote(table)} AS s`;
  let last: SqlScalar[] | null = null,
    exhausted = false,
    queries = 0;
  ...
  function copyKey(row: readonly SqlScalar[]): SqlScalar[] {
    ...
    for (const value of row.slice(0, keys.length)) {
      ...
      if (size > MAX_KEY_BYTES)
        throw unsupported("Continuation key exceeds the 1 MiB retained-key budget");
    }
    ...
    if (
      withoutRowid === 0 &&
      !(typeof key[0] === "bigint" || (typeof key[0] === "number" && Number.isSafeInteger(key[0])))
    )
      throw badResult();
    return key;
  }
      ...
      for (const stage of stages) {
        ...
        if (after !== null) {
          for (let i = 0; i < stage; i++) {
            predicates.push(`${expression(keys[i]!)} = ?`);
            params.push(after[i]!);
          }
          const key = keys[stage]!;
          predicates.push(`${expression(key)} ${key.descending ? "<" : ">"} ?`);
          params.push(after[stage]!);
        }
        ...
        if (
          result.rowArrays.length > remaining ||
          result.columnCount !== keys.length + columns.length
        )
          throw badResult();
        for (const row of result.rowArrays) {
          ...
          if (
            ...
            (same(last, next) ||
              (withoutRowid === 0 &&
                (settings.reverse
                  ? BigInt(next[0] as number | bigint) >= BigInt(last[0] as number | bigint)
                  : BigInt(next[0] as number | bigint) <= BigInt(last[0] as number | bigint))))
          )
            throw badResult();
          last = next;
...
export interface TableScan<Row extends Record<string, unknown> = Record<string, unknown>>
  extends AsyncIterableIterator<Row> {
...
function cancelledOnly(cause: unknown): boolean {
  if (cause instanceof AggregateError)
    return cause.errors.length > 0 && cause.errors.every(cancelledOnly);
  return (
    cause instanceof FrankenSQLiteError &&
    (cause.code === "ERR_FSQLITE_TRANSACTION_CANCELLED" ||
      cause.code === "ERR_FSQLITE_JOB_CANCELLED")
  );
}
...
export function scanTable<Row extends Record<string, unknown> = Record<string, unknown>>(
  ...
  if (
    ...
    (waitTimeoutMs !== undefined &&
      (!Number.isInteger(waitTimeoutMs) || waitTimeoutMs < 1 || waitTimeoutMs > 2147483647))
  ) {
  ...
  const signal = AbortSignal.any(
    callerSignal === undefined ? [controller.signal] : [controller.signal, callerSignal],
  );
  ...
  let buffered: (Record<string, unknown> | undefined)[] = [],
    position = 0;
  let pagesRead = 0,
    pageQueries = 0,
    rowsRead = 0,
    ...
    maxBufferedRows = 0;
  let pending: {
    resolve: (result: IteratorResult<Row>) => void;
  ...
  function fail(cause: unknown): void {
    ...
    else if (failure.cause !== cause)
      failure = {
        cause: new AggregateError([failure.cause, cause], "Table scan and cleanup both failed", {
          cause: failure.cause,
        }),
      };
  }
  ...
  function expired(): void {
    stop({
      cause: new FrankenSQLiteError({
        code: "ERR_FSQLITE_SCAN_IDLE_TIMEOUT",
        transient: false,
        message:
          "Table scan consumer was inactive; its retained snapshot was released. This is not successful EOF.",
      }),
    });
  }
  ...
  function settle(): void {
    ...
    pending = null;
    if (failure !== null) {
      demand?.reject(failure.cause);
      rejectDone(failure.cause);
    } else {
      demand?.resolve({ done: true, value: undefined });
      resolveDone();
    }
  }
  ...
  function cancellation(cause: unknown): FrankenSQLiteError {
    const error = new FrankenSQLiteError({
      code: "ERR_FSQLITE_SCAN_CANCELLED",
      transient: false,
      message:
        "Table scan cancelled before completion; delivered rows are only a prefix, not successful EOF",
    });
    error.cause = cause;
    return error;
  }
      ...
      unobserve = observeQueueClose(queue, (error) => {
        ...
        stop({
          cause:
            error ??
            (observing
              ? cancellation("Owning queue is closing")
  ...
  const iterator: TableScan<Row> = Object.freeze({
    ...
    get stats() {
      return Object.freeze({
        ...
        rowsRead,
        rowsYielded,
        bufferedRows: buffered.length - position,
        maxBufferedRows,
      });
    },
    done,
    next(): Promise<IteratorResult<Row>> {
      ...
      if (pending !== null)
        return Promise.reject(
          new FrankenSQLiteError({
            code: "ERR_FSQLITE_SCAN_NEXT_PENDING",
            message: "Await the outstanding scan next() before requesting another row",
            transient: false,
          }),
        );
      const result = new Promise<IteratorResult<Row>>((resolve, reject) => {
    ...
    async throw(cause?: unknown): Promise<IteratorResult<Row>> {
      stop({ cause });
      await done;
      throw cause;
    },
