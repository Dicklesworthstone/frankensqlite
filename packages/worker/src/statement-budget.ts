...
export class PreparedStatementError extends Error {
  ...
  constructor(readonly code: "ERR_FSQLITE_STATEMENT_INPUT" | "ERR_FSQLITE_STATEMENT_LIMIT" |
    "ERR_FSQLITE_STATEMENT_TOO_LARGE" | "ERR_FSQLITE_STATEMENT_METADATA", message: string) {
    super(message);
...
export function resolvePreparedStatementLimits(
  ...
  if (typeof options !== "object" || options === null || Array.isArray(options)) {
    throw new PreparedStatementError("ERR_FSQLITE_STATEMENT_INPUT", "preparedStatementLimits must be an object");
  }
  ...
  if (!Number.isSafeInteger(maxStatements) || maxStatements < 1 || maxStatements > 4096) {
    throw new PreparedStatementError("ERR_FSQLITE_STATEMENT_INPUT", "maxStatements must be an integer in 1..4096");
  }
  if (!Number.isSafeInteger(maxBytes) || maxBytes < 256 || maxBytes > 1024 ** 3) {
    throw new PreparedStatementError("ERR_FSQLITE_STATEMENT_INPUT", "maxBytes must be an integer in 256..1073741824");
  }
...
export class PreparedStatementBudget {
  ...
  get stats(): PreparedStatementStats {
    return Object.freeze({ ...this.limits, statements: this.#statements,
      bytes: this.#bytes, rejectedStatements: this.#rejected });
  }
  ...
  reserve(sql: string): StatementReservation {
    ...
    let released = false;
    let refused = false;
    const refuse = (code: "ERR_FSQLITE_STATEMENT_LIMIT" | "ERR_FSQLITE_STATEMENT_TOO_LARGE", message: string): never => {
    ...
    if (!Number.isSafeInteger(size) || size > this.limits.maxBytes) {
      refuse("ERR_FSQLITE_STATEMENT_TOO_LARGE", "Prepared statement exceeds its metadata byte limit");
    }
    if (this.#statements >= this.limits.maxStatements || size > this.limits.maxBytes - this.#bytes) {
