...
before(async () => {
  const module = ts.transpileModule(await readFile(source, 'utf8'), { compilerOptions: {
    target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.ESNext,
  } }).outputText;
  server = createServer((request, response) => {
    response.setHeader('Cache-Control', 'no-store');
    response.setHeader('Content-Type', request.url === '/' ? 'text/html' : 'text/javascript');
    response.end(request.url === '/ownership.js' ? module : request.url === '/worker.js' ? worker : '<!doctype html><title>Snapshot session ownership</title>');
  });
...
test('shared sessions coexist; exclusive admission fails without waiting', async () => {
  const result = await run(async () => {
    ...
    await a.close();
    const stillBlocked = await L.acquire('indexeddb-snapshot', name, 'exclusive').catch(e => e.code);
    await b.close();
    const c = await L.acquire('indexeddb-snapshot', name, 'exclusive');
    const modes = [a.mode, b.mode, c.mode]; await c.close();
    return {modes, blocked, stillBlocked};
  });
  assert.deepEqual(result, {modes:['shared','shared','exclusive'], blocked:'ERR_FSQLITE_SNAPSHOT_OWNED', stillBlocked:'ERR_FSQLITE_SNAPSHOT_OWNED'});
});

test('exclusive ownership excludes both kinds of independently running worker', async () => {
  const result = await run(async () => {
    const a = startOwner(), b = startOwner(), c = startOwner();
    ...
    a.worker.terminate(); b.worker.terminate(); c.worker.terminate();
    return {first:first.mode, shared:shared.code, exclusive:exclusive.code, next:next.mode};
  });
  assert.deepEqual(result, {first:'exclusive', shared:'ERR_FSQLITE_SNAPSHOT_OWNED', exclusive:'ERR_FSQLITE_SNAPSHOT_OWNED', next:'exclusive'});
});

test('termination releases ownership; no stale callback has to cooperate', async () => {
  const result = await run(async () => {
    const a = startOwner(), b = startOwner();
    const config = {action:'acquire', backend:'opfs-snapshot', name:crypto.randomUUID(), mode:'exclusive'};
    await a.ask(config);
    ...
    await b.ask({action:'close'}); b.worker.terminate();
...
test('namespaces distinguish backends, escaping, and lone surrogates', async () => {
  const result = await run(async () => {
    ...
    const names = [prefix, prefix+'\\', prefix+'"', prefix+'\ud800', prefix+'\ud801', prefix+'\ufffd'];
    const leases = [];
    ...
    await Promise.all(leases.map(lease => lease.close())); return count;
  });
...
test('close joins release, is idempotent, and retires reusable ownership', async () => {
  const result = await run(async () => {
    ...
    const mismatches = [lease.matches('indexeddb-snapshot', name, 'exclusive'), lease.matches('opfs-snapshot', name, undefined)];
    const first = lease.close(), same = first === lease.close();
    const retired = lease.matches('opfs-snapshot', name, 'exclusive');
    await first;
    const next = await L.acquire('opfs-snapshot', name, 'exclusive'); await next.close();
    return {matches, mismatches, same, retired, frozen:Object.isFrozen(lease)};
  });
  assert.deepEqual(result, {matches:true, mismatches:[false,false], same:true, retired:false, frozen:true});
});
...
test('invalid policies and names never create a held or pending lock', async () => {
  const result = await run(async () => {
    ...
    const errors = [];
    for (const args of [['memory','x','exclusive'], ['opfs-snapshot','',undefined], ['opfs-snapshot',':memory:',undefined],
      ['opfs-snapshot','x','invalid'], ['opfs-snapshot','a\0b','shared'], ['opfs-snapshot','x'.repeat(257),'shared']]) {
  ...
  assert.deepEqual(result, {errors:Array(6).fill('ERR_FSQLITE_SNAPSHOT_OWNERSHIP_INPUT'), same:true});
});
