...
fn test_csqlite_writes_fsqlite_reads() {
    asupersync::test_utils::run_test(|| async {
        ...
        // Phase 2: Read with FrankenSQLite
        {
            ...
            let conn = match fsqlite::Connection::open(path_str).await {
                Ok(c) => c,
                Err(e) => {
                    compat_log!(
                        "csqlite→fsqlite",
            ...
            // Verify row count
            let rows = match conn.query("SELECT COUNT(*) FROM t").await {
                Ok(r) => r,
                Err(e) => {
...
fn test_compat_edge_cases_csqlite_to_fsqlite() {
    asupersync::test_utils::run_test(|| async {
        ...
        // Verify with FrankenSQLite
        {
            ...
            // NULL row
            let rows = match conn
                .query("SELECT txt, num, flt, blb FROM edge WHERE id = 1")
                .await
            {
                Ok(r) => r,
                Err(e) => {
            ...
            // Integer extremes
            let rows = conn
                .query("SELECT num FROM edge WHERE id = 3")
                .await
                .unwrap();
            ...
            let rows = conn
                .query("SELECT num FROM edge WHERE id = 4")
                .await
                .unwrap();
            if let Some(fsqlite_types::SqliteValue::Integer(n)) = rows.first().unwrap().get(0) {
                assert_eq!(*n, i64::MIN);
            ...
            // Large blob
            let rows = conn
                .query("SELECT blb FROM edge WHERE id = 5")
                .await
                .unwrap();
            ...
            // Large text
            let rows = conn
                .query("SELECT txt FROM edge WHERE id = 6")
                .await
                .unwrap();
...
fn test_compat_bidirectional_roundtrip() {
    asupersync::test_utils::run_test(|| async {
        ...
        // Step 2: FrankenSQLite reads and adds more
        {
            ...
            for i in 50..100 {
                if let Err(e) = conn
                    .execute(&format!("INSERT INTO t VALUES ({i}, 'fsqlite_{i}')"))
                    .await
                {
