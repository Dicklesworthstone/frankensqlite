...
impl MemoryFile {
    ...
    fn write_page_batch_into_storage(&self, cx: &Cx, writes: &[(u64, &[u8])]) -> Result<()> {
        checkpoint_or_abort(cx)?;
        if writes.is_empty() {
            return Ok(());
        }
...
impl VfsFile for MemoryFile {
    ...
    async fn write_page_batch(&self, cx: &Cx, writes: &[(u64, &[u8])]) -> Result<()> {
        self.write_page_batch_into_storage(cx, writes)
    }
    ...
    fn write_page_batch_tracked<'a>(
        ...
        let mut source_completion = VfsWriteCompletionSource::new(completion);
        async move {
            let result = self.write_page_batch_into_storage(cx, writes);
            if result.is_ok() {
                source_completion.complete_success();
            } else {
                source_completion.complete_error();
