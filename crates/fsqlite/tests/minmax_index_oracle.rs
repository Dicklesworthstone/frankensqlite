...
async fn uses_index_seek(conn: &Connection, sql: &str) -> bool {
...
/// Byte-exact comparison of one MIN/MAX query across frank and C SQLite.
...
fn minmax_index_seek_matches_sqlite() {
    asupersync::test_utils::run_test(|| async {
        ...
        cmp(&f, &r, "SELECT MIN(v) FROM n").await;
        cmp(&f, &r, "SELECT MAX(v) FROM n").await;
        ...
        cmp(&f, &r, "SELECT MIN(v) FROM e").await;
        cmp(&f, &r, "SELECT MAX(v) FROM e").await;
        cmp(&f, &r, "SELECT COALESCE(MIN(v), 99) FROM e").await;

        // Opcode gate: ASC and DESC indexed MIN/MAX use index-end seeks; unsafe shapes still decline.
        assert!(
            uses_index_seek(&f, "SELECT MIN(v) FROM t").await,
            "MIN(v) (indexed ASC) must use the index seek"
        );
        assert!(
            uses_index_seek(&f, "SELECT MAX(v) FROM t").await,
            "MAX(v) (indexed ASC) must use the index seek"
        );
        assert!(
            uses_index_seek(&f, "SELECT MIN(c) FROM t").await,
            "MIN(c) (NOCASE index, NOCASE compare) must use the index seek"
        );
        assert!(
            !uses_index_seek(&f, "SELECT MIN(u) FROM t").await,
            "MIN(u) (no index) must full-scan"
        );
        assert!(
            uses_index_seek(&f, "SELECT MIN(d) FROM t").await
                && has_op(&f, "SELECT MIN(d) FROM t", "Last").await
                && has_op(&f, "SELECT MIN(d) FROM t", "Prev").await,
            "MIN(d) (DESC index) must seek Last and walk back through trailing NULLs with Prev"
        );
        assert!(
            uses_index_seek(&f, "SELECT MAX(d) FROM t").await
                && has_op(&f, "SELECT MAX(d) FROM t", "Rewind").await
                && !has_op(&f, "SELECT MAX(d) FROM t", "Next").await,
            "MAX(d) (DESC index) must use a single Rewind end seek without a Next scan"
        );
        assert!(
            !uses_index_seek(&f, "SELECT MIN(c COLLATE BINARY) FROM t").await,
