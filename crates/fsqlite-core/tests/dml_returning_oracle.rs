...
fn insert_forms() {
    asupersync::test_utils::run_test(|| async {
        agree(
            &["CREATE TABLE a(x INT)", "CREATE TABLE b(x INT)", "INSERT INTO a VALUES (1),(2),(3)",
              "INSERT INTO b SELECT x*10 FROM a WHERE x >= 2"],
            "SELECT x FROM b ORDER BY x", "INSERT ... SELECT",
        ).await;
        agree(
            ...
            "SELECT id, n, s FROM t ORDER BY id", "INSERT DEFAULT VALUES",
        ).await;
        agree(
            &["CREATE TABLE t(a INT, b INT)", "INSERT INTO t VALUES (1+1, 2*3),(abs(-4), 5%2)"],
            "SELECT a, b FROM t ORDER BY a", "multi-row INSERT with expressions",
        ).await;
    });
}
...
fn update_forms() {
    asupersync::test_utils::run_test(|| async {
        agree(
            ...
            "SELECT id, v FROM t ORDER BY id", "UPDATE with expression + WHERE",
        ).await;
        agree(
            &["CREATE TABLE t(id INT, v INT)", "CREATE TABLE m(id INT, mul INT)",
              "INSERT INTO t VALUES (1,10),(2,20)", "INSERT INTO m VALUES (1,2),(2,3)",
              "UPDATE t SET v = v * (SELECT mul FROM m WHERE m.id = t.id)"],
            "SELECT id, v FROM t ORDER BY id", "UPDATE with correlated subquery",
        ).await;
        agree(
            &["CREATE TABLE t(id INT, v INT)", "CREATE TABLE m(id INT, add INT)",
              "INSERT INTO t VALUES (1,10),(2,20),(3,30)", "INSERT INTO m VALUES (1,5),(3,7)",
              "UPDATE t SET v = v + m.add FROM m WHERE m.id = t.id"],
            "SELECT id, v FROM t ORDER BY id", "UPDATE ... FROM",
        ).await;
    });
}
...
fn delete_forms() {
    asupersync::test_utils::run_test(|| async {
        agree(
            &["CREATE TABLE t(id INT)", "INSERT INTO t VALUES (1),(2),(3),(4)", "DELETE FROM t WHERE id % 2 = 0"],
            "SELECT id FROM t ORDER BY id", "DELETE with WHERE",
        ).await;
        agree(
            &["CREATE TABLE t(id INT)", "CREATE TABLE keep(id INT)", "INSERT INTO t VALUES (1),(2),(3)", "INSERT INTO keep VALUES (2)",
              "DELETE FROM t WHERE id NOT IN (SELECT id FROM keep)"],
            "SELECT id FROM t ORDER BY id", "DELETE with NOT IN subquery",
        ).await;
    });
}
...
fn returning_clause() {
    asupersync::test_utils::run_test(|| async {
        agree_returning(
            ...
            "SELECT id, v FROM t ORDER BY id", "RETURNING on INSERT",
        ).await;
        agree_returning(
            &["CREATE TABLE t(id INT, v INT)", "INSERT INTO t VALUES (1,10),(2,20)"],
            "UPDATE t SET v = v + 1 WHERE id = 2 RETURNING id, v",
            "SELECT id, v FROM t ORDER BY id", "RETURNING on UPDATE",
        ).await;
        agree_returning(
            &["CREATE TABLE t(id INT, v INT)", "INSERT INTO t VALUES (1,10),(2,20),(3,30)"],
            "DELETE FROM t WHERE v >= 20 RETURNING id",
            "SELECT id FROM t ORDER BY id", "RETURNING on DELETE",
        ).await;
    });
}
...
fn limit_offset_edges() {
    asupersync::test_utils::run_test(|| async {
        agree(&["CREATE TABLE t(x INT)", "INSERT INTO t VALUES (1),(2),(3),(4),(5)"], "SELECT x FROM t ORDER BY x LIMIT 2 OFFSET 1", "LIMIT/OFFSET").await;
        agree(&["CREATE TABLE t(x INT)", "INSERT INTO t VALUES (1),(2),(3)"], "SELECT x FROM t ORDER BY x LIMIT -1", "LIMIT -1 means no limit").await;
        agree(&["CREATE TABLE t(x INT)", "INSERT INTO t VALUES (1),(2)"], "SELECT x FROM t ORDER BY x LIMIT 5 OFFSET 10", "OFFSET past end is empty").await;
        agree(&["CREATE TABLE t(x INT)", "INSERT INTO t VALUES (1),(2),(3),(4)"], "SELECT x FROM t ORDER BY x LIMIT 1, 2", "LIMIT offset, count comma syntax").await;
    });
}
