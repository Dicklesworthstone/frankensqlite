...
async function fixture(t) {
  ...
  function start(mode, owner, barrier) {
    const worker = new Worker(new URL('./helpers/durable-jobs-thread.mjs', import.meta.url), {
      ...
      execArgv: ['--experimental-loader', new URL('./helpers/source-loader.mjs', import.meta.url).href],
    });
    workers.push(worker);
    ...
    void ready.catch(() => {});
    void result.catch(() => {});
    const fail = error => { readyReject(error); resultReject(error); };
  ...
  return { sql, queue, start, time: value => { now = value; } };
}

test('four independent SQLite connections claim and complete jobs without duplicate effects', { timeout: 30_000 }, async t => {
...
test('a committed lease survives forced worker termination and is reclaimed at expiry', { timeout: 30_000 }, async t => {
...
test('termination between application SQL and completion cannot leave partial effects', { timeout: 30_000 }, async t => {
