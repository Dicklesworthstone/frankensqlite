...
function causes(error) {
  return [error, ...(error?.errors ?? []).flatMap(causes), ...(error?.cause ? causes(error.cause) : [])];
}
...
function stored(f) {
  ...
  try { return oracle.prepare('SELECT id FROM items ORDER BY id').all().map(r => r.id); }
  finally { oracle.close(); }
}

for (const sql of ['COMMIT', '  -- end early\nEND TRANSACTION;', 'ROLLBACK', 'SAVEPOINT manual', 'RELEASE manual']) {
...
test('unowned raw worker request is rejected while the public callback is suspended', async () => {
  ...
  await f.db.transaction(async tx => {
    ...
    const foreign = await f.host.handle({ kind: 'execute', requestId: 999, sql: "INSERT INTO items VALUES (2,'foreign')" });
...
test('nested transactions use distinct parent-linked scopes and preserve siblings', async () => {
  ...
  assert.deepEqual(stored(f), [1,3]);
  assert.deepEqual(f.worker.requests.filter(r => r.kind === 'transaction').map(({action,transactionId,parentId}) =>
    [action,transactionId,parentId]), [['begin','1',undefined], ['begin','2','1'], ['rollback','2',undefined],
      ['begin','3','1'], ['commit','3',undefined], ['commit','1',undefined]]);
  await f.db.close();
});
...
test('transaction script creates triggers atomically and preflights all top-level statements', async () => {
  ...
  await assert.rejects(f.db.transaction(tx => tx.executeBatch("INSERT INTO items VALUES(2,'no'); /*tail*/ COMMIT;")),
    hasCode('ERR_FSQLITE_TRANSACTION_SQL'));
...
test('stream chunks, prepare and cleanup are bound to the stream child scope', async () => {
  ...
  await f.db.transaction(async parent => {
    ...
    await assert.rejects(parent.executeStream('INSERT INTO items VALUES (?,?)', [[700,'x'],[0,'duplicate']], { batchSize: 1 }));
...
test('caught SQL failures fence every remaining operation but still permit cleanup', async () => {
  ...
  await assert.rejects(f.db.transaction(async tx => {
    ...
    await assert.rejects(tx.execute("INSERT INTO items VALUES (2,'later')"), hasCode('ERR_FSQLITE_TRANSACTION_ABORTED'));
    await stmt.finalize();
  }));
  assert.deepEqual(stored(f), []);
...
test('public callbacks receive only their handle, never the internal mutable scope', async () => {
  ...
  await f.db.transaction(async function(tx) {
    ...
    await tx.transaction(function(child) { assert.equal(arguments.length, 1); return child.query('SELECT 1'); });
  });
  await f.db.close();
});
...
test('prepared handles cannot be retagged to enter a child and expired ids cannot be reused', async () => {
  ...
  await f.db.transaction(async parent => {
    ...
    await parent.transaction(async child => {
      const forged = await f.host.handle({ kind: 'statement-execute', requestId: 999,
        statementId: '1', transactionId: '2', params: [8,'bad'] });
  ...
  const stale = await f.host.handle({ kind: 'execute', requestId: 1000, transactionId: '1', sql: "INSERT INTO items VALUES (2,'stale')" });
...
test('deferred commit rejection rolls back schema and data before returning control', async () => {
  ...
  assert.deepEqual(stored(f), []);
  assert.equal((await f.db.query("SELECT count(*) AS n FROM sqlite_master WHERE name='children'")).rows[0].n, 0);
  await f.db.close();
});
