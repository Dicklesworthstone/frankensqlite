...
fn view_semantics_match_rusqlite_oracle() {
    asupersync::test_utils::run_test(|| async {
        ...
        check(
            "view aliases+expr",
            fq(&f, "SELECT eid, ename, dep, annual FROM v_emp ORDER BY eid").await,
            rq(&r, "SELECT eid, ename, dep, annual FROM v_emp ORDER BY eid"),
            &mut diffs,
        );
        ...
        check(
            ...
            fq(
                &f,
                "SELECT ename FROM v_emp WHERE dep='eng' AND annual > 1100 ORDER BY ename",
            )
            .await,
            rq(
                &r,
        ...
        check(
            "view join",
            fq(&f, "SELECT ename, flr FROM v_join ORDER BY ename").await,
            rq(&r, "SELECT ename, flr FROM v_join ORDER BY ename"),
            &mut diffs,
        );
        // view with aggregate
        check(
            "view aggregate",
            fq(&f, "SELECT dept, n, total, hi FROM v_agg ORDER BY dept").await,
            rq(&r, "SELECT dept, n, total, hi FROM v_agg ORDER BY dept"),
            &mut diffs,
        );
        // aggregate view with an outer HAVING-like filter
        check(
            "view agg outer filter",
            fq(
                &f,
                "SELECT dept FROM v_agg WHERE total >= 190 ORDER BY dept",
            )
            .await,
            rq(
                &r,
        ...
        check(
            "view inner orderby + outer limit",
            fq(&f, "SELECT id, salary FROM v_ordered LIMIT 3").await,
            rq(&r, "SELECT id, salary FROM v_ordered LIMIT 3"),
            &mut diffs,
        );
        // outer ORDER BY overrides the inner one
        check(
            "view outer orderby override",
            fq(&f, "SELECT id FROM v_ordered ORDER BY id").await,
            rq(&r, "SELECT id FROM v_ordered ORDER BY id"),
            &mut diffs,
        );
        // nested view
        check(
            "view nested",
            fq(&f, "SELECT dept, total FROM v_nested ORDER BY dept").await,
            rq(&r, "SELECT dept, total FROM v_nested ORDER BY dept"),
            &mut diffs,
        );
        // DISTINCT view
        check(
            "view distinct",
            fq(&f, "SELECT dept FROM v_depts ORDER BY dept").await,
            rq(&r, "SELECT dept FROM v_depts ORDER BY dept"),
            &mut diffs,
        );
        ...
        check(
            "view scalar subquery",
            fq(&f, "SELECT (SELECT max(total) FROM v_agg)").await,
            rq(&r, "SELECT (SELECT max(total) FROM v_agg)"),
            &mut diffs,
        );
        ...
        check(
            "count over view",
            fq(&f, "SELECT count(*) FROM v_agg").await,
            rq(&r, "SELECT count(*) FROM v_agg"),
            &mut diffs,
        );

        assert!(
            diffs.is_empty(),
            "{} VIEW-semantics divergence(s) vs rusqlite:\n{}",
            diffs.len(),
            diffs.join("\n")
        );
    });
