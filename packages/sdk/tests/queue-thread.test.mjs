...
async function threaded(t) {
  ...
  const queue=await FrankenDBQueue.open({worker,requestLimits:{maxPendingRequests:1}},{maxPendingJobs:32});
...
test('real thread: concurrent callers serialize full transactions through a one-request transport',options,async t=>{
  const f=await threaded(t),q=f.queue;
  const jobs=Array.from({length:20},(_,id)=>q.transaction(async tx=>{
    assert.equal((await tx.query('SELECT count(*) AS n FROM items')).rows[0].n,id);
    await tx.execute('INSERT INTO items VALUES(?,?)',[id,`v${id}`]);
    const statement=await tx.prepare('SELECT value FROM items WHERE id=?');return (await statement.get([id])).value;
  }));
  const exported=q.export(),closed=q.close();
  assert.deepEqual(await Promise.all(jobs),Array.from({length:20},(_,id)=>`v${id}`));
  assert.ok((await exported).length>=512);await closed;
  assert.deepEqual(onDisk(q.path),Array.from({length:20},(_,id)=>[id,`v${id}`]));
  assert.equal(q.stats.pendingJobs,0);assert.equal(q.stats.completedJobs,22);assert.equal(q.stats.state,'closed');
});

test('real thread: active cancellation waits for in-flight SQL and undoes trigger effects before the next job',options,async t=>{
  const f=await threaded(t),q=f.queue,c=new AbortController();let laterRan=false;
  await q.transaction(tx=>tx.executeBatch('CREATE TABLE audit(id);CREATE TRIGGER a AFTER INSERT ON items BEGIN INSERT INTO audit VALUES(new.id);END;'));
  ...
  const next=q.transaction(tx=>{laterRan=true;return tx.execute('INSERT INTO items VALUES(?,?)',[100,'after']);});
  await f.paused();c.abort('cancel');assert.equal((await f.cancelAck()).accepted,true);await drain();
  assert.equal(first.outcome.status,'pending');assert.equal(laterRan,false);
  f.resume();await first.settled;await next;assert.equal(first.outcome.status,'rejected');
  assert.deepEqual((await q.transaction(tx=>tx.query('SELECT * FROM audit'))).rowArrays,[[100]]);
  await q.close();assert.deepEqual(onDisk(q.path),[[100,'after']]);
});

test('real thread: late abort cannot turn a committed job into failure or release the next job early',options,async t=>{
  const f=await threaded(t),q=f.queue,c=new AbortController();let nextRan=false;
  await f.configure({commit:true});
  const first=q.transaction(async tx=>{await tx.execute("INSERT INTO items VALUES(1,'committed')");return 42;},{signal:c.signal});
  const next=q.transaction(tx=>{nextRan=true;return tx.execute("INSERT INTO items VALUES(2,'next')");});
  await f.paused();c.abort('late');await drain();assert.equal(nextRan,false);
  f.resume();assert.equal(await first,42);await next;await q.close();
  assert.deepEqual(onDisk(q.path),[[1,'committed'],[2,'next']]);assert.equal(q.stats.failedJobs,0);
});

test('real thread: an actual worker crash settles pending jobs and rolls back the uncommitted file',options,async t=>{
  const f=await threaded(t),q=f.queue;let called=false;await f.configure({row:2});
  const first=observe(q.transaction(async tx=>{
    await tx.execute('INSERT INTO items VALUES(?,?)',[1,'uncommitted']);
    await tx.execute('INSERT INTO items VALUES(?,?)',[2,'blocked']);
  }));
  const second=observe(q.transaction(()=>{called=true;}));await f.paused();
  const exited=new Promise(resolve=>f.native.once('exit',resolve));f.crash();
  await Promise.all([first.settled,second.settled,exited]);
  assert.equal(first.outcome.status,'rejected');assert.equal(second.outcome.status,'rejected');assert.equal(called,false);
  await observe(q.close()).settled;assert.equal(q.stats.pendingJobs,0);assert.equal(q.stats.state,'closed');
