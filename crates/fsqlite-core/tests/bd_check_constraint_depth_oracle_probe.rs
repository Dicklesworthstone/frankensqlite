...
fn check_constraint_depth_match_rusqlite_oracle() {
    asupersync::test_utils::run_test(|| async {
        ...
        ex(
            &f,
            &r,
            "INSERT INTO t VALUES (6,10,'A','Xhi','toolong',1,5)",
        )
        .await; // name too long
        ...
        check(
            ...
            fq(
                &f,
                "SELECT id,score,grade,code,name,lo,hi FROM t ORDER BY id",
            )
            .await,
            rq(
                &r,
        ...
        check(
            "update violate score",
            fq(&f, "SELECT id,score FROM t WHERE id=1").await,
            rq(&r, "SELECT id,score FROM t WHERE id=1"),
            &mut diffs,
        );
        ...
        check(
            "update violate grade",
            fq(&f, "SELECT id,grade FROM t WHERE id=1").await,
            rq(&r, "SELECT id,grade FROM t WHERE id=1"),
            &mut diffs,
        );
        // UPDATE that would violate the table-level CHECK
        ex(&f, &r, "UPDATE t SET lo=100, hi=1 WHERE id=1").await; // rejected (lo>hi)
        check(
            "update violate table check",
            fq(&f, "SELECT id,lo,hi FROM t WHERE id=1").await,
            rq(&r, "SELECT id,lo,hi FROM t WHERE id=1"),
            &mut diffs,
        );
        // UPDATE setting a value to NULL satisfies the table CHECK
        ex(&f, &r, "UPDATE t SET hi=NULL WHERE id=1").await; // allowed (NULL branch)
        check(
            "update null satisfies check",
            fq(&f, "SELECT id,lo,hi FROM t WHERE id=1").await,
            rq(&r, "SELECT id,lo,hi FROM t WHERE id=1"),
            &mut diffs,
        );
        ...
        assert!(
            diffs.is_empty(),
            "{} CHECK-constraint divergence(s) vs rusqlite:\n{}",
            diffs.len(),
            diffs.join("\n")
        );
    });
