...
function transport(t, workerData = {}) {
  const thread = new Worker(
    new URL("../../worker/tests/helpers/checkpoint-thread.mjs", import.meta.url),
    { workerData },
  );
  ...
  const worker = {
    ...
    postMessage(request, transfer) {
      worker.sent.push(structuredClone(request));
      thread.postMessage(request, transfer);
    },
    terminate() {
    ...
    control(testControl, value) {
      return new Promise((resolve, reject) => {
        const n = ++id;
        controls.set(n, { resolve, reject });
        thread.postMessage({ testControl, id: n, value });
      });
    },
    notice(name) {
      return new Promise((resolve) => {
        notices.set(name, resolve);
      });
    },
  };
  thread.on("message", (data) => {
    if (data.testControl) {
      const c = controls.get(data.id);
    ...
    if (data.notice) {
      notices.get(data.notice)?.();
      notices.delete(data.notice);
      return;
    }
    for (const l of listeners.message) l({ data });
  });
  thread.on("error", (error) => {
    for (const c of controls.values()) c.reject(error);
    controls.clear();
...
async function open(t, queue = false, workerData = {}, queueOptions = {}) {
  const worker = transport(t, workerData),
    options = { worker, dbName: crypto.randomUUID(), persistence: "indexeddb-snapshot" };
  const db = queue
    ? await FrankenDBQueue.open(options, { checkpointOnCommit: true, ...queueOptions })
...
for (const fault of [
  ...
  test(`real thread recovers ${fault} without export, publication or repeated SQL`, async (t) => {
    ...
    const before = await worker.control("stats"),
      id = worker.sent.findLast((r) => r.kind === "checkpoint").publicationId;
    const saved = await db.recoverCheckpoint();
    assert.equal(saved.revision, id);
    assert.equal(saved.parentRevision, null);
    assert.ok(Object.isFrozen(saved));
    assert.equal(db.snapshotRevision, id);
    const after = await worker.control("stats");
    assert.equal(after.saves, before.saves);
    assert.deepEqual(after.events, before.events);
    assert.deepEqual(after.head, before.head);
    assert.deepEqual(await inspectImage(after.head.bytes), [7]);
    await db.execute("INSERT INTO items VALUES(8)");
    const next = await db.checkpoint();
...
test("successful normal checkpoints retain concurrent FIFO lineage", async (t) => {
  ...
  assert.equal(saved[1].parentRevision, saved[0].revision);
  assert.equal(saved[2].parentRevision, saved[1].revision);
  const requests = worker.sent.filter((r) => r.kind === "checkpoint");
  assert.deepEqual(
    saved.map((r) => r.revision),
    requests.map((r) => r.publicationId),
  );
  await assert.rejects(
    db.recoverCheckpoint(),
    (e) => e.code === "ERR_FSQLITE_SNAPSHOT_RECOVERY_EMPTY",
  );
  await db.close();
});
...
for (const corruption of ["bytes", "revision", "absent"]) {
  test(`recovery refuses authoritative ${corruption} damage and leaves the receipt fence`, async (t) => {
    ...
    await worker.control("corrupt", corruption);
    const before = await worker.control("stats");
    await assert.rejects(
      db.recoverCheckpoint(),
      (e) =>
...
for (const fault of ["bad-receipt", "bad-envelope", "wrong-token", "wrong-parent", "worker-ack"]) {
  test(`a second ${fault} during recovery retains the same candidate for another readback`, async (t) => {
    ...
    assert.equal(receipt.revision, publication);
    assert.equal((await worker.control("stats")).saves, 1);
    await db.close();
  });
}
...
test("overlapping uncertain checkpoint replies cannot skip an unacknowledged revision", async (t) => {
  ...
  await assert.rejects(
    db.recoverCheckpoint(),
    (e) => e.code === "ERR_FSQLITE_SNAPSHOT_NOT_CONFIRMED",
  );
  assert.equal(db.snapshotRevision, null);
  await assert.rejects(db.checkpoint(), receiptFailure);
  await db.close();
...
test("no in-flight checkpoint is abandoned to admit recovery", async (t) => {
  ...
  await assert.rejects(
    db.recoverCheckpoint(),
    (e) => e.code === "ERR_FSQLITE_SNAPSHOT_RECOVERY_PENDING",
  );
  await pending;
  await db.close();
});
...
test("simultaneous recovery callers share one read; new publications wait for it", async (t) => {
  ...
  await worker.control("fault", "bad-receipt");
  await assert.rejects(db.checkpoint(), receiptFailure);
  await worker.control("hold");
  const held = worker.notice("confirm-held");
  const a = db.recoverCheckpoint(),
    b = db.recoverCheckpoint();
  assert.equal(a, b);
  await held;
  await assert.rejects(db.checkpoint(), receiptFailure);
  assert.equal(worker.sent.filter((r) => r.kind === "checkpoint-recover").length, 1);
  await worker.control("release");
  await a;
  await db.close();
});
...
test("readback acknowledges the captured image, never later unsaved memory writes", async (t) => {
  ...
  await worker.control("fault", "worker-ack");
  await assert.rejects(db.checkpoint());
  await db.execute("INSERT INTO items VALUES(2)");
  await db.recoverCheckpoint();
  const state = await worker.control("stats");
  ...
  await db.checkpoint();
  assert.deepEqual(await inspectImage((await worker.control("stats")).head.bytes), [1, 2]);
  await db.close();
});
...
test("older workers preserve ordinary checkpoints and never falsely advertise recovery", async (t) => {
  ...
  await db.checkpoint();
  assert.equal(worker.sent.findLast((r) => r.kind === "checkpoint").publicationId, undefined);
  await assert.rejects(
    db.recoverCheckpoint(),
    (e) => e.code === "ERR_FSQLITE_SNAPSHOT_RECOVERY_UNAVAILABLE",
  );
  await db.close();
});
...
for (const capability of [false, 0, 2, "1", {}]) {
  test(`unsupported recovery capability ${JSON.stringify(capability)} fails opening`, async (t) => {
    ...
    await assert.rejects(
      FrankenDB.open({ worker, dbName: crypto.randomUUID(), persistence: "indexeddb-snapshot" }),
      receiptFailure,
    );
  });
}
...
for (const fault of ["store-ack", "bad-receipt"]) {
  test(`queued ${fault} recovery clears the commit fence without replay or duplicate notifications`, async (t) => {
    const { db: queue, worker } = await open(t, true);
    const changes = [];
    ...
    try {
      await queue.transaction(async (tx) => {
        calls++;
        await tx.execute("INSERT INTO items VALUES(9)");
        return "kept";
      });
    } catch (error) {
      committed = error;
    }
    assert.ok(committed instanceof FrankenCheckpointCommitError);
    assert.equal(committed.value, "kept");
    assert.equal(queue.stats.checkpointRecoveryRequired, true);
    await assert.rejects(
      queue.transaction(() => {
        calls++;
      }),
      (e) => /RECOVERY/.test(e.code),
    );
    const before = await worker.control("stats"),
      sequence = queue.changeSequence;
    const saved = await queue.recoverCheckpoint();
    assert.equal(queue.snapshotRevision, saved.revision);
    assert.equal(queue.stats.checkpointRecoveryRequired, false);
    assert.equal(queue.changeSequence, sequence);
    const after = await worker.control("stats");
    assert.equal(after.saves, before.saves);
    assert.deepEqual(after.events, before.events);
...
test("quota failure is not confirmed; explicit new publication still recovers the queue", async (t) => {
  const { db: queue, worker } = await open(t, true);
  await worker.control("fault", "quota");
  await assert.rejects(
    queue.transaction((tx) => tx.execute("INSERT INTO items VALUES(6)")),
    FrankenCheckpointCommitError,
  );
  await assert.rejects(
    queue.recoverCheckpoint(),
    (e) => e.code === "ERR_FSQLITE_SNAPSHOT_NOT_CONFIRMED",
  );
  assert.equal(queue.stats.checkpointRecoveryRequired, true);
  await queue.checkpoint();
  assert.equal(queue.stats.checkpointRecoveryRequired, false);
  assert.deepEqual(await inspectImage((await worker.control("stats")).head.bytes), [6]);
  await queue.close();
});
...
test("queue holds capacity and drains active recovery before close, including late abort", async (t) => {
  ...
  await worker.control("fault", "bad-receipt");
  await assert.rejects(
    queue.transaction((tx) => tx.execute("INSERT INTO items VALUES(5)")),
    FrankenCheckpointCommitError,
  );
  await worker.control("hold");
  const held = worker.notice("confirm-held"),
    controller = new AbortController();
  const recovery = queue.recoverCheckpoint({ signal: controller.signal });
  await held;
  assert.equal(queue.stats.pendingJobs, 1);
  controller.abort("too late to abandon read");
  ...
  assert.equal(queue.stats.pendingJobs, 0);
});
...
test("pre-aborted queued recovery runs no storage work and cannot clear the fence", async (t) => {
  ...
  await assert.rejects(
    queue.recoverCheckpoint({ signal: AbortSignal.abort("stop") }),
    (e) => e.code === "ERR_FSQLITE_JOB_CANCELLED",
  );
  assert.equal(queue.stats.checkpointRecoveryRequired, true);
  assert.equal(worker.sent.filter((r) => r.kind === "checkpoint-recover").length, 0);
  await queue.recoverCheckpoint();
  await queue.close();
});

test("malformed readback after a generic store failure fences publication until confirmation", async (t) => {
  const { db, worker } = await open(t);
  await worker.control("fault", "store-ack");
  await assert.rejects(db.checkpoint());
  await worker.control("fault", "bad-envelope");
  await assert.rejects(db.recoverCheckpoint(), receiptFailure);
  await assert.rejects(db.checkpoint(), receiptFailure);
  await db.recoverCheckpoint();
  assert.equal((await worker.control("stats")).saves, 1);
  await db.close();
...
test("read-only recovery preserves retained prepared handles and SQL values", async (t) => {
  const { db, worker } = await open(t),
    statement = await db.prepare("SELECT ? AS value");
  await worker.control("fault", "bad-receipt");
  await assert.rejects(db.checkpoint());
...
test("queue wait timeout does not interrupt another active recovery or clear its fence early", async (t) => {
  ...
  await worker.control("fault", "bad-receipt");
  await assert.rejects(
    queue.transaction((tx) => tx.execute("INSERT INTO items VALUES(4)")),
    FrankenCheckpointCommitError,
  );
  await worker.control("hold");
  const held = worker.notice("confirm-held"),
    active = queue.recoverCheckpoint();
  await held;
  await assert.rejects(
    queue.recoverCheckpoint({ waitTimeoutMs: 1 }),
    (e) => e.code === "ERR_FSQLITE_JOB_WAIT_TIMEOUT",
  );
  assert.equal(queue.stats.checkpointRecoveryRequired, true);
...
test("memory sessions and closed sessions cannot use snapshot recovery", async (t) => {
  const worker = transport(t),
    db = await FrankenDB.open({ worker });
  assert.equal(db.checkpointRecoverySupported, false);
  await assert.rejects(
    db.recoverCheckpoint(),
    (e) => e.code === "ERR_FSQLITE_SNAPSHOT_RECOVERY_UNAVAILABLE",
