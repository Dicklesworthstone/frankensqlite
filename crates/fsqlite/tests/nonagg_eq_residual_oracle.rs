...
fn frank_rows(c: &Connection, sql: &str) -> Vec<Vec<String>> {
    let mut r: Vec<Vec<String>> = c.query(sql).unwrap_or_else(|e| panic!("frank `{sql}`: {e}")).iter().map(|row| row.values().iter().map(render).collect()).collect();
    r.sort(); r
}
...
fn cmp(f: &Connection, r: &rusqlite::Connection, sql: &str, l: &str) { assert_eq!(frank_rows(f, sql), sqlite_rows(r, sql), "[{l}] diverged: `{sql}`"); }
fn check(label: &str, ddl: &[&str]) {
    let (f, r) = setup(ddl);
    for i in 1..=600_i64 {
        let a = if i % 17 == 0 { "NULL".to_owned() } else { format!("{}", i % 20) };
        let c = if i % 19 == 0 { "NULL".to_owned() } else { format!("{}", i % 12) };
        ins(&f, &r, &format!("INSERT INTO t VALUES ({i}, {a}, {c}, 'v{}', 'k{}');", i % 7, i % 9));
    }
    ...
    for sql in seeks {
        cmp(&f, &r, sql, label);
        assert!(has_seek(&f, sql), "[{label}] non-covering eq+residual must seek: `{sql}`");
    }
...
fn nonagg_eq_residual_matches_sqlite() {
    check("single idx_a", &["CREATE TABLE t (id INTEGER PRIMARY KEY, a INTEGER, c INTEGER, x TEXT, s TEXT);", "CREATE INDEX idx_a ON t(a);", "CREATE INDEX idx_c ON t(c);"]);
    check("shadowed idx_a", &["CREATE TABLE t (id INTEGER PRIMARY KEY, a INTEGER, c INTEGER, x TEXT, s TEXT);", "CREATE INDEX idx_ax ON t(a, x);", "CREATE INDEX idx_a ON t(a);"]);
}
#[test]
fn nonagg_eq_residual_text_prefix() {
    let (f, r) = setup(&["CREATE TABLE t (id INTEGER PRIMARY KEY, s TEXT, c INTEGER, x TEXT);", "CREATE INDEX idx_s ON t(s);"]);
    for i in 1..=300_i64 { ins(&f, &r, &format!("INSERT INTO t VALUES ({i}, 'k{}', {}, 'p{}');", i % 8, i % 10, i % 4)); }
    for sql in ["SELECT c FROM t WHERE s = 'k3' AND c = 4", "SELECT * FROM t WHERE s = 'k3' AND c > 5"] {
