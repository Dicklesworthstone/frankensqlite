...
fn check_update(upd: &str, no_rewind: bool) {
    ...
    if no_rewind {
        assert!(
            !has_op(&f, upd, "Rewind"),
            "rowid-IN UPDATE must not full-scan (Rewind): `{upd}`"
        );
    } else {
        assert!(
            has_op(&f, upd, "Rewind"),
            "control UPDATE should full-scan: `{upd}`"
        );
    }
...
fn update_rowid_in_matches_sqlite() {
    ...
    check_update(
        "UPDATE t SET x = 'r' WHERE id IN (5, 25, 45) AND c = 5",
        true,
    ); // eq residual
    check_update(
        "UPDATE t SET a = a + 1 WHERE id IN (10, 20, 30, 40) AND c > 3",
        true,
    ); // range residual + expr SET
    check_update(
        "UPDATE t SET x = 'm' WHERE id IN (100, 200, 300) AND c != 5 AND x = 'v3'",
        true,
    ); // multi
    check_update(
        "UPDATE t SET x = 'n' WHERE id IN (5, 25, 45) AND c = 999",
        true,
    ); // residual matches nothing
    check_update(
        "UPDATE t SET id = id + 5000 WHERE id IN (5, 25, 45) AND c = 5",
        true,
    ); // residual + ROWID rewrite
