...
function chunks(f) {
  return f.worker.requests.filter((r) => r.kind === "statement-execute-many");
}
...
test("25,001 async rows use bounded chunks, one prepared handle and one outer commit", async () => {
  ...
  assert.deepEqual((await f.db.query("SELECT count(*), sum(id) FROM t")).rowArrays, [
    [25001, 312537501],
  ]);
...
test("a later chunk constraint failure rolls back earlier chunks and trigger effects", async () => {
  ...
  await f.db.execute(
    "CREATE TRIGGER inserted AFTER INSERT ON t BEGIN INSERT INTO audit VALUES(new.id); END",
  );
  ...
  await assert.rejects(f.db.executeStream(INSERT, rows, { batchSize: 3 }), (e) => {
    ...
    assert.equal(e.phase, "execute");
    assert.equal(e.rowIndex, 6);
...
test("a synchronous generator failure rolls back all chunks and preserves its cause", async () => {
  ...
  await assert.rejects(f.db.executeStream(INSERT, rows(), { batchSize: 1 }), (e) => {
    assert.equal(e.phase, "source");
    assert.equal(e.rowIndex, 3);
...
test("an async producer failure closes its iterator once before releasing ownership", async () => {
  ...
  const rows = {
    [Symbol.asyncIterator]() {
      return {
        async next() {
          if (++pulls === 3) throw cause;
          return { done: false, value: [pulls, `v${pulls}`] };
        },
        async return() {
          returns++;
          closing.resolve();
...
test("source pulls stop while a worker chunk is suspended; foreign work is refused", async () => {
  ...
  const f = await fixture({
    async beforeBatch(sql) {
      if (sql.startsWith("SAVEPOINT fsqlite_bulk_")) {
        blocked.resolve();
        await release.promise;
      }
    },
  ...
  await assert.rejects(
    f.db.execute(INSERT, [90, "foreign"]),
...
test("byte budget flushes before row limit and copies reused row and blob buffers", async () => {
  ...
  const result = await f.db.executeStream("INSERT INTO blobs VALUES (?, ?)", rows(), {
    batchSize: 100,
    maxBatchBytes: 121,
  });
  assert.equal(result.batches, 4); // 16 + 16*2 + 12 = 60 accounted bytes per row.
  assert.deepEqual(
    chunks(f).map((r) => r.parameterSets.length),
    [2, 2, 2, 1],
  );
  const read = (await f.db.query("SELECT id, hex(data) FROM blobs ORDER BY id")).rowArrays;
  assert.deepEqual(
    read,
    Array.from({ length: 7 }, (_, i) => [i + 1, (i + 1).toString(16).padStart(2, "0").repeat(12)]),
  );
  await f.db.close();
});
...
test("oversized or invalid later rows fail with their global index and undo earlier work", async () => {
  for (const value of ["x".repeat(100), undefined, { invalid: true }, [1]]) {
    ...
    await assert.rejects(
      f.db.executeStream(INSERT, rows(), { batchSize: 1, maxBatchBytes: 80 }),
      (e) => {
        assert.equal(e.phase, "input");
        assert.equal(e.rowIndex, 2);
        return true;
      },
    );
    assert.equal(returned, 1);
...
test("WITH and RETURNING statements that produce rows reject without consuming a source", async () => {
  ...
  for (const sql of [
    ...
    "INSERT INTO t VALUES(1,'a') RETURNING id",
  ]) {
...
test("a failed BEGIN does not consume rows or roll back an existing manual transaction", async () => {
  ...
  assert.deepEqual(await f.read(), [[90, "manual"]]);
  await f.db.close();
});
...
test("iterator and statement cleanup failures do not mask the primary SQL error", async () => {
  ...
  await assert.rejects(f.db.executeStream(INSERT, iterator, { batchSize: 1 }), (e) => {
    assert.equal(e.phase, "execute");
    assert.equal(e.rowIndex, 1);
    assert.match(e.cause.cause.message, /UNIQUE/);
    assert.deepEqual(
      e.cleanupErrors.map((x) => x.message),
      ["source close failed", "finalize failed"],
    );
    return true;
  });
  assert.deepEqual(await f.read(), []);
...
test("malformed iterator results close once, finalize and roll back", async () => {
  ...
  await assert.rejects(
    f.db.executeStream(INSERT, rows),
    (e) => e.phase === "source" && e.rowIndex === 0,
  );
  assert.equal(returns, 1);
  assert.deepEqual(await f.read(), []);
  await f.db.close();
...
test("safe-integer count overflow is rejected rather than returning a rounded total", async () => {
  ...
  const client = {
    async prepare() {
      return { statementId: "1", columnCount: 0 };
    },
    async executePreparedMany() {
      return { executions: 1, changes: Number.MAX_SAFE_INTEGER };
    },
  ...
  await assert.rejects(
    executeRowStream(
      ...
      streamOptions(INSERT, { batchSize: 1 }),
    ),
...
test("independent connections make progress while an import waits for source data", async () => {
  ...
  const started = deferred(),
    release = deferred();
  const pending = a.db.executeStream(
    INSERT,
    (async function* () {
      started.resolve();
      await release.promise;
  ...
  await b.db.executeStream(INSERT, [[2, "b"]]);
  assert.deepEqual(await b.read(), [[2, "b"]]);
  release.resolve();
  await pending;
  assert.deepEqual(await a.read(), [[1, "a"]]);
  await a.db.close();
  await b.db.close();
});
...
test("a deferred foreign-key failure at outer COMMIT undoes every released chunk", async () => {
  ...
  await assert.rejects(
    f.db.executeStream("INSERT INTO child VALUES (?)", [[1], [2], [3]], { batchSize: 1 }),
    /FOREIGN KEY/,
  );
...
test("committed streamed data reopens with exact values and clean SQLite integrity", async () => {
  const f = await fixture();
  const path = f.handles[0].path;
  await f.db.executeStream(
    INSERT,
...
test("streamed integers preserve signed 64-bit limits and reject wider bigint payloads", async () => {
  ...
  assert.deepEqual(
    (await f.db.query("SELECT CAST(id AS TEXT), value FROM t ORDER BY id")).rowArrays,
    [
...
const isStreamCancelled = (e) =>
  e instanceof FrankenStreamError && e.code === "ERR_FSQLITE_STREAM_CANCELLED";
...
test("progress is immutable, cumulative and explicitly uncommitted", async () => {
  ...
  assert.deepEqual(result, { executions: 5, changes: 5, batches: 3 });
  await f.db.close();
});
...
test("async progress applies backpressure and no additional input is prefetched", async () => {
  ...
  await started.promise;
  assert.equal(pulls, 2);
  assert.equal(chunks(f).length, 1);
  release.resolve();
  await pending;
...
test("a progress rejection closes the source and rolls back already reported chunks", async () => {
  const f = await fixture();
  const cause = new Error("validation rejected imported rows");
  let returned = 0;
  function* rows() {
    try {
      for (let i = 1; i <= 10; i++) yield [i, `v${i}`];
    } finally {
      returned++;
    }
  }
  ...
  assert.equal(returned, 1);
  assert.equal(chunks(f).length, 2);
  assert.deepEqual(await f.read(), []);
  await f.db.close();
});
...
test("cancellation between chunks undoes every previous chunk and trigger effect", async () => {
  ...
  let returned = 0;
  await f.db.executeBatch(
    "CREATE TABLE audit(id); CREATE TRIGGER tr AFTER INSERT ON t BEGIN INSERT INTO audit VALUES(new.id); END;",
  );
  function* rows() {
    try {
      for (let i = 1; i <= 10; i++) yield [i, `v${i}`];
  ...
  await assert.rejects(
    f.db.executeStream(INSERT, rows(), {
      ...
      onProgress(p) {
        if (p.batches === 2) controller.abort("stop importing");
      },
    }),
...
test("in-flight cancellation waits for the current SQL then undoes earlier successful chunks", async () => {
  ...
  f.handles[0].prepare = async (sql) => {
    const stmt = await prepare(sql),
      execute = stmt.executeWithParams.bind(stmt);
  ...
  let settled = false;
  const pending = f.db.executeStream(
    INSERT,
    Array.from({ length: 10 }, (_, i) => [i + 1, `v${i}`]),
  ...
  const rejection = assert.rejects(
    pending,
    (e) => isStreamCancelled(e) && e.cause.code === "ERR_FSQLITE_BULK_CANCELLED",
  );
  await blocked.promise;
  controller.abort();
  await new Promise((resolve) => setImmediate(resolve));
  assert.equal(settled, false);
  assert.equal(f.worker.requests.filter((r) => r.kind === "cancel-bulk").length, 1);
  release.resolve();
  await rejection;
  assert.equal(chunks(f).length, 2);
  assert.deepEqual(await f.read(), []);
...
test("a real SQL failure takes precedence over a coincident abort", async () => {
  ...
  f.handles[0].prepare = async (sql) => {
    ...
    stmt.executeWithParams = async (params) => {
      try {
        ...
        if (params[0] === 5) controller.abort();
      }
    };
    return stmt;
  };
  ...
  await assert.rejects(
    f.db.executeStream(INSERT, rows, { batchSize: 3, signal: controller.signal }),
    (e) => {
      assert.equal(e.phase, "execute");
      assert.equal(e.rowIndex, 4);
      assert.match(e.cause.cause.message, /UNIQUE/);
      return true;
    },
  );
...
test("abort after COMMIT dispatch cannot turn the real committed result into cancellation", async () => {
  ...
  const f = await fixture({
    beforeBatch(sql) {
      if (sql === "COMMIT") controller.abort("too late");
    },
  });
  const result = await f.db.executeStream(
    INSERT,
  ...
  const reopened = new DatabaseSync(path);
  try {
    assert.equal(reopened.prepare("SELECT count(*) AS n FROM t").get().n, 2);
  } finally {
    reopened.close();
  }
});
...
test("cancellation preserves iterator cleanup failures without losing the abort cause", async () => {
  ...
  await assert.rejects(
    f.db.executeStream(INSERT, source, {
      batchSize: 1,
      signal: controller.signal,
      onProgress() {
        controller.abort(reason);
    ...
    (e) => {
      assert.ok(isStreamCancelled(e));
      assert.equal(e.cause, reason);
      assert.deepEqual(e.cleanupErrors, [cleanup]);
      return true;
    },
  );
  assert.deepEqual(await f.read(), []);
...
test("a failed cancellation rollback makes the connection terminal, not silently reusable", async () => {
  ...
  await assert.rejects(
    ...
    (e) => {
      ...
      assert.ok(isStreamCancelled(e.cause));
      assert.equal(e.errors[1].code, "ERR_FSQLITE_TRANSACTION_CONNECTION_UNUSABLE");
      assert.match(e.errors[1].cause.message, /rollback unavailable/);
      return true;
    },
  );
    ...
    const pending = f.db.executeStream(
      ...
      ],
      {
        ...
        onProgress(p) {
          ...
          if (abort && p.batches === 2) controller.abort();
        },
      },
    );
    if (abort) await assert.rejects(pending, isStreamCancelled);
    else await pending;
    assert.equal(active.size, 0);
    await f.db.close();
  }
...
test("progress does not claim success when a deferred constraint later rejects commit", async () => {
  ...
  const progress = [];
  await f.db.executeBatch(
    "PRAGMA foreign_keys=ON; CREATE TABLE p(id PRIMARY KEY); CREATE TABLE c(id REFERENCES p(id) DEFERRABLE INITIALLY DEFERRED)",
  );
  await assert.rejects(
    f.db.executeStream("INSERT INTO c VALUES (?)", [[1], [2]], {
      batchSize: 1,
      onProgress(p) {
        progress.push(p);
      },
  ...
  assert.deepEqual(progress, [
    { executions: 1, changes: 1, batches: 1, committed: false },
    { executions: 2, changes: 2, batches: 2, committed: false },
  ]);
