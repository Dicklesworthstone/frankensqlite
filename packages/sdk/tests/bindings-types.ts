...
export async function bindingTypes(db: FrankenDB): Promise<void> {
  ...
  accepts<Promise<number>>(
    db.execute("UPDATE users SET name = :name WHERE id = :id", { id: 1, name: "Ada" }),
  );
  accepts<Promise<import("../src/types").QueryResult<User>>>(
    db.query<User>("SELECT * FROM users WHERE id = :id", values),
  );
