...
const model = installIndexedDbModel(),
  fixture = sqliteSnapshotWorker();
let fault = workerData?.fault,
  hold,
  release,
  saves = 0,
  confirms = 0;
...
IndexedDbSnapshotStore.prototype.save = async function (...args) {
  ...
  if (fault === "store-ack") {
    fault = null;
    throw new Error("lost store acknowledgement after commit");
  }
  return saved;
};
IndexedDbSnapshotStore.prototype.confirmPublication = async function (...args) {
  confirms++;
  if (hold) {
    parentPort.postMessage({ notice: "confirm-held" });
    await hold;
  }
...
parentPort.on("message", (request) => {
  if (request.testControl) {
    try {
      ...
      else if (request.testControl === "hold")
        hold = new Promise((r) => {
          release = r;
        });
      else if (request.testControl === "release") {
        hold = null;
        release?.();
      } else if (request.testControl === "corrupt") {
        const state = [...model.databases.values()][0],
          head = state.values.get("head");
      ...
      } else if (request.testControl === "stats")
        result = {
          saves,
          confirms,
          events: fixture.events,
          requestQueue: fixture.host.requestQueue,
      ...
      parentPort.postMessage({
        testControl: request.testControl,
        id: request.id,
        error: String(error),
      });
    }
  ...
  void fixture.host
    ...
    .then((response) => {
      ...
      if (response.kind === "ready" && workerData?.capability !== undefined)
        response.data.checkpointRecovery = workerData.capability;
      if (
        (request.kind === "checkpoint" || request.kind === "checkpoint-recover") &&
        response.kind === "checkpoint-result"
      ) {
        // Fault injection acts on wire data, not the store's immutable receipt.
        response = structuredClone(response);
        const value = fault;
        if (value === "bad-receipt") {
          fault = null;
          response.data.sha256 = "not-a-hash";
        }
        if (value === "bad-envelope") {
          fault = null;
          response.data = null;
        }
        if (value === "wrong-token") {
          fault = null;
          response.data.revision = crypto.randomUUID();
        }
        if (value === "wrong-parent") {
          fault = null;
          response.data.parentRevision = crypto.randomUUID();
        }
        if (value === "worker-ack") {
          fault = null;
          response = {
