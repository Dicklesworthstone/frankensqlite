...
async fn seek_shape(conn: &Connection, sql: &str) -> (bool, bool, bool) {
...
async fn populate_prefix_table(
    f: &Connection,
...
fn minmax_prefix_desc_matches_sqlite() {
    asupersync::test_utils::run_test(|| async {
        ...
        for (table, index, direction) in [
            ...
            populate_prefix_table(&f, &r, table, index, direction).await;
        }
            ...
            for aggregate in ["MIN", "MAX"] {
                for predicate in [
                    ...
                    cmp(
                        &f,
                        &r,
                        &format!("SELECT {aggregate}(b) FROM {table} WHERE {predicate}"),
                    )
                    .await;
                }
        ...
        for (table, index, direction) in [
            ...
            for aggregate in ["MIN", "MAX"] {
                cmp(
                    &f,
                    &r,
                    &format!("SELECT {aggregate}(b) FROM {table} WHERE a = 1"),
                )
                .await;
            }
        }
        ...
        assert_eq!(
            seek_shape(&f, "SELECT MAX(b) FROM t_desc WHERE a = 5").await,
            (true, false, false),
            "MAX(b) on (a, b DESC) must SeekGE the block front, covering (no SeekLE / SeekRowid)"
        );
        assert_eq!(
            seek_shape(&f, "SELECT MIN(b) FROM t_desc WHERE a = 5").await,
            (true, false, false),
        ...
        assert_eq!(
            seek_shape(&f, "SELECT MIN(b) FROM t_asc WHERE a = 5").await,
            (true, false, false),
            "MIN(b) on (a, b ASC) must SeekGE the block front without a table lookup"
        );
        assert_eq!(
            seek_shape(&f, "SELECT MAX(b) FROM t_asc WHERE a = 5").await,
            (false, true, false),
