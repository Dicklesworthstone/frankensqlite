...
export class ChangesetHttpError extends Error {
  ...
  constructor(
    ...
    this.name = "ChangesetHttpError";
    this.code = `ERR_FSQLITE_HTTP_${kind}`;
  }
}
function fail(kind: FailureCode, message: string): never {
  throw new ChangesetHttpError(kind, message);
}
function bound(value: unknown, fallback: number, maximum: number): number {
  ...
  if (typeof n !== "number" || !Number.isSafeInteger(n) || n < 1 || n > maximum)
    fail("INPUT", `Expected an integer in 1..${maximum}`);
  return n;
}
function field(record: unknown, key: string): unknown {
  if (typeof record !== "object" || record === null || Array.isArray(record))
    fail("PROTOCOL", "Expected an HTTP delivery record");
  const d = Object.getOwnPropertyDescriptor(record, key);
  if (d === undefined || !Object.hasOwn(d, "value"))
    fail("PROTOCOL", "HTTP delivery fields must be own data properties");
  return d.value;
}
function identity(value: unknown, maximum: number): string {
  if (typeof value !== "string" || !value.length || value.length > maximum || value.includes("\0"))
    fail("PROTOCOL", "Invalid delivery identity");
  const bytes = encoder.encode(value);
  if (bytes.length > maximum || utf8.decode(bytes) !== value)
    fail("PROTOCOL", "Invalid identity encoding");
  return value;
}
function digest(value: unknown): string {
  if (typeof value !== "string" || !/^[0-9a-f]{64}$/.test(value))
    fail("PROTOCOL", "Invalid SHA-256 digest");
  return value;
}
function integer(value: unknown, maximum: number): number {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 0 || value > maximum)
    fail("PROTOCOL", "Invalid delivery counter");
  return value;
}
function ownedBytes(value: unknown, maximum: number): Uint8Array {
  ...
  if (
    ...
    fail("INPUT", "Use fixed, non-shared changeset buffers");
...
function encode(message: WireEnvelope, maximum: number, format: WireFormat) {
  ...
  const meta = metadata(
    {
      protocol: field(message, "protocol"),
      receiverId: field(message, "receiverId"),
...
function receipt(value: unknown, meta: Metadata, format: WireFormat): WireReceipt {
  ...
  if (
    ...
    field(value, "deliveryId") !== meta.deliveryId ||
    field(value, "sha256") !== meta.sha256 ||
    field(value, "byteLength") !== meta.byteLength ||
    field(value, "confirmed") !== true
  )
    fail("PROTOCOL", "The HTTP receipt does not confirm this exact delivery");
  const applied = integer(field(value, "applied"), 100_000),
    omitted = integer(field(value, "omitted"), 100_000);
  const replayed = field(value, "replayed");
  if (applied + omitted > 100_000 || typeof replayed !== "boolean")
    fail("PROTOCOL", "Invalid HTTP receipt decision");
...
class Budget {
  ...
  checkpoint(): void {
    ...
    if (this.signal.aborted)
      throw new ChangesetHttpError(
        this.signal.reason === this.#reason ? "TIMEOUT" : "CANCELLED",
        "HTTP delivery interrupted; a dispatched request may have committed",
        "not-sent",
        undefined,
...
function declaredLength(headers: Headers, maximum: number): number | null {
  ...
  if (encoding !== null && encoding.toLowerCase() !== "identity")
    fail("PROTOCOL", "Content encoding is not supported on the changeset endpoint");
  ...
  if (!Number.isSafeInteger(length) || length > maximum)
    fail("LIMIT", "HTTP body exceeds its byte budget");
  return length;
}
...
async function readBody(
  ...
  try {
    ...
    let output = new Uint8Array(Math.min(4096, maximum)),
      used = 0;
    while (true) {
      budget.checkpoint();
      ...
      if (
        chunk.byteLength > maximum - used ||
        (expected !== null && chunk.byteLength > expected - used)
      )
        fail("LIMIT", "HTTP body exceeds its declared or configured byte budget");
      if (used + chunk.byteLength > output.length) {
        const grown = new Uint8Array(
          Math.min(maximum, Math.max(used + chunk.byteLength, output.length * 2)),
        );
...
function endpoint(input: string | URL, insecure: boolean): string {
  ...
  try {
    ...
    return fail("INPUT", "Use an absolute changeset endpoint URL");
  }
  ...
  if (
    ...
    url.password ||
    url.hash ||
    (url.protocol !== "https:" && !(insecure && local && url.protocol === "http:"))
  )
    fail(
      "INPUT",
      "Use HTTPS without URL credentials or fragments; development HTTP is loopback-only",
    );
  return url.href;
}
...
export function createChangesetHttpTransport(
  url: string | URL,
...
function createHttpTransport(
  ...
  const address = endpoint(url, insecure),
    credentials = options.headers;
  const send = options.fetch ?? globalThis.fetch?.bind(globalThis);
  if (
    typeof send !== "function" ||
    (credentials !== undefined && typeof credentials !== "function")
  )
    fail("INPUT", "A fetch implementation and valid credential provider are required");
  return async (message, controls = {}) => {
    ...
    try {
      ...
      const timeoutMs =
        inputTimeout === undefined
          ? timeout
          : Math.min(timeout, bound(inputTimeout, timeout, 2_147_483_647));
      ...
      if (
        ...
        fail("PROTOCOL", "An inspectable, unredirected HTTP response is required");
      if (response.status !== 200)
        throw new ChangesetHttpError(
          "HTTP",
          "The changeset endpoint did not acknowledge delivery",
          "unknown",
          response.status,
        );
      ...
      if (cause instanceof ChangesetHttpError) {
        throw new ChangesetHttpError(
          cause.code.slice("ERR_FSQLITE_HTTP_".length) as FailureCode,
          cause.message,
      ...
      try {
        ...
        if (cancelled instanceof ChangesetHttpError)
          throw new ChangesetHttpError(
            cancelled.code.endsWith("TIMEOUT") ? "TIMEOUT" : "CANCELLED",
            cancelled.message,
            sent ? "unknown" : "not-sent",
            undefined,
            { cause },
      ...
      throw new ChangesetHttpError(
        sent ? "NETWORK" : "INPUT",
        "HTTP delivery failed; preserve the delivery identity before retrying",
        sent ? "unknown" : "not-sent",
        undefined,
        { cause },
      );
...
function origins(input: readonly string[] | undefined): ReadonlySet<string> {
  if (input !== undefined && (!Array.isArray(input) || input.length > 64))
    fail("INPUT", "Configure at most 64 exact origins");
  ...
  for (const item of input ?? []) {
    ...
    try {
      ...
      return fail("INPUT", "Invalid allowed origin");
    }
    if (
      typeof item !== "string" ||
      item.length > 2048 ||
      ...
      fail(
        "INPUT",
        "Origins must be exact HTTP(S) origins, without paths, credentials or wildcards",
      );
    result.add(item);
  }
  return result;
...
function corsHeaders(input: readonly string[] | undefined): ReadonlySet<string> {
  if (input !== undefined && (!Array.isArray(input) || input.length > 32))
    fail("INPUT", "Configure at most 32 additional CORS header names");
  const result = new Set(["authorization", "content-type"]);
  for (const header of input ?? []) {
    if (
      typeof header !== "string" ||
      header.length > 128 ||
      !/^[!#$%&'+.^_`|~0-9a-z-]+$/i.test(header)
    )
      fail("INPUT", "Invalid CORS header name");
    const name = header.toLowerCase();
    if (
      ["cookie", "cookie2", "host", "origin", "connection", "transfer-encoding"].includes(name) ||
      name.startsWith("sec-") ||
      name.startsWith("proxy-")
    )
      fail("INPUT", "CORS cannot authorize ambient credentials or connection headers");
    result.add(name);
  }
  return result;
}
...
function errorResponse(status: number, origin: string | null, extra?: HeadersInit): Response {
  ...
  return httpResponse(
    status,
    { error: "ERR_FSQLITE_HTTP_REJECTED", outcome: "unknown" },
    origin,
    extra,
  );
}
...
function createHttpHandler(
  ...
  if (
    ...
    (authorizeDelivery !== undefined && typeof authorizeDelivery !== "function")
  )
    fail("INPUT", "A receiver and explicit authorization callback are required");
  const receive = method.bind(receiver);
  const maximum = bound(options.maxMessageBytes, 8 * 1024 * 1024, MAX_PAYLOAD);
  const timeout = bound(options.timeoutMs, 30_000, 2_147_483_647);
  const capacity = bound(options.maxInFlight, 1, 64);
  const allowed = origins(options.allowedOrigins),
    allowedHeaders = corsHeaders(options.allowedHeaders);
  let active = 0;
  return async (request) => {
    let admitted = false,
      ...
      receiving = false;
    try {
      if (!(request instanceof Request) || request.url.length > 8192)
        return errorResponse(400, null);
      const requestedOrigin = request.headers.get("origin");
      if (requestedOrigin !== null) {
        let parsed: URL;
        try {
          parsed = new URL(requestedOrigin);
        } catch {
          return errorResponse(403, null);
        }
        if (
          parsed.origin !== requestedOrigin ||
          !["https:", "http:"].includes(parsed.protocol) ||
          (requestedOrigin !== new URL(request.url).origin && !allowed.has(requestedOrigin))
        )
          return errorResponse(403, null);
      ...
      if (request.method === "OPTIONS") {
        ...
        const requested =
          names === "" ? [] : names.split(",").map((name) => name.trim().toLowerCase());
        if (requested.length > 34 || requested.some((name) => !allowedHeaders.has(name)))
          return errorResponse(403, origin);
        return httpResponse(204, null, origin, {
      ...
      active++;
      admitted = true;
      budget = new Budget({ signal: request.signal, timeoutMs: timeout });
      budget.checkpoint();
      ...
      try {
        permitted = (await authorize(context)) === true;
      } catch {
        permitted = false;
      }
      budget.checkpoint();
      if (!permitted) return errorResponse(403, origin);
      const wire = await readBody(
        request.body,
        request.headers,
      ...
      if (authorizeDelivery !== undefined) {
        try {
          permitted = (await authorizeDelivery(context, meta)) === true;
        } catch {
          permitted = false;
        }
        budget.checkpoint();
      ...
      const result = await receive(envelope, {
        signal: budget.signal,
      ...
      if (cause instanceof ChangesetHttpError) {
        if (cause.code.endsWith("TIMEOUT") || cause.code.endsWith("CANCELLED"))
          return errorResponse(408, origin);
      ...
      try {
        ...
        if (request instanceof Request && request.body !== null && !request.body.locked)
