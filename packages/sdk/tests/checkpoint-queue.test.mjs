...
class SqliteTransport {
  ...
  async init(config) {
    ...
    if (saved !== null) {
      const path = join(s.dir, "restored.sqlite");
      writeFileSync(path, saved.bytes);
      s.raw = new DatabaseSync(path);
      s.revision = saved.revision;
    } else s.raw = new DatabaseSync(":memory:");
    const ready = {
      path: config.dbName ?? ":memory:",
...
function stateFor(t) {
  const s = {
    dir: mkdtempSync(join(tmpdir(), "fsqlite-checkpoint-queue-")),
    inits: 0,
    stack: [],
    events: [],
    attempts: 0,
...
async function openQueue(
  t,
  name,
  options = { checkpointOnCommit: true },
...
async function fixture(t, options, persistence) {
  const model = installIndexedDbModel(),
...
async function savedValue(t, name) {
  ...
  return {
    saved,
    value: db.prepare("SELECT value FROM counts").get().value,
...
test("checkpoint policy default is unchanged for memory and snapshot queues", async (t) => {
  for (const persistence of ["memory", "indexeddb-snapshot"]) {
    ...
    assert.equal(queue.snapshotRevision, null);
  }
});
for (const input of [null, 1, "true", {}]) {
  test(`invalid checkpoint policy ${JSON.stringify(input)} opens no database`, async (t) => {
    const state = stateFor(t);
    await assert.rejects(
      FrankenDBQueue.open({ worker: state }, { checkpointOnCommit: input }),
      TypeError,
    );
...
test("checkpoint policy rejects memory mode and joins database cleanup", async (t) => {
  const state = stateFor(t);
  await assert.rejects(
    FrankenDBQueue.open({ worker: state }, { checkpointOnCommit: true }),
    (e) => e.code === "ERR_FSQLITE_CHECKPOINT_MODE",
  );
...
test("mode rejection retains close errors", async (t) => {
  ...
  await assert.rejects(
    ...
    (e) => {
      ...
      assert.equal(e.errors[0].code, "ERR_FSQLITE_CHECKPOINT_MODE");
      assert.equal(e.errors[1], state.closeError);
      return true;
    },
  );
});
test("acknowledged job restores its SQL and trigger effects from a real SQLite image", async (t) => {
  ...
  assert.equal(stored.effects, 1);
  assert.equal(queue.snapshotRevision, stored.saved.revision);
  assert.equal(state.checkpoints, 1);
  await queue.close();
  const restored = await openQueue(t, name);
  assert.equal(value(restored.state), 7);
  assert.equal(restored.queue.snapshotRevision, stored.saved.revision);
});
test("publication retains FIFO capacity and close waits for all accepted commits to checkpoint", async (t) => {
  ...
  const first = queue
    .transaction((tx) => tx.execute("UPDATE counts SET value=value+1"))
    .then(() => {
      settled = true;
    });
  ...
  const second = queue.transaction(async (tx) => {
    later = true;
    return tx.execute("UPDATE counts SET value=value+1");
  });
  await assert.rejects(
    queue.transaction(() => {}),
  ...
  assert.equal(closing, queue.close());
  await turn();
  ...
  await second;
  await closing;
  assert.equal((await savedValue(t, name)).value, 2);
...
test("put success is not acknowledgement: wait for the IndexedDB transaction complete event", async (t) => {
  const { queue, name } = await fixture(t);
  ...
  t.mock.method(ModelObjectStore.prototype, "put", function (...args) {
    ...
    result.addEventListener("success", () => {
      const advance = transaction.advance.bind(transaction);
      transaction.advance = () => {};
      staged.resolve();
  ...
  const pending = queue
    .transaction((tx) => tx.execute("UPDATE counts SET value=4"))
    .then(() => {
      settled = true;
    });
  ...
  release.resolve();
  await pending;
  assert.equal((await savedValue(t, name)).value, 4);
});
test("post-COMMIT publication failure exposes committed value, not a retryable SQL failure", async (t) => {
  ...
  assert.equal(failure.code, "ERR_FSQLITE_COMMITTED_CHECKPOINT_FAILED");
  assert.equal(failure.sqlCommitted, true);
  assert.equal(failure.checkpointConfirmed, false);
  assert.equal(failure.value, "committed-value");
  assert.equal(failure.cause, cause);
  assert.equal(failure.previousRevision, null);
  assert.equal(isTransactionConflict(failure), false);
  assert.equal(value(state), 1);
  assert.equal(await savedValue(t, name), null);
...
test("failed checkpoint fences already queued callbacks before even journal SQL executes", async (t) => {
  ...
  await saving.promise;
  const before = state.sql;
  const queued = queue.transaction(() => {
    later = true;
  });
  const refused = assert.rejects(
    queued,
    (e) => e.code === "ERR_FSQLITE_CHECKPOINT_RECOVERY_REQUIRED",
  );
  release.resolve();
  await firstFailure;
  await refused;
  assert.equal(later, false);
  assert.equal(state.sql, before);
  assert.equal(queue.stats.failedJobs, 2);
});
test("recovery checkpoint saves once, releases the fence and never replays the committed callback", async (t) => {
  const { queue, state, name } = await fixture(t);
  const failure = await failPublication(queue, state);
  const accepted = queue.stats.acceptedJobs;
  await assert.rejects(
    queue.transaction(() => {}),
    (e) => e.code === "ERR_FSQLITE_CHECKPOINT_RECOVERY_REQUIRED" && e.cause === failure,
  );
  await assert.rejects(
  ...
  const bytes = await queue.export();
  ...
  assert.equal(queue.snapshotRevision, receipt.revision);
  await queue.transaction((tx) => tx.execute("UPDATE counts SET value=value+1"));
  const stored = await savedValue(t, name);
  assert.equal(stored.value, 2);
  assert.equal(stored.effects, 2);
});
test("failed and pre-cancelled recovery barriers do not clear the fence", async (t) => {
  const { queue, state } = await fixture(t);
  await failPublication(queue, state);
  await assert.rejects(queue.checkpoint(), /quota/);
  assert.equal(queue.stats.checkpointRecoveryRequired, true);
  const control = new AbortController();
  control.abort("not now");
  const before = state.checkpoints;
  await assert.rejects(queue.checkpoint({ signal: control.signal }));
  assert.equal(state.checkpoints, before);
  assert.equal(queue.stats.checkpointRecoveryRequired, true);
});
test("failing SQL rolls back without checkpointing or requiring storage recovery", async (t) => {
  ...
  await assert.rejects(
    queue.transaction(async (tx) => {
      await tx.execute("UPDATE counts SET value=8");
      throw cause;
    }),
    (e) => e === cause,
  );
...
test("whole SQL retry checkpoints only the final successful attempt", async (t) => {
  ...
  state.beforeBoundary = (action, parent) => {
    if (action === "commit" && parent === undefined && failures-- > 0) throw busy();
  };
  ...
  assert.equal(state.checkpoints, 1);
  const saved = await savedValue(t, name);
  assert.equal(saved.value, 1);
  assert.equal(saved.effects, 1);
});
test("BUSY-shaped checkpoint errors are outside the retry loop and cannot duplicate committed SQL", async (t) => {
  ...
  await assert.rejects(
    ...
    (e) => e instanceof FrankenCheckpointCommitError,
  );
...
test("competing snapshot queues preserve CAS winner and expose the losing committed image for recovery", async (t) => {
  ...
  let failure;
  await assert.rejects(
    loser.transaction((tx) => tx.execute("UPDATE counts SET value=20")),
    (e) => {
      failure = e;
      return e.cause?.code === "ERR_FSQLITE_SNAPSHOT_CONFLICT";
    },
  );
  assert.equal(failure.previousRevision, baseline);
  assert.equal(value(b), 20);
  assert.equal(value(a), 10);
  assert.equal((await savedValue(t, name)).value, 10);
  assert.equal(loser.stats.checkpointRecoveryRequired, true);
  await assert.rejects(loser.checkpoint(), (e) => e.code === "ERR_FSQLITE_SNAPSHOT_CONFLICT");
  const image = await loser.export();
  assert.ok(image.length > 0);
  assert.equal(loser.snapshotRevision, baseline);
});
test("lost publication acknowledgement remains unknown until authoritative reopen reconciles lineage", async (t) => {
  ...
  let failure;
  await assert.rejects(
    queue.transaction((tx) => tx.execute("UPDATE counts SET value=3")),
    (e) => {
      failure = e;
      return e instanceof FrankenCheckpointCommitError;
    },
  );
  assert.equal(failure.checkpointConfirmed, false);
  assert.equal((await savedValue(t, name)).value, 3);
  assert.equal(queue.snapshotRevision, null);
  assert.equal(queue.stats.checkpointRecoveryRequired, true);
...
test("late cancellation cannot abandon saving an acknowledged COMMIT", async (t) => {
  ...
  const result = await queue.transaction(
    async (tx) => {
      await tx.execute("UPDATE counts SET value=2");
      return "saved";
    },
    { timeoutMs: 100, signal: control.signal },
  );
...
test("publication failure outranks late cancellation and preserves the original storage cause", async (t) => {
  ...
  await assert.rejects(
    queue.transaction((tx) => tx.execute("UPDATE counts SET value=1"), { signal: control.signal }),
    (e) => e instanceof FrankenCheckpointCommitError && e.cause === cause,
  );
  assert.equal(value(state), 1);
});
test("pre-commit cancellation rolls back and never checkpoints", async (t) => {
  ...
  await assert.rejects(
    queue.transaction(
      async (tx) => {
        await tx.execute("UPDATE counts SET value=5");
        control.abort("early");
      },
...
test("local committed notifications survive checkpoint failure; recovery emits no duplicate", async (t) => {
  ...
  assert.equal(changes[0].commits, 1n);
...
test("read-only transaction jobs still checkpoint under the explicit per-transaction policy", async (t) => {
  ...
  assert.equal(
    await queue.transaction(
      async (tx) => (await tx.query("SELECT value FROM counts")).rows[0].value,
    ),
    0,
  );
  assert.equal(state.checkpoints, 1);
  assert.equal((await savedValue(t, name)).value, 0);
});
test("unresolved checkpoint makes close reject after releasing resources, with one shared promise", async (t) => {
  ...
  assert.equal(first, queue.close());
...
test("unresolved checkpoint plus close failure retains both causes and committed-state evidence", async (t) => {
  ...
  await assert.rejects(
    queue.close(),
    (e) =>
      e instanceof AggregateError && e.errors[0] === failure && e.errors[1] === state.closeError,
  );
  assert.equal(state.closed, true);
});
test("failed export cannot clear checkpoint recovery", async (t) => {
  ...
  await assert.rejects(queue.export(), /export failed/);
  assert.equal(queue.stats.checkpointRecoveryRequired, true);
});
...
test("an IndexedDB abort after put success retains the previous snapshot and fences the new commit", async (t) => {
  ...
  await assert.rejects(
    queue.transaction((tx) => tx.execute("UPDATE counts SET value=2")),
    (e) => e instanceof FrankenCheckpointCommitError,
  );
  const stored = await savedValue(t, name);
  assert.equal(stored.value, 1);
  assert.equal(stored.saved.revision, old);
  assert.equal(value(state), 2);
  assert.equal(queue.stats.checkpointRecoveryRequired, true);
});
test("a recovery barrier already behind the failing job can unblock later accepted jobs in FIFO order", async (t) => {
  ...
  state.beforeSave = async () => {
    if (state.checkpoints === 1) {
      saving.resolve();
      await release.promise;
      throw new Error("temporary quota");
    }
  };
  const first = queue.transaction((tx) => tx.execute("UPDATE counts SET value=1"));
...
test("listener failure does not turn a saved commit into failure or suppress its checkpoint", async (t) => {
  ...
  await queue.transaction((tx) => tx.execute("UPDATE counts SET value=9"));
  await listenerFailed;
  assert.equal((await savedValue(t, name)).value, 9);
  assert.equal(state.checkpoints, 1);
  assert.equal(queue.stats.checkpointRecoveryRequired, false);
});
test("callback return values from a failed SQL attempt are never exposed as committed checkpoint values", async (t) => {
  ...
  state.beforeBoundary = (action) => {
    if (action === "commit") throw failed;
  };
  await assert.rejects(
    queue.transaction(async (tx) => {
      await tx.execute("UPDATE counts SET value=2");
      return { doNotPublish: true };
    }),
    (e) => e === failed,
  );
  assert.equal(state.checkpoints, 0);
...
for (const [name, change] of [
  ...
  [
    "non-v4 revision",
    (saved) => {
      saved.revision = "00000000-0000-1000-8000-000000000001";
    },
  ],
  [
    "empty digest",
  ...
  [
    "unaligned length",
    (saved) => {
      saved.byteLength = 513;
    },
  ],
  [
    "oversized length",
    (saved) => {
      saved.byteLength = 64 * 1024 * 1024 + 512;
    },
  ],
  [
    "non-finite length",
    (saved) => {
      saved.byteLength = NaN;
    },
  ],
  [
    "malformed parent",
    (saved) => {
      saved.parentRevision = "";
    },
  ],
  [
    "self-parent",
    (saved) => {
      saved.parentRevision = saved.revision;
    },
  ],
]) {
  test(`receipt rejects ${name} without acknowledging or replaying the committed write`, async (t) => {
    ...
    await assert.rejects(
      queue.transaction((tx) => tx.execute("UPDATE counts SET value=1")),
      (error) => {
        assert.ok(error instanceof FrankenCheckpointCommitError);
        assert.equal(error.cause.code, "ERR_FSQLITE_SNAPSHOT_RECEIPT");
        return true;
      },
    );
    assert.equal(queue.snapshotRevision, null);
...
test("receipt binds parent to the last acknowledged snapshot and cannot silently skip a generation", async (t) => {
  ...
  state.afterSave = (saved) => {
    saved.parentRevision = unrelatedRevision;
  };
  await assert.rejects(
    queue.transaction((tx) => tx.execute("UPDATE counts SET value=2")),
    (e) => e.cause?.code === "ERR_FSQLITE_SNAPSHOT_RECEIPT",
  );
  assert.equal(queue.snapshotRevision, previous);
  assert.equal(value(state), 2);
});
test("receipt capture owns frozen scalar metadata rather than returning transport-owned aliases", async (t) => {
  ...
  assert.notEqual(receipt, wire);
  wire.revision = unrelatedRevision;
  wire.sha256 = "";
  assert.equal(receipt.revision, revision);
  assert.equal(queue.snapshotRevision, revision);
...
test("initialization rejects malformed saved metadata and closes its worker before exposing a database", async (t) => {
  installIndexedDbModel();
  const state = stateFor(t);
  state.afterReady = (ready) => {
    ready.snapshot = { revision: "", parentRevision: null, byteLength: 0, sha256: "" };
  };
  await assert.rejects(
    FrankenDB.open({
      worker: state,
    ...
    (e) => e.code === "ERR_FSQLITE_SNAPSHOT_RECEIPT",
  );
  assert.equal(state.closed, true);
...
test("out-of-order checkpoint acknowledgements cannot advance or roll back acknowledged lineage", async (t) => {
  ...
  let replies = 0;
  state.afterSave = async () => {
    if (++replies === 1) {
      firstSaved.resolve();
      await release.promise;
    }
  };
  const first = db.checkpoint();
  const rejected = assert.rejects(first, (e) => e.code === "ERR_FSQLITE_SNAPSHOT_RECEIPT");
  await firstSaved.promise;
  try {
    await assert.rejects(db.checkpoint(), (e) => e.code === "ERR_FSQLITE_SNAPSHOT_RECEIPT");
  } finally {
    release.resolve();
    await rejected;
  }
...
test("normal concurrent checkpoint calls acknowledge their FIFO parent chain without false conflict", async (t) => {
  ...
  assert.equal(db.snapshotRevision, third.revision);
  assert.equal(state.checkpoints, 3);
  await db.close();
});
test("initialization rejects snapshot getters without invoking them, retaining cleanup errors", async (t) => {
  ...
  state.afterReady = (ready) =>
    Object.defineProperty(ready, "snapshot", {
      get() {
        invoked = true;
        return null;
      },
