...
use crate::{
    ...
    WalFecGroupMeta, WalFecGroupMetaInit, WalFecGroupRecord, WalFrameHeader, WalHeader, WalSalts,
...
fn native_export_repairs_real_fec_without_changing_source_data() {
    run_test(async {
        ...
        let originals: Vec<_> = ["", "-wal", "-wal-fec"]
            .into_iter()
            .map(|suffix| {
        ...
        for (path, bytes) in originals {
            assert_eq!(host_fs::read(&path).unwrap(), bytes);
        }
    });
}
...
fn native_export_uses_a_later_commit_to_anchor_a_repaired_terminal() {
    run_test(async {
        ...
        let mut damaged = host_fs::read(&wal_path).unwrap();
        let terminal = WAL_HEADER_SIZE + 3 * FRAME_SIZE;
        let previous = WalFrameHeader::from_bytes(&damaged[terminal..])
            .unwrap()
            .checksum;
...
fn failed_recovery_and_input_budget_do_not_create_output() {
    run_test(async {
        ...
        assert!(matches!(
            export_database(&cx, &options).await,
...
fn sidecar_writer_contention_does_not_leak_source_recovery_locks() {
    run_test(async {
        let (options, _) = fixture(8, &[1]);
        let cx = request_context().unwrap();
        let guard = host_fs::open_existing_regular_file_no_follow(&companion(
            &options.source,
            "-wal-fec.lock",
        ))
        .unwrap();
...
fn export_header_is_published_only_after_body_sync() {
    ...
    write_image(&mut file, &cx, &image, |file| {
        synced.push(file.get_ref().clone());
        Ok(())
    })
    .unwrap();
...
fn failed_body_sync_never_writes_the_valid_header() {
    let mut file = Cursor::new(Vec::new());
    assert!(
        write_image(&mut file, &Cx::new(), &empty_database(17), |_| {
...
fn bounded_reader_refuses_short_and_growing_snapshots() {
    ...
    assert_eq!(
        read_exact_snapshot(&mut Cursor::new(vec![1; 4]), &cx, 4).unwrap(),
        vec![1; 4]
    );
    cx.cancel();
    assert!(matches!(
        read_exact_snapshot(&mut Cursor::new(vec![]), &cx, 0),
...
fn cancelled_export_does_not_reserve_destination() {
    run_test(async {
        ...
        assert!(
            !NativeVfs::new()
                .path_entry_exists(&Cx::new(), &options.destination)
                .unwrap()
        );
    });
}
...
fn final_sync_failure_does_not_return_a_success_receipt() {
    ...
    assert!(
        write_image(&mut file, &Cx::new(), &image, |_| {
            calls += 1;
            if calls == 2 {
...
fn readback_rejects_changed_bytes_and_unexpected_tail() {
    ...
    let image = empty_database(19);
    assert_eq!(
        verify_image(&mut Cursor::new(image.clone()), &cx, &image).unwrap(),
        blake3::hash(&image)
    );
...
fn native_in_place_repair_preserves_original_backup_and_publishes_index() {
    use crate::wal_index::{
        WAL_SHM_SEGMENT_BYTES, read_shared_wal_index_header, validate_shared_wal_index_wal_binding,
    };
    run_test(async {
        let (options, _) = fixture(8, &[1]);
        let wal_path = companion(&options.source, "-wal");
        ...
        let expected =
            crate::wal_fec::replay::recover_wal_fec_image(&original, &sidecar, options.replay)
                .unwrap()
                .complete_image()
                .unwrap()
                .into_owned();
        ...
        let region = observer
            .file
            .shm_map(&cx, 0, u32::try_from(WAL_SHM_SEGMENT_BYTES).unwrap(), true)
            .unwrap();
        let (wal_observer, _) = vfs
            .open(
                &cx,
                Some(&wal_path),
        ...
        let terminal =
            WalFrameHeader::from_bytes(&expected[expected.len() - FRAME_SIZE..]).unwrap();
...
fn native_in_place_repair_uses_certificate_without_rewriting_it() {
    run_test(async {
        ...
        assert_eq!(
            host_fs::read(&companion(&options.source, "-wal-cert")).unwrap(),
            certificate
        );
    });
}
...
fn native_in_place_contention_and_precancellation_do_not_reserve_backup() {
    run_test(async {
        ...
        assert!(repair_wal(&cx, &options).await.is_err());
        assert!(
            !vfs.path_entry_exists(&Cx::new(), &options.destination)
                .unwrap()
        );
    });
}
...
fn write_fixture_certificate(options: &Options, identity: [u8; 16]) -> Vec<u8> {
    ...
    let mut certificate = ParallelWalCommitCertificate {
        ...
        certificate_epoch: 7,
        commit_seq_lo: CommitSeq::new(7),
        commit_seq_hi: CommitSeq::new(7),
        durable_segment_epoch: 7,
        lane_count: 1,
        lane_record_counts: vec![4],
        db_size_pages: 1,
        page_set_size: 1,
    ...
    let record = ParallelWalDurableCertificateRecord::new(
        WalGenerationIdentity::from_header(&header),
        1,
        4,
        identity,
        certificate,
...
fn optional_bad_or_oversized_certificates_do_not_block_checksum_backed_export() {
    run_test(async {
        for oversized in [false, true] {
            ...
            if oversized {
                options.replay.max_certificate_bytes = 1;
            }
            let cx = request_context().unwrap();
            let report = export_database(&cx, &options).await.unwrap();
            ...
            assert_eq!(host_fs::read(&options.destination).unwrap(), expected);
            assert_eq!(
                host_fs::read(&certificate).unwrap(),
                b"unusable optional proof"
            );
        }
    });
}
