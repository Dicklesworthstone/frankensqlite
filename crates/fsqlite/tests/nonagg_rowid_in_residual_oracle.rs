...
fn nonagg_rowid_in_residual_matches_sqlite() {
    ...
    for sql in seeks { cmp(&f, &r, sql, "rowid-in-resid"); assert!(has_seek(&f, sql), "rowid IN+residual must seek: `{sql}`"); }
    // Exact rowid IN (no residual) still seeks byte-identically.
    for sql in ["SELECT id FROM t WHERE id IN (1,2,3)", "SELECT x FROM t WHERE id IN (5,6,7)"] {
        cmp(&f, &r, sql, "exact"); assert!(has_seek(&f, sql), "exact rowid IN must seek: `{sql}`");
    }
}
