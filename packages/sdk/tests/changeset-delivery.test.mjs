...
import {
  ...
  ChangesetReceiver,
} from "../src/changeset-delivery.ts";
...
class SqlTarget {
  constructor(path = ":memory:") {
    ...
    this.transactions = 0;
    this.db.exec(
      "PRAGMA foreign_keys=ON; CREATE TABLE IF NOT EXISTS items(id INTEGER PRIMARY KEY, value TEXT)",
    );
  }
  ...
  async transaction(work) {
    ...
    try {
      ...
      return this.resultOverride ?? value;
    } catch (error) {
...
function envelope(bytes = payload(), id = "source:1") {
  return {
    protocol: CHANGESET_DELIVERY_PROTOCOL,
...
function setup(t, options = {}, path) {
  const target = new SqlTarget(path);
  t.after(() => target.db.close());
  let confirmations = 0;
  const receiver = new ChangesetReceiver(target, {
    receiverId: "replica:A",
    tables: ["items"],
    confirmCommit: async () => {
      confirmations++;
...
test("native session payload reaches SQL and acknowledgement binds exact bytes", async (t) => {
  ...
  assert.deepEqual(ack, {
    ...
    byteLength: message.changeset.length,
    applied: 2,
    omitted: 0,
    replayed: false,
    confirmed: true,
  ...
  assert.deepEqual(rows(s.target), [
    { id: 1, value: "native" },
    { id: 2, value: "second" },
  ]);
});
...
test("confirmation failure leaves a replayable receipt, never a success ACK", async (t) => {
  ...
  const s = setup(t, {
    confirmCommit: async () => {
      if (++confirmations === 1) throw new Error("checkpoint failed");
    },
  });
  await assert.rejects(s.receiver.receive(envelope()), phase("receiver-confirm"));
  assert.deepEqual(rows(s.target), [{ id: 1, value: "one" }]);
  const before = s.target.sql.filter((sql) =>
    sql.startsWith('INSERT OR ABORT INTO main."items"'),
  ).length;
  const ack = await s.receiver.receive(envelope());
  assert.equal(ack.replayed, true);
  assert.equal(confirmations, 2);
  assert.equal(
    s.target.sql.filter((sql) => sql.startsWith('INSERT OR ABORT INTO main."items"')).length,
    before,
  );
});
...
test("lost SQL COMMIT response is recovered through the same identity", async (t) => {
  ...
  s.target.afterCommit = undefined;
  assert.equal((await s.receiver.receive(envelope())).replayed, true);
  assert.equal(s.confirmations(), 1);
  assert.equal(rows(s.target).length, 1);
});
...
test("lost successful receiver response never repeats row SQL or local conflict policy", async (t) => {
  ...
  assert.equal(s.confirmations(), 2);
});
...
for (const [name, change] of [
  ...
  [
    "digest syntax",
    (m) => {
      m.sha256 = "X".repeat(64);
    },
  ],
  [
    "identity",
    (m) => {
      m.deliveryId = "bad\0id";
    },
  ],
  [
    "invalid UTF-16",
  ...
  [
    "inherited field",
    (m) => {
      const id = m.deliveryId;
      delete m.deliveryId;
      Object.setPrototypeOf(m, { deliveryId: id });
    },
  ],
  [
    "accessor field",
    (m) => {
      Object.defineProperty(m, "deliveryId", {
        get() {
  ...
  [
    "corrupt codec",
    (m) => {
      m.changeset = new Uint8Array([1]);
      m.sha256 = createHash("sha256").update(m.changeset).digest("hex");
    },
  ],
]) {
...
test("buffer admission rejects shared, resizable, detached and oversized bytes", async (t) => {
  ...
  for (const bytes of [
    ...
    new Uint8Array(new ArrayBuffer(0, { maxByteLength: 8 })),
    detached,
    new Uint8Array(129),
  ]) {
    await assert.rejects(
      s.receiver.receive({ ...envelope(), changeset: bytes }),
      phase("validate"),
    );
  }
  assert.equal(s.target.transactions, 0);
...
test("intrinsic buffer capture ignores hostile subclass iterators and length getters", async (t) => {
  class HostileBytes extends Uint8Array {
    ...
    [Symbol.iterator]() {
      throw new Error("shadow iterator");
    }
  }
  const bytes = payload(),
    s = setup(t),
...
test("caller mutation after admission cannot change SQL or its acknowledged digest", async (t) => {
  const s = setup(t),
    m = envelope(),
    sha = m.sha256;
  const pending = s.receiver.receive(m);
  m.changeset.fill(0);
  m.sha256 = "0".repeat(64);
  m.deliveryId = "changed";
  const ack = await pending;
  assert.equal(ack.sha256, sha);
  assert.equal(ack.deliveryId, "source:1");
  assert.deepEqual(rows(s.target), [{ id: 1, value: "one" }]);
});

test("allowlist is captured and applied again on replay", async (t) => {
  const names = ["items"];
  const s = setup(t, { tables: names });
  names[0] = "other";
  await s.receiver.receive(envelope());
  const restricted = new ChangesetReceiver(s.target, {
    receiverId: "replica:A",
    tables: [],
    confirmCommit: async () => assert.fail("unauthorized confirmation"),
  });
...
test("SQL conflicts roll back all rows and no receipt is confirmed", async (t) => {
  const s = setup(t);
  s.target.db.exec("INSERT INTO items VALUES (2,'existing')");
  ...
  assert.deepEqual(rows(s.target), [{ id: 2, value: "existing" }]);
  assert.equal(s.confirmations(), 0);
});
...
test("explicit omissions are retained and reconfirmed on replay", async (t) => {
  ...
  const first = await s.receiver.receive(envelope()),
    again = await s.receiver.receive(envelope());
  assert.equal(first.applied, 0);
  assert.equal(first.omitted, 1);
  assert.equal(again.omitted, 1);
...
test("deferred foreign-key failure cannot produce a confirmed receipt", async (t) => {
  const s = setup(t);
  s.target.db.exec(
    "CREATE TABLE parent(id PRIMARY KEY); CREATE TABLE child(id PRIMARY KEY, p REFERENCES parent DEFERRABLE INITIALLY DEFERRED)",
  );
  const receiver = new ChangesetReceiver(s.target, {
    receiverId: "replica:A",
    tables: ["child"],
    confirmCommit: async () => assert.fail("COMMIT failed"),
  });
  const bytes = encodeChangeset([
    {
      name: "child",
      primaryKey: [1, 0],
      changes: [{ operation: "insert", indirect: false, new: [1n, 99n] }],
    },
  ]);
...
test("same receiver rejects overlapping requests without growing a queue", async (t) => {
  ...
  const first = s.receiver.receive(envelope());
  await entered.promise;
  await assert.rejects(
    s.receiver.receive(envelope()),
    (error) => error.code === "ERR_FSQLITE_DELIVERY_BUSY",
  );
  assert.equal(s.target.transactions, 1);
  finish.release();
  await first;
  assert.equal((await s.receiver.receive(envelope())).replayed, true);
});
...
test("already cancelled input never starts SQL or confirmation", async (t) => {
  ...
  await assert.rejects(
    s.receiver.receive(envelope(), { signal: controller.signal }),
    (error) => error.code === "ERR_FSQLITE_DELIVERY_CANCELLED",
  );
  assert.equal(s.target.transactions, 0);
  assert.equal(s.confirmations(), 0);
});
...
test("a malformed post-COMMIT adapter result is not acknowledged", async (t) => {
  const s = setup(t);
  s.target.resultOverride = { applied: 99, omitted: 0, replayed: false };
  await assert.rejects(
    s.receiver.receive(envelope()),
    (error) => error.code === "ERR_FSQLITE_DELIVERY_RECEIPT",
  );
  assert.equal(s.confirmations(), 0);
  s.target.resultOverride = undefined;
  assert.equal((await s.receiver.receive(envelope())).replayed, true);
});
...
test("receipt and receiver rows reopen together in a separate process", async (t) => {
  ...
  const child = spawnSync(
    ...
    console.log(JSON.stringify({row,receipt})); db.close();`,
      path,
    ],
    { encoding: "utf8" },
  );
  ...
  assert.deepEqual(JSON.parse(child.stdout), {
    row: { value: "one" },
    receipt: { applied: 1, omitted: 0 },
  });
  const second = setup(t, {}, path);
  assert.equal((await second.receiver.receive(envelope())).replayed, true);
});

test("configuration rejects implicit confirmation and SDK/system table access", () => {
  for (const options of [
    {},
    { confirmCommit: async () => {}, tables: ["__fsqlite_changeset_outbox"] },
    { confirmCommit: async () => {}, tables: ["sqlite_schema"] },
    { confirmCommit: async () => {}, tables: ["items", "ITEMS"] },
    { confirmCommit: async () => {}, maxMessageBytes: 0 },
    { confirmCommit: async () => {}, maxMessageBytes: null },
  ]) {
    assert.throws(
      () => new ChangesetReceiver({}, { receiverId: "replica:A", tables: ["items"], ...options }),
    );
  }
});
...
test("monotonic deadline reaches SQL checkpoints even when timer tasks are starved", async (t) => {
  const s = setup(t);
  const original = s.target.execute.bind(s.target);
  s.target.execute = async (sql, params) => {
    ...
    if (sql.startsWith('INSERT OR ABORT INTO main."items"')) {
      ...
      while (performance.now() < until) {
        /* Deliberately starve timer tasks. */
      }
    }
    return value;
  };
...
async function pair(t, options = {}) {
  const source = new SqlTarget(options.sourcePath);
  t.after(() => source.db.close());
  ...
  const record = (sql, params, deliveryId) =>
    outbox.record((tx) => tx.execute(sql, params), { tables: ["items"], deliveryId });
  return {
    source,
    outbox,
    destination,
...
async function recordOne(p) {
  return p.record("INSERT INTO items VALUES (?,?)", [1n, "one"], "source:1");
}
function adapt(outbox, overrides = {}) {
  return {
    pending: (options) => outbox.pending(options),
    read: (id) => outbox.read(id),
    acknowledge: (id, sha) => outbox.acknowledge(id, sha),
    ...overrides,
  };
}
...
test("source and receiver confirmations bracket every delivery and acknowledgement", async (t) => {
  ...
  const outbox = adapt(p.outbox, {
    ...
    read: async (id) => {
      events.push("read");
      return p.outbox.read(id);
    },
    acknowledge: async (id, sha) => {
      events.push("ack");
      return p.outbox.acknowledge(id, sha);
    },
  });
  const pump = new ChangesetDeliveryPump(outbox, {
    ...p.pumpOptions,
    confirmSource: async () => {
      events.push("source-confirm");
    },
    deliver: async (m, o) => {
      events.push("send");
      return p.destination.receiver.receive(m, o);
...
test("transport failure before submission preserves the oldest message and its successor", async (t) => {
  ...
  assert.equal((await p.outbox.pending()).length, 2);
  assert.deepEqual(rows(p.destination.target), []);
...
test("lost remote ACK keeps source bytes and replay never reapplies receiver writes", async (t) => {
  ...
  await assert.rejects(
    p
      .pump({
        deliver: async (m, o) => {
          await p.destination.receiver.receive(m, o);
  ...
  assert.notEqual((await p.outbox.read("source:1")).changeset, null);
  const before = p.destination.target.sql.filter((sql) =>
    sql.startsWith('INSERT OR ABORT INTO main."items"'),
  ).length;
  assert.equal((await p.pump().run()).replays, 1);
  assert.equal(
    p.destination.target.sql.filter((sql) => sql.startsWith('INSERT OR ABORT INTO main."items"'))
      .length,
    before,
  );
});
  ...
  test(`source confirmation ${failedConfirmation} fails before any remote SQL`, async (t) => {
    ...
    await assert.rejects(
      p
        .pump({
          confirmSource: async () => {
            if (++confirmations === failedConfirmation) throw new Error("source checkpoint");
          },
          deliver: async () => {
            sends++;
            assert.fail("source unconfirmed");
          },
        })
        .run(),
...
test("receiver confirmation failure cannot reclaim the source; replay confirms again", async (t) => {
  ...
  const p = await pair(t, {
    receiverOptions: {
      confirmCommit: async () => {
        if (++confirmations === 1) throw new Error("receiver checkpoint");
      },
    },
  });
  await recordOne(p);
  ...
  assert.equal((await p.outbox.pending()).length, 1);
  assert.equal(rows(p.destination.target).length, 1);
  assert.equal((await p.pump().run()).replays, 1);
  assert.equal(confirmations, 2);
});
...
test("failed source acknowledgement retries delivery, never the original source callback", async (t) => {
  const p = await pair(t);
  let callbacks = 0;
  await p.outbox.record(
    async (tx) => {
      callbacks++;
      await tx.execute("INSERT INTO items VALUES(1,'one')");
    },
    { tables: ["items"], deliveryId: "source:1" },
  );
  const pump = new ChangesetDeliveryPump(
    adapt(p.outbox, {
      acknowledge: async () => {
...
test("lost source acknowledgement response is recovered before observing an empty outbox", async (t) => {
  ...
  const pump = new ChangesetDeliveryPump(
    adapt(p.outbox, {
      acknowledge: async (id, sha) => {
        await p.outbox.acknowledge(id, sha);
        throw new Error("lost source commit response");
      },
    }),
    p.pumpOptions,
  );
  ...
  const result = await p
    .pump({
      ...
      deliver: async () => assert.fail("already acknowledged"),
    })
    .run();
...
test("failed post-ACK source checkpoint cannot be bypassed by an empty pending list", async (t) => {
  ...
  await assert.rejects(
    p
      .pump({
        confirmSource: async () => {
          if (++confirmations === 3) throw new Error("post-ack checkpoint");
        },
      })
      .run(),
    phase("source-confirm"),
...
for (const [name, mutate] of [
  ...
  [
    "accessor",
    (a) => {
      Object.defineProperty(a, "confirmed", {
        get() {
  ...
  [
    ...
    (a) => {
      delete a.confirmed;
      Object.setPrototypeOf(a, { confirmed: true });
    },
  ],
]) {
  test(`reject mismatched ${name} receipt without acknowledging source`, async (t) => {
    ...
    await assert.rejects(
      p
        .pump({
          deliver: async (m, o) => {
            const ack = { ...(await p.destination.receiver.receive(m, o)) };
            mutate(ack);
            return ack;
          },
        })
...
test("sender must explicitly accept the receivers retained omission decision", async (t) => {
  const p = await pair(t, { receiverOptions: { onConflict: () => "omit" } });
  await recordOne(p);
...
test("run and byte limits leave the next ordered message pending", async (t) => {
  ...
  assert.equal(one.stopped, "limit");
  assert.equal(one.deliveries, 1);
  assert.equal(reads, 1);
  assert.equal((await p.outbox.pending())[0].deliveryId, "source:2");
  const two = await pump.run({ maxDeliveries: 1 });
  assert.equal(two.deliveries, 1);
  assert.equal(two.stopped, "limit");
  ...
  test(`${mode} byte admission refuses an oversized head before payload load`, async (t) => {
    ...
    const pump = new ChangesetDeliveryPump(
      adapt(p.outbox, {
        read: async () => {
          reads++;
          assert.fail("oversized load");
        },
      }),
      { ...p.pumpOptions, ...(mode === "message" ? { maxMessageBytes: 1 } : {}) },
    );
    await assert.rejects(
      pump.run(mode === "run" ? { maxBytes: 1 } : {}),
      (error) => error.code === "ERR_FSQLITE_DELIVERY_LIMIT",
    );
...
test("corrupt source bytes never reach transport", async (t) => {
  ...
  await assert.rejects(
    p.pump({ deliver: async () => assert.fail("corrupt send") }).run(),
    phase("source-read"),
  );
  assert.equal((await p.outbox.pending()).length, 1);
});
...
test("acknowledgement racing selection and payload read is confirmed, not sent twice", async (t) => {
  ...
  let raced = false,
    confirmations = 0;
  const wrapped = adapt(p.outbox, {
    pending: async (o) => {
      const selected = await p.outbox.pending(o);
      if (selected.length && !raced) {
        raced = true;
        const loaded = await p.outbox.read(selected[0].deliveryId);
  ...
  const result = await new ChangesetDeliveryPump(wrapped, {
    ...p.pumpOptions,
    confirmSource: async () => {
      confirmations++;
    },
    deliver: async () => assert.fail("already confirmed by peer"),
  }).run();
  assert.equal(result.deliveries, 0);
  assert.equal(result.alreadyAcknowledged, 1);
  assert.equal(confirmations, 2);
});
...
test("missing or changed selected metadata stops instead of skipping work", async (t) => {
  ...
  for (const read of [
    async () => null,
    async (id) => {
      const loaded = await p.outbox.read(id);
      return { ...loaded, delivery: { ...loaded.delivery, sequence: 999n } };
    },
  ]) {
    await assert.rejects(
      new ChangesetDeliveryPump(adapt(p.outbox, { read }), p.pumpOptions).run(),
      phase("source-read"),
    );
    assert.equal((await p.outbox.pending()).length, 1);
  }
...
test("cancellation during a successful source ACK still drains source confirmation", async (t) => {
  ...
  const wrapped = adapt(p.outbox, {
    acknowledge: async (id, sha) => {
      const result = await p.outbox.acknowledge(id, sha);
      controller.abort("after ack");
      return result;
    },
  });
  await assert.rejects(
    new ChangesetDeliveryPump(wrapped, {
      ...p.pumpOptions,
      confirmSource: async () => {
        confirmations++;
      },
...
test("delivery timeout passes only the remaining budget to transport and drains it", async (t) => {
  ...
  const pump = p.pump({
    ...
    deliver: async (m, o) => {
      ...
      entered.release();
      await finish.promise;
      return p.destination.receiver.receive(m);
    },
  });
  ...
  await entered.promise;
  assert.ok(seenTimeout > 0 && seenTimeout < 200);
  await pause(210);
  assert.equal(settled, false);
  finish.release();
  await checked;
...
for (const cut of ["receiver-committed", "source-acknowledged"]) {
  test(`SIGKILL at ${cut} recovers delivery from independently reopened files`, async (t) => {
    ...
    const sourcePath = join(directory, "source.db"),
      receiverPath = join(directory, "receiver.db");
    const p = await pair(t, { sourcePath, receiverPath });
    await recordOne(p);
    ...
    const child = spawnSync(
      ...
      source.db.close(); destination.db.close();`,
        sourcePath,
        receiverPath,
        cut,
      ],
    ...
    assert.match(child.stdout, new RegExp("CUT:" + cut));
    ...
    assert.equal(
      (
        await recovered.outbox.record(async () => assert.fail("source operation replayed"), {
          tables: ["items"],
          deliveryId: "source:1",
        })
      ).replayed,
    ...
    assert.equal(
      recovered.destination.target.db
        .prepare("SELECT count(*) n FROM __fsqlite_changeset_receipts")
        .get().n,
      1,
    );
  });
...
test("real SQLite image barriers retain source bytes until a receiver snapshot is confirmed", async (t) => {
  ...
  const save = (target, files, kind) => {
    ...
    target.db.exec(`VACUUM INTO '${path.replaceAll("'", "''")}'`);
    files.push(path);
  };
  const p = await pair(t, {
    receiverOptions: {
      confirmCommit: async () => {
        ...
        if (receiverImages.length === 1)
          throw new Error("publication completed but response was lost");
      },
    },
  });
  receiverTarget = p.destination.target;
  await recordOne(p);
  const pump = p.pump({
    confirmSource: async () => {
      save(p.source, sourceImages, "source");
    },
  });
  await assert.rejects(pump.run(), phase("transport"));
  const readImage = (path, sql) => {
  ...
  assert.deepEqual(
    readImage(
      sourceImages.at(-1),
      "SELECT acknowledged, length(payload) > 0 retained FROM __fsqlite_changeset_outbox",
    ),
    { acknowledged: 0, retained: 1 },
  );
  assert.deepEqual(readImage(receiverImages[0], "SELECT value FROM items"), { value: "one" });
  assert.deepEqual(
    readImage(receiverImages[0], "SELECT applied FROM __fsqlite_changeset_receipts"),
    { applied: 1 },
  );
  ...
  assert.deepEqual(
    readImage(
      sourceImages.at(-1),
      "SELECT acknowledged, length(payload) retained FROM __fsqlite_changeset_outbox",
    ),
    { acknowledged: 1, retained: 0 },
  );
...
test("two file-backed senders overlap, then recover without duplicating receiver SQL", async (t) => {
  ...
  const first = await pair(t, options);
  ...
  const attempts = await Promise.allSettled([
    first.pump({ deliver: deliver(first) }).run(),
    second.pump({ deliver: deliver(second) }).run(),
  ]);
  ...
  assert.ok(
    attempts.every(
      (r) =>
        r.status === "fulfilled" ||
        r.reason.phase === "transport" ||
        r.reason.phase === "source-ack",
    ),
  );
  await first.pump().run();
  assert.deepEqual(rows(first.destination.target), [{ id: 1, value: "one" }]);
  assert.equal(
    first.destination.target.db.prepare("SELECT count(*) n FROM __fsqlite_changeset_receipts").get()
      .n,
    1,
  );
...
test("invalid run admission never confirms, reads or submits and releases the instance", async (t) => {
  ...
  for (const bad of [
    ...
    await assert.rejects(pump.run(bad));
...
test("a claimed successful ACK that leaves the same pending head cannot spin or skip it", async (t) => {
  ...
  await assert.rejects(pump.run(), phase("source-read"));
