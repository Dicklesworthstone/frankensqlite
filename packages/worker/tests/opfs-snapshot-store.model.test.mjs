...
import { databaseImage, deferred, installOpfsModel } from "./helpers/opfs-model.mjs";
...
const fileKey = (model) => {
  assert.equal(model.files.size, 1);
  return [...model.files.keys()][0];
};
...
function rewriteEnvelope(bytes, change) {
  ...
  new DataView(result.buffer).setUint32(8, header.length);
  result.set(header, 12);
  result.set(bytes.slice(12 + length), 12 + header.length);
  return result;
}

test("OPFS checkpoints reopen complete images and revision lineage", async (t) => {
  const m = fixture(t),
    a = await OpfsSnapshotStore.open("notes/日本語");
  ...
  assert.equal(two.parentRevision, one.revision);
  assert.equal(two.byteLength, 65536);
  assert.match(two.sha256, /^[a-f0-9]{64}$/);
  a.close();
  const b = await OpfsSnapshotStore.open("notes/日本語");
  const saved = await b.load();
  assert.deepEqual(saved.bytes, databaseImage(7, 65536));
  assert.equal(saved.revision, two.revision);
  assert.equal(m.counts.publications, 2);
  b.close();
});

test("OPFS stale and create-only writers cannot overwrite the winning checkpoint", async (t) => {
  const m = fixture(t),
    a = await OpfsSnapshotStore.open("shared"),
    b = await OpfsSnapshotStore.open("shared");
  ...
  assert.equal((await b.load()).revision, next.revision);
  assert.equal(m.counts.publications, 2);
});
...
test("OPFS lock spans stream close but unrelated names stay independent", async (t) => {
  ...
  const saving = a.save(databaseImage(1), null);
  await entered.promise;
  let loaded = false;
  const reading = a.load().then((value) => {
    loaded = true;
    return value;
  });
...
test("OPFS captures caller-owned views before asynchronous hashing", async (t) => {
  fixture(t);
  const a = await OpfsSnapshotStore.open("copy");
  const backing = new Uint8Array(1024);
  backing.set(databaseImage(9), 100);
  const view = backing.subarray(100, 612),
    expected = new Uint8Array(view);
  const pending = a.save(view, null);
  backing.fill(255);
  await pending;
  assert.deepEqual((await a.load()).bytes, expected);
...
for (const phase of ["create", "write", "beforeClose"]) {
  test(`OPFS ${phase} failure preserves an existing head and allows retry`, async (t) => {
    const m = fixture(t),
      a = await OpfsSnapshotStore.open(phase);
    const saved = await a.save(databaseImage(4), null),
      key = fileKey(m),
      original = m.files.get(key).slice();
    ...
    assert.deepEqual(m.files.get(key), original);
    delete m.hooks[phase];
    await a.save(databaseImage(6), saved.revision);
    assert.equal((await a.load()).bytes[100], 6);
  });
  test(`OPFS initial ${phase} failure removes only its new empty staging entry`, async (t) => {
    ...
    delete m.hooks[phase];
    await a.save(databaseImage(8), null);
    assert.equal((await a.load()).bytes[100], 8);
  });
}

test("OPFS lost close acknowledgement is confirmed without rewriting or replay", async (t) => {
  const m = fixture(t),
    a = await OpfsSnapshotStore.open("receipt");
  const previous = await a.save(databaseImage(1), null),
    revision = crypto.randomUUID();
  m.hooks.afterClose = () => {
    throw new Error("lost acknowledgement");
  };
  await assert.rejects(
    a.save(databaseImage(2), previous.revision, revision),
  ...
  assert.equal(m.counts.publications, count);
  assert.equal((await a.load()).bytes[100], 2);
  await assert.rejects(
    a.confirmPublication(crypto.randomUUID(), previous.revision),
    code("NOT_CONFIRMED"),
  );
...
test("OPFS lost initial receipt is retained rather than deleted", async (t) => {
  const m = fixture(t),
    a = await OpfsSnapshotStore.open("first-receipt"),
    revision = crypto.randomUUID();
  m.hooks.afterClose = () => {
    throw new Error("lost acknowledgement");
  };
...
test("OPFS corruption and malformed envelopes cannot be loaded or overwritten", async (t) => {
  const m = fixture(t),
    a = await OpfsSnapshotStore.open("corrupt");
  const saved = await a.save(databaseImage(), null),
    key = fileKey(m),
    original = m.files.get(key).slice();
  const badHash = original.slice();
...
test("OPFS names preserve distinct unpaired UTF-16 surrogates", async (t) => {
  const m = fixture(t),
    a = await OpfsSnapshotStore.open("\ud800"),
    b = await OpfsSnapshotStore.open("\ud801");
  await a.save(databaseImage(1), null);
  await b.save(databaseImage(2), null);
  assert.equal(m.files.size, 2);
  assert.equal((await a.load()).bytes[100], 1);
  assert.equal((await b.load()).bytes[100], 2);
});

test("OPFS close fences queued work without interrupting an admitted publication", async (t) => {
  const m = fixture(t),
    a = await OpfsSnapshotStore.open("closed"),
    entered = deferred(),
    release = deferred();
  m.hooks.beforeClose = async () => {
    entered.resolve();
  ...
  const saving = a.save(databaseImage(2), null);
  await entered.promise;
  const reading = a.load();
  const rejected = assert.rejects(reading, code("CLOSED"));
  a.close();
  a.close();
  release.resolve();
  const saved = await saving;
  await rejected;
  await assert.rejects(a.load(), code("CLOSED"));
  await assert.rejects(a.save(databaseImage(), saved.revision), code("CLOSED"));
  const b = await OpfsSnapshotStore.open("closed");
  assert.equal((await b.load()).revision, saved.revision);
});
