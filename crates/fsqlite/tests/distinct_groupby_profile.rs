...
async fn measure_batched(conn: &Connection, sql: &str, batches: u64, per_batch: u64) -> (f64, f64) {
    for _ in 0..20 {
        let _ = conn.query(sql).await.unwrap();
    }
...
async fn distinct_routing(conn: &Connection, sql: &str) -> &'static str {
...
fn distinct_groupby_indexed_or_scan() {
    asupersync::test_utils::run_test(|| async {
        ...
        conn.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, k INTEGER, u INTEGER, h INTEGER);")
            .await
            .unwrap();
        conn.execute("CREATE INDEX idx_k ON t(k);").await.unwrap();
        conn.execute("CREATE INDEX idx_h ON t(h);").await.unwrap();
        ...
        eprintln!(
         ...
         lever is NOT in this binary)",
            distinct_routing(&conn, "SELECT DISTINCT k FROM t").await
        );
        for (label, sql) in cases {
            eprintln!(
                "  [{label:32}] {:9.1} ns/query",
        ...
        for _ in 0..5 {
            k_stats.push(measure_batched(&conn, "SELECT DISTINCT k FROM t", 1, 200).await);
            u_stats.push(measure_batched(&conn, "SELECT DISTINCT u FROM t", 1, 200).await);
        }
        let pool = |s: &[(f64, f64)]| {
