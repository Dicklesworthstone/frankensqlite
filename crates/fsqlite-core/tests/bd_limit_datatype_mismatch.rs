...
fn non_integer_limit_reports_datatype_mismatch() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert_eq!(
            err_of(setup, "SELECT * FROM t LIMIT 'x'").await,
            "datatype mismatch"
        );
        assert_eq!(
            err_of(setup, "SELECT * FROM t LIMIT x'00'").await,
            "datatype mismatch"
        );
        assert_eq!(
            err_of(setup, "SELECT * FROM t LIMIT 1.5").await,
            "datatype mismatch"
        );
        ...
        let rows = c
            .query_with_params("SELECT * FROM t LIMIT '2'", &[])
            .await
            .unwrap();
