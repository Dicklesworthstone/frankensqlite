//! bd-xa2qv — Oracle-parity e2e: UPSERT with MULTIPLE ON CONFLICT clauses on a
//! WITHOUT ROWID table.
//!
//! WITHOUT ROWID mirror of `upsert_multi_on_conflict_oracle_e2e`. SQLite 3.35.0+
//! allows a chain of `ON CONFLICT` clauses on a single INSERT, each targeting a
//! different UNIQUE/PK constraint. When the candidate row violates a constraint,
//! the clauses are examined **in written order** and the first clause whose
//! conflict target is violated is applied (its `DO UPDATE` rewrites the specific
//! conflicting row, or `DO NOTHING` silently skips the insert). A conflict on a
//! constraint that no clause targets raises the default constraint error. A
//! final targetless `ON CONFLICT DO ...` matches any conflict. On a WITHOUT
//! ROWID table the clustered PRIMARY KEY and every secondary UNIQUE index are
//! probed the same way. Every scenario asserts per-statement agreement with
//! rusqlite, then compares resulting table state.
...
// Clustered PK `a`, secondary UNIQUE `b`, payload `v`.
const T: &str = "CREATE TABLE t(a INTEGER PRIMARY KEY, b INTEGER UNIQUE, v) WITHOUT ROWID";

/// Bead example: the candidate conflicts on the SECOND target (b); the second
/// clause must apply -> (1,2,200). This is exactly what the pre-fix WITHOUT
/// ROWID path got wrong (it only recognized the first clause's target).
#[test]
fn wr_multi_conflict_on_second_target() {
    asupersync::test_utils::run_test(|| async {
        scenario(
            ...
            "wr_multi_conflict_on_second_target",
        )
        .await;
    });
}
...
#[test]
fn wr_multi_conflict_on_first_target() {
    asupersync::test_utils::run_test(|| async {
        scenario(
            &[
                T,
            ...
            "wr_multi_conflict_on_first_target",
        )
        .await;
    });
}
...
/// order) decides.
#[test]
fn wr_multi_conflict_both_targets_clause_order_wins() {
    asupersync::test_utils::run_test(|| async {
        scenario(
            &[
                T,
            ...
            "wr_multi_conflict_both_a_then_b",
        )
        .await;
        scenario(
            &[
            ...
            "wr_multi_conflict_both_b_then_a",
        )
        .await;
    });
}
...
#[test]
fn wr_multi_conflict_targets_different_rows() {
    asupersync::test_utils::run_test(|| async {
        scenario(
            &[
                T,
            ...
            "wr_multi_conflict_targets_different_rows",
        )
        .await;
    });
}
...
#[test]
fn wr_multi_conflict_do_nothing_clause() {
    asupersync::test_utils::run_test(|| async {
        ...
        scenario(
            ...
            "wr_multi_conflict_2nd_do_nothing",
        )
        .await;
        ...
        scenario(
            ...
            &["SELECT a,b,v FROM t ORDER BY a"], // (1,2,10)
            "wr_multi_conflict_1st_do_nothing",
        )
        .await;
    });
}
...
#[test]
fn wr_multi_conflict_trailing_bare_do_nothing() {
    asupersync::test_utils::run_test(|| async {
        scenario(
            &[
                "CREATE TABLE t(a INTEGER PRIMARY KEY, b INTEGER UNIQUE, c INTEGER UNIQUE, v) WITHOUT ROWID",
            ...
            "wr_multi_conflict_trailing_bare_do_nothing",
        )
        .await;
    });
}
...
#[test]
fn wr_multi_conflict_no_conflict_inserts() {
    asupersync::test_utils::run_test(|| async {
        scenario(
            &[
                T,
            ...
            "wr_multi_conflict_no_conflict_inserts",
        )
        .await;
    });
}
...
#[test]
fn wr_multi_conflict_untargeted_constraint_errors() {
    asupersync::test_utils::run_test(|| async {
        scenario(
            &[
                "CREATE TABLE t(a INTEGER PRIMARY KEY, b INTEGER UNIQUE, c INTEGER UNIQUE, v) WITHOUT ROWID",
            ...
            "wr_multi_conflict_untargeted_constraint_errors",
        )
        .await;
    });
}
...
#[test]
fn wr_multi_conflict_excluded_and_existing_refs() {
    asupersync::test_utils::run_test(|| async {
        scenario(
            &[
                T,
            ...
            "wr_multi_conflict_excluded_and_existing_refs",
        )
        .await;
    });
}

/// A `DO UPDATE ... WHERE` whose predicate is false on a matching conflict is a
/// no-op: the row is neither updated nor inserted (the conflict is still
/// "handled"), across a multi-clause chain.
#[test]
fn wr_multi_conflict_do_update_where_false_skips() {
    asupersync::test_utils::run_test(|| async {
        scenario(
            &[
                T,
                "INSERT INTO t VALUES(1,2,10)",
                // conflict on b; WHERE false -> no update, no insert -> (1,2,10).
                "INSERT INTO t VALUES(9,2,99) ON CONFLICT(a) DO UPDATE SET v=100 ON CONFLICT(b) DO UPDATE SET v=200 WHERE v<0",
            ],
            &["SELECT a,b,v FROM t ORDER BY a"], // (1,2,10)
            "wr_multi_conflict_do_update_where_false_skips",
        )
        .await;
        // WHERE true on the matching clause -> update applies -> (1,2,200).
        scenario(
            &[
                T,
                "INSERT INTO t VALUES(1,2,10)",
            ...
            &["SELECT a,b,v FROM t ORDER BY a"], // (1,2,200)
            "wr_multi_conflict_do_update_where_true_applies",
        )
        .await;
    });
}

/// Regression for the single-clause target-specificity corollary on a WITHOUT
/// ROWID table: an explicit `ON CONFLICT(a) DO NOTHING` must NOT swallow a
/// conflict on a different constraint (b); stock raises the constraint error.
#[test]
fn wr_single_do_nothing_nontarget_conflict_errors() {
    asupersync::test_utils::run_test(|| async {
        scenario(
            &[
                "CREATE TABLE t(a INTEGER PRIMARY KEY, b INTEGER UNIQUE, c) WITHOUT ROWID",
            ...
            "wr_single_do_nothing_nontarget_conflict_errors",
        )
        .await;
    });
}

/// Candidate NOT NULL / CHECK constraints are enforced (ABORT) for an upsert row
/// on a WITHOUT ROWID table — even when the row would conflict and DO UPDATE —
/// matching stock. Guards against blanket-IGNORE of candidate constraints.
#[test]
fn wr_upsert_candidate_check_and_notnull_enforced() {
    asupersync::test_utils::run_test(|| async {
        ...
        scenario(
            &[
                "CREATE TABLE t(a INTEGER PRIMARY KEY, b UNIQUE, v CHECK(v>0)) WITHOUT ROWID",
                "INSERT INTO t VALUES(1,2,10)",
                "INSERT INTO t VALUES(1,8,-5) ON CONFLICT(a) DO UPDATE SET v=100",
            ],
            &["SELECT a,b,v FROM t ORDER BY a"], // (1,2,10) — rejected
            "wr_upsert_conflicting_candidate_check_enforced",
        )
        .await;
        ...
        scenario(
            ...
            "wr_upsert_do_nothing_candidate_check_enforced",
        )
        .await;
    });
}
...
/// algorithm propagates to the non-target constraints — WITHOUT ROWID.
#[test]
fn wr_upsert_or_ignore_untargeted_conflict_ignored() {
    asupersync::test_utils::run_test(|| async {
        scenario(
            &[
                "CREATE TABLE t(a INTEGER PRIMARY KEY, b INTEGER UNIQUE, c INTEGER UNIQUE, v) WITHOUT ROWID",
            ...
            "wr_upsert_or_ignore_untargeted_conflict_ignored",
        )
        .await;
    });
}
