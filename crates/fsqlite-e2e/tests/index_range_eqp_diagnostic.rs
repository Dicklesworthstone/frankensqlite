...
async fn eqp(conn: &Connection, sql: &str) -> String {
    match conn.query(&format!("EXPLAIN QUERY PLAN {sql}")).await {
        Ok(rows) => rows
            .iter()
...
async fn row_count(conn: &Connection, sql: &str) -> usize {
    conn.query(sql).await.map(|r| r.len()).unwrap_or(usize::MAX)
}
...
fn diagnostic_index_range_plan_choice() {
    asupersync::test_utils::run_test(|| async {
        ...
        f.execute("CREATE TABLE t (id INTEGER PRIMARY KEY, k INTEGER, v REAL, w TEXT);")
            .await
            .unwrap();
        f.execute("CREATE INDEX idx_t_k ON t(k);").await.unwrap();
        ...
        for s in 0..samples {
            ...
            let t = Instant::now();
            for _ in 0..execs {
                let _ = f.query(&seek_sql).await.unwrap();
            }
            seek_us.push(t.elapsed().as_micros() as f64 / execs as f64);

            let t = Instant::now();
            for _ in 0..execs {
                let _ = f.query(&scan_sql).await.unwrap();
            }
        ...
        for s in 0..samples {
            ...
            let t = Instant::now();
            for _ in 0..execs {
                let _ = ph_seek.query_with_params(&params).await.unwrap();
            }
            ph_seek_us.push(t.elapsed().as_micros() as f64 / execs as f64);

            let t = Instant::now();
            for _ in 0..execs {
                let _ = ph_scan.query_with_params(&params).await.unwrap();
            }
...
fn diagnostic_composite_desc_order_by() {
    asupersync::test_utils::run_test(|| async {
        ...
        f.execute("CREATE TABLE ct (id INTEGER PRIMARY KEY, a INTEGER, b INTEGER, w TEXT);")
            .await
            .unwrap();
        f.execute("CREATE INDEX idx_ct_ab ON ct(a, b);")
            .await
            .unwrap();
        ...
        for s in 0..samples {
            ...
            let seek_sql = format!(
                "SELECT id, b FROM ct WHERE a = {av} AND b > {lo} ORDER BY b DESC, id DESC"
            );
            let scan_sql = format!(
