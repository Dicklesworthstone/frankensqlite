...
fn alter_table_schema_rewrite_match_rusqlite_oracle() {
    asupersync::test_utils::run_test(|| async {
        ...
        check(
            "rename col new name query",
            fq(&f, "SELECT a, qty FROM t ORDER BY a").await,
            rq(&r, "SELECT a, qty FROM t ORDER BY a"),
            &mut diffs,
        );
        check(
            "rename col old name errors",
            fq(&f, "SELECT b FROM t").await,
            rq(&r, "SELECT b FROM t"),
            &mut diffs,
        );
        // index still usable under the renamed column
        check(
            "rename col index usable",
            fq(&f, "SELECT a FROM t WHERE qty > 5 ORDER BY a").await,
            rq(&r, "SELECT a FROM t WHERE qty > 5 ORDER BY a"),
            &mut diffs,
        );
        // dependent view rewritten to use the new column name internally
        check(
            "rename col view rewritten",
            fq(&f, "SELECT a, bee FROM vt ORDER BY a").await,
            rq(&r, "SELECT a, bee FROM vt ORDER BY a"),
            &mut diffs,
        );
        ...
        check(
            "rename col trigger fires",
            fq(&f, "SELECT msg FROM log ORDER BY msg").await,
            rq(&r, "SELECT msg FROM log ORDER BY msg"),
            &mut diffs,
        );
        // CHECK constraint still enforced under the new name (insert violating qty<0 fails)
        ex(&f, &r, "INSERT INTO t VALUES (5,-1,'bad')").await;
        check(
            "rename col check enforced",
        ...
        check(
            ...
            fq(&f, "SELECT a, qty FROM items ORDER BY a").await,
            rq(&r, "SELECT a, qty FROM items ORDER BY a"),
            &mut diffs,
        );
        check(
            "rename table view follows",
            fq(&f, "SELECT a, bee FROM vt ORDER BY a").await,
            rq(&r, "SELECT a, bee FROM vt ORDER BY a"),
            &mut diffs,
        );
        check(
            "rename table old name errors",
            fq(&f, "SELECT * FROM t").await,
            rq(&r, "SELECT * FROM t"),
            &mut diffs,
        );
        ...
        ex(
            &f,
            &r,
            "ALTER TABLE items ADD COLUMN note TEXT DEFAULT 'none'",
        )
        .await;
        check(
            "add col with default",
            fq(&f, "SELECT a, note FROM items ORDER BY a").await,
            rq(&r, "SELECT a, note FROM items ORDER BY a"),
            &mut diffs,
        );
        ex(
            &f,
        ...
        .await;
        check(
            "add col not-null default",
            fq(&f, "SELECT a, score FROM items ORDER BY a").await,
            rq(&r, "SELECT a, score FROM items ORDER BY a"),
            &mut diffs,
        );
        ...
        ex(
            &f,
            &r,
            "ALTER TABLE items ADD COLUMN qty2 INTEGER GENERATED ALWAYS AS (qty*2) VIRTUAL",
        )
        .await;
        check(
            "add generated col",
            fq(&f, "SELECT a, qty2 FROM items ORDER BY a").await,
            rq(&r, "SELECT a, qty2 FROM items ORDER BY a"),
            &mut diffs,
        );
        ...
        check(
            ...
            fq(
                &f,
                "SELECT name FROM pragma_table_info('items') ORDER BY name",
            )
            .await,
            rq(
                &r,
        ...
        assert!(
            diffs.is_empty(),
            "{} ALTER-TABLE schema-rewrite divergence(s) vs rusqlite:\n{}",
            diffs.len(),
            diffs.join("\n")
        );
    });
