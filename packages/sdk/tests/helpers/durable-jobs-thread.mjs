...
async function run() {
  ...
  if (workerData.mode.startsWith("managed-")) {
    const runner = DurableJobWorker.start(
      jobs,
      async (lease) => {
        ...
        return {
          result: "done",
          apply: async (tx) => {
            await tx.execute("INSERT INTO effects(job_id, owner) VALUES (?,?)", [
              lease.id,
              lease.owner,
            ]);
      ...
      },
      {
        ...
        concurrency: workerData.mode === "managed-consume" ? 2 : 1,
        stopWhenIdle: true,
        clock: () => 1000,
        leaseMs: 30_000,
      },
