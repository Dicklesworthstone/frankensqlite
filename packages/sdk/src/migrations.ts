...
export class FrankenMigrationError extends Error {
  ...
  constructor(
    readonly code: "ERR_FSQLITE_MIGRATION_INPUT" | "ERR_FSQLITE_MIGRATION_SQL" |
      "ERR_FSQLITE_MIGRATION_HISTORY" | "ERR_FSQLITE_MIGRATION_DRIFT",
    message: string,
    readonly version?: number,
...
function commandWords(sql: string): string[] {
  ...
  for (let i = 0; i < sql.length && words.length < 3;) {
    if (/[\t\n\v\f\r \uFEFF]/.test(sql[i]!) || (words.length === 0 && sql[i] === ";")) { i++; continue; }
    if (sql.startsWith("--", i)) {
      const end = sql.indexOf("\n", i + 2); i = end < 0 ? sql.length : end + 1; continue;
    }
    if (sql.startsWith("/*", i)) {
      const end = sql.indexOf("*/", i + 2); i = end < 0 ? sql.length : end + 2; continue;
    }
...
function validateStatement(sql: string, version: number): void {
  try {
    ...
    if (parameterLayout(sql).count !== 0) throw new Error("Migration SQL must not contain bind parameters");
    const [verb, object, third] = commandWords(sql);
    const allowed = verb === "CREATE"
      ? ["TABLE", "INDEX", "VIEW", "TRIGGER"].includes(object ?? "") || (object === "UNIQUE" && third === "INDEX")
      : verb === "ALTER" ? object === "TABLE"
      : verb === "DROP" ? ["TABLE", "INDEX", "VIEW", "TRIGGER"].includes(object ?? "")
      : ["INSERT", "UPDATE", "DELETE", "REPLACE", "WITH"].includes(verb ?? "");
    if (!allowed) throw new Error("Use transactional DDL/DML, not PRAGMA, ATTACH, VACUUM, TEMP or virtual-table setup");
  } catch (cause: unknown) {
    throw new FrankenMigrationError("ERR_FSQLITE_MIGRATION_SQL",
      `Migration ${version} must contain one supported SQL statement per entry`, version, { cause });
  }
}

function capturePlan(input: readonly SchemaMigration[]): readonly SchemaMigration[] {
  if (!Array.isArray(input) || input.length > MAX_MIGRATIONS) invalid(`A migration plan accepts at most ${MAX_MIGRATIONS} migrations`);
  const length = input.length;
  const result: SchemaMigration[] = [];
  let previous = 0, statements = 0, units = 0;
  for (let i = 0; i < length; i++) {
    ...
    const version = entry.version, name = entry.name, source = entry.statements;
    if (!Number.isSafeInteger(version) || version <= previous) invalid("Versions must be positive safe integers in strictly increasing order", version);
    if (typeof name !== "string" || name.trim().length === 0 || name.length > 256 || name.includes("\0")) invalid("Migration names must contain 1..256 characters without NUL", version);
    if (!Array.isArray(source) || source.length === 0 || source.length > MAX_STATEMENTS - statements) invalid(`A plan accepts 1..${MAX_STATEMENTS} total SQL statements`, version);
    const count = source.length, captured: string[] = [];
    statements += count;
    for (let j = 0; j < count; j++) {
      if (!Object.hasOwn(source, j)) invalid("Statement arrays must not contain holes", version);
      const sql: unknown = source[j];
      if (typeof sql !== "string" || sql.length === 0) invalid("Migration statements must be nonempty strings", version);
      units += sql.length;
      if (units > MAX_SQL_UNITS) invalid("Migration SQL exceeds the 4 Mi UTF-16-code-unit plan limit", version);
...
async function temporarySchema(tx: FrankenTransaction): Promise<string> {
  const result = await tx.query("SELECT type, name, tbl_name, sql FROM temp.sqlite_master ORDER BY type COLLATE BINARY, name COLLATE BINARY LIMIT 4097");
  if (result.rowArrays.length > 4096) historyError("Too many temporary schema objects to verify a migration");
...
async function readHistory(tx: FrankenTransaction): Promise<History> {
  const catalog = await tx.query(
    "SELECT name, type, sql FROM main.sqlite_master WHERE name = ? COLLATE NOCASE", [MIGRATION_HISTORY_TABLE]);
  if (catalog.rowArrays.length === 0) return { schema: null, entries: Object.freeze([]) };
  const object = catalog.rowArrays[0]!;
  if (catalog.rowArrays.length !== 1 || object[0] !== MIGRATION_HISTORY_TABLE || object[1] !== "table" || typeof object[2] !== "string") {
  ...
  const names = ["version", "name", "sha256"], types = ["INTEGER", "TEXT", "TEXT"];
  if (columns.rowArrays.length !== 3) historyError("Unsupported migration history table shape");
  for (let i = 0; i < 3; i++) {
    const row = columns.rowArrays[i]!;
    if (row.length !== 7 || integer(row[0]) !== i || row[1] !== names[i] ||
        typeof row[2] !== "string" || row[2].toUpperCase() !== types[i] ||
        integer(row[3]) !== (i === 0 ? 0 : 1) || row[4] !== null ||
        integer(row[5]) !== (i === 0 ? 1 : 0) || integer(row[6]) !== 0) {
  ...
  const rows = await tx.query(`SELECT version, name, sha256 FROM ${TABLE} ORDER BY version LIMIT ${MAX_MIGRATIONS + 1}`);
  if (rows.rowArrays.length > MAX_MIGRATIONS) historyError("Migration history exceeds the supported bound");
  const entries: MigrationIdentity[] = [];
  let previous = 0;
  for (const row of rows.rowArrays) {
    const version = integer(row[0]), name = row[1], sha256 = row[2];
    if (row.length !== 3 || version === undefined || version <= previous || typeof name !== "string" ||
        name.length === 0 || name.length > 256 || typeof sha256 !== "string" || !/^[0-9a-f]{64}$/.test(sha256)) {
...
function verifyPrefix(history: History, plan: readonly MigrationIdentity[]): void {
  for (let i = 0; i < history.entries.length; i++) {
    const actual = history.entries[i]!, expected = plan[i];
    if (actual.version !== expected?.version) historyError("Applied history must be an exact prefix of this plan; missing, reordered or newer versions cannot be skipped", actual.version);
    if (actual.name !== expected.name || actual.sha256 !== expected.sha256) {
      throw new FrankenMigrationError("ERR_FSQLITE_MIGRATION_DRIFT",
        `Applied migration ${actual.version} differs from this plan; restore the original definition and add a new version`, actual.version);
    }
  }
}
...
export class FrankenMigrationPlan {
  ...
  inspect(db: FrankenDB, options?: TransactionOptions): Promise<MigrationStatus> {
    if (!(db instanceof FrankenDB)) return Promise.reject(new TypeError("inspect requires a FrankenDB connection"));
    return db.transaction(async tx => {
      const identities = await this.#manifest();
      const history = await readHistory(tx);
      verifyPrefix(history, identities);
      return Object.freeze({ currentVersion: history.entries.at(-1)?.version ?? 0,
        applied: history.entries, pending: Object.freeze(identities.slice(history.entries.length)) });
    }, options);
  }
  ...
  apply(db: FrankenDB, options?: TransactionOptions): Promise<MigrationResult> {
    if (!(db instanceof FrankenDB)) return Promise.reject(new TypeError("apply requires a FrankenDB connection"));
  ...
  async #manifest(): Promise<readonly MigrationIdentity[]> {
    ...
    if (!globalThis.crypto?.subtle) invalid("Secure-context Web Crypto is required for migration checksums");
    ...
    for (const migration of this.#migrations) {
      ...
      const bytes = new TextEncoder().encode(JSON.stringify([
        "frankensqlite-sdk-migration", 1, migration.version, migration.name, migration.statements,
      ]));
      const digest = new Uint8Array(await crypto.subtle.digest("SHA-256", bytes));
      identities.push(Object.freeze({ version: migration.version, name: migration.name,
  ...
  async #apply(tx: FrankenTransaction): Promise<MigrationResult> {
    ...
    if (databases.rowArrays.some((row: readonly unknown[]) => row[1] !== "main" && row[1] !== "temp")) {
    ...
    if (initialCount < identities.length && history.schema === null) {
      ...
      if (history.schema === null || history.entries.length !== 0) historyError("Could not initialize an empty migration history");
    }
    ...
    for (let i = initialCount; i < identities.length; i++) {
      ...
      if (await temporarySchema(tx) !== temporary) historyError("A migration must not leave temporary schema changes behind", identity.version);
      ...
      history = await readHistory(tx);
      verifyPrefix(history, identities);
      if (history.schema !== schema || history.entries.length !== i) historyError("Migration SQL changed the reserved history table", identity.version);
      await tx.execute(`INSERT INTO ${TABLE} (version, name, sha256) VALUES (?, ?, ?)`,
        [identity.version, identity.name, identity.sha256]);
      history = await readHistory(tx);
      verifyPrefix(history, identities);
      if (history.schema !== schema || history.entries.length !== i + 1) historyError("Migration history insertion did not record exactly one version", identity.version);
    }
    return Object.freeze({ previousVersion, currentVersion: identities.at(-1)?.version ?? 0,
