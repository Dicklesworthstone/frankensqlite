...
fn explicit_txn_multirow_insert_sees_prior_row_and_is_atomic() {
    asupersync::test_utils::run_test(|| async {
        ...
        check(
            "after conflict txn",
            fq(&f, "SELECT id,v FROM g ORDER BY id,v").await,
            rq(&r, "SELECT id,v FROM g ORDER BY id,v"),
            &mut diffs,
        );
        // exactly one row with id=1, holding the prior 'a' (no duplicate slipped in)
        check(
            "id=1 rows",
            fq(&f, "SELECT v FROM g WHERE id=1").await,
            rq(&r, "SELECT v FROM g WHERE id=1"),
            &mut diffs,
        );
        ...
        ex(
            &f,
            &r,
            "INSERT INTO g VALUES (30,'p'),(31,'q'),(30,'p2'),(32,'r')",
        )
        .await;
        ex(&f, &r, "COMMIT").await;
        check(
            "intra-stmt dup aborts",
            fq(&f, "SELECT id,v FROM g ORDER BY id").await,
            rq(&r, "SELECT id,v FROM g ORDER BY id"),
            &mut diffs,
        );
        ...
        assert!(
            diffs.is_empty(),
            "{} bd-q2bju divergence(s) vs rusqlite:\n{}",
            diffs.len(),
            diffs.join("\n")
        );
    });
