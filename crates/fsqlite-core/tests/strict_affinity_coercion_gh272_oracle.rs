...
fn strict_text_accepts_numeric_gh272() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert!(d.is_empty(), "GH#272 STRICT TEXT divergence:\n{}", d.join("\n"));
...
fn strict_integer_accepts_lossless_real_gh272() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert!(d.is_empty(), "GH#272 STRICT INTEGER divergence:\n{}", d.join("\n"));
        assert_agree_query(&f, &r, "SELECT a, typeof(a) FROM t ORDER BY a").await;
    });
}
...
fn strict_multi_column_text_affinity_gh272() {
    asupersync::test_utils::run_test(|| async {
        ...
        let d = apply_checked(&f, &r, &[
            "INSERT INTO t VALUES ('42', '1.5', 9)",
        ]).await;
        assert!(d.is_empty(), "GH#272 STRICT multi-col divergence:\n{}", d.join("\n"));
        assert_agree_query(&f, &r, "SELECT a, typeof(a), b, typeof(b), c, typeof(c) FROM t").await;
    });
}
...
fn strict_update_integer_into_text_gh272() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert!(d.is_empty(), "GH#272 STRICT UPDATE divergence:\n{}", d.join("\n"));
