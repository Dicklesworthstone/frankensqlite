...
async fn fts_rowids(conn: &Connection, fts: &str) -> Vec<i64> {
...
fn delete_all_then_reinsert_stored() {
    asupersync::test_utils::run_test(|| async {
        ...
        let before = fts_rowids(&conn, "t").await;
        eprintln!("[stored] rowids before delete = {before:?}");
        assert_eq!(before, vec![1, 2, 3]);

        let affected = conn.execute("DELETE FROM t").await.expect("delete-all");
        eprintln!("[stored] DELETE FROM t affected = {affected}");

        let after = fts_rowids(&conn, "t").await;
        ...
        let again = fts_rowids(&conn, "t").await;
...
fn delete_all_rejected_on_contentless_without_contentless_delete() {
    asupersync::test_utils::run_test(|| async {
        ...
        let res = conn.execute("DELETE FROM t").await;
