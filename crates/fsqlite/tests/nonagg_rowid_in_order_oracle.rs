...
fn nonagg_rowid_in_order_matches_sqlite() {
    ...
    for sql in seeks { cmp_ord(&f, &r, sql, "rowid-in-order"); assert!(has_seek(&f, sql), "rowid IN + ORDER BY rowid must seek: `{sql}`"); }
    ...
    for sql in declines { cmp_ord(&f, &r, sql, "declines"); assert!(!has_seek(&f, sql), "non-rowid/multi-term ORDER BY must NOT seek: `{sql}`"); }
    // No ORDER BY still seeks (byte-identical to before this lever).
    assert!(has_seek(&f, "SELECT id FROM t WHERE id IN (1,2,3)"), "bare rowid IN must still seek");
}
