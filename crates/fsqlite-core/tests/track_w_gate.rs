...
fn wtest_insert_truth_matrix_values_all_params_hits_direct_lane() {
    asupersync::test_utils::run_test(|| async {
        ...
        let stmt = conn
            .prepare("INSERT INTO tm1 VALUES (?1, ?2, ?3)")
            .await
            .unwrap();
        reset_hot_path_profile();
        for i in 0..20 {
...
fn wtest_insert_truth_matrix_operator_expression_values_hits_direct_lane() {
    asupersync::test_utils::run_test(|| async {
        ...
        let rows = conn
            .query("SELECT computed FROM tm3 ORDER BY id")
            .await
            .unwrap();
...
fn wtest_insert_truth_matrix_autoincrement_hits_fast_lane() {
    asupersync::test_utils::run_test(|| async {
        ...
        let stmt = conn
            .prepare("INSERT INTO tm4 (name) VALUES (?1)")
            .await
            .unwrap();
        reset_hot_path_profile();
        for i in 0..10 {
        ...
        let count = conn.query("SELECT COUNT(*) FROM tm4").await.unwrap();
...
fn wtest_insert_truth_matrix_returning_stays_off_direct_lane() {
    asupersync::test_utils::run_test(|| async {
        ...
        let result = conn
            .query("INSERT INTO tm5 VALUES (1, 'hello') RETURNING id, val")
            .await;
        match result {
            Ok(rows) => {
...
fn wtest_update_simple_where_hits_fast_lane() {
    asupersync::test_utils::run_test(|| async {
        ...
        let rows = conn.query("SELECT val FROM wu1 ORDER BY id").await.unwrap();
...
fn wtest_delete_simple_where_hits_fast_lane() {
    asupersync::test_utils::run_test(|| async {
        ...
        let stmt = conn.prepare("DELETE FROM wd1 WHERE id = ?1").await.unwrap();
        reset_hot_path_profile();
        ...
        let rows = conn.query("SELECT id FROM wd1").await.unwrap();
        assert_eq!(rows.len(), 1);
...
fn wtest_update_with_trigger_falls_back_with_attribution() {
    asupersync::test_utils::run_test(|| async {
        ...
        conn.execute("CREATE TABLE wut_log (id INTEGER PRIMARY KEY, msg TEXT)")
            .await
            .unwrap();
        conn.execute("INSERT INTO wut VALUES (1, 'old')")
            .await
            .unwrap();
        conn.execute(
        ...
        let log = conn.query("SELECT msg FROM wut_log").await.unwrap();
        assert_eq!(log.len(), 1);
...
fn wtest_delete_with_fk_cascade_falls_back_with_attribution() {
    asupersync::test_utils::run_test(|| async {
        ...
        let children = conn.query("SELECT id FROM wfk_child").await.unwrap();
    ...
    asupersync::test_utils::run_test(|| async {
        ...
        for _ in 0..1000 {
            ...
            match roll {
                ...
                _ => {
                    let rows = conn.query("SELECT COUNT(*) FROM moltp").await.unwrap();
                    assert_eq!(
                        rows[0].values()[0],
        ...
        let final_rows = conn.query("SELECT COUNT(*) FROM moltp").await.unwrap();
        assert_eq!(
...
fn wtest_w1_function_cache_preserved_across_prepared_reuse() {
    asupersync::test_utils::run_test(|| async {
        ...
        let count = conn.query("SELECT COUNT(*) FROM w1t").await.unwrap();
...
fn wtest_w9_same_connection_read_after_write_no_data_loss() {
    asupersync::test_utils::run_test(|| async {
        ...
        for i in 1..=50 {
            ...
            let count_rows = conn.query("SELECT COUNT(*) FROM w9t").await.unwrap();
            assert_eq!(
...
fn wtest_w9_same_connection_update_then_read_sees_new_value() {
    asupersync::test_utils::run_test(|| async {
        ...
        let rows = conn
            .query("SELECT val FROM w9u WHERE id = 2")
            .await
            .unwrap();
        assert_eq!(rows.len(), 1);
        assert_eq!(
        ...
        let other = conn
            .query("SELECT val FROM w9u WHERE id = 1")
            .await
            .unwrap();
...
fn wtest_w9_same_connection_delete_then_read_sees_removal() {
    asupersync::test_utils::run_test(|| async {
        ...
        let delete_stmt = conn.prepare("DELETE FROM w9d WHERE id = ?1").await.unwrap();
        ...
        let rows = conn.query("SELECT id FROM w9d ORDER BY id").await.unwrap();
...
fn wtest_cross_connection_prepared_insert_visible_after_commit() {
    asupersync::test_utils::run_test(|| async {
        ...
        let conn_a = Connection::open(&path).await.unwrap();
        conn_a.execute("PRAGMA journal_mode = WAL").await.unwrap();
        conn_a
            .execute("CREATE TABLE xconn (id INTEGER PRIMARY KEY, val TEXT)")
            .await
            .unwrap();

        let conn_b = Connection::open(&path).await.unwrap();
        ...
        let rows = conn_b
            .query("SELECT val FROM xconn WHERE id = 1")
            .await
            .unwrap();
...
fn wtest_cross_connection_prepared_insert_within_explicit_txn() {
    asupersync::test_utils::run_test(|| async {
        ...
        let pre_commit = conn_b.query("SELECT COUNT(*) FROM xtxn").await.unwrap();
        assert_eq!(
        ...
        let post_commit = conn_b.query("SELECT COUNT(*) FROM xtxn").await.unwrap();
        assert_eq!(
...
fn wtest_explicit_txn_batch_insert_engine_reuse_and_fast_lane() {
    asupersync::test_utils::run_test(|| async {
        ...
        let count = conn.query("SELECT COUNT(*) FROM txn_batch").await.unwrap();
...
fn wtest_fk_pragma_toggle_dynamic_fast_lane_eligibility() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert!(
            off_snap.prepared_update_delete_fast_lane_hits
                + off_snap.prepared_direct_delete_executions
                >= 1,
...
fn wtest_multi_table_prepared_dml_no_interference() {
    asupersync::test_utils::run_test(|| async {
        ...
        let ins_a = conn
            .prepare("INSERT INTO mt_a VALUES (?1, ?2)")
            .await
            .unwrap();
        let ins_b = conn
            .prepare("INSERT INTO mt_b VALUES (?1, ?2)")
            .await
            .unwrap();
        let upd_a = conn
            ...
            .unwrap();
        let del_b = conn
            .prepare("DELETE FROM mt_b WHERE id = ?1")
            .await
            .unwrap();
        ...
        let a_count = conn.query("SELECT COUNT(*) FROM mt_a").await.unwrap();
        assert_eq!(a_count[0].values()[0], SqliteValue::Integer(20));
        let b_count = conn.query("SELECT COUNT(*) FROM mt_b").await.unwrap();
