...
async function image() {
  ...
  await db.executeBatch(
    "CREATE TABLE items(id INTEGER PRIMARY KEY,value TEXT);INSERT INTO items VALUES(1,'first'),(2,'second');CREATE TABLE audit(id);CREATE TRIGGER a AFTER INSERT ON items BEGIN INSERT INTO audit VALUES(new.id);END;",
  );
...
async function fixture(t, options = {}) {
  const bytes = await image(),
    fixtures = [];
  const pool = await FrankenSnapshotPool.open(bytes, {
    ...options,
    worker: () => {
      const f = sqliteSnapshotWorker(options.hooks?.(fixtures.length) ?? {});
      fixtures.push(f);
      options.configure?.(f, fixtures.length - 1);
      return f.worker;
    },
  });
...
function block(fixture) {
  const started = deferred(),
    gate = deferred(),
    seen = [];
  const core = fixture.handles[0],
    query = core.query,
    queryWithParams = core.queryWithParams;
...
test(
  "replicas import the same owned image without detaching the caller buffer",
  opts,
  async (t) => {
    ...
    assert.equal(
      (await pool.query("SELECT value FROM items WHERE id=?", [2])).rows[0].value,
      "second",
    );
    bytes.fill(0);
    assert.equal((await pool.query("SELECT count(*) AS n FROM items")).rows[0].n, 2);
    assert.ok(Object.isFrozen(pool.snapshot));
    assert.ok(Object.isFrozen(pool.stats));
  },
);

test(
  "separate replicas execute overlapping queries; FIFO starts more callers than slots",
  opts,
  async (t) => {
    const { pool, fixtures } = await fixture(t, { workers: 2, maxPendingQueries: 8 });
    const a = block(fixtures[0]),
      b = block(fixtures[1]);
    const pending = Array.from({ length: 8 }, (_, n) => pool.query("SELECT ? AS n", [n]));
    await Promise.all([a.started, b.started]);
    assert.equal(pool.stats.activeQueries, 2);
    assert.equal(pool.stats.waitingQueries, 6);
    b.release();
    ...
    a.release();
    assert.deepEqual(
      (await Promise.all(pending)).map((result) => result.rows[0].n),
      [0, 1, 2, 3, 4, 5, 6, 7],
    );
...
test(
  "overload refuses work before transport, then released capacity can be reused",
  opts,
  async (t) => {
...
test(
  "waiting bindings and blob aliases are captured before application mutation",
  opts,
  async (t) => {
    const { pool, fixtures } = await fixture(t, { workers: 1 });
    const gate = block(fixtures[0]),
      first = pool.query("SELECT 1");
    await gate.started;
    const backing = Uint8Array.of(0, 10, 20, 30),
      params = [backing.subarray(1, 3), "original"];
...
test(
  "read-only admission refuses settings, scripts, maintenance and EXPLAIN pragma escape",
  opts,
  async (t) => {
    ...
    for (const sql of [
      ...
      assert.equal(
        fixtures.reduce((n, f) => n + f.worker.requests.length, 0),
        before,
        sql,
      );
    }
...
test(
  "WITH DML is rejected by the actual core query-only policy; data and triggers stay unchanged",
  opts,
  async (t) => {
    const { pool, fixtures } = await fixture(t, { workers: 1 });
    for (const sql of [
    ...
    assert.deepEqual((await pool.query("SELECT * FROM items ORDER BY id")).rowArrays, [
      [1, "first"],
      [2, "second"],
    ]);
    ...
    try {
      assert.deepEqual(reference.prepare("PRAGMA integrity_check").all().map(Object.values), [
        ["ok"],
      ]);
    } finally {
      reference.close();
...
test(
  "SELECT, recursive CTE, EXPLAIN SELECT, comments and quoted semicolons work",
  opts,
  async (t) => {
    const { pool } = await fixture(t);
    assert.equal(
      (await pool.query("; /*lead*/ SELECT ';PRAGMA query_only=OFF' AS value -- tail")).rows[0]
        .value,
      ";PRAGMA query_only=OFF",
    );
    assert.deepEqual(
      (
        await pool.query(
          "WITH RECURSIVE x(n) AS (SELECT 1 UNION ALL SELECT n+1 FROM x WHERE n<1000) SELECT sum(n) AS n FROM x",
        )
      ).rowArrays,
      [[500500]],
    );
    assert.ok(
      (await pool.query("EXPLAIN /*a*/ QUERY /*b*/ PLAN SELECT * FROM items")).rows.length > 0,
    );
  },
);

test(
  "SQL failures settle one job, restore accounting, and do not poison successors",
  opts,
  async (t) => {
    ...
    await bad;
    assert.equal((await good).rows[0].n, 42);
    assert.equal(pool.stats.failedQueries, 1);
    assert.equal(pool.stats.pendingBytes, 0);
  },
);
...
test("waiting cancellation removes a query without executing it", opts, async (t) => {
  ...
  await gate.started;
  const cancelled = assert.rejects(pool.query("SELECT 2", [], { signal: controller.signal }), {
    code: "ERR_FSQLITE_POOL_CANCELLED",
    cause: "stop",
  });
  controller.abort("stop");
  await cancelled;
  assert.equal(pool.stats.waitingQueries, 0);
  assert.equal(pool.stats.pendingQueries, 1);
  gate.release();
  await first;
...
test(
  "active cancellation retains its slot until SQL settles; unrelated replicas continue",
  opts,
  async (t) => {
    ...
    gate.release();
    assert.equal((await active).error.code, "ERR_FSQLITE_POOL_CANCELLED");
    assert.equal(pool.stats.pendingQueries, 0);
  },
);
...
test(
  "waiting deadline is checked again at dispatch even when its timer cannot run",
  opts,
  async (t) => {
    const { pool, fixtures } = await fixture(t, { workers: 1 });
    const gate = block(fixtures[0]),
    ...
    while (performance.now() < until) {
      /* Deliberately hold task delivery. */
    }
    gate.release();
    await first;
    assert.equal((await timed).error.code, "ERR_FSQLITE_POOL_TIMEOUT");
    assert.deepEqual(gate.seen, ["SELECT 1"]);
  },
);

test(
  "close is idempotent, drains active and accepted queued work, then closes every replica",
  opts,
  async (t) => {
    const { pool, fixtures } = await fixture(t, { workers: 1 });
    const gate = block(fixtures[0]),
    ...
    assert.equal(fixtures[0].worker.terminateCount, 1);
...
test(
  "reentrant parameter getters cannot enqueue after close or evade capacity",
  opts,
  async (t) => {
    const { pool } = await fixture(t, { workers: 1 });
    let closed;
...
test("pre-aborted, invalid and oversized input never reaches a replica", opts, async (t) => {
  ...
  c.abort("already");
  await assert.rejects(pool.query("SELECT 1", [], { signal: c.signal }), {
    code: "ERR_FSQLITE_POOL_CANCELLED",
  });
  await assert.rejects(pool.query("SELECT ?", ["x".repeat(1000)]), {
    code: "ERR_FSQLITE_REQUEST_TOO_LARGE",
  });
  await assert.rejects(pool.query("SELECT ?", [new Uint8Array(new ArrayBuffer(1000), 0, 1)]), {
    code: "ERR_FSQLITE_REQUEST_TOO_LARGE",
  });
  await assert.rejects(pool.query("SELECT ?", [new Uint8Array(new SharedArrayBuffer(1))]), {
    code: "ERR_FSQLITE_POOL_INPUT",
  });
...
test(
  "partial initialization failure waits for and closes successfully opened replicas",
  opts,
  async () => {
    const bytes = await image(),
      f = sqliteSnapshotWorker();
    let calls = 0;
    await assert.rejects(
      FrankenSnapshotPool.open(bytes, {
        workers: 2,
        worker() {
    ...
    assert.deepEqual(f.events.slice(-2), ["close", "free"]);
  },
);

test(
  "duplicate worker factories are rejected rather than sharing one mutable connection",
  opts,
  async () => {
    const bytes = await image(),
      f = sqliteSnapshotWorker();
    await assert.rejects(
      FrankenSnapshotPool.open(bytes, { workers: 2, worker: () => f.worker }),
      AggregateError,
    );
    assert.equal(f.worker.terminateCount, 1);
  },
...
test("invalid images and worker limits reject before invoking a worker factory", opts, async () => {
  ...
  for (const workers of [0, 9, NaN, 1.5])
    await assert.rejects(FrankenSnapshotPool.open(bytes, { workers, worker }));
...
test(
  "missing query_only enforcement fails initialization rather than admitting mutable replicas",
  opts,
  async () => {
    const bytes = await image(),
      f = sqliteSnapshotWorker();
    ...
    const error = (
      await observed(FrankenSnapshotPool.open(bytes, { workers: 1, worker: () => f.worker }))
    ).error;
...
async function changedImage() {
  ...
  const db = await FrankenDB.open({ worker: f.worker });
  await db.executeBatch(
    "CREATE TABLE items(id INTEGER PRIMARY KEY,value TEXT);INSERT INTO items VALUES(10,'new'),(20,'generation'),(30,'three');",
  );
  const bytes = await db.export();
  await db.close();
  return bytes;
...
test(
  "refresh waits for every old read and atomically publishes all replacement replicas",
  opts,
  async (t) => {
    const { pool, fixtures } = await fixture(t, { workers: 2 });
    const replacement = await changedImage(),
      initial = pool.snapshot;
    const a = block(fixtures[0]),
      b = block(fixtures[1]);
    const old = [
      pool.query("SELECT count(*) AS n FROM items"),
      pool.query("SELECT count(*) AS n FROM items"),
    ];
    await Promise.all([a.started, b.started]);
    ...
    assert.equal(pool.stats.refreshing, true);
    b.release();
    await old[1];
    assert.equal(fixtures.length, 2);
    assert.equal(pool.snapshot, initial);
    a.release();
    const refreshed = await refresh;
    assert.equal(refreshed.snapshot.generation, 2);
    assert.equal(refreshed.cleanupErrors.length, 0);
    assert.equal(refreshed.snapshot.sha256, createHash("sha256").update(replacement).digest("hex"));
    for (const result of await Promise.all(old)) {
      assert.equal(result.rows[0].n, 2);
      assert.equal(result.snapshot, initial);
    }
    for (const result of await Promise.all(later)) {
      assert.equal(result.rows[0].n, 3);
      assert.equal(result.snapshot, refreshed.snapshot);
    }
    assert.equal(fixtures.length, 4);
    assert.ok(fixtures.slice(0, 2).every((f) => f.worker.terminateCount === 1));
    assert.equal(pool.stats.pendingSnapshotBytes, 0);
    assert.equal(pool.stats.refreshing, false);
  },
);

test(
  "pending refresh owns its copied image and only one refresh may be admitted",
  opts,
  async (t) => {
    const { pool, fixtures } = await fixture(t, { workers: 1 });
    const replacement = await changedImage(),
      expected = createHash("sha256").update(replacement).digest("hex");
    const gate = block(fixtures[0]),
      old = pool.query("SELECT 1");
    await gate.started;
    const pending = pool.refresh(replacement);
    replacement.fill(0);
...
test(
  "failed refresh preserves the old generation but refuses queries waiting for the new one",
  opts,
  async (t) => {
    const { pool, fixtures } = await fixture(t, {
      workers: 2,
...
test(
  "successful refresh reports old cleanup errors without pretending publication failed",
  opts,
  async (t) => {
    const { pool } = await fixture(t, {
      workers: 2,
    ...
    assert.ok(Object.isFrozen(result.cleanupErrors));
    assert.equal((await pool.query("SELECT count(*) AS n FROM items")).rows[0].n, 3);
  },
);
...
test("close drains an accepted refresh, then closes its new workers too", opts, async (t) => {
  ...
  const refreshed = pool.refresh(await changedImage()),
    read = pool.query("SELECT count(*) AS n FROM items");
  await started.promise;
  const closed = pool.close();
  assert.equal(pool.close(), closed);
  await assert.rejects(pool.refresh(await image()), { code: "ERR_FSQLITE_POOL_CLOSED" });
  assert.equal(pool.stats.state, "closing");
  assert.equal(fixtures[0].worker.terminateCount, 0);
  stage.resolve();
  await refreshed;
  assert.equal((await read).rows[0].n, 3);
  await closed;
  assert.ok(fixtures.every((f) => f.worker.terminateCount === 1));
  assert.equal(pool.stats.state, "closed");
});

test(
  "queries can expire or cancel behind an in-flight refresh without cancelling the refresh",
  opts,
  async (t) => {
    const stage = deferred(),
      started = deferred();
    ...
    const refresh = pool.refresh(await changedImage());
    await started.promise;
    const c = new AbortController(),
      cancelled = observed(pool.query("SELECT 9", [], { signal: c.signal }));
    const expired = observed(pool.query("SELECT 8", [], { waitTimeoutMs: 1 }));
    c.abort();
    ...
    assert.equal(pool.stats.refreshing, true);
    stage.resolve();
    assert.equal((await refresh).snapshot.generation, 2);
  },
);

test(
  "a reused worker on refresh is refused before disturbing its old connection",
  opts,
  async () => {
    const bytes = await image(),
      f = sqliteSnapshotWorker();
    ...
    try {
      ...
      assert.equal(f.worker.terminateCount, 0);
      assert.equal((await pool.query("SELECT count(*) AS n FROM items")).rows[0].n, 2);
    } finally {
      await pool.close();
    }
...
test(
  "invalid refresh input creates no barrier and leaves later queries usable",
  opts,
  async (t) => {
    const { pool } = await fixture(t, { workers: 1 });
...
test(
  "repeated successful refreshes advance identity even for byte-identical images",
  opts,
  async (t) => {
    const { pool, bytes, fixtures } = await fixture(t, { workers: 2 });
    const hash = pool.snapshot.sha256;
    for (let generation = 2; generation <= 4; generation++) {
      ...
      assert.equal(result.snapshot.sha256, hash);
    }
    await pool.close();
...
test(
  "a replica crashing while another replica initializes cannot produce a successful open",
  opts,
  async () => {
    const bytes = await image(),
      gate = deferred(),
      ...
      FrankenSnapshotPool.open(bytes, {
        ...
        worker() {
          const index = fixtures.length,
            f = sqliteSnapshotWorker(index === 1 ? { beforeImport: () => gate.promise } : {});
          ...
          f.worker.addEventListener = (type, fn) => {
            if (type === "error") errors.add(fn);
            add(type, fn);
          };
          f.worker.removeEventListener = (type, fn) => {
            if (type === "error") errors.delete(fn);
            remove(type, fn);
          };
          if (index === 0)
            f.worker.addEventListener("message", (event) => {
              if (event.data.kind === "query-result") ready.resolve();
            });
          fixtures.push(f);
          return f.worker;
        },
...
test(
  "named bindings execute unchanged SQL by native SQLite slots across snapshot generations",
  opts,
  async (t) => {
    const references = [],
      stops = [];
        ...
        async load() {
          return {
            FrankenDB: {
              async import(bytes) {
                const path = join(
                  await mkdtemp(join(tmpdir(), "fsqlite-pool-bindings-")),
                  "db.sqlite",
                );
                await writeFile(path, bytes);
                reference = sqliteBindingFixture(path);
          ...
          void host.handle(structuredClone(request)).then(
            (response) => {
              for (const listener of listeners.message)
                listener({ data: structuredClone(response) });
            },
            (cause) => {
              for (const listener of listeners.error) listener({ message: String(cause) });
            },
          );
        },
        terminate() {
          if (reference && !stopped) {
            stopped = reference.shutdown();
            stops.push(stopped);
          }
        },
      };
    };
    pool = await FrankenSnapshotPool.open(await image(), {
      workers: 2,
      worker,
    ...
    for (const result of results)
      assert.deepEqual(result.rowArrays, [
        [2, 2, "NUL\0λ", Uint8Array.of(0, 255), 9223372036854775807n],
      ]);
    assert.equal(references.length, 2);
    assert.ok(
      references.every((reference) => reference.requests.some((request) => request.sql === sql)),
    );
    ...
    assert.deepEqual(next.rowArrays, [["generation"]]);
    assert.equal(next.snapshot, refresh.snapshot);
    await pool.close();
    await Promise.all(stops);
...
test(
  "an old worker crashing during staged refresh closes both generations without publishing",
  opts,
  async (t) => {
    ...
    const { pool, fixtures } = await fixture(t, {
      ...
      configure(f, index) {
        ...
        f.worker.addEventListener = (type, fn) => {
          if (type === "error") errors[index].add(fn);
          add(type, fn);
        };
        f.worker.removeEventListener = (type, fn) => {
          if (type === "error") errors[index].delete(fn);
          remove(type, fn);
        };
      },
    });
    ...
    for (const listener of [...errors[0]])
      listener({ message: "old worker failed during refresh" });
    assert.equal((await queued).error.code, "ERR_FSQLITE_POOL_UNUSABLE");
    release.resolve();
    assert.ok((await refresh).error);
    ...
    assert.equal(pool.stats.pendingSnapshotBytes, 0);
    assert.ok(fixtures.every((f) => f.worker.terminateCount === 1));
    assert.ok(errors.every((set) => set.size === 0));
  },
);
