...
function fixture(t) {
  ...
  function connection(hooks = {}) {
    ...
    const host = {
      ...
      init(name = "db", mode, persistence = "opfs-snapshot", extra = {}) {
        ...
        return result({
          ...
          config: {
            dbName: name,
            persistence,
            ...(mode === undefined ? {} : { snapshotOwnership: mode }),
            ...extra,
          },
        });
      },
      ...
      query: async (sql) =>
        (await result({ kind: "query", sql })).data.rows.map((row) => ({ ...row })),
      save: async () =>
        (await result({ kind: "checkpoint", publicationId: crypto.randomUUID() })).data,
...
for (const backend of ["opfs-snapshot", "indexeddb-snapshot"]) {
  ...
  test(`${backend}: default/shared sessions coexist but still use snapshot CAS`, async (t) => {
    ...
    await a.sql("CREATE TABLE items(id INTEGER PRIMARY KEY)");
    await a.save();
    ...
    await assert.rejects(c.init("db", "exclusive", backend), { code: OWNED });
    await b.close();
    await c.init("db", "exclusive", backend);
  });
  ...
  test(`${backend}: failed import releases a new owner, not a reused live owner`, async (t) => {
    ...
    await seed.init("db", "exclusive", backend);
    await seed.sql("CREATE TABLE items(id INTEGER PRIMARY KEY)");
    await seed.save();
    await seed.close();
    const hooks = {
      beforeImport() {
  ...
  test(`${backend}: ownership is acquired before asynchronous image import`, async (t) => {
    ...
    const seed = connection();
    await seed.init("db", "exclusive", backend);
    await seed.sql("CREATE TABLE items(id INTEGER)");
    await seed.save();
    await seed.close();
...
test("queued close retains ownership through checkpoint publication and core cleanup", async (t) => {
  ...
  finish.resolve();
  await saving;
  assert.equal((await closing).kind, "close-result");
  assert.ok(a.events.includes("free"));
  await b.init();
  assert.deepEqual(await b.query("SELECT * FROM items"), [{ id: 1 }]);
});
...
test("queued close retains ownership while an earlier SQL operation is outstanding", async (t) => {
  ...
  const b = connection();
  await a.init();
  await a.sql("CREATE TABLE items(id INTEGER)");
  const inserting = a.result({ kind: "execute", sql: "INSERT INTO items VALUES(1)" });
  await entered.promise;
...
test("same-name reinitialization never drops the lease during old handle destructors", async (t) => {
  ...
  const a = connection({
    beforeClose() {
      observations.push(storage.sessions.size);
    },
  });
  await a.init();
  await a.sql("CREATE TABLE items(id INTEGER)");
...
test("reinitialization to another namespace retires only the old lease after core cleanup", async (t) => {
  ...
  assert.equal(storage.sessions.size, 1);
  await b.init("one");
  await assert.rejects(connection().init("two"), { code: OWNED });
});
...
test("transport failure during import disposes the candidate before releasing ownership", async (t) => {
  ...
  const a = connection({
    ...
    b = connection();
  const opening = a.init();
  const failed = assert.rejects(opening, /channel failed/);
  await entered.promise;
  ...
  await failed;
  await closing;
  assert.deepEqual(a.events, ["close", "free"]);
  assert.equal(storage.sessions.size, 0);
  await b.init();
});
...
test("explicit policy fails without Web Locks before constructing a database", async (t) => {
  ...
  try {
    ...
    for (const mode of ["exclusive", "shared"])
      await assert.rejects(a.init("db", mode, "indexeddb-snapshot"), {
    ...
    assert.equal(ready.data.snapshotOwnership, undefined);
    await a.close();
  } finally {
    navigator.locks = saved;
  }
});
...
test("ownership policies are captured once at admission and reject invalid values", async (t) => {
  ...
  const opening = a.result({ kind: "init", config });
  assert.equal(reads, 1);
  assert.equal((await opening).data.snapshotOwnership, "exclusive");
  assert.equal(reads, 1);
  for (const policy of [null, false, 0, "other"]) {
    await assert.rejects(connection().init("other", policy), {
      code: "ERR_FSQLITE_SNAPSHOT_OWNERSHIP_INPUT",
    });
  }
  await assert.rejects(connection().init("db", "exclusive", "memory"), {
    code: "ERR_FSQLITE_SNAPSHOT_OWNERSHIP_INPUT",
  });
  assert.equal(storage.sessions.size, 1);
});
