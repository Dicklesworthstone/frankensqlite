...
pub const OWNERSHIP_SCAN_WINDOW_BYTES: usize = 64 * 1024;
...
#[derive(Debug)]
pub struct PrivatePageOwnership {
    file: File,
    total_pages: u32,
    marked_pages: u64,
}
...
impl PrivatePageOwnership {
    pub fn create(parent: &Path, total_pages: u32) -> Result<Self> {
    ...
    pub fn first_unowned(&self, page_size: PageSize) -> Result<Option<PageNumber>> {
        let lock_byte_page = fsqlite_pager::lock_byte_page(page_size);
        // Heap, not stack: this window is 64 KiB and the engine's deeply nested
        // `block_on`'d futures already run close to their stack budget. The
        // residency claim is "fixed", not "stack-resident" -- one boxed window
        // per call keeps that claim while costing nothing measurable.
        let mut window = vec![0_u8; OWNERSHIP_SCAN_WINDOW_BYTES].into_boxed_slice();
    ...
    pub const fn marked_pages(&self) -> u64 {
        self.marked_pages
    }
}
