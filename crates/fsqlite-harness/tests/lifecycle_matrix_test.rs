...
async fn setup_table(db_str: &str) -> Connection {
...
async fn count_rows(conn: &Connection) -> i64 {
    let rows = conn.query("SELECT count(*) FROM t1").await.expect("count");
...
fn create_close_reopen_preserves_schema_and_data() {
    asupersync::test_utils::run_test(|| async {
        ...
        {
            let conn = setup_table(&db_str).await;
...

        {
            ...
            assert_eq!(
                read_val(&conn, 1).await.as_deref(),
                Some("baseline"),
...
fn begin_commit_reopen_persists() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert_eq!(
            read_val(&conn, 2).await.as_deref(),
            Some("committed"),
...
fn nested_savepoints_mixed_rollback_release() {
    asupersync::test_utils::run_test(|| async {
...

        {
            ...
            conn.execute("SAVEPOINT sp_outer").await.expect("sp_outer");
            conn.execute("INSERT INTO t1 VALUES (2, 'outer')")
                .await
                .expect("outer insert");

            // Level 2: inner savepoint
            conn.execute("SAVEPOINT sp_inner").await.expect("sp_inner");
            ...
            conn.execute("SAVEPOINT sp_inner2")
                .await
                .expect("sp_inner2");
            conn.execute("INSERT INTO t1 VALUES (4, 'inner2')")
                .await
                .expect("inner2 insert");
            conn.execute("RELEASE sp_inner2")
                .await
                .expect("release inner2");
        ...
        assert_eq!(
            read_val(&conn, 4).await.as_deref(),
            Some("inner2"),
...
fn savepoint_rollback_all_then_rollback_transaction() {
    asupersync::test_utils::run_test(|| async {
...

        {
            ...
            conn.execute("ROLLBACK").await.expect("rollback txn");
        }
    ...
    asupersync::test_utils::run_test(|| async {
...

        {
            ...
            conn.execute("INSERT INTO t1 VALUES (1)")
                .await
                .expect("insert");
            conn.execute("DROP TABLE t1").await.expect("drop");
            conn.execute("CREATE TABLE t1 (y TEXT)")
                .await
                .expect("recreate");
        ...
        let conn = Connection::open(&db_str).await.expect("reopen");
        let rows = conn.query("SELECT y FROM t1").await.expect("query");
        assert_eq!(
            rows.len(),
            1,
...
fn transaction_across_schema_and_data_changes() {
    asupersync::test_utils::run_test(|| async {
        ...
        let conn = Connection::open(&db_str).await.expect("reopen");
        let rows1 = conn.query("SELECT val FROM t1").await.expect("t1");
        let rows2 = conn.query("SELECT x FROM t2").await.expect("t2");
...
fn wal_mode_savepoint_rollback_to_with_reopen() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert!(
            read_val(&conn, 2).await.is_none(),
            "bead_id={BEAD_ID} case=wal_sp_discarded_gone"
        );
        assert_eq!(
            read_val(&conn, 3).await.as_deref(),
            Some("after_rollback"),
    ...
    asupersync::test_utils::run_test(|| async {
...

        {
            ...
            conn.execute("INSERT INTO t1 VALUES (2, 'to_delete')")
                .await
                .expect("insert");
            conn.execute("DELETE FROM t1 WHERE id = 2")
                .await
                .expect("delete");
        }
...
fn empty_transaction_commit_is_safe() {
    asupersync::test_utils::run_test(|| async {
...

        {
            ...
            conn.execute("BEGIN").await.expect("begin");
            // No changes
            conn.execute("COMMIT").await.expect("commit empty txn");
        }
...
fn empty_transaction_rollback_is_safe() {
    asupersync::test_utils::run_test(|| async {
...

        {
            ...
            conn.execute("BEGIN").await.expect("begin");
            // No changes
            conn.execute("ROLLBACK").await.expect("rollback empty txn");
        }
...
fn savepoint_without_begin_acts_as_transaction() {
    asupersync::test_utils::run_test(|| async {
...

        {
            ...
            conn.execute("SAVEPOINT sp_auto").await.expect("sp_auto");
            conn.execute("INSERT INTO t1 VALUES (2, 'sp_auto')")
                .await
                .expect("insert");
            conn.execute("RELEASE sp_auto").await.expect("release");
        }
...
fn multiple_tables_in_single_transaction_persist_atomically() {
    asupersync::test_utils::run_test(|| async {
        ...
        let conn = Connection::open(&db_str).await.expect("reopen");
        let r1 = conn.query("SELECT x FROM t1").await.expect("t1");
        let r2 = conn.query("SELECT y FROM t2").await.expect("t2");
        let r3 = conn.query("SELECT z FROM t3").await.expect("t3");
