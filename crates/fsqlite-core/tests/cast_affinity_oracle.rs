...
fn cast_text_to_integer() {
    asupersync::test_utils::run_test(|| async {
        ...
        agree(
            &[],
            "SELECT CAST('-7xyz' AS INTEGER)",
            "sign + leading numeric",
        )
        .await;
        agree(
            &[],
            "SELECT CAST('1e3' AS INTEGER)",
            "integer cast stops at 'e' -> 1",
        )
        .await;
    });
}
...
fn cast_real_to_integer_truncates() {
    asupersync::test_utils::run_test(|| async {
        agree(
            ...
            "truncate toward zero (positive)",
        )
        .await;
        agree(
            &[],
            "SELECT CAST(-3.99 AS INTEGER)",
            "truncate toward zero (negative)",
        )
        .await;
    });
}
...
fn cast_to_real_and_text() {
    asupersync::test_utils::run_test(|| async {
        agree(
            ...
            "text -> real with trailing garbage",
        )
        .await;
        agree(
            &[],
            "SELECT CAST('2.5e2' AS REAL)",
            "text -> real with exponent",
        )
        .await;
...
fn cast_null_and_numeric() {
    asupersync::test_utils::run_test(|| async {
        agree(
            &[],
            "SELECT CAST(NULL AS INTEGER), CAST(NULL AS TEXT)",
            "CAST NULL stays NULL",
        )
        .await;
        agree(
            &[],
            "SELECT CAST('42.0' AS NUMERIC)",
            "NUMERIC of integral value -> integer",
        )
        .await;
        agree(
            &[],
            "SELECT CAST('42.5' AS NUMERIC)",
            "NUMERIC of fractional value -> real",
        )
        .await;
    });
}
    ...
    asupersync::test_utils::run_test(|| async {
        ...
        agree(
            &[
                ...
                "INSERT INTO t VALUES ('007'),('3.14'),('abc')",
            ],
        ...
        agree(
            &[],
            "SELECT 5 = '5', '5' = 5, 5 < '5', 'abc' < 5",
            "bare literal-vs-literal comparison applies no affinity",
        )
        .await;
    });
}
    ...
    asupersync::test_utils::run_test(|| async {
        agree(
            &[
                ...
                "INSERT INTO t VALUES (X'01'),(X'00'),(5),('x')",
            ],
        ...
        agree(
            &[
                "CREATE TABLE t(v)",
                "INSERT INTO t VALUES (NULL),(1),(2.5),('a'),(X'ff')",
            ],
