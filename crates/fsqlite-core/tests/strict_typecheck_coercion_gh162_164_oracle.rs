...
fn strict_integer_accepts_lossless_text_and_real_gh162_164() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert!(
            d.is_empty(),
            "GH#162/#164 STRICT INTEGER accept divergence:\n{}",
            d.join("\n")
        );
...
fn strict_integer_rejects_nonlossless_gh162_164() {
    asupersync::test_utils::run_test(|| async {
        ...
        assert!(
            d.is_empty(),
            "GH#162/#164 STRICT INTEGER reject divergence:\n{}",
            d.join("\n")
        );
        assert_agree_query(&f, &r, "SELECT a, typeof(a) FROM t").await;
    });
...
fn strict_text_and_real_cross_type_gh162_164() {
    asupersync::test_utils::run_test(|| async {
        ...
        let r = rusqlite::Connection::open_in_memory().unwrap();
        for s in [
            "CREATE TABLE tt (a TEXT) STRICT",
            "CREATE TABLE tr (a REAL) STRICT",
            "CREATE TABLE tb (a BLOB) STRICT",
        ] {
            f.execute(s).await.unwrap();
            r.execute_batch(s).unwrap();
        }
        ...
        assert!(
            d.is_empty(),
            "GH#162/#164 cross-type divergence:\n{}",
            d.join("\n")
        );
