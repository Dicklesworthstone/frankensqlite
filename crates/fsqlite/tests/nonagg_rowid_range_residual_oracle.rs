...
fn nonagg_rowid_range_residual_matches_sqlite() {
    ...
    for sql in seeks { cmp(&f, &r, sql, "rowid-range-resid"); assert!(has_seek(&f, sql), "rowid range+residual must seek: `{sql}`"); }
    // Bare rowid range (no residual) still seeks byte-identically.
    for sql in ["SELECT id FROM t WHERE id BETWEEN 10 AND 20", "SELECT x FROM t WHERE id > 595"] {
        cmp(&f, &r, sql, "bare"); assert!(has_seek(&f, sql), "bare rowid range must seek: `{sql}`");
    }
}
