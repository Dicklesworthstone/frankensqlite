...
fn update_set_semantics_match_rusqlite_oracle() {
    asupersync::test_utils::run_test(|| async {
        ...
        check("b gets old a", fq(&f, "SELECT id,a,b FROM t ORDER BY id").await,
              rq(&r, "SELECT id,a,b FROM t ORDER BY id"), &mut diffs);
        ...
        check("swap old values", fq(&f, "SELECT id,a,b FROM sw ORDER BY id").await,
              rq(&r, "SELECT id,a,b FROM sw ORDER BY id"), &mut diffs);
        ...
        check("multi-col old refs", fq(&f, "SELECT a,b,c FROM m").await,
              rq(&r, "SELECT a,b,c FROM m"), &mut diffs);
        ...
        check("where old image", fq(&f, "SELECT id,v FROM w ORDER BY id").await,
              rq(&r, "SELECT id,v FROM w ORDER BY id"), &mut diffs);
        ...
        ex(&f, &r, "UPDATE p SET base = (SELECT COALESCE(sum(amt),0) FROM q WHERE q.pid=p.id)").await;
        check("subquery in set", fq(&f, "SELECT id,base FROM p ORDER BY id").await,
              rq(&r, "SELECT id,base FROM p ORDER BY id"), &mut diffs);
        ...
        check("gencol after update", fq(&f, "SELECT id,base,dbl FROM g ORDER BY id").await,
              rq(&r, "SELECT id,base,dbl FROM g ORDER BY id"), &mut diffs);
        ...
        check("null arithmetic in set", fq(&f, "SELECT a,b FROM n").await,
              rq(&r, "SELECT a,b FROM n"), &mut diffs);

        // updating the same column twice is a parse error on both engines
        check("dup column set errors", fq(&f, "UPDATE t SET a=1, a=2").await,
              rq(&r, "UPDATE t SET a=1, a=2"), &mut diffs);

        assert!(diffs.is_empty(), "{} UPDATE-SET-semantics divergence(s) vs rusqlite:\n{}", diffs.len(), diffs.join("\n"));
    });
}
