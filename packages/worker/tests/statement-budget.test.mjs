...
async function fixture(t, statementLimits = {}, hooks = {}) {
  ...
  const core = () => {
    ...
    const result = (stmt, params = []) => {
      ...
      return {
        ...
        columnTypes: [],
        rows,
        rowArrays: rows.map((r) => columns.map((c) => r[c])),
        changes: 0,
      };
    };
    return {
      ...
      async query(sql) {
        return result(raw.prepare(sql));
      },
      async queryWithParams(sql, params) {
        return result(raw.prepare(sql), params);
      },
      ...
      async prepare(sql) {
        ...
        const check = () => {
          assert.equal(item.freed, false, "use after free");
        };
        const handle = {
          get sql() {
            return hooks.sql ? hooks.sql(sql) : sql;
          },
          get columnCount() {
            return hooks.count ? hooks.count(columns.length) : columns.length;
          },
          columnNames() {
            return hooks.names ? hooks.names(columns) : columns;
          },
          free() {
            item.freeCount++;
            check();
  ...
  t.after(async () => {
    ...
    for (const r of records) if (!r.closed) r.raw.close();
  });
...
const prepare = async (f, sql = "SELECT value FROM data", transactionId) =>
  ok(
    await f.send({ kind: "prepare", sql, ...(transactionId ? { transactionId } : {}) }),
    "prepare-result",
  ).data.statementId;
const boundary = (f, action, id = "1", parentId) =>
  f.send({ kind: "transaction", action, transactionId: id, ...(parentId ? { parentId } : {}) });
...
for (const input of [
  ...
  test(`invalid prepared policy ${JSON.stringify(input)} is rejected`, () => {
    assert.throws(
      () => resolvePreparedStatementLimits(input),
...
test("retained handles remain charged after request completion; saturation never evicts one", async (t) => {
  ...
  for (const id of [first, second]) {
    assert.deepEqual(
      ok(await f.send({ kind: "statement-query", statementId: id })).data.rowArrays,
      [[7]],
    );
  }
...
test("metadata growth hitting aggregate capacity releases only the candidate", async (t) => {
  ...
  assert.equal(f.statements[1].freeCount, 1);
  assert.equal(f.statements[0].freeCount, 0);
  assert.equal(f.host.preparedStatements.bytes, bytes);
  assert.deepEqual(
    ok(await f.send({ kind: "statement-query", statementId: first })).data.rowArrays,
    [[7]],
  );
});
...
for (const field of ["sql", "count", "names"]) {
  test(`throwing ${field} metadata cannot strand an unpublished handle`, async (t) => {
    const hooks = {
      [field]() {
        throw Object.assign(new Error("metadata failure"), { code: "PROBE_METADATA" });
      },
    };
...
for (const [label, hooks] of [
  ...
  ["negative columns", { count: () => -1 }],
  ["noninteger columns", { count: () => 1.5 }],
  ["huge columns", { count: () => 32769 }],
  ["non-array names", { names: () => ({ length: 1 }) }],
  ["wrong name count", { names: () => [] }],
  ["sparse names", { names: () => new Array(1) }],
...
test("column metadata is captured once, copied by index and isolated from later mutation", async (t) => {
  ...
  names[0] = "changed";
  assert.deepEqual(response.data.columnNames, ["value"]);
...
test("metadata callback cancellation cannot publish a now-unowned prepared statement", async (t) => {
  ...
  ok(await boundary(f, "begin"));
  hooks.names = (names) => {
    void f.send({ kind: "cancel-transaction", targetTransactionId: "1" });
    return names;
  };
  error(
    await f.send({ kind: "prepare", sql: "SELECT 1", transactionId: "1" }),
...
for (const moment of ["await", "metadata"]) {
  test(`transport failure during ${moment} never publishes a candidate or frees an active operation`, async (t) => {
    ...
    let close;
    if (moment === "await")
      hooks.afterPrepare = async () => {
        started.resolve();
        await release.promise;
      };
    else
      hooks.names = (names) => {
        close = f.host.failTransport(failure);
        return names;
      };
    ...
    if (moment === "await") {
      ...
      close = f.host.failTransport(failure);
      await turn();
      assert.equal(f.statements[1].freeCount, 0);
    ...
    assert.equal(f.host.preparedStatements.statements, 0);
    assert.deepEqual(
      f.statements.map((s) => s.freeCount),
      [1, 1],
    );
  });
}
...
for (const cleanup of [new Error("free failed"), undefined, null, false]) {
  test(`unpublished cleanup failure fences SQL, even when thrown value is ${String(cleanup)}`, async (t) => {
    const hooks = {
      names() {
        throw new Error("primary metadata failure");
      },
      free() {
        throw cleanup;
      },
    ...
    assert.equal(failure.cause.message, "primary metadata failure");
...
test("explicit finalize failure fences the connection, does not retry free, and drains other handles", async (t) => {
  ...
  let observed;
  hooks.free = (item) => {
    observed ??= f.host.preparedStatements.statements;
    if (item === f.statements[0]) throw new Error("bad free");
  };
...
test("returning an already-owned handle is rejected without freeing that handle", async (t) => {
  ...
  const f = await fixture(t, {}, hooks);
  const id = await prepare(f);
  hooks.alias = f.statements[0].handle;
  error(await f.send({ kind: "prepare", sql: "SELECT 99" }), metadata);
  assert.equal(f.host.preparedStatements.statements, 1);
  assert.equal(f.statements[0].freeCount, 0);
  assert.deepEqual(ok(await f.send({ kind: "statement-query", statementId: id })).data.rowArrays, [
    [7],
  ]);
});
...
for (const action of ["commit", "rollback"]) {
  test(`transaction ${action} releases its prepared resources without manual finalization`, async (t) => {
    const f = await fixture(t, { maxStatements: 1 });
    ok(await boundary(f, "begin"));
    await prepare(f, "SELECT value FROM data", "1");
    ok(await boundary(f, action));
    assert.equal(f.host.preparedStatements.statements, 0);
    await prepare(f);
  });
}
...
test("a rejected child prepare is recoverable without releasing its parent statement", async (t) => {
  const f = await fixture(t, { maxStatements: 1 });
  ok(await boundary(f, "begin"));
  const parent = await prepare(f, "SELECT value FROM data", "1");
  ok(await boundary(f, "begin", "2", "1"));
  error(await f.send({ kind: "prepare", sql: "SELECT 2", transactionId: "2" }), limit);
  ok(await boundary(f, "rollback", "2"));
  assert.deepEqual(
    ok(await f.send({ kind: "statement-query", statementId: parent, transactionId: "1" })).data
      .rowArrays,
    [[7]],
  );
  ok(await boundary(f, "commit"));
  assert.equal(f.host.preparedStatements.statements, 0);
});
...
test("failed replacement preserves prepared accounting; successful replacement releases it", async (t) => {
  ...
  ok(await f.send({ kind: "init", config: {} }));
  assert.equal(f.host.preparedStatements.statements, 0);
  await prepare(f);
  assert.equal(f.statements[0].freeCount, 1);
});
...
test("ordinary close remains behind an admitted prepare and releases its reservation once", async (t) => {
  ...
  const pending = f.send({ kind: "prepare", sql: "SELECT 1" });
  await started.promise;
  ...
  assert.equal(f.host.preparedStatements.statements, 1);
  release.resolve();
  ok(await pending, "prepare-result");
  ok(await close, "close-result");
  assert.equal(f.statements[0].freeCount, 1);
  assert.equal(f.host.preparedStatements.statements, 0);
});
...
test("reservation release is idempotent and cannot resurrect an expired lease", () => {
  ...
  const lease = budget.reserve("SELECT 1");
  lease.grow(16);
  lease.release();
  lease.release();
  assert.equal(budget.stats.statements, 0);
  assert.equal(budget.stats.bytes, 0);
  assert.throws(() => lease.grow(1));
  const next = budget.reserve("SELECT 2");
  next.release();
});
