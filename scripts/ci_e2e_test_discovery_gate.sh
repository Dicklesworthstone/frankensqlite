...
# truth as this gate, execute each target in default libtest mode, and account
# for exactly one successful process receipt per target. When that structural
# contract is present, every tracked integration target is covered without a
# hand-maintained literal allowlist.
#
...
#
# It also cannot prove that `#[ignore]` cases ran: default libtest mode skips
# those functions. Ignored helper/performance cases and ignored correctness
# defects require a separate, explicit release inventory and evidence policy.
#
...
# Populate this from each comment-sanitized workflow below: documentation must
# never turn a disabled literal command into configured coverage.
: > "$tmp/named_raw"
...
for workflow in "$workflows_dir"/*.yml "$workflows_dir"/*.yaml; do
  [ -f "$workflow" ] || continue
  # Comments are documentation, not executable workflow structure. Strip both
  # full-line and ordinary trailing comments before matching so commented-out
  # commands cannot erase the uncovered-test baseline.
  sed -E \
    -e '/^[[:space:]]*#/d' \
    -e 's/[[:space:]]+#.*$//' \
    "$workflow" > "$tmp/workflow_without_comments"
  {
    grep -hoE -- '--test[= ][A-Za-z0-9_-]+' \
...
if [ "$mode" = "--write" ]; then
  {
    echo "# bd-ohk1x baseline: fsqlite-e2e integration targets with no configured"
    echo "# default-libtest CI process receipt."
    echo "#"
    echo "# Generated by scripts/ci_e2e_test_discovery_gate.sh --write"
    echo "# Do not hand-sort; regenerate instead."
    echo "#"
    echo "# Every name here lacks a configured default-libtest process receipt. That is a target-level"
    echo "# gap, not an approval -- shrinking this file is the goal. Removing a"
    echo "# name requires actually wiring the target into a workflow."
...
echo "fsqlite-e2e integration targets: ${total} total, ${named} configured for default-libtest CI execution, ${unrun} uncovered."
if [ -n "$dynamic_all_workflow" ]; then
  echo "dynamic all-target coverage: ${dynamic_all_workflow}"
fi
