...
fn nonagg_rowid_eq_residual_matches_sqlite() {
    ...
    for sql in seeks { cmp(&f, &r, sql, "rowid-eq-resid"); assert!(has_seek(&f, sql), "rowid=const+residual must seek: `{sql}`"); }
    // Bare `rowid = const` (no residual) still seeks via the existing RowidLookup directive.
    for sql in ["SELECT id FROM t WHERE id = 3", "SELECT x FROM t WHERE id = 300"] {
        cmp(&f, &r, sql, "bare"); assert!(has_seek(&f, sql), "bare rowid = const must seek: `{sql}`");
    }
}
