...
fn rowid_autoinc_matches_rusqlite_oracle() {
    asupersync::test_utils::run_test(|| async {
        ...
        check(
            "ipk rowid alias",
            fq(&f, "SELECT rowid, id, v FROM a ORDER BY id").await,
            rq(&r, "SELECT rowid, id, v FROM a ORDER BY id"),
            &mut diffs,
        );
        check(
            "last_insert_rowid a",
            fq(&f, "SELECT last_insert_rowid()").await,
            rq(&r, "SELECT last_insert_rowid()"),
            &mut diffs,
        );
        ...
        check(
            "implicit rowid reuse",
            fq(&f, "SELECT rowid, v FROM b ORDER BY rowid").await,
            rq(&r, "SELECT rowid, v FROM b ORDER BY rowid"),
            &mut diffs,
        );
        ...
        check(
            "sqlite_sequence c",
            fq(&f, "SELECT name, seq FROM sqlite_sequence WHERE name='c'").await,
            rq(&r, "SELECT name, seq FROM sqlite_sequence WHERE name='c'"),
            &mut diffs,
        );

        // WITHOUT ROWID
        for s in [
            "CREATE TABLE d(k TEXT PRIMARY KEY, v INTEGER) WITHOUT ROWID",
            "INSERT INTO d VALUES ('b',2),('a',1),('c',3)",
        ] {
            ex(&f, &r, s).await;
        }
        check(
            "without rowid order",
            fq(&f, "SELECT k, v FROM d ORDER BY k").await,
            rq(&r, "SELECT k, v FROM d ORDER BY k"),
            &mut diffs,
        );
        check(
            "without rowid pk lookup",
            fq(&f, "SELECT v FROM d WHERE k='b'").await,
            rq(&r, "SELECT v FROM d WHERE k='b'"),
            &mut diffs,
        );
        ...
        check(
            "rowid aliases",
            fq(&f, "SELECT rowid, _rowid_, oid FROM a WHERE id=50").await,
            rq(&r, "SELECT rowid, _rowid_, oid FROM a WHERE id=50"),
            &mut diffs,
        );
        check(
            "max/typeof",
            fq(&f, "SELECT max(rowid), typeof(rowid) FROM a").await,
            rq(&r, "SELECT max(rowid), typeof(rowid) FROM a"),
            &mut diffs,
        );

        assert!(
            diffs.is_empty(),
            "{} rowid/autoinc divergence(s) vs rusqlite:\n{}",
            diffs.len(),
            diffs.join("\n")
        );
    });
